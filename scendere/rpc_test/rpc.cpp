...
#include <scendere/boost/beast/http.hpp>
...
#include <scendere/test_common/telemetry.hpp>
...
public:
	...
	{
		sock.async_connect (scendere::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port_a), [this] (boost::system::error_code const & ec) {
			if (!ec)
			{
...
public:
	...
	void reset ()
	{
		scendere::thread_role::set (scendere::thread_role::name::unknown);
	}
...
{
public:
	rpc_context (std::shared_ptr<scendere::rpc> & rpc_a, std::unique_ptr<scendere::ipc::ipc_server> & ipc_server_a, std::unique_ptr<scendere::ipc_rpc_processor> & ipc_rpc_processor_a, std::unique_ptr<scendere::node_rpc_config> & node_rpc_config_a, std::unique_ptr<scoped_io_thread_name_change> & io_scope_a)
	{
		rpc = std::move (rpc_a);
	...
	std::shared_ptr<scendere::rpc> rpc;
	std::unique_ptr<scendere::ipc::ipc_server> ipc_server;
	std::unique_ptr<scendere::ipc_rpc_processor> ipc_rpc_processor;
	std::unique_ptr<scendere::node_rpc_config> node_rpc_config;
	std::unique_ptr<scoped_io_thread_name_change> io_scope;
};

std::shared_ptr<scendere::node> add_ipc_enabled_node (scendere::system & system, scendere::node_config & node_config, scendere::node_flags const & node_flags)
{
	node_config.ipc_config.transport_tcp.enabled = true;
	node_config.ipc_config.transport_tcp.port = scendere::get_available_port ();
	return system.add_node (node_config, node_flags);
}

std::shared_ptr<scendere::node> add_ipc_enabled_node (scendere::system & system, scendere::node_config & node_config)
{
	return add_ipc_enabled_node (system, node_config, scendere::node_flags ());
}

std::shared_ptr<scendere::node> add_ipc_enabled_node (scendere::system & system)
{
...
void reset_confirmation_height (scendere::store & store, scendere::account const & account)
{
...
void wait_response_impl (scendere::system & system, rpc_context const & rpc_ctx, boost::property_tree::ptree & request, std::chrono::duration<double, std::nano> const & time, boost::property_tree::ptree & response_json)
{
...
boost::property_tree::ptree wait_response (scendere::system & system, rpc_context const & rpc_ctx, boost::property_tree::ptree & request, std::chrono::duration<double, std::nano> const & time = 5s)
{
...
void check_block_response_count (scendere::system & system, rpc_context const & rpc_ctx, boost::property_tree::ptree & request, uint64_t size_count)
{
...
rpc_context add_rpc (scendere::system & system, std::shared_ptr<scendere::node> const & node_a)
{
	auto scoped_thread_name_io (std::make_unique<scoped_io_thread_name_change> ());
	auto node_rpc_config (std::make_unique<scendere::node_rpc_config> ());
	auto ipc_server (std::make_unique<scendere::ipc::ipc_server> (*node_a, *node_rpc_config));
	scendere::rpc_config rpc_config (node_a->network_params.network, scendere::get_available_port (), true);
	const auto ipc_tcp_port = ipc_server->listening_tcp_port ();
	debug_assert (ipc_tcp_port.has_value ());
	auto ipc_rpc_processor (std::make_unique<scendere::ipc_rpc_processor> (system.io_ctx, rpc_config, ipc_tcp_port.value ()));
	auto rpc (std::make_shared<scendere::rpc> (system.io_ctx, rpc_config, *ipc_rpc_processor));
	rpc->start ();
...
TEST (rpc, wrapped_task)
{
	...
	scendere::node_rpc_config node_rpc_config;
	...
	auto handler_l (std::make_shared<scendere::json_handler> (node, node_rpc_config, "", response_handler_l));
	auto task (handler_l->create_worker_task ([] (std::shared_ptr<scendere::json_handler> const &) {
...
TEST (rpc, account_balance)
{
	...
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (scendere::error_common::invalid_index).message (), response2.get<std::string> ("error"));
}
...
TEST (rpc, account_weight)
{
	...
	scendere::change_block block (latest, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (block).code);
...
TEST (rpc, validate_account_invalid)
{
	...
	auto const rpc_ctx = add_rpc (system, node);
	std::string account;
	scendere::dev::genesis_key.pub.encode_account (account);
	account[0] ^= 0x1;
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", scendere::dev::genesis_key.pub.to_account ());
	request.put ("destination", scendere::dev::genesis_key.pub.to_account ());
	request.put ("amount", "100");
	ASSERT_EQ (node->balance (scendere::dev::genesis_key.pub), scendere::dev::constants.genesis_amount);
	auto response (wait_response (system, rpc_ctx, request, 10s));
	std::string block_text (response.get<std::string> ("block"));
	scendere::block_hash block;
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->ledger.block_or_pruned_exists (block));
	ASSERT_EQ (node->latest (scendere::dev::genesis_key.pub), block);
	ASSERT_NE (node->balance (scendere::dev::genesis_key.pub), scendere::dev::constants.genesis_amount);
}
...
TEST (rpc, send_fail)
{
	...
	request.put ("amount", "100");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (scendere::error_common::account_not_found_wallet).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, send_work)
{
	...
	request.put ("work", "1");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (scendere::error_common::invalid_work).message (), response.get<std::string> ("error"));
	request.erase ("work");
	request.put ("work", scendere::to_string_hex (*node->work_generate_blocking (node->latest (scendere::dev::genesis_key.pub))));
...
TEST (rpc, send_work_disabled)
{
	...
	request.put ("amount", "100");
	auto response (wait_response (system, rpc_ctx, request, 10s));
	ASSERT_EQ (std::error_code (scendere::error_common::disabled_work_generation).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, send_idempotent)
{
	...
	request.put ("action", "send");
	request.put ("source", scendere::dev::genesis_key.pub.to_account ());
	request.put ("destination", scendere::account{}.to_account ());
	request.put ("amount", (scendere::dev::constants.genesis_amount - (scendere::dev::constants.genesis_amount / 4)).convert_to<std::string> ());
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->ledger.block_or_pruned_exists (block));
	ASSERT_EQ (node->balance (scendere::dev::genesis_key.pub), scendere::dev::constants.genesis_amount / 4);
	auto response2 (wait_response (system, rpc_ctx, request));
	...
	ASSERT_EQ (std::error_code (scendere::error_common::insufficient_balance).message (), response3.get<std::string> ("error"));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3560
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3561
// CI run in which it failed: https://github.com/scenderecurrency/scendere-node/runs/4280938039?check_suite_focus=true#step:5:1895
TEST (rpc, DISABLED_send_epoch_2)
{
	...
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, scendere::epoch::epoch_1));
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, scendere::epoch::epoch_2));

	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv, false);

	auto target_difficulty = scendere::dev::network_params.work.threshold (scendere::work_version::work_1, scendere::block_details (scendere::epoch::epoch_2, true, false, false));
	...
	request.put ("action", "send");
	request.put ("source", scendere::dev::genesis_key.pub.to_account ());
	request.put ("destination", scendere::keypair ().pub.to_account ());
	request.put ("amount", "1");

	// Test that the correct error is given if there is insufficient work
	auto insufficient = system.work_generate_limited (scendere::dev::genesis->hash (), min_difficulty, target_difficulty);
	request.put ("work", scendere::to_string_hex (insufficient));
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_common::invalid_work);
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, scendere::dev::genesis->account ().to_account ());
}
...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		if (!node->store.account.get (transaction, scendere::dev::genesis_key.pub, info))
		{
...
TEST (rpc, wallet_create_seed)
{
	...
	scendere::random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
	auto prv = scendere::deterministic_key (seed, 0);
	auto pub (scendere::pub_key (prv));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "wallet_create");
...
TEST (rpc, wallet_export)
{
	...
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	scendere::wallet_store store (error, kdf, transaction, scendere::dev::genesis->account (), 1, "0", wallet_json);
	ASSERT_FALSE (error);
	ASSERT_TRUE (store.exists (transaction, scendere::dev::genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	destination->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::keypair key;
	auto source_id = scendere::random_wallet_id ();
	...
	ASSERT_TRUE (destination->exists (scendere::dev::genesis_key.pub));
	auto transaction (node->wallets.tx_begin_read ());
...
TEST (rpc, block)
{
	...
	request.put ("hash", node->latest (scendere::dev::genesis->account ()).to_string ());
	auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, block_account)
{
	...
	request.put ("hash", scendere::dev::genesis->hash ().to_string ());
	auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::keypair key;
	auto genesis (node->latest (scendere::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, 1));
	...
	std::vector<scendere::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (scendere::block_hash (i->second.get<std::string> ("")));
	}
...
TEST (rpc, frontier)
{
	scendere::system system;
	auto node = add_ipc_enabled_node (system);
	std::unordered_map<scendere::account, scendere::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			source[key.pub] = hash;
			node->store.confirmation_height.put (transaction, key.pub, { 0, scendere::block_hash (0) });
			node->store.account.put (transaction, key.pub, scendere::account_info (hash, 0, 0, 0, 0, 0, scendere::epoch::epoch_0));
		}
	}
	scendere::keypair key;
	auto const rpc_ctx = add_rpc (system, node);
	...
	request.put ("account", scendere::account{}.to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	auto response (wait_response (system, rpc_ctx, request));
	auto & frontiers_node (response.get_child ("frontiers"));
	std::unordered_map<scendere::account, scendere::block_hash> frontiers;
	...
	ASSERT_EQ (1, frontiers.erase (scendere::dev::genesis_key.pub));
...
TEST (rpc, history)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto change (system.wallet (0)->change_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), scendere::dev::genesis_key.pub, node0->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	scendere::state_block usend (scendere::dev::genesis->account (), node0->latest (scendere::dev::genesis->account ()), scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, scendere::dev::genesis->account (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (node0->latest (scendere::dev::genesis->account ())));
	scendere::state_block ureceive (scendere::dev::genesis->account (), usend.hash (), scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount, usend.hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (usend.hash ()));
	scendere::state_block uchange (scendere::dev::genesis->account (), ureceive.hash (), scendere::keypair ().pub, scendere::dev::constants.genesis_amount, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (ureceive.hash ()));
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (scendere::process_result::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (scendere::process_result::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (scendere::process_result::progress, node0->ledger.process (transaction, uchange).code);
	}
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (scendere::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend.hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (scendere::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (scendere::dev::constants.genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
	ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), std::get<3> (history_l[4]));
}
...
TEST (rpc, account_history)
{
	...
	auto const rpc_ctx = add_rpc (system, node0);
	{
		...
		request.put ("account", scendere::dev::genesis->account ().to_account ());
		request.put ("count", 100);
		auto response (wait_response (system, rpc_ctx, request, 10s));
	...
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, account2, node0->config.receive_minimum.number ()));
	...
	// Test filter for receive state blocks
	{
		...
		boost::property_tree::ptree other_account;
		other_account.put ("", scendere::dev::genesis_key.pub.to_account ());
		boost::property_tree::ptree filtered_accounts;
		filtered_accounts.push_back (std::make_pair ("", other_account));
		request.add_child ("account_filter", filtered_accounts);
		request.put ("count", 100);
...
TEST (rpc, history_count)
{
	...
	auto change (system.wallet (0)->change_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), scendere::dev::genesis_key.pub, node->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "history");
...
TEST (rpc, history_pruning)
{
	...
	auto change (std::make_shared<scendere::change_block> (scendere::dev::genesis->hash (), scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work.generate (scendere::dev::genesis->hash ())));
	node0->process_active (change);
	auto send (std::make_shared<scendere::send_block> (change->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - node0->config.receive_minimum.number (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work.generate (change->hash ())));
	node0->process_active (send);
	auto receive (std::make_shared<scendere::receive_block> (send->hash (), send->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work.generate (send->hash ())));
	node0->process_active (receive);
	auto usend (std::make_shared<scendere::state_block> (scendere::dev::genesis->account (), receive->hash (), scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, scendere::dev::genesis->account (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (receive->hash ())));
	auto ureceive (std::make_shared<scendere::state_block> (scendere::dev::genesis->account (), usend->hash (), scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount, usend->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (usend->hash ())));
	auto uchange (std::make_shared<scendere::state_block> (scendere::dev::genesis->account (), ureceive->hash (), scendere::keypair ().pub, scendere::dev::constants.genesis_amount, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node0->work_generate_blocking (ureceive->hash ())));
...
TEST (rpc, process_block)
{
	...
	auto const rpc_ctx = add_rpc (system, node1);
	scendere::keypair key;
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	boost::property_tree::ptree request;
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_TIMELY (10s, node1->latest (scendere::dev::genesis_key.pub) == send.hash ());
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_blocks::invalid_block);
...
TEST (rpc, process_block_async)
{
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_common::is_not_state_block);
		ASSERT_EQ (ec.message (), response.get<std::string> ("error"));
	}

	auto state_send (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 100, scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest)));
	std::string json1;
	...
	request.put ("block", json1);
	{
		...
		ASSERT_TIMELY (10s, node1->latest (scendere::dev::genesis_key.pub) == state_send->hash ());
	}
}
...
TEST (rpc, process_republish)
{
	...
	scendere::keypair key;
	auto latest (node1.latest (scendere::dev::genesis_key.pub));
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node3->work_generate_blocking (latest));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	send.serialize_json (json);
	request.put ("block", json);
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_TIMELY (10s, node2.latest (scendere::dev::genesis_key.pub) == send.hash ());
}
...
TEST (rpc, process_subtype_send)
{
	...
	scendere::keypair key;
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	scendere::state_block send (scendere::dev::genesis->account (), latest, scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	send.serialize_json (json);
	...
	auto response (wait_response (system, rpc_ctx, request));
	std::error_code ec (scendere::error_rpc::invalid_subtype_balance);
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	...
	ASSERT_TIMELY (10s, system.nodes[1]->latest (scendere::dev::genesis_key.pub) == send.hash ());
}
...
TEST (rpc, process_subtype_open)
{
	...
	scendere::keypair key;
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	auto send = std::make_shared<scendere::state_block> (scendere::dev::genesis->account (), latest, scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*send).code);
	ASSERT_EQ (scendere::process_result::progress, node2.process (*send).code);
	auto const rpc_ctx = add_rpc (system, node1);
	node1->scheduler.manual (send);
	scendere::state_block open (key.pub, 0, key.pub, scendere::Gxrb_ratio, send->hash (), key.prv, key.pub, *node1->work_generate_blocking (key.pub));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
...
TEST (rpc, process_subtype_receive)
{
	...
	auto & node2 = *system.add_node ();
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	auto send = std::make_shared<scendere::state_block> (scendere::dev::genesis->account (), latest, scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*send).code);
	ASSERT_EQ (scendere::process_result::progress, node2.process (*send).code);
	auto const rpc_ctx = add_rpc (system, node1);
	node1->scheduler.manual (send);
	scendere::state_block receive (scendere::dev::genesis_key.pub, send->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount, send->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (send->hash ()));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	...
	request.put ("subtype", "open");
	auto response2 (wait_response (system, rpc_ctx, request));
	ec = scendere::error_rpc::invalid_subtype_previous;
	ASSERT_EQ (response2.get<std::string> ("error"), ec.message ());
	request.put ("subtype", "receive");
	auto response3 (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (receive.hash ().to_string (), response3.get<std::string> ("hash"));
	ASSERT_TIMELY (10s, node2.latest (scendere::dev::genesis_key.pub) == receive.hash ());
}
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	auto latest (node->latest (scendere::dev::genesis_key.pub));
	auto min_difficulty = node->network_params.work.entry;
	auto max_difficulty = node->network_params.work.epoch_1;
	scendere::state_block send (scendere::dev::genesis->account (), latest, scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, system.work_generate_limited (latest, min_difficulty, max_difficulty));
	ASSERT_LT (scendere::dev::network_params.work.difficulty (send), max_difficulty);
	ASSERT_GE (scendere::dev::network_params.work.difficulty (send), min_difficulty);
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	send.serialize_json (json);
	...
	request.put ("subtype", "send");
	auto response (wait_response (system, rpc_ctx, request));
	std::error_code ec (scendere::error_process::insufficient_work);
	ASSERT_EQ (1, response.count ("error"));
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
}
...
TEST (rpc, keepalive)
{
	scendere::system system;
	auto node0 = add_ipc_enabled_node (system);
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work));
	node1->start ();
...
TEST (rpc, peers)
{
	scendere::system system;
	auto node = add_ipc_enabled_node (system);
	auto port = scendere::get_available_port ();
	auto const node2 = system.add_node (scendere::node_config (port, system.logging));
	scendere::endpoint endpoint (boost::asio::ip::make_address_v6 ("fc00::1"), 4000);
...
TEST (rpc, pending)
{
	...
	scendere::keypair key1;
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 100));
	...
	request.put ("count", "100");
	{
		...
		scendere::block_hash hash (blocks_node.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		auto & blocks_node (response.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		scendere::block_hash hash (blocks_node.begin ()->first);
		ASSERT_EQ (block1->hash (), hash);
	...
	request.put ("threshold", "100"); // Threshold test
	{
		...
		auto & blocks_node (response.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<scendere::block_hash, scendere::uint128_union> blocks;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			scendere::block_hash hash;
			hash.decode_hex (i->first);
			scendere::uint128_union amount;
	...
	request.put ("min_version", "true");
	{
		...
		auto & blocks_node (response.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<scendere::block_hash, scendere::uint128_union> amounts;
		std::unordered_map<scendere::block_hash, scendere::account> sources;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			scendere::block_hash hash;
			hash.decode_hex (i->first);
		...
		ASSERT_EQ (sources[block1->hash ()], scendere::dev::genesis_key.pub);
	}
	...
	rpc_ctx.io_scope->reset ();
	node->store.confirmation_height.put (node->store.tx_begin_write (), scendere::dev::genesis_key.pub, { 2, block1->hash () });
	auto block2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 200));
	auto block3 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 300));
	auto block4 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 400));
	rpc_ctx.io_scope->renew ();
	...
	request.put ("count", "2");
	{
		...
		scendere::block_hash hash (blocks_node.begin ()->first);
		scendere::block_hash hash1 ((++blocks_node.begin ())->first);
...
TEST (rpc, receivable_offset_and_sorting)
{
	...
	auto block1 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 200);
	auto block2 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 100);
	auto block3 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 400);
	auto block4 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 300);
	auto block5 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 300);
	auto block6 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 300);
	...
	scendere::confirmation_height_info confirmation_height_info;
	ASSERT_FALSE (node->store.confirmation_height.get (node->store.tx_begin_read (), scendere::dev::genesis->account (), confirmation_height_info));
	...
	auto hash_exists = [] (boost::property_tree::ptree & node, bool match_first, scendere::block_hash hash) {
		std::stringstream ss;
	...
	request.put ("offset", "0");
	{
		...
		ASSERT_EQ (block3->hash (), scendere::block_hash{ itr->first });
		...
		++itr;
		ASSERT_EQ (block1->hash (), scendere::block_hash{ itr->first });
	...
	request.put ("offset", "3");
	{
		...
		++itr;
		ASSERT_EQ (block2->hash (), scendere::block_hash{ itr->first });
...
TEST (rpc, pending_burn)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::constants.burn_account, 100));
	auto const rpc_ctx = add_rpc (system, node);
	node->scheduler.flush ();
	ASSERT_TIMELY (5s, !node->active.active (*block1));
	ASSERT_TIMELY (5s, node->ledger.cache.cemented_count == 2 && node->confirmation_height_processor.current ().is_zero () && node->confirmation_height_processor.awaiting_processing_size () == 0);
	boost::property_tree::ptree request;
	request.put ("action", "pending");
	request.put ("account", scendere::dev::constants.burn_account.to_account ());
	request.put ("count", "100");
	{
		auto response (wait_response (system, rpc_ctx, request));
		auto & blocks_node (response.get_child ("blocks"));
...
TEST (rpc, search_receivable)
{
	...
	auto latest (node->latest (scendere::dev::genesis_key.pub));
	scendere::send_block block (latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - node->config.receive_minimum.number (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node->work_generate_blocking (latest));
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (scendere::process_result::progress, node->ledger.process (transaction, block).code);
	}
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	request.put ("wallet", wallet);
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_TIMELY (10s, node->balance (scendere::dev::genesis_key.pub) == scendere::dev::constants.genesis_amount);
}
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Scendere %1%") % SCENDERE_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
	...
	auto genesis_open (node1->latest (scendere::dev::genesis_key.pub));
...
TEST (rpc, work_generate)
{
	...
	auto verify_response = [&node, &rpc_ctx, &system] (auto & request, auto & hash) {
		...
		uint64_t work;
		ASSERT_FALSE (scendere::from_string_hex (work_text, work));
		auto result_difficulty (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, hash, work));
		auto response_difficulty_text (response.template get<std::string> ("difficulty"));
		uint64_t response_difficulty;
		ASSERT_FALSE (scendere::from_string_hex (response_difficulty_text, response_difficulty));
		ASSERT_EQ (result_difficulty, response_difficulty);
		auto multiplier = response.template get<double> ("multiplier");
		ASSERT_NEAR (scendere::difficulty::to_multiplier (result_difficulty, node->default_difficulty (scendere::work_version::work_1)), multiplier, 1e-6);
	};
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", scendere::to_string_hex (difficulty));
		auto response (wait_response (system, rpc_ctx, request, 10s));
		...
		ASSERT_NEAR (scendere::difficulty::to_multiplier (result_difficulty, node->default_difficulty (scendere::work_version::work_1)), multiplier, 1e-10);
	...
	{
		uint64_t difficulty (node->max_work_generate_difficulty (scendere::work_version::work_1) + 1);
		request.put ("difficulty", scendere::to_string_hex (difficulty));
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_rpc::difficulty_limit);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", scendere::to_string_hex (static_cast<uint64_t> (0xff00000000000000)));
	...
	}
	{
		...
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	}
	{
		double max_multiplier (scendere::difficulty::to_multiplier (node->max_work_generate_difficulty (scendere::work_version::work_1), node->default_difficulty (scendere::work_version::work_1)));
...
TEST (rpc, work_generate_block_high)
{
	...
	auto const rpc_ctx = add_rpc (system, node);
	scendere::keypair key;
	scendere::state_block block (key.pub, 0, scendere::dev::genesis_key.pub, scendere::Gxrb_ratio, 123, key.prv, key.pub, *node->work_generate_blocking (key.pub));
	scendere::block_hash hash (block.root ().as_block_hash ());
	auto block_difficulty (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, hash, block.block_work ()));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("json_block", "true");
	...
	request.add_child ("block", json);
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (scendere::error_rpc::block_work_enough).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_low)
{
	...
	auto const rpc_ctx = add_rpc (system, node);
	scendere::keypair key;
	scendere::state_block block (key.pub, 0, scendere::dev::genesis_key.pub, scendere::Gxrb_ratio, 123, key.prv, key.pub, 0);
	auto threshold (node->default_difficulty (block.work_version ()));
	block.block_work_set (system.work_generate_limited (block.root ().as_block_hash (), threshold, scendere::difficulty::from_multiplier (node->config.max_work_generate_multiplier / 10, threshold)));
	scendere::block_hash hash (block.root ().as_block_hash ());
	auto block_difficulty (scendere::dev::network_params.work.difficulty (block));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("difficulty", scendere::to_string_hex (block_difficulty + 1));
	request.put ("json_block", "false");
	std::string json;
	block.serialize_json (json);
	request.put ("block", json);
...
TEST (rpc, work_generate_block_root_mismatch)
{
	...
	request.put ("block", json);
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (scendere::error_rpc::block_root_mismatch).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	auto epoch2 = system.upgrade_genesis_epoch (*node, scendere::epoch::epoch_2);
	ASSERT_NE (nullptr, epoch2);
	scendere::keypair key;
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	ASSERT_NE (nullptr, send_block);
	scendere::state_block block (key.pub, 0, scendere::dev::genesis_key.pub, scendere::Gxrb_ratio, send_block->hash (), key.prv, key.pub, 0);
	auto threshold (scendere::dev::network_params.work.threshold (block.work_version (), scendere::block_details (scendere::epoch::epoch_2, false, true, false)));
...
TEST (rpc, work_cancel)
{
	...
	while (!done)
	{
		system.work.generate (scendere::work_version::work_1, hash1, node1->network_params.work.base, [&done] (boost::optional<uint64_t> work_a) {
			done = !work_a;
		});
...
TEST (rpc, work_peer_bad)
{
	...
	node2.work_generate (scendere::work_version::work_1, hash1, node2.network_params.work.base, [&work] (boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	ASSERT_TIMELY (5s, scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, hash1, work) >= scendere::dev::network_params.work.threshold_base (scendere::work_version::work_1));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3629
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3639
TEST (rpc, DISABLED_work_peer_one)
{
	...
	scendere::keypair key1;
	std::atomic<uint64_t> work (0);
	node2.work_generate (scendere::work_version::work_1, key1.pub, node1->network_params.work.base, [&work] (boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	ASSERT_TIMELY (5s, scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, key1.pub, work) >= scendere::dev::network_params.work.threshold_base (scendere::work_version::work_1));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3629
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3636
TEST (rpc, DISABLED_work_peer_many)
{
	scendere::system system1 (1);
	scendere::system system2;
	scendere::system system3 (1);
	scendere::system system4 (1);
	...
	{
		scendere::keypair key1;
		node1.work_generate (scendere::work_version::work_1, key1.pub, node1.network_params.work.base, [&work] (boost::optional<uint64_t> work_a) {
			work = *work_a;
		});
		while (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, key1.pub, work) < scendere::dev::network_params.work.threshold_base (scendere::work_version::work_1))
		{
			system1.poll ();
			system2.poll ();
			system3.poll ();
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3629
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3637
TEST (rpc, DISABLED_work_version_invalid)
{
	...
	request.put ("version", "work_invalid");
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (1, response.count ("error"));
		ASSERT_EQ (std::error_code (scendere::error_rpc::bad_work_version).message (), response.get<std::string> ("error"));
	}
...
TEST (rpc, block_count_pruning)
{
	...
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	auto send1 (std::make_shared<scendere::send_block> (latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest)));
	node1->process_active (send1);
	auto receive1 (std::make_shared<scendere::receive_block> (send1->hash (), send1->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (send1->hash ())));
...
TEST (rpc, available_supply)
{
	...
	rpc_ctx.io_scope->reset ();
	auto block2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, 0, 100)); // Sending to burning 0 account
	rpc_ctx.io_scope->renew ();
...
TEST (rpc, mrai_to_raw)
{
	...
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (scendere::Mxrb_ratio.convert_to<std::string> (), response1.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", scendere::Mxrb_ratio.convert_to<std::string> ());
	auto response1 (wait_response (system, rpc_ctx, request1));
...
TEST (rpc, krai_to_raw)
{
	...
	request1.put ("amount", "1");
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (scendere::kxrb_ratio.convert_to<std::string> (), response1.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", scendere::kxrb_ratio.convert_to<std::string> ());
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ ("1", response1.get<std::string> ("amount"));
}

TEST (rpc, scendere_to_raw)
{
	scendere::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "scendere_to_raw");
	request1.put ("amount", "1");
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ (scendere::Mxrb_ratio.convert_to<std::string> (), response1.get<std::string> ("amount"));
}

TEST (rpc, raw_to_scendere)
{
	scendere::system system;
	auto node1 = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request1;
	request1.put ("action", "raw_to_scendere");
	request1.put ("amount", scendere::Mxrb_ratio.convert_to<std::string> ());
	auto response1 (wait_response (system, rpc_ctx, request1));
	ASSERT_EQ ("1", response1.get<std::string> ("amount"));
}
...
TEST (rpc, account_representative_set)
{
	...
	// create a 2nd account and send it some scendere
	scendere::keypair key2;
	wallet.insert_adhoc (key2.prv);
	auto key2_open_block_hash = wallet.send_sync (scendere::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number ());
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub, key2_open_block->representative ());
...
TEST (rpc, account_representative_set_epoch_2)
{
	...
	auto target_difficulty = scendere::dev::network_params.work.threshold (scendere::work_version::work_1, scendere::block_details (scendere::epoch::epoch_2, false, false, false));
	ASSERT_LT (node->network_params.work.entry, target_difficulty);
	auto min_difficulty = node->network_params.work.entry;
	...
	request.put ("action", "account_representative_set");
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
	request.put ("representative", scendere::keypair ().pub.to_account ());
...
TEST (rpc, bootstrap)
{
	scendere::system system0;
	auto node = add_ipc_enabled_node (system0);
	scendere::system system1 (1);
	auto node1 = system1.nodes[0];
	auto latest (node1->latest (scendere::dev::genesis_key.pub));
	scendere::send_block send (latest, scendere::dev::genesis->account (), 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	{
		auto transaction (node1->store.tx_begin_write ());
		ASSERT_EQ (scendere::process_result::progress, node1->ledger.process (transaction, send).code);
	}
	...
	while (node->latest (scendere::dev::genesis->account ()) != node1->latest (scendere::dev::genesis->account ()))
	{
...
TEST (rpc, representatives)
{
	...
	std::vector<scendere::account> representatives;
	...
	ASSERT_EQ (scendere::dev::genesis->account (), representatives[0]);
}
...
TEST (rpc, wallet_change_seed)
{
	...
	scendere::random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
	{
		auto transaction (node->wallets.tx_begin_read ());
		scendere::raw_key seed0;
		scendere::random_pool::generate_block (seed0.bytes.data (), seed0.bytes.size ());
...
TEST (rpc, wallet_frontiers)
{
	scendere::system system0;
	auto node = add_ipc_enabled_node (system0);
	system0.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto const rpc_ctx = add_rpc (system0, node);
	boost::property_tree::ptree request;
	...
	auto response (wait_response (system0, rpc_ctx, request));
	auto & frontiers_node (response.get_child ("frontiers"));
	std::vector<scendere::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (scendere::account (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (node->latest (scendere::dev::genesis->account ()), frontiers[0]);
}
...
TEST (rpc, work_validate)
{
	...
	request.put ("action", "work_validate");
	request.put ("hash", hash.to_string ());
	request.put ("work", scendere::to_string_hex (work1));
	{
		auto response (wait_response (system, rpc_ctx, request));
		...
		uint64_t difficulty;
		ASSERT_FALSE (scendere::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, node1->default_difficulty (scendere::work_version::work_1));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, scendere::difficulty::to_multiplier (difficulty, node1->default_difficulty (scendere::work_version::work_1)), 1e-6);
	}
	uint64_t work2 (0);
	request.put ("work", scendere::to_string_hex (work2));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (0, response.count ("valid"));
		...
		uint64_t difficulty;
		ASSERT_FALSE (scendere::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (node1->default_difficulty (scendere::work_version::work_1), difficulty);
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, scendere::difficulty::to_multiplier (difficulty, node1->default_difficulty (scendere::work_version::work_1)), 1e-6);
	}
	auto result_difficulty (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, hash, work1));
	ASSERT_GE (result_difficulty, node1->default_difficulty (scendere::work_version::work_1));
	request.put ("work", scendere::to_string_hex (work1));
	request.put ("difficulty", scendere::to_string_hex (result_difficulty));
	{
		auto response (wait_response (system, rpc_ctx, request));
	...
	request.put ("work", scendere::to_string_hex (work1));
	request.put ("difficulty", scendere::to_string_hex (difficulty4));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (result_difficulty >= difficulty4, response.get<bool> ("valid"));
		ASSERT_EQ (result_difficulty >= node1->default_difficulty (scendere::work_version::work_1), response.get<bool> ("valid_all"));
	...
	request.put ("work", scendere::to_string_hex (work3));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_TRUE (response.get<bool> ("valid"));
		ASSERT_TRUE (response.get<bool> ("valid_all"));
...
TEST (rpc, work_validate_epoch_2)
{
	...
	request.put ("work", scendere::to_string_hex (work));
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (0, response.count ("valid"));
		ASSERT_FALSE (response.get<bool> ("valid_all"));
		...
		ASSERT_FALSE (scendere::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, scendere::difficulty::to_multiplier (difficulty, node->network_params.work.epoch_2), 1e-6);
	};
	...
	rpc_ctx.io_scope->renew ();
	{
		...
		ASSERT_FALSE (scendere::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, scendere::difficulty::to_multiplier (difficulty, node->default_difficulty (scendere::work_version::work_1)), 1e-6);
	};
}
...
TEST (rpc, bootstrap_any)
{
	...
	auto node = add_ipc_enabled_node (system0);
	scendere::system system1 (1);
	auto latest (system1.nodes[0]->latest (scendere::dev::genesis_key.pub));
	scendere::send_block send (latest, scendere::dev::genesis->account (), 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system1.nodes[0]->work_generate_blocking (latest));
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (scendere::process_result::progress, system1.nodes[0]->ledger.process (transaction, send).code);
	}
	auto const rpc_ctx = add_rpc (system0, node);
	boost::property_tree::ptree request;
...
TEST (rpc, republish)
{
	...
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (send).code);
	scendere::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, *node1->work_generate_blocking (key.pub));
	ASSERT_EQ (scendere::process_result::progress, node1->process (open).code);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	request.put ("action", "republish");
	request.put ("hash", send.hash ().to_string ());
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_TIMELY (10s, system.nodes[1]->balance (scendere::dev::genesis_key.pub) != scendere::dev::constants.genesis_amount);
	auto & blocks_node (response.get_child ("blocks"));
	std::vector<scendere::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
	...
	ASSERT_EQ (1, blocks.size ());
	ASSERT_EQ (scendere::dev::genesis->hash (), blocks[0]);
...
TEST (rpc, deterministic_key)
{
	...
	scendere::account account0 (system0.wallet (0)->deterministic_insert ());
	scendere::account account1 (system0.wallet (0)->deterministic_insert ());
	scendere::account account2 (system0.wallet (0)->deterministic_insert ());
	auto const rpc_ctx = add_rpc (system0, node);
	boost::property_tree::ptree request;
...
TEST (rpc, accounts_balances)
{
	...
	entry.put ("", scendere::dev::genesis_key.pub.to_account ());
	...
	for (auto & balances : response.get_child ("balances"))
	{
		...
		ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), account_text);
...
TEST (rpc, accounts_representatives)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	auto response_representative (response.get_child ("representatives").get<std::string> (scendere::dev::genesis->account ().to_account ()));
	ASSERT_EQ (response_representative, scendere::dev::genesis->account ().to_account ());
}
...
TEST (rpc, accounts_frontiers)
{
	...
	for (auto & frontiers : response.get_child ("frontiers"))
	{
		...
		ASSERT_EQ (node->latest (scendere::dev::genesis->account ()), scendere::block_hash{ frontier_text });
	}
}
...
TEST (rpc, accounts_pending)
{
	...
	boost::property_tree::ptree accounts;
	child.put ("", scendere::dev::genesis_key.pub.to_account ());
...
TEST (rpc, accounts_receivable)
{
	...
	request.put ("count", "100");
	{
		...
		for (auto & blocks : response.get_child ("blocks"))
		{
			...
			scendere::block_hash hash1 (blocks.second.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		for (auto & blocks : response.get_child ("blocks"))
		{
			std::string account_text (blocks.first);
			ASSERT_EQ (key1.pub.to_account (), account_text);
			scendere::block_hash hash1 (blocks.second.begin ()->first);
			ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, blocks)
{
	...
	boost::property_tree::ptree entry;
	boost::property_tree::ptree peers_l;
	entry.put ("", node->latest (scendere::dev::genesis->account ()).to_string ());
	peers_l.push_back (std::make_pair ("", entry));
	...
	for (auto & blocks : response.get_child ("blocks"))
	{
		...
		ASSERT_EQ (node->latest (scendere::dev::genesis->account ()).to_string (), hash_text);
...
TEST (rpc, wallet_info)
{
	...
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	...
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, 1));
	ASSERT_EQ (4, node->ledger.cache.block_count);

	scendere::account account (system.wallet (0)->deterministic_insert ());
	{
		auto transaction (node->wallets.tx_begin_write ());
...
TEST (rpc, wallet_balances)
{
	...
	auto send (system0.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, 1));
	rpc_ctx.io_scope->renew ();
...
TEST (rpc, pending_exists)
{
	...
	scendere::keypair key1;
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto hash0 (node->latest (scendere::dev::genesis->account ()));
	auto block1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 100));
	node->scheduler.flush ();
	ASSERT_TIMELY (5s, !node->active.active (*block1));
	ASSERT_TIMELY (5s, node->ledger.cache.cemented_count == 2 && node->confirmation_height_processor.current ().is_zero () && node->confirmation_height_processor.awaiting_processing_size () == 0);
	...
	node->store.pending.exists (node->store.tx_begin_read (), scendere::pending_key (scendere::dev::genesis_key.pub, block1->hash ()));
...
TEST (rpc, wallet_pending)
{
	...
	auto block1 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 100);
	ASSERT_TIMELY (5s, node->get_confirmation_height (node->store.tx_begin_read (), scendere::dev::genesis_key.pub) == 2);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	scendere::block_hash hash1{ pending.second.begin ()->second.get<std::string> ("") };
	ASSERT_EQ (block1->hash (), hash1);
}
...
TEST (rpc, wallet_receivable)
{
	...
	auto iterations (0);
	auto block1 (system0.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 100));
	node->scheduler.flush ();
...
TEST (rpc, work_get)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	system.wallet (0)->work_cache_blocking (scendere::dev::genesis_key.pub, node->latest (scendere::dev::genesis_key.pub));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	uint64_t work (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, scendere::dev::genesis->account (), work);
	ASSERT_EQ (scendere::to_string_hex (work), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
	request.put ("work", scendere::to_string_hex (work0));
	auto response (wait_response (system, rpc_ctx, request));
	std::string success (response.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	uint64_t work1 (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, scendere::dev::genesis->account (), work1);
...
TEST (rpc, wallet_republish)
{
	...
	auto node1 = add_ipc_enabled_node (system);
	scendere::keypair key;
	while (key.pub < scendere::dev::genesis_key.pub)
	{
		scendere::keypair key1;
		key.pub = key1.pub;
		key.prv = key1.prv;
...
TEST (rpc, delegators)
{
	...
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (send).code);
	scendere::open_block open (send.hash (), scendere::dev::genesis_key.pub, key.pub, key.prv, key.pub, *node1->work_generate_blocking (key.pub));
	ASSERT_EQ (scendere::process_result::progress, node1->process (open).code);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	...
	ASSERT_EQ ("100", delegators.get<std::string> (scendere::dev::genesis_key.pub.to_account ()));
...
TEST (rpc, delegators_parameters)
{
	...
	if (scendere::dev::genesis_key.pub.number () < key.pub.number ())
	{
		ASSERT_EQ ("100", delegators2.get<std::string> (scendere::dev::genesis_key.pub.to_account ()));
	}
	else
	{
	...
	if (scendere::dev::genesis_key.pub.number () > key.pub.number ())
	{
		last_account = scendere::dev::genesis_key.pub;
	}
	request.put ("start", scendere::account (last_account.number () - 1).to_account ());
...
TEST (rpc, account_info)
{
	...
	// Test for a non existing account
	{
		...
		ASSERT_EQ (error.get (), std::error_code (scendere::error_common::account_not_found).message ());
	}
	...
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node1->process (send).code);
	auto time (scendere::seconds_since_epoch ());
	{
		auto transaction = node1->store.tx_begin_write ();
		node1->store.confirmation_height.put (transaction, scendere::dev::genesis_key.pub, { 1, scendere::dev::genesis->hash () });
	}
	rpc_ctx.io_scope->renew ();
	...
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
	{
		...
		ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), open_block);
		std::string representative_block (response.get<std::string> ("representative_block"));
		ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), representative_block);
		...
		ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), confirmation_height_frontier);
	...
	request.put ("representative", "1");
	{
		...
		ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), representative2);
	}
	...
	scendere::keypair key1;
	{
		latest = node1->latest (scendere::dev::genesis_key.pub);
		scendere::send_block send1 (latest, key1.pub, 50, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
		ASSERT_EQ (scendere::process_result::progress, node1->process (send1).code);
		scendere::send_block send2 (send1.hash (), key1.pub, 25, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (send1.hash ()));
		ASSERT_EQ (scendere::process_result::progress, node1->process (send2).code);

		scendere::state_block state_change (scendere::dev::genesis_key.pub, send2.hash (), key1.pub, 25, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (send2.hash ()));
		ASSERT_EQ (scendere::process_result::progress, node1->process (state_change).code);

		scendere::open_block open (send1.hash (), scendere::dev::genesis_key.pub, key1.pub, key1.prv, key1.pub, *node1->work_generate_blocking (key1.pub));
		ASSERT_EQ (scendere::process_result::progress, node1->process (open).code);
	}
	...
	request.put ("include_confirmed", true);
	{
		...
		ASSERT_EQ (confirmed_representative, scendere::dev::genesis_key.pub.to_account ());

		auto confirmed_frontier (response.get<std::string> ("confirmed_frontier"));
		ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), confirmed_frontier);
...
TEST (rpc, json_block_input)
{
	...
	scendere::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	scendere::state_block send (scendere::dev::genesis->account (), node1->latest (scendere::dev::genesis_key.pub), scendere::dev::genesis->account (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, 0);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	...
	scendere::state_block block (json_error, response.get_child ("block"));
	ASSERT_FALSE (json_error);

	ASSERT_FALSE (scendere::validate_message (key.pub, send.hash (), block.block_signature ()));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	scendere::send_block send_from_json (json_error, response.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	auto check_blocks = [node] (boost::property_tree::ptree & response) {
		for (auto & blocks : response.get_child ("blocks"))
		{
			...
			ASSERT_EQ (scendere::dev::constants.genesis_amount.convert_to<std::string> (), amount_text);
			...
			ASSERT_EQ (scendere::dev::constants.genesis_amount.convert_to<std::string> (), balance_text);
			...
			ASSERT_EQ (scendere::block_hash (0).to_string (), successor_text); // Genesis block doesn't have successor yet
		}
	};
	boost::property_tree::ptree request;
	...
	std::string random_hash = scendere::block_hash ().to_string ();
	...
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (scendere::error_blocks::not_found).message (), response.get<std::string> ("error"));
	}
	...
	request.put ("receive_hash", "1");
	{
		...
		for (auto & blocks : response.get_child ("blocks"))
		{
			...
			ASSERT_EQ (scendere::block_hash (0).to_string (), receive_hash);
		}
	}
}
...
TEST (rpc, blocks_info_receive_hash)
{
	...
	// do 4 sends
	auto send1 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 1);
	auto send2 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 2);
	auto send3 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 3);
	auto send4 = system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 4);
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, scendere::Gxrb_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), key.pub, scendere::Gxrb_ratio, send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (0)->change_action (scendere::dev::genesis_key.pub, key.pub));
	ASSERT_NE (nullptr, change);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	request.put ("action", "blocks_info");
	...
	ASSERT_EQ (change_successor, scendere::block_hash (0).to_string ()); // Change block doesn't have successor yet
}
...
{
	scendere::system system;
	scendere::node_config node_config0 (scendere::get_available_port (), system.logging);
	node_config0.receive_minimum = scendere::dev::constants.genesis_amount; // Prevent auto-receive & receive1 block conflicts
	auto & node0 = *system.add_node (node_config0);
	scendere::node_config node_config1 (scendere::get_available_port (), system.logging);
	...
	ASSERT_FALSE (amount.is_initialized ()); // Cannot calculate amount
	bool json_error{ false };
	scendere::receive_block receive_from_json (json_error, response2.get_child ("contents"));
	ASSERT_FALSE (json_error);
	...
	ASSERT_EQ (successor_text, scendere::block_hash (0).to_string ()); // receive1 block doesn't have successor yet
}
...
TEST (rpc, ledger)
{
	...
	scendere::keypair key;
	auto latest (node->latest (scendere::dev::genesis_key.pub));
	auto genesis_balance (scendere::dev::constants.genesis_amount);
	auto send_amount (genesis_balance - 100);
	genesis_balance -= send_amount;
	scendere::send_block send (latest, key.pub, genesis_balance, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node->work_generate_blocking (latest));
	ASSERT_EQ (scendere::process_result::progress, node->process (send).code);
	scendere::open_block open (send.hash (), scendere::dev::genesis_key.pub, key.pub, key.prv, key.pub, *node->work_generate_blocking (key.pub));
	...
	request.put ("representative", true);
	{
		...
		for (auto & account : response.get_child ("accounts"))
		{
			...
			ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), representative.get ());
		}
	}
	// Test threshold
	request.put ("count", 2);
	...
	scendere::send_block send2 (send.hash (), key.pub, genesis_balance, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node->work_generate_blocking (send.hash ()));
	rpc_ctx.io_scope->reset ();
...
TEST (rpc, block_create)
{
	...
	scendere::send_block send (latest, key.pub, 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, send_work);
	auto open_work = *node1->work_generate_blocking (key.pub);
	scendere::open_block open (send.hash (), scendere::dev::genesis_key.pub, key.pub, key.prv, key.pub, open_work);
	auto const rpc_ctx = add_rpc (system, node1);
	boost::property_tree::ptree request;
	...
	request.put ("work", scendere::to_string_hex (send_work));
	auto response (wait_response (system, rpc_ctx, request));
	std::string send_hash (response.get<std::string> ("hash"));
	ASSERT_EQ (send.hash ().to_string (), send_hash);
	std::string send_difficulty (response.get<std::string> ("difficulty"));
	ASSERT_EQ (scendere::to_string_hex (scendere::dev::network_params.work.difficulty (send)), send_difficulty);
	auto send_text (response.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (send_text);
	boost::property_tree::read_json (block_stream, block_l);
	auto send_block (scendere::deserialize_block_json (block_l));
	...
	request1.put ("representative", scendere::dev::genesis_key.pub.to_account ());
	request1.put ("source", send.hash ().to_string ());
	request1.put ("work", scendere::to_string_hex (open_work));
	auto response1 (wait_response (system, rpc_ctx, request1));
	...
	auto open_block (scendere::deserialize_block_json (block_l));
	...
	scendere::change_block change (open.hash (), key.pub, key.prv, key.pub, change_work);
	request1.put ("type", "change");
	request1.put ("work", scendere::to_string_hex (change_work));
	...
	auto change_block (scendere::deserialize_block_json (block_l));
	ASSERT_EQ (change.hash (), change_block->hash ());
	rpc_ctx.io_scope->reset ();
	ASSERT_EQ (scendere::process_result::progress, node1->process (change).code);
	scendere::send_block send2 (send.hash (), key.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *node1->work_generate_blocking (send.hash ()));
	ASSERT_EQ (scendere::process_result::progress, node1->process (send2).code);
	rpc_ctx.io_scope->renew ();
	boost::property_tree::ptree request2;
	...
	request2.put ("work", scendere::to_string_hex (*node1->work_generate_blocking (change.hash ())));
	...
	auto receive_block (scendere::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
	request.put ("previous", scendere::dev::genesis->hash ().to_string ());
	request.put ("representative", scendere::dev::genesis_key.pub.to_account ());
	request.put ("balance", (scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", scendere::to_string_hex (*node->work_generate_blocking (scendere::dev::genesis->hash ())));
	auto response (wait_response (system, rpc_ctx, request));
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (scendere::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (scendere::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	rpc_ctx.io_scope->reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (scendere::process_result::progress, process_result.code);
}
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("representative", scendere::dev::genesis_key.pub.to_account ());
	request.put ("balance", scendere::Gxrb_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", scendere::to_string_hex (*node->work_generate_blocking (key.pub)));
	auto response (wait_response (system, rpc_ctx, request));
	std::string state_hash (response.get<std::string> ("hash"));
	auto state_text (response.get<std::string> ("block"));
	std::stringstream block_stream (state_text);
	...
	ASSERT_EQ (scendere::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (scendere::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, scendere::dev::network_params.work.threshold (state_block->work_version (), scendere::block_details (scendere::epoch::epoch_0, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	rpc_ctx.io_scope->reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (scendere::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, scendere::epoch::epoch_0);
...
TEST (rpc, block_create_state_request_work)
{
	...
	std::unique_ptr<scendere::state_block> epoch2;
	{
		scendere::system system (1);
		system.upgrade_genesis_epoch (*system.nodes.front (), scendere::epoch::epoch_1);
		epoch2 = system.upgrade_genesis_epoch (*system.nodes.front (), scendere::epoch::epoch_2);
	}
	...
	for (auto previous : previous_test_input)
	{
		...
		boost::property_tree::read_json (block_stream, block_l);
		auto block (scendere::deserialize_block_json (block_l));
		ASSERT_NE (nullptr, block);
		ASSERT_GE (scendere::dev::network_params.work.difficulty (*block), node->default_difficulty (scendere::work_version::work_1));
	}
}
...
TEST (rpc, block_create_open_epoch_v2)
{
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (scendere::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, scendere::dev::network_params.work.threshold (state_block->work_version (), scendere::block_details (scendere::epoch::epoch_2, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	rpc_ctx.io_scope->reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (scendere::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, scendere::epoch::epoch_2);
	ASSERT_TRUE (state_block->sideband ().details.is_receive);
	ASSERT_FALSE (node->latest (key.pub).is_zero ());
}
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	auto send_block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	ASSERT_NE (nullptr, send_block);
	scendere::state_block open (key.pub, 0, scendere::dev::genesis_key.pub, scendere::Gxrb_ratio, send_block->hash (), key.prv, key.pub, *node->work_generate_blocking (key.pub));
	ASSERT_EQ (scendere::process_result::progress, node->process (open).code);
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, scendere::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	...
	request.put ("representative", scendere::dev::genesis_key.pub.to_account ());
	request.put ("balance", (2 * scendere::Gxrb_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_send_epoch_v2)
{
	...
	request.put ("link", scendere::dev::genesis_key.pub.to_string ());
	auto response (wait_response (system, rpc_ctx, request));
	std::string state_hash (response.get<std::string> ("hash"));
	auto state_text (response.get<std::string> ("block"));
	std::stringstream block_stream (state_text);
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (scendere::dev::network_params.work.difficulty (*state_block));
	ASSERT_GT (difficulty, scendere::dev::network_params.work.threshold (state_block->work_version (), scendere::block_details (scendere::epoch::epoch_2, true, false, false)));
	rpc_ctx.io_scope->reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (scendere::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, scendere::epoch::epoch_2);
...
TEST (rpc, wallet_create_fail)
{
	...
	for (int i = 0; i < 127; i++)
	{
		node->wallets.create (scendere::random_wallet_id ());
	}
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "wallet_create");
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (scendere::error_common::wallet_lmdb_max_dbs).message (), response.get<std::string> ("error"));
}
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (scendere::dev::genesis_key.pub));

	// Make sure using special wallet key as pubkey fails
	scendere::public_key bad_key (1);
	...
	std::error_code ec (scendere::error_common::bad_public_key);
...
TEST (rpc, online_reps)
{
	...
	ASSERT_EQ (node2->online_reps.online (), scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio);
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), item->second.get<std::string> (""));
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), item2->first);
	auto weight2 (item2->second.get<std::string> ("weight"));
	ASSERT_EQ (node2->weight (scendere::dev::genesis_key.pub).convert_to<std::string> (), weight2);
	//Test accounts filter
	rpc_ctx.io_scope->reset ();
	auto new_rep (system.wallet (1)->deterministic_insert ());
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, new_rep, node1->config.receive_minimum.number ()));
	rpc_ctx.io_scope->renew ();
	ASSERT_NE (nullptr, send);
	...
	rpc_ctx.io_scope->reset ();
	auto change (system.wallet (0)->change_action (scendere::dev::genesis_key.pub, new_rep));
	rpc_ctx.io_scope->renew ();
	ASSERT_NE (nullptr, change);
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (scendere::dev::genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		scendere::keypair key1;
		scendere::send_block send (previous_genesis_chain_hash, key1.pub, scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio - 1, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
	...
	std::shared_ptr<scendere::block> frontier;
	{
		auto transaction = node->store.tx_begin_read ();
	...
	// Begin process for confirming the block (and setting confirmation height)
	{
		...
		auto write_guard = node->write_database_queue.wait (scendere::writer::testing);
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		std::error_code ec (scendere::error_rpc::confirmation_height_not_processing);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	...
	scendere::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == scendere::dev::constants.genesis_amount || tally_num == (scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	ASSERT_TRUE (node->active.list_recently_cemented ().empty ());
	auto send1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	auto send3 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, scendere::Gxrb_ratio));
	...
	scendere::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == scendere::dev::constants.genesis_amount || tally_num == (scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio) || tally_num == (scendere::dev::constants.genesis_amount - 2 * scendere::Gxrb_ratio) || tally_num == (scendere::dev::constants.genesis_amount - 3 * scendere::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm_confirmed)
{
	...
	config.peering_port = scendere::get_available_port ();
	config.callback_address = "localhost";
	config.callback_port = scendere::get_available_port ();
	config.callback_target = "/";
	config.logging.init (path);
	auto node = add_ipc_enabled_node (system, config);
	{
		auto transaction (node->store.tx_begin_read ());
		ASSERT_TRUE (node->ledger.block_confirmed (transaction, scendere::dev::genesis->hash ()));
	}
	ASSERT_EQ (0, node->stats.count (scendere::stat::type::error, scendere::stat::detail::http_callback, scendere::stat::dir::out));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "block_confirm");
	request.put ("hash", scendere::dev::genesis->hash ().to_string ());
	...
	ASSERT_EQ (scendere::dev::genesis->hash (), confirmed.begin ()->winner->hash ());
	// Check callback
	ASSERT_TIMELY (10s, node->stats.count (scendere::stat::type::error, scendere::stat::detail::http_callback, scendere::stat::dir::out) != 0);
	// Callback result is error because callback target port isn't listening
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::error, scendere::stat::detail::http_callback, scendere::stat::dir::out));
	node->stop ();
}
...
TEST (rpc, stats_clear)
{
	...
	auto const rpc_ctx = add_rpc (system, node);
	scendere::keypair key;
	node->stats.inc (scendere::stat::type::ledger, scendere::stat::dir::in);
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::ledger, scendere::stat::dir::in));
	boost::property_tree::ptree request;
	request.put ("action", "stats_clear");
	auto response (wait_response (system, rpc_ctx, request));
	std::string success (response.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_EQ (0, node->stats.count (scendere::stat::type::ledger, scendere::stat::dir::in));
...
TEST (rpc, unchecked)
{
	...
	auto const rpc_ctx = add_rpc (system, node);
	scendere::keypair key{};
	auto open = std::make_shared<scendere::state_block> (key.pub, 0, key.pub, 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	auto open2 = std::make_shared<scendere::state_block> (key.pub, 0, key.pub, 2, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
...
TEST (rpc, unchecked_get)
{
	...
	request.put ("hash", open->hash ().to_string ());
	{
		...
		ASSERT_LE (timestamp, scendere::seconds_since_epoch ());
	}
	request.put ("json_block", true);
	{
		auto response (wait_response (system, rpc_ctx, request));
...
TEST (rpc, unopened)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::account account1 (1), account2 (account1.number () + 1);
	auto genesis (node->latest (scendere::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, account2, 10));
	ASSERT_NE (nullptr, send2);
	auto const rpc_ctx = add_rpc (system, node);
	{
		boost::property_tree::ptree request;
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", scendere::account (account2.number () + 1).to_account ());
		auto response (wait_response (system, rpc_ctx, request));
		auto & accounts (response.get_child ("accounts"));
...
TEST (rpc, unopened_burn)
{
	...
	auto genesis (node->latest (scendere::dev::genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::constants.burn_account, 1));
	ASSERT_NE (nullptr, send);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "unopened");
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3512
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3514
TEST (rpc, DISABLED_wallet_history)
{
	...
	auto node = add_ipc_enabled_node (system, node_config);
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto timestamp1 (scendere::seconds_since_epoch ());
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto timestamp2 (scendere::seconds_since_epoch ());
	auto receive (system.wallet (0)->receive_action (send->hash (), scendere::dev::genesis_key.pub, node->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	scendere::keypair key;
	auto timestamp3 (scendere::seconds_since_epoch ());
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	system.deadline_set (10s);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<4> (history_l[0]));
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<4> (history_l[1]));
	...
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<4> (history_l[2]));
	ASSERT_EQ (std::to_string (timestamp1), std::get<5> (history_l[2]));
	// Genesis block
	ASSERT_EQ ("receive", std::get<0> (history_l[3]));
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	ASSERT_EQ (scendere::dev::constants.genesis_amount.convert_to<std::string> (), std::get<2> (history_l[3]));
	ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), std::get<3> (history_l[3]));
	ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), std::get<4> (history_l[3]));
}
...
TEST (rpc, sign_hash)
{
	...
	request.put ("key", key.prv.to_string ());
	auto response (wait_response (system, rpc_ctx, request, 10s));
	std::error_code ec (scendere::error_rpc::sign_hash_disabled);
	ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	...
	ASSERT_FALSE (scendere::validate_message (key.pub, send.hash (), signature));
}
...
TEST (rpc, memory_stats)
{
	...
	auto vote (std::make_shared<scendere::vote> (key.pub, key.prv, 0, 0, hashes));
...
TEST (rpc, block_confirmed)
{
	...
	auto response (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (scendere::error_blocks::invalid_block_hash).message (), response.get<std::string> ("error"));

	request.put ("hash", "0");
	auto response1 (wait_response (system, rpc_ctx, request));
	ASSERT_EQ (std::error_code (scendere::error_blocks::not_found).message (), response1.get<std::string> ("error"));
	...
	// Open an account directly in the ledger
	{
		...
		scendere::send_block send1 (latest, key.pub, 300, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (scendere::process_result::progress, node->ledger.process (transaction, send1).code);

		scendere::open_block open1 (send1.hash (), scendere::dev::genesis->account (), key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	auto send = std::make_shared<scendere::send_block> (latest, key.pub, 10, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
...
TEST (rpc, database_txn_tracker)
{
	...
	// First try when database tracking is disabled
	{
		...
		{
			auto response (wait_response (system, rpc_ctx, request));
			std::error_code ec (scendere::error_common::tracking_not_enabled);
			ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &rpc_ctx, &request] () {
		auto response (wait_response (system, rpc_ctx, request));
		std::error_code ec (scendere::error_common::invalid_amount);
		ASSERT_EQ (response.get<std::string> ("error"), ec.message ());
	};
	...
	std::thread thread ([&store = node->store, &keep_txn_alive_promise, &txn_created_promise] () {
		...
		scendere::thread_role::set (scendere::thread_role::name::rpc_process_container);
	...
	auto thread_name = scendere::thread_role::get_string (scendere::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	...
	auto node = add_ipc_enabled_node (system);
	auto const rpc_ctx = add_rpc (system, node);
	ASSERT_EQ (node->default_difficulty (scendere::work_version::work_1), node->network_params.work.epoch_2);
	boost::property_tree::ptree request;
	...
	auto expected_multiplier{ 1.0 };
	{
		...
		ASSERT_FALSE (scendere::from_string_hex (network_minimum_text, network_minimum));
		ASSERT_EQ (node->default_difficulty (scendere::work_version::work_1), network_minimum);
		auto network_receive_minimum_text (response.get<std::string> ("network_receive_minimum"));
		uint64_t network_receive_minimum;
		ASSERT_FALSE (scendere::from_string_hex (network_receive_minimum_text, network_receive_minimum));
		ASSERT_EQ (node->default_receive_difficulty (scendere::work_version::work_1), network_receive_minimum);
		auto multiplier (response.get<double> ("multiplier"));
		...
		ASSERT_FALSE (scendere::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (scendere::difficulty::from_multiplier (expected_multiplier, node->default_difficulty (scendere::work_version::work_1)), network_current);
		auto network_receive_current_text (response.get<std::string> ("network_receive_current"));
		uint64_t network_receive_current;
		ASSERT_FALSE (scendere::from_string_hex (network_receive_current_text, network_receive_current));
		auto network_receive_current_multiplier (scendere::difficulty::to_multiplier (network_receive_current, network_receive_minimum));
		auto network_receive_current_normalized_multiplier (scendere::dev::network_params.work.normalized_multiplier (network_receive_current_multiplier, network_receive_minimum));
...
TEST (rpc, simultaneous_calls)
{
	...
	scendere::thread_runner runner (system.io_ctx, node->config.io_threads);
	scendere::node_rpc_config node_rpc_config;
	scendere::ipc::ipc_server ipc_server (*node, node_rpc_config);
	scendere::rpc_config rpc_config{ scendere::dev::network_params.network, scendere::get_available_port (), true };
	const auto ipc_tcp_port = ipc_server.listening_tcp_port ();
	ASSERT_TRUE (ipc_tcp_port.has_value ());
	rpc_config.rpc_process.num_ipc_connections = 8;
	scendere::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config, ipc_tcp_port.value ());
	scendere::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_block_count");
	request.put ("account", scendere::dev::genesis_key.pub.to_account ());
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (scendere::dev::genesis_key.pub.to_account ());
	account_text[4] = '-';
	request.put ("account", account_text);
	auto response2 (wait_response (system, rpc_ctx, request));
	std::string frontier (response2.get<std::string> ("frontier"));
	ASSERT_EQ (scendere::dev::genesis->hash ().to_string (), frontier);
...
TEST (rpc, epoch_upgrade)
{
	...
	scendere::keypair epoch_signer (scendere::dev::genesis_key);
	auto send1 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, scendere::dev::genesis->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 1, key1.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::dev::genesis->hash ()))); // to opened account
	ASSERT_EQ (scendere::process_result::progress, node->process (*send1).code);
	auto send2 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, send1->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 2, key2.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1->hash ()))); // to unopened account (pending)
	ASSERT_EQ (scendere::process_result::progress, node->process (*send2).code);
	auto send3 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, send2->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 3, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send2->hash ()))); // to burn (0)
	ASSERT_EQ (scendere::process_result::progress, node->process (*send3).code);
	scendere::account max_account (std::numeric_limits<scendere::uint256_t>::max ());
	auto send4 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, send3->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 4, max_account, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send3->hash ()))); // to max account
	ASSERT_EQ (scendere::process_result::progress, node->process (*send4).code);
	auto open (std::make_shared<scendere::state_block> (key1.pub, 0, key1.pub, 1, send1->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (scendere::process_result::progress, node->process (*open).code);
	// Check accounts epochs
	{
		...
		{
			scendere::account_info info (i->second);
			ASSERT_EQ (info.epoch (), scendere::epoch::epoch_0);
		}
	}
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	// Check upgrade
	{
		...
		ASSERT_TRUE (node->store.account.exists (transaction, std::numeric_limits<scendere::uint256_t>::max ()));
		ASSERT_FALSE (node->store.account.exists (transaction, 0));
	}

	rpc_ctx.io_scope->reset ();
	// Epoch 2 upgrade
	auto genesis_latest (node->latest (scendere::dev::genesis_key.pub));
	auto send5 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, genesis_latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 5, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (genesis_latest))); // to burn (0)
	ASSERT_EQ (scendere::process_result::progress, node->process (*send5).code);
	auto send6 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, send5->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 6, key1.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send5->hash ()))); // to key1 (again)
	ASSERT_EQ (scendere::process_result::progress, node->process (*send6).code);
	auto key1_latest (node->latest (key1.pub));
	auto send7 (std::make_shared<scendere::state_block> (key1.pub, key1_latest, key1.pub, 0, key3.pub, key1.prv, key1.pub, *system.work.generate (key1_latest))); // to key3
	ASSERT_EQ (scendere::process_result::progress, node->process (*send7).code);
	{
		// Check pending entry
		auto transaction (node->store.tx_begin_read ());
		scendere::pending_info info;
		ASSERT_FALSE (node->store.pending.get (transaction, scendere::pending_key (key3.pub, send7->hash ()), info));
	...
	// Check upgrade
	{
		...
		{
			scendere::account_info info (i->second);
			ASSERT_EQ (info.epoch (), scendere::epoch::epoch_2);
		}
		ASSERT_TRUE (node->store.account.exists (transaction, key1.pub));
		ASSERT_TRUE (node->store.account.exists (transaction, key2.pub));
...
TEST (rpc, account_lazy_start)
{
	...
	auto send1 = std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, scendere::dev::genesis->hash (), scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::dev::genesis->hash ()));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*send1).code);
...
TEST (rpc, receive)
{
	...
	auto send1 (wallet->send_action (scendere::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number (), *node->work_generate_blocking (scendere::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, node->balance (scendere::dev::genesis_key.pub) != scendere::dev::constants.genesis_amount);
	ASSERT_TIMELY (10s, !node->store.account.exists (node->store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet->send_action (scendere::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (send1->hash ())));
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	...
	request.put ("block", send2->hash ().to_string ());
	{
		...
		ASSERT_EQ (info.head, scendere::block_hash{ receive_text });
	}
	// Trying to receive the same block should fail with unreceivable
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (scendere::error_process::unreceivable).message (), response.get<std::string> ("error"));
	}
	// Trying to receive a non-existing block should fail
	request.put ("block", scendere::block_hash (send2->hash ().number () + 1).to_string ());
	{
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (std::error_code (scendere::error_blocks::not_found).message (), response.get<std::string> ("error"));
	}
...
TEST (rpc, receive_unopened)
{
	...
	scendere::keypair key1;
	auto send1 (wallet->send_action (scendere::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (scendere::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, !node->balance (scendere::dev::genesis_key.pub) != scendere::dev::constants.genesis_amount);
	...
	request.put ("block", send1->hash ().to_string ());
	{
		...
		ASSERT_EQ (info.representative, scendere::dev::genesis_key.pub);
	}
	rpc_ctx.io_scope->reset ();

	// Test receiving for an unopened with a different wallet representative
	scendere::keypair key2;
	auto prev_amount (node->balance (scendere::dev::genesis_key.pub));
	auto send2 (wallet->send_action (scendere::dev::genesis_key.pub, key2.pub, node->config.receive_minimum.number () - 1, *node->work_generate_blocking (send1->hash ())));
	ASSERT_TIMELY (5s, !node->balance (scendere::dev::genesis_key.pub) != prev_amount);
...
TEST (rpc, receive_work_disabled)
{
	...
	auto send1 (wallet->send_action (scendere::dev::genesis_key.pub, key1.pub, node->config.receive_minimum.number () - 1, *worker_node.work_generate_blocking (scendere::dev::genesis->hash ()), false));
...
TEST (rpc, receive_pruned)
{
	...
	wallet1->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::keypair key1;
	wallet2->insert_adhoc (key1.prv);
	auto send1 (wallet1->send_action (scendere::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number (), *node2->work_generate_blocking (scendere::dev::genesis->hash ())));
	ASSERT_TIMELY (5s, node2->balance (scendere::dev::genesis_key.pub) != scendere::dev::constants.genesis_amount);
	ASSERT_TIMELY (10s, !node2->store.account.exists (node2->store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet1->send_action (scendere::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number () - 1, *node2->work_generate_blocking (send1->hash ())));
	// Extra send frontier
	auto send3 (wallet1->send_action (scendere::dev::genesis_key.pub, key1.pub, node2->config.receive_minimum.number (), *node2->work_generate_blocking (send1->hash ())));
	// Pruning
...
TEST (rpc, telemetry_single)
{
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (scendere::error_rpc::requires_port_and_address).message (), response.get<std::string> ("error"));
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (scendere::error_common::invalid_ip_address).message (), response.get<std::string> ("error"));
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (scendere::error_common::invalid_port).message (), response.get<std::string> ("error"));
	}
	...
	request.put ("port", node->network.endpoint ().port ());
	{
		...
		scendere::jsonconfig config (response);
		...
		scendere::compare_default_telemetry_response_data (telemetry_data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (scendere::work_version::work_1), node->node_id);
	}
}
...
TEST (rpc, telemetry_all)
{
	...
	std::atomic<bool> done{ false };
	auto node = system.nodes.front ();
	node1->telemetry->get_metrics_single_peer_async (node1->network.find_channel (node->network.endpoint ()), [&done] (scendere::telemetry_data_response const & telemetry_data_response_a) {
	...
	request.put ("action", "telemetry");
	{
		...
		ASSERT_FALSE (telemetry_data.deserialize_json (config, should_ignore_identification_metrics));
		scendere::compare_default_telemetry_response_data_excluding_signature (telemetry_data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (scendere::work_version::work_1));
	...
	scendere::jsonconfig config (metrics);
	scendere::telemetry_data data;
	auto const should_ignore_identification_metrics = false;
	ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
	scendere::compare_default_telemetry_response_data (data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (scendere::work_version::work_1), node->node_id);
...
TEST (rpc, telemetry_self)
{
	...
	node1->network.udp_channels.insert (scendere::endpoint (boost::asio::ip::make_address_v6 ("::1"), scendere::test_node_port ()), 0);
	...
	auto const should_ignore_identification_metrics = false;
	{
		...
		scendere::jsonconfig config (response);
		ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
		scendere::compare_default_telemetry_response_data (data, node1->network_params, node1->config.bandwidth_limit, node1->default_difficulty (scendere::work_version::work_1), node1->node_id);
	}
	...
	{
		auto response (wait_response (system, rpc_ctx, request, 10s));
		ASSERT_EQ (std::error_code (scendere::error_rpc::peer_not_found).message (), response.get<std::string> ("error"));
	}
}
...
TEST (rpc, confirmation_active)
{
	...
	auto send1 (std::make_shared<scendere::send_block> (scendere::dev::genesis->hash (), scendere::public_key (), scendere::dev::constants.genesis_amount - 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::dev::genesis->hash ())));
	auto send2 (std::make_shared<scendere::send_block> (send1->hash (), scendere::public_key (), scendere::dev::constants.genesis_amount - 200, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1->hash ())));
	node1->process_active (send1);
...
TEST (rpc, confirmation_info)
{
	...
	auto send (std::make_shared<scendere::send_block> (scendere::dev::genesis->hash (), scendere::public_key (), scendere::dev::constants.genesis_amount - 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::dev::genesis->hash ())));
