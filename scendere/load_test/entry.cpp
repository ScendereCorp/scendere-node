...
#include <scendere/boost/asio/connect.hpp>
#include <scendere/boost/asio/ip/tcp.hpp>
#include <scendere/boost/asio/spawn.hpp>
...
void write_config_files (boost::filesystem::path const & data_path, int index)
{
	...
	toml.write (scendere::get_node_toml_config_path (data_path));

	scendere::rpc_config rpc_config{ daemon_config.node.network_params.network };
	rpc_config.port = rpc_port_start + index;
	rpc_config.enable_control = true;
	rpc_config.rpc_process.ipc_port = ipc_port_start + index;

	// Write rpc config
	scendere::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (scendere::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the scendere_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the scendere_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "scendere_node";
	...
	{
		std::cerr << "scendere_node executable could not be found in " << node_path << std::endl;
		return 1;
	}
	...
	{
		auto rpc_filepath = running_executable_filepath.parent_path () / "scendere_rpc";
		if (running_executable_filepath.has_extension ())
		{
	...
	{
		std::cerr << "scendere_rpc executable could not be found in " << rpc_path << std::endl;
		return 1;
	}
	...
	{
		auto data_path = scendere::unique_path ();
		boost::filesystem::create_directory (data_path);
		write_config_files (data_path, i);
		data_paths.push_back (std::move (data_path));
	}

	auto current_network = scendere::dev::network_params.network.get_current_network_as_string ();
	...
	scendere::signal_handler_impl = [&ioc] () {
		ioc.stop ();
	};

	std::signal (SIGINT, &scendere::signal_handler);
	std::signal (SIGTERM, &scendere::signal_handler);
	...
	std::thread t ([send_count, &ioc, &primary_node_results, &resolver, &node_count, &destination_count] () {
		...
		wallet_add_rpc (ioc, primary_node_results, wallet, scendere::dev::genesis_key.prv.to_string ());
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			boost::asio::spawn (ioc, [&ioc, &primary_node_results, &wallet, destination_account, &send_calls_remaining] (boost::asio::yield_context yield) {
				send_receive (ioc, wallet, scendere::dev::genesis->account ().to_account (), destination_account->as_string, send_calls_remaining, primary_node_results, yield);
			});
		}
	...
	scendere::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
