...
TEST (bulk_pull, no_address)
{
	...
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	auto req = std::make_unique<scendere::bulk_pull> (scendere::dev::network_params.network);
	req->start = 1;
	req->end = 2;
	connection->requests.push (std::unique_ptr<scendere::message>{});
	auto request (std::make_shared<scendere::bulk_pull_server> (connection, std::move (req)));
...
TEST (bulk_pull, genesis_to_end)
{
	...
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	auto req = std::make_unique<scendere::bulk_pull> (scendere::dev::network_params.network);
	req->start = scendere::dev::genesis_key.pub;
	req->end.clear ();
	connection->requests.push (nullptr);
	auto request (std::make_shared<scendere::bulk_pull_server> (connection, std::move (req)));
	ASSERT_EQ (system.nodes[0]->latest (scendere::dev::genesis_key.pub), request->current);
...
TEST (bulk_pull, end_not_owned)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 100));
	scendere::block_hash latest (system.nodes[0]->latest (scendere::dev::genesis_key.pub));
	scendere::open_block open (0, 1, 2, scendere::keypair ().prv, 4, 5);
	open.hashables.account = key2.pub;
	open.hashables.representative = key2.pub;
	open.hashables.source = latest;
	open.refresh ();
	open.signature = scendere::sign_message (key2.prv, key2.pub, open.hash ());
	system.nodes[0]->work_generate_blocking (open);
	ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->process (open).code);
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	auto req = std::make_unique<scendere::bulk_pull> (scendere::dev::network_params.network);
	req->start = key2.pub;
	req->end = scendere::dev::genesis->hash ();
	connection->requests.push (std::unique_ptr<scendere::message>{});
	auto request (std::make_shared<scendere::bulk_pull_server> (connection, std::move (req)));
	ASSERT_EQ (request->current, request->request->end);
}
...
TEST (bulk_pull, by_block)
{
	...
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	auto req = std::make_unique<scendere::bulk_pull> (scendere::dev::network_params.network);
	req->start = scendere::dev::genesis->hash ();
	req->end.clear ();
	connection->requests.push (std::unique_ptr<scendere::message>{});
	auto request (std::make_shared<scendere::bulk_pull_server> (connection, std::move (req)));
	auto block (request->get_next ());
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (block->hash (), scendere::dev::genesis->hash ());
...
TEST (bulk_pull, count_limit)
{
	...
	auto send1 (std::make_shared<scendere::send_block> (node0->latest (scendere::dev::genesis_key.pub), scendere::dev::genesis_key.pub, 1, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (node0->latest (scendere::dev::genesis_key.pub))));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*send1).code);
	auto receive1 (std::make_shared<scendere::receive_block> (send1->hash (), send1->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*receive1).code);

	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node0, scendere::socket::endpoint_type_t::server), node0));
	auto req = std::make_unique<scendere::bulk_pull> (scendere::dev::network_params.network);
...
TEST (bootstrap_processor, process_one)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, 100));
	ASSERT_NE (nullptr, send);

	node_config.peering_port = scendere::get_available_port ();
	node_flags.disable_rep_crawler = true;
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), node_config, system.work, node_flags));
	scendere::block_hash hash1 (node0->latest (scendere::dev::genesis_key.pub));
	scendere::block_hash hash2 (node1->latest (scendere::dev::genesis_key.pub));
	ASSERT_NE (hash1, hash2);
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_NE (node1->latest (scendere::dev::genesis_key.pub), node0->latest (scendere::dev::genesis_key.pub));
	ASSERT_TIMELY (10s, node1->latest (scendere::dev::genesis_key.pub) == node0->latest (scendere::dev::genesis_key.pub));
...
TEST (bootstrap_processor, process_two)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::block_hash hash1 (node0->latest (scendere::dev::genesis_key.pub));
	ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, 50));
	scendere::block_hash hash2 (node0->latest (scendere::dev::genesis_key.pub));
	ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, 50));
	scendere::block_hash hash3 (node0->latest (scendere::dev::genesis_key.pub));
	ASSERT_NE (hash1, hash2);
...
TEST (bootstrap_processor, process_state)
{
	...
	ASSERT_EQ (scendere::process_result::progress, node0->process (*block1).code);
	ASSERT_EQ (scendere::process_result::progress, node0->process (*block2).code);
	...
	ASSERT_EQ (node0->latest (scendere::dev::genesis_key.pub), block2->hash ());
	ASSERT_NE (node1->latest (scendere::dev::genesis_key.pub), block2->hash ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_NE (node1->latest (scendere::dev::genesis_key.pub), node0->latest (scendere::dev::genesis_key.pub));
	ASSERT_TIMELY (10s, node1->latest (scendere::dev::genesis_key.pub) == node0->latest (scendere::dev::genesis_key.pub));
...
TEST (bootstrap_processor, process_new)
{
	...
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, node1->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	ASSERT_TIMELY (10s, !node1->balance (key2.pub).is_zero ());
	auto receive (node2->block (node2->latest (key2.pub)));
	ASSERT_NE (nullptr, receive);
	scendere::uint128_t balance1 (node1->balance (scendere::dev::genesis_key.pub));
	scendere::uint128_t balance2 (node1->balance (key2.pub));
	...
	auto node3 (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work));
	...
	ASSERT_EQ (balance1, node3->balance (scendere::dev::genesis_key.pub));
	node3->stop ();
}
...
TEST (bootstrap_processor, pull_diamond)
{
	...
	auto send1 (std::make_shared<scendere::send_block> (node0->latest (scendere::dev::genesis_key.pub), key.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (node0->latest (scendere::dev::genesis_key.pub))));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*send1).code);
	auto open (std::make_shared<scendere::open_block> (send1->hash (), 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*open).code);
	auto send2 (std::make_shared<scendere::send_block> (open->hash (), scendere::dev::genesis_key.pub, std::numeric_limits<scendere::uint128_t>::max () - 100, key.prv, key.pub, *system.work.generate (open->hash ())));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*send2).code);
	auto receive (std::make_shared<scendere::receive_block> (send1->hash (), send2->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (scendere::process_result::progress, node0->process (*receive).code);
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work));
	ASSERT_FALSE (node1->init_error ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node1->balance (scendere::dev::genesis_key.pub) == 100);
	ASSERT_EQ (100, node1->balance (scendere::dev::genesis_key.pub));
	node1->stop ();
}
...
TEST (bootstrap_processor, DISABLED_pull_requeue_network_error)
{
	...
	{
		scendere::unique_lock<scendere::mutex> lock (node1->bootstrap_initiator.connections->mutex);
		ASSERT_FALSE (attempt->stopped);
		++attempt->pulling;
		node1->bootstrap_initiator.connections->pulls.emplace_back (scendere::dev::genesis_key.pub, send1->hash (), scendere::dev::genesis->hash (), attempt->incremental_id);
		node1->bootstrap_initiator.connections->request_pull (lock);
		node2->stop ();
	}
	ASSERT_TIMELY (5s, attempt == nullptr || attempt->requeued_pulls == 1);
	ASSERT_EQ (0, node1->stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_failed_account, scendere::stat::dir::in)); // Requeue is not increasing failed attempts
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3558
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3559
// CI run in which it failed: https://github.com/scenderecurrency/scendere-node/runs/4280675502?check_suite_focus=true#step:6:398
TEST (bootstrap_processor, DISABLED_push_diamond)
{
	...
	auto receive (std::make_shared<scendere::receive_block> (send1->hash (), send2->hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*receive).code);
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (scendere::dev::genesis_key.pub) == 100);
	ASSERT_EQ (100, node0->balance (scendere::dev::genesis_key.pub));
	node1->stop ();
}
...
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3517
TEST (bootstrap_processor, DISABLED_push_diamond_pruning)
{
	...
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), config, system.work, node_flags, 1));
	ASSERT_FALSE (node1->init_error ());
	auto latest (node0->latest (scendere::dev::genesis_key.pub));
	auto send1 (std::make_shared<scendere::send_block> (latest, key.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest)));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*send1).code);
	auto open (std::make_shared<scendere::open_block> (send1->hash (), 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*open).code);
	// 1st bootstrap
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (key.pub) == scendere::dev::constants.genesis_amount);
...
TEST (bootstrap_processor, push_one)
{
	...
	scendere::keypair key1;
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work));
	auto wallet (node1->wallets.create (scendere::random_wallet_id ()));
	ASSERT_NE (nullptr, wallet);
	wallet->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::uint128_t balance1 (node1->balance (scendere::dev::genesis_key.pub));
	auto send (wallet->send_action (scendere::dev::genesis_key.pub, key1.pub, 100));
	ASSERT_NE (nullptr, send);
	ASSERT_NE (balance1, node1->balance (scendere::dev::genesis_key.pub));
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (scendere::dev::genesis_key.pub) != balance1);
	node1->stop ();
}
...
TEST (bootstrap_processor, lazy_hash_pruning)
{
	...
	scendere::blocks_confirm (*node1, { send1, receive1, change1, change2 }, true);
...
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3640
TEST (bootstrap_processor, DISABLED_lazy_unclear_state_link)
{
	...
	ASSERT_EQ (scendere::process_result::progress, node1->process (*receive).code);
	// Start lazy bootstrap with last block in chain known
	auto node2 = system.add_node (scendere::node_config (scendere::get_available_port (), system.logging), node_flags);
	...
	ASSERT_EQ (0, node2->stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_failed_account, scendere::stat::dir::in));
}
...
TEST (bootstrap_processor, lazy_unclear_state_link_not_existing)
{
	...
	ASSERT_TIMELY (5s, node2->ledger.block_or_pruned_exists (open->hash ()));
	ASSERT_TIMELY (5s, node2->ledger.block_or_pruned_exists (send2->hash ()));
	ASSERT_EQ (1, node2->stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_failed_account, scendere::stat::dir::in));
}
...
TEST (bootstrap_processor, DISABLED_lazy_destinations)
{
	...
	auto send2 = builder
				 ...
				 .build_shared ();
	ASSERT_EQ (scendere::process_result::progress, node1->process (*send2).code);
	auto open (std::make_shared<scendere::open_block> (send1->hash (), key1.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (scendere::process_result::progress, node1->process (*open).code);
	auto state_open = builder
					  ...
					  .build_shared ();
	ASSERT_EQ (scendere::process_result::progress, node1->process (*state_open).code);
...
TEST (bootstrap_processor, lazy_pruning_missing_block)
{
	...
	// Confirm last block to prune previous
	scendere::blocks_confirm (*node1, { send1, send2, open, state_open }, true);
	...
	// Start lazy bootstrap with last block in sender chain
	config.peering_port = scendere::get_available_port ();
	auto node2 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), config, system.work, node_flags, 1));
	node2->network.udp_channels.insert (node1->network.endpoint (), node1->network_params.network.protocol_version);
	node2->bootstrap_initiator.bootstrap_lazy (send2->hash ());
	// Check processed blocks
	...
	ASSERT_FALSE (node2->ledger.block_or_pruned_exists (state_open->hash ()));
	{
		...
		ASSERT_TRUE (node2->unchecked.exists (transaction, scendere::unchecked_key (send2->root ().as_block_hash (), send2->hash ())));
	}
...
TEST (bootstrap_processor, multiple_attempts)
{
	...
	auto node2 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), node_config, system.work));
...
{
	{
		std::shared_ptr<scendere::frontier_req_server> hold; // Destructing tcp acceptor on non-existent io_context
		{
			scendere::system system (1);
			auto connection (std::make_shared<scendere::bootstrap_server> (nullptr, system.nodes[0]));
			auto req = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
			req->start.clear ();
			req->age = std::numeric_limits<decltype (req->age)>::max ();
			req->count = std::numeric_limits<decltype (req->count)>::max ();
			connection->requests.push (std::unique_ptr<scendere::message>{});
			hold = std::make_shared<scendere::frontier_req_server> (connection, std::move (req));
		}
	}
	ASSERT_TRUE (true);
}
...
TEST (frontier_req, begin)
{
	...
	req->count = std::numeric_limits<decltype (req->count)>::max ();
	connection->requests.push (std::unique_ptr<scendere::message>{});
	auto request (std::make_shared<scendere::frontier_req_server> (connection, std::move (req)));
	ASSERT_EQ (scendere::dev::genesis_key.pub, request->current);
	ASSERT_EQ (scendere::dev::genesis->hash (), request->frontier);
}
...
TEST (frontier_req, end)
{
	...
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	auto req = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	req->start = scendere::dev::genesis_key.pub.number () + 1;
	req->age = std::numeric_limits<decltype (req->age)>::max ();
	req->count = std::numeric_limits<decltype (req->count)>::max ();
	connection->requests.push (std::unique_ptr<scendere::message>{});
	auto request (std::make_shared<scendere::frontier_req_server> (connection, std::move (req)));
...
TEST (frontier_req, count)
{
	...
	scendere::keypair key1 ("ED5AE0A6505B14B67435C29FD9FEEBC26F597D147BC92F6D795FFAD7AFD3D967");
	scendere::state_block_builder builder;
	...
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	req->start.clear ();
	req->age = std::numeric_limits<decltype (req->age)>::max ();
	req->count = 1;
...
TEST (frontier_req, time_bound)
{
	...
	auto req2 (std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network));
	req2->start.clear ();
	req2->age = 1;
	req2->count = std::numeric_limits<decltype (req2->count)>::max ();
	auto connection2 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));
	connection2->requests.push (std::unique_ptr<scendere::message>{});
	auto request2 (std::make_shared<scendere::frontier_req_server> (connection, std::move (req2)));
...
TEST (frontier_req, confirmed_frontier)
{
	scendere::system system (1);
	auto node1 = system.nodes[0];
	scendere::keypair key_before_genesis;
	// Public key before genesis in accounts table
	while (key_before_genesis.pub.number () >= scendere::dev::genesis_key.pub.number ())
	{
		key_before_genesis = scendere::keypair ();
	}
	scendere::keypair key_after_genesis;
	// Public key after genesis in accounts table
	while (key_after_genesis.pub.number () <= scendere::dev::genesis_key.pub.number ())
	{
		key_after_genesis = scendere::keypair ();
	}
	scendere::state_block_builder builder;
	...
	ASSERT_EQ (scendere::process_result::progress, node1->process (*receive2).code);
	...
	req->header.flag_set (scendere::message_header::frontier_req_only_confirmed);
	...
	auto connection2 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req2 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	...
	req2->header.flag_set (scendere::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req2->header.frontier_req_is_only_confirmed_present ());
	connection2->requests.push (std::unique_ptr<scendere::message>{});
	auto request2 (std::make_shared<scendere::frontier_req_server> (connection2, std::move (req2)));
	ASSERT_EQ (scendere::dev::genesis_key.pub, request2->current);
	ASSERT_EQ (scendere::dev::genesis->hash (), request2->frontier);

	// Request starting with account after genesis (confirmed only)
	auto connection3 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req3 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	...
	req3->header.flag_set (scendere::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req3->header.frontier_req_is_only_confirmed_present ());
	connection3->requests.push (std::unique_ptr<scendere::message>{});
	auto request3 (std::make_shared<scendere::frontier_req_server> (connection3, std::move (req3)));
	ASSERT_TRUE (request3->current.is_zero ());
	ASSERT_TRUE (request3->frontier.is_zero ());

	// Request for all accounts (unconfirmed blocks)
	auto connection4 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req4 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	req4->start.clear ();
	...
	connection4->requests.push (std::unique_ptr<scendere::message>{});
	auto request4 (std::make_shared<scendere::frontier_req_server> (connection4, std::move (req4)));
	...
	auto connection5 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req5 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	...
	connection5->requests.push (std::unique_ptr<scendere::message>{});
	auto request5 (std::make_shared<scendere::frontier_req_server> (connection5, std::move (req5)));
	...
	scendere::blocks_confirm (*node1, { send1, receive1 }, true);
	ASSERT_TIMELY (5s, node1->block_confirmed (send1->hash ()) && node1->block_confirmed (receive1->hash ()));
	auto connection6 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req6 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	...
	req6->header.flag_set (scendere::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req6->header.frontier_req_is_only_confirmed_present ());
	connection6->requests.push (std::unique_ptr<scendere::message>{});
	auto request6 (std::make_shared<scendere::frontier_req_server> (connection6, std::move (req6)));
	...
	scendere::blocks_confirm (*node1, { send2, receive2 }, true);
	ASSERT_TIMELY (5s, node1->block_confirmed (send2->hash ()) && node1->block_confirmed (receive2->hash ()));
	auto connection7 (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*node1, scendere::socket::endpoint_type_t::server), node1));
	auto req7 = std::make_unique<scendere::frontier_req> (scendere::dev::network_params.network);
	...
	req7->header.flag_set (scendere::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req7->header.frontier_req_is_only_confirmed_present ());
	connection7->requests.push (std::unique_ptr<scendere::message>{});
	auto request7 (std::make_shared<scendere::frontier_req_server> (connection7, std::move (req7)));
...
TEST (bulk, genesis)
{
	...
	scendere::keypair key2;
	auto send (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send);
	scendere::block_hash latest3 (node1->latest (scendere::dev::genesis_key.pub));
	ASSERT_NE (latest1, latest3);

	node2->bootstrap_initiator.bootstrap (node1->network.endpoint (), false);
	ASSERT_TIMELY (10s, node2->latest (scendere::dev::genesis_key.pub) == node1->latest (scendere::dev::genesis_key.pub));
	ASSERT_EQ (node2->latest (scendere::dev::genesis_key.pub), node1->latest (scendere::dev::genesis_key.pub));
	node2->stop ();
}
...
TEST (bulk, offline_send)
{
	...
	scendere::keypair key2;
	auto wallet (node2->wallets.create (scendere::random_wallet_id ()));
	wallet->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, node1->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	ASSERT_NE (std::numeric_limits<scendere::uint256_t>::max (), node1->balance (scendere::dev::genesis_key.pub));
	node1->block_processor.flush ();
	...
	ASSERT_TIMELY (10s, node2->balance (scendere::dev::genesis_key.pub) != std::numeric_limits<scendere::uint256_t>::max ());
...
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3613
TEST (bulk, DISABLED_genesis_pruning)
{
	...
	ASSERT_EQ (latest1, latest2);
	scendere::keypair key2;
	auto send1 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send2);
	auto send3 (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 100));
	...
	ASSERT_TIMELY (25s, node2->stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::initiate, scendere::stat::dir::out) >= 1 && !node2->bootstrap_initiator.in_progress ());
...
TEST (bulk_pull_account, basics)
{
	...
	auto send1 (system.wallet (0)->send_action (scendere::dev::genesis->account (), key1.pub, 25));
	auto send2 (system.wallet (0)->send_action (scendere::dev::genesis->account (), key1.pub, 10));
	auto send3 (system.wallet (0)->send_action (scendere::dev::genesis->account (), key1.pub, 2));
	ASSERT_TIMELY (5s, system.nodes[0]->balance (key1.pub) == 25);
	auto connection (std::make_shared<scendere::bootstrap_server> (std::make_shared<scendere::socket> (*system.nodes[0], scendere::socket::endpoint_type_t::server), system.nodes[0]));

	{
		auto req = std::make_unique<scendere::bulk_pull_account> (scendere::dev::network_params.network);
		req->account = key1.pub;
		req->minimum_amount = 5;
		req->flags = scendere::bulk_pull_account_flags ();
		connection->requests.push (std::unique_ptr<scendere::message>{});
		auto request (std::make_shared<scendere::bulk_pull_account_server> (connection, std::move (req)));
		...
		ASSERT_EQ (scendere::uint128_union (10), block_data.second.get ()->amount);
		ASSERT_EQ (scendere::dev::genesis->account (), block_data.second.get ()->source);
...

	{
		...
		req->flags = scendere::bulk_pull_account_flags::pending_address_only;
		auto request (std::make_shared<scendere::bulk_pull_account_server> (connection, std::move (req)));
