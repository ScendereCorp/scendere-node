...
#include <scendere/boost/asio/ip/network_v6.hpp>
...
TEST (socket, max_connections)
{
	...
	auto server_port = scendere::get_available_port ();
	...
	auto server_socket = std::make_shared<scendere::server_socket> (*node, listen_endpoint, 2);
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);

	// successful incoming connections are stored in server_sockets to keep them alive (server side)
	std::vector<std::shared_ptr<scendere::socket>> server_sockets;
	server_socket->on_connection ([&server_sockets] (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const & ec_a) {
	...
	auto client1 = std::make_shared<scendere::client_socket> (*node);
	client1->async_connect (dst_endpoint, connect_handler);

	auto client2 = std::make_shared<scendere::client_socket> (*node);
	client2->async_connect (dst_endpoint, connect_handler);

	auto client3 = std::make_shared<scendere::client_socket> (*node);
	client3->async_connect (dst_endpoint, connect_handler);

	auto get_tcp_accept_failures = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_failure, scendere::stat::dir::in);
	};

	auto get_tcp_accept_successes = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_success, scendere::stat::dir::in);
	};
	...
	auto client4 = std::make_shared<scendere::client_socket> (*node);
	client4->async_connect (dst_endpoint, connect_handler);

	auto client5 = std::make_shared<scendere::client_socket> (*node);
	...
	auto client6 = std::make_shared<scendere::client_socket> (*node);
	client6->async_connect (dst_endpoint, connect_handler);

	auto client7 = std::make_shared<scendere::client_socket> (*node);
	client7->async_connect (dst_endpoint, connect_handler);

	auto client8 = std::make_shared<scendere::client_socket> (*node);
...
TEST (socket, max_connections_per_ip)
{
	...
	auto server_socket = std::make_shared<scendere::server_socket> (*node, listen_endpoint, max_global_connections);
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	...
	std::vector<std::shared_ptr<scendere::socket>> client_list;
	client_list.reserve (max_ip_connections + 1);

	for (auto idx = 0; idx < max_ip_connections + 1; ++idx)
	{
		auto client = std::make_shared<scendere::client_socket> (*node);
		client->async_connect (dst_endpoint, connect_handler);
		client_list.push_back (client);
	}

	auto get_tcp_max_per_ip = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_ip, scendere::stat::dir::in);
	};
...
TEST (socket, limited_subnet_address)
{
	...
	auto network = scendere::socket_functions::get_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
{
	auto address = boost::asio::ip::make_address ("a41d:b7b2:8298:cf45:672e:bd1a:e7fb:f713");
	auto first_address = scendere::socket_functions::first_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
{
	auto address = boost::asio::ip::make_address ("a41d:b7b2:8298:cf45:672e:bd1a:e7fb:f713");
	auto last_address = scendere::socket_functions::last_ipv6_subnet_address (address.to_v6 (), 32); // network prefix = 32.
...
TEST (socket, count_subnetwork_connections)
{
	...
	auto connection0 = std::make_shared<scendere::client_socket> (*node);
	auto connection1 = std::make_shared<scendere::client_socket> (*node);
	auto connection2 = std::make_shared<scendere::client_socket> (*node);
	auto connection3 = std::make_shared<scendere::client_socket> (*node);
	auto connection4 = std::make_shared<scendere::client_socket> (*node);
	auto connection5 = std::make_shared<scendere::client_socket> (*node);
	auto connection6 = std::make_shared<scendere::client_socket> (*node);

	scendere::address_socket_mmap connections_per_address;
	...
	ASSERT_EQ (4, scendere::socket_functions::count_subnetwork_connections (connections_per_address, address1.to_v6 (), 32));
}
...
TEST (socket, max_connections_per_subnetwork)
{
	...
	auto get_tcp_max_per_subnetwork = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_subnetwork, scendere::stat::dir::in);
	};
...
TEST (socket, disconnection_of_silent_connections)
{
	...
	auto server_socket = std::make_shared<scendere::server_socket> (*node, listen_endpoint, 1);
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	...
	std::shared_ptr<scendere::socket> server_data_socket;
	server_socket->on_connection ([&server_data_socket] (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const & ec_a) {
		server_data_socket = new_connection;
		return true;
	});

	// Instantiates a client to simulate an incoming connection.
	auto client_socket = std::make_shared<scendere::client_socket> (*node);
	...
	auto get_tcp_io_timeout_drops = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_io_timeout_drop, scendere::stat::dir::in);
	};
	auto get_tcp_silent_connection_drops = [&node] () {
		return node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_silent_connection_drop, scendere::stat::dir::in);
	};
...
TEST (socket, drop_policy)
{
	...
	scendere::inactive_node inactivenode (scendere::unique_path (), node_flags);
	auto node = inactivenode.node;

	scendere::thread_runner runner (node->io_ctx, 1);

	std::vector<std::shared_ptr<scendere::socket>> connections;

	auto func = [&] (size_t total_message_count, scendere::buffer_drop_policy drop_policy) {
		auto server_port (scendere::get_available_port ());
		boost::asio::ip::tcp::endpoint endpoint (boost::asio::ip::address_v6::any (), server_port);

		auto server_socket = std::make_shared<scendere::server_socket> (*node, endpoint, 1);
		boost::system::error_code ec;
		server_socket->start (ec);
		ASSERT_FALSE (ec);

		// Accept connection, but don't read so the writer will drop.
		server_socket->on_connection ([&connections] (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const & ec_a) {
			connections.push_back (new_connection);
			return true;
		});

		auto client = std::make_shared<scendere::client_socket> (*node);
		scendere::transport::channel_tcp channel{ *node, client };
		scendere::util::counted_completion write_completion (static_cast<unsigned> (total_message_count));
		...
		[&channel, total_message_count, node, &write_completion, &drop_policy, client] (boost::system::error_code const & ec_a) mutable {
			for (int i = 0; i < total_message_count; i++)
			{
				...
				channel.send_buffer (
				scendere::shared_const_buffer (std::move (buff)), [&write_completion, client] (boost::system::error_code const & ec, size_t size_a) mutable {
					client.reset ();
	...
	func (scendere::socket::queue_size_max * 2 + 1, scendere::buffer_drop_policy::no_socket_drop);
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_no_socket_drop, scendere::stat::dir::out));
	ASSERT_EQ (0, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_drop, scendere::stat::dir::out));

	func (scendere::socket::queue_size_max + 1, scendere::buffer_drop_policy::limiter);
	// The stats are accumulated from before
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_no_socket_drop, scendere::stat::dir::out));
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_drop, scendere::stat::dir::out));
...
TEST (socket, concurrent_writes)
{
	...
	scendere::util::counted_completion read_count_completion (total_message_count);
	std::function<void (std::shared_ptr<scendere::socket> const &)> reader = [&read_count_completion, &total_message_count, &reader] (std::shared_ptr<scendere::socket> const & socket_a) {
...
#pragma GCC diagnostic pop
	...
	auto server_socket = std::make_shared<scendere::server_socket> (*node, endpoint, max_connections);
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	std::vector<std::shared_ptr<scendere::socket>> connections;
	...
	server_socket->on_connection ([&connections, &reader] (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const & ec_a) {
		if (ec_a)
		{
	...
	scendere::util::counted_completion connection_count_completion (client_count);
	std::vector<std::shared_ptr<scendere::socket>> clients;
...
#pragma GCC diagnostic ignored "-Wunused-lambda-capture"
		...
		client_threads.emplace_back ([&client, &message_count] () {
			for (int i = 0; i < message_count; i++)
			{
				...
				buff.push_back ('A' + i);
				client->async_write (scendere::shared_const_buffer (std::move (buff)));
			}
		});
#ifndef _WIN32
#pragma GCC diagnostic pop
	...
	ASSERT_EQ (node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_success, scendere::stat::dir::in), client_count);
	// We may exhaust max connections and have some tcp accept failures, but no more than the client count
	ASSERT_LT (node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_failure, scendere::stat::dir::in), client_count);
...
TEST (socket_timeout, connect)
{
	...
	std::shared_ptr<scendere::node> node = system.nodes[0];
	...
	boost::asio::ip::tcp::endpoint endpoint (boost::asio::ip::make_address_v6 ("::ffff:10.255.254.253"), scendere::get_available_port ());
	...
	ASSERT_TRUE (ec);
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_connect_error, scendere::stat::dir::in));

	// check that the socket was closed due to tcp_io_timeout timeout
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_io_timeout_drop, scendere::stat::dir::out));
}
...
TEST (socket_timeout, read)
{
	...
	boost::asio::ip::tcp::endpoint endpoint (boost::asio::ip::address_v6::loopback (), scendere::get_available_port ());
	...
	ASSERT_TRUE (ec);
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_read_error, scendere::stat::dir::in));
...
TEST (socket_timeout, write)
{
	...
	socket->async_connect (endpoint, [&socket, &ec, &done] (boost::system::error_code const & ec_a) {
		...
		{
			socket->async_write (scendere::shared_const_buffer{ buffer }, [&ec, &done] (boost::system::error_code const & ec_a, size_t size_a) {
				if (ec_a)
				{
					ec = ec_a;
					done = true;
	...
	ASSERT_TIMELY (10s, done == true);
	ASSERT_TRUE (ec);
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_error, scendere::stat::dir::in));
...
TEST (socket_timeout, read_overlapped)
{
	...
	acceptor.async_accept (newsock, [&newsock] (boost::system::error_code const & ec_a) {
		debug_assert (!ec_a);
		auto buffer = std::make_shared<std::vector<uint8_t>> (1);
		scendere::async_write (newsock, scendere::shared_const_buffer (buffer), [] (boost::system::error_code const & ec_a, size_t size_a) {
			debug_assert (!ec_a);
...
TEST (socket_timeout, write_overlapped)
{
	...
	socket->async_connect (endpoint, [&socket, &ec, &done] (boost::system::error_code const & ec_a) {
		...
		socket->async_write (scendere::shared_const_buffer{ buffer1 }, [] (boost::system::error_code const & ec_a, size_t size_a) {
			debug_assert (size_a == 1);
		});
		for (auto i = 0; i < 1024; ++i)
		{
			socket->async_write (scendere::shared_const_buffer{ buffer2 }, [&ec, &done] (boost::system::error_code const & ec_a, size_t size_a) {
				if (ec_a)
				{
					ec = ec_a;
					done = true;
