...
unsigned constexpr scendere::wallet_store::version_current;

TEST (wallet, no_special_keys_accounts)
{
	bool init;
	scendere::mdb_env env (init, scendere::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	scendere::wallet_store wallet (init, kdf, transaction, scendere::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (init);
	...
	for (uint64_t account = 0; account < scendere::wallet_store::special_count; account++)
	{
		scendere::account account_l (account);
...
TEST (wallet, one_item_iteration)
{
	...
	{
		ASSERT_EQ (key1.pub, scendere::uint256_union (i->first));
		scendere::raw_key password;
		wallet.wallet_key (password, transaction);
		scendere::raw_key key;
		key.decrypt (scendere::wallet_value (i->second).key, password, (scendere::uint256_union (i->first)).owords[0].number ());
...
TEST (wallet, two_item_iteration)
{
	...
	std::unordered_set<scendere::public_key> pubs;
	std::unordered_set<scendere::raw_key> prvs;
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	{
		auto transaction (env.tx_begin_write ());
		scendere::wallet_store wallet (init, kdf, transaction, scendere::dev::genesis->account (), 1, "0");
		...
		for (auto i (wallet.begin (transaction)), j (wallet.end ()); i != j; ++i)
		{
			...
			wallet.wallet_key (password, transaction);
			scendere::raw_key key;
			key.decrypt (scendere::wallet_value (i->second).key, password, (i->first).owords[0].number ());
			prvs.insert (key);
		}
	}
...
TEST (wallet, insufficient_spend_one)
{
	...
	auto block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, 500));
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key1.pub, scendere::dev::constants.genesis_amount));
}
...
TEST (wallet, spend_all_one)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, std::numeric_limits<scendere::uint128_t>::max ()));
	scendere::account_info info2;
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.store.account.get (transaction, scendere::dev::genesis_key.pub, info2);
...
TEST (wallet, send_async)
{
	...
	std::thread thread ([&system] () {
		ASSERT_TIMELY (10s, system.nodes[0]->balance (scendere::dev::genesis_key.pub).is_zero ());
	});
	std::atomic<bool> success (false);
	system.wallet (0)->send_async (scendere::dev::genesis_key.pub, key2.pub, std::numeric_limits<scendere::uint128_t>::max (), [&success] (std::shared_ptr<scendere::block> const & block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
...
TEST (wallet, change)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->change_action (scendere::dev::genesis_key.pub, key2.pub));
	auto block2 (system.nodes[0]->rep_block (scendere::dev::genesis_key.pub));
...
TEST (wallet, partial_spend)
{
	...
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	scendere::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<scendere::uint128_t>::max () - 500, system.nodes[0]->balance (scendere::dev::genesis_key.pub));
}
...
TEST (wallet, spend_no_previous)
{
	scendere::system system (1);
	{
		...
		ASSERT_FALSE (system.nodes[0]->store.account.get (transaction, scendere::dev::genesis_key.pub, info1));
		for (auto i (0); i < 50; ++i)
		{
...
TEST (account, encode_zero)
{
	 ...
	 */
	//TO-CHANGE NUMEROS CARACTERES DA HASH 60 + 5 (4 letras + underscore) scen_
	ASSERT_EQ (65, str0.size ());
	ASSERT_EQ (65, str0.size ());
	scendere::account number1;
...
{
	scendere::raw_key value0 (0);
	scendere::fan fan (value0, 1024);
	for (auto & i : fan.values)
	{
		ASSERT_NE (value0, *i);
	}
	scendere::raw_key value1;
	fan.value (value1);
...
{
	auto error (false);
	scendere::mdb_env env (error, scendere::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	scendere::wallet_store wallet (error, kdf, transaction, scendere::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (scendere::dev::genesis->account (), wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	scendere::keypair key;
...
TEST (wallet, serialize_json_empty)
{
	...
	auto transaction (env.tx_begin_write ());
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	scendere::wallet_store wallet1 (error, kdf, transaction, scendere::dev::genesis->account (), 1, "0");
	ASSERT_FALSE (error);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	scendere::wallet_store wallet2 (error, kdf, transaction, scendere::dev::genesis->account (), 1, "1", serialized);
	ASSERT_FALSE (error);
	scendere::raw_key password1;
	scendere::raw_key password2;
...
TEST (wallet_store, move)
{
	...
	scendere::wallet_store wallet2 (error, kdf, transaction, scendere::dev::genesis->account (), 1, "1");
	ASSERT_FALSE (error);
	scendere::keypair key2;
	...
	std::vector<scendere::public_key> keys;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		...
		uint64_t work (0);
		if (!wallet->store.work_get (transaction, scendere::dev::genesis_key.pub, work))
		{
			done = scendere::dev::network_params.work.difficulty (scendere::dev::genesis->work_version (), scendere::dev::genesis->hash (), work) >= system.nodes[0]->default_difficulty (scendere::dev::genesis->work_version ());
		}
...
TEST (wallet, work_generate)
{
	...
	auto & node1 (*system.nodes[0]);
	auto wallet (system.wallet (0));
	scendere::uint128_t amount1 (node1.balance (scendere::dev::genesis_key.pub));
	uint64_t work1;
	wallet->insert_adhoc (scendere::dev::genesis_key.prv);
	...
	scendere::keypair key;
	auto block (wallet->send_action (scendere::dev::genesis_key.pub, key.pub, 100));
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_TIMELY (10s, node1.ledger.account_balance (transaction, scendere::dev::genesis_key.pub) != amount1);
	...
	while (again)
	{
		...
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		again = wallet->store.work_get (transaction, account1, work1) || scendere::dev::network_params.work.difficulty (block->work_version (), node1.ledger.latest_root (block_transaction, account1), work1) < node1.default_difficulty (block->work_version ());
	}
}
...
TEST (wallet, work_cache_delayed)
{
	...
	scendere::keypair key;
	auto block1 (wallet->send_action (scendere::dev::genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block1->hash (), node1.latest (scendere::dev::genesis_key.pub));
	auto block2 (wallet->send_action (scendere::dev::genesis_key.pub, key.pub, 100));
	ASSERT_EQ (block2->hash (), node1.latest (scendere::dev::genesis_key.pub));
	ASSERT_EQ (block2->hash (), node1.wallets.delayed_work->operator[] (scendere::dev::genesis_key.pub));
	auto threshold (node1.default_difficulty (scendere::work_version::work_1));
	auto again (true);
	system.deadline_set (10s);
	while (again)
	{
		...
		{
			again = scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, block2->hash (), work1) < threshold;
		}
	}
	ASSERT_GE (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, block2->hash (), work1), threshold);
}
...
TEST (wallet, insert_locked)
{
	...
	ASSERT_TRUE (wallet->insert_adhoc (scendere::keypair ().prv).is_zero ());
}
...
TEST (wallet, reseed)
{
	...
	scendere::kdf kdf{ scendere::dev::network_params.kdf_work };
	scendere::wallet_store wallet (init, kdf, transaction, scendere::dev::genesis->account (), 1, "0");
	scendere::raw_key seed1;
	seed1 = 1;
	scendere::raw_key seed2;
	seed2 = 2;
	wallet.seed_set (transaction, seed1);
	scendere::raw_key seed3;
...
TEST (wallet, no_work)
{
	...
	scendere::keypair key2;
	auto block (system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, std::numeric_limits<scendere::uint128_t>::max (), false));
	ASSERT_NE (nullptr, block);
	ASSERT_NE (0, block->block_work ());
	ASSERT_GE (scendere::dev::network_params.work.difficulty (*block), scendere::dev::network_params.work.threshold (block->work_version (), block->sideband ().details));
	auto transaction (system.wallet (0)->wallets.tx_begin_read ());
	uint64_t cached_work (0);
	system.wallet (0)->store.work_get (transaction, scendere::dev::genesis_key.pub, cached_work);
...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key2.pub, scendere::Gxrb_ratio));
		ASSERT_EQ (scendere::dev::constants.genesis_amount - scendere::Gxrb_ratio * i, system.nodes[0]->balance (scendere::dev::genesis_key.pub));
	}
}
...
TEST (wallet, change_seed)
{
	...
	uint32_t index (4);
	auto prv = scendere::deterministic_key (seed1, index);
	pub = scendere::pub_key (prv);
	wallet->insert_adhoc (scendere::dev::genesis_key.prv, false);
	auto block (wallet->send_action (scendere::dev::genesis_key.pub, pub, 100));
	ASSERT_NE (nullptr, block);
...
TEST (wallet, epoch_2_validation)
{
	...
	{
		auto send = wallet.send_action (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.pub, amount, 1);
		ASSERT_NE (nullptr, send);
		ASSERT_EQ (scendere::epoch::epoch_2, send->sideband ().details.epoch);
		ASSERT_EQ (scendere::epoch::epoch_0, send->sideband ().source_epoch); // Not used for send state blocks

		auto receive = wallet.receive_action (send->hash (), scendere::dev::genesis_key.pub, amount, send->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive);
		if (scendere::dev::network_params.work.difficulty (*receive) < node.network_params.work.base)
		{
			ASSERT_GE (scendere::dev::network_params.work.difficulty (*receive), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (scendere::epoch::epoch_2, receive->sideband ().details.epoch);
			ASSERT_EQ (scendere::epoch::epoch_2, receive->sideband ().source_epoch);
			break;
		}
	}
	...
	// Test a change block
	ASSERT_NE (nullptr, wallet.change_action (scendere::dev::genesis_key.pub, scendere::keypair ().pub, 1));
}
...
TEST (wallet, epoch_2_receive_propagation)
{
	...
	while (++tries < max_tries)
	{
		...
		auto amount = node.config.receive_minimum.number ();
		auto send1 = wallet.send_action (scendere::dev::genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send1);
		ASSERT_NE (nullptr, wallet.receive_action (send1->hash (), scendere::dev::genesis_key.pub, amount, send1->link ().as_account (), 1));
		...
		// Send a block
		auto send2 = wallet.send_action (scendere::dev::genesis_key.pub, key.pub, amount, 1);
		ASSERT_NE (nullptr, send2);

		auto receive2 = wallet.receive_action (send2->hash (), key.pub, amount, send2->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive2);
		if (scendere::dev::network_params.work.difficulty (*receive2) < node.network_params.work.base)
		{
			ASSERT_GE (scendere::dev::network_params.work.difficulty (*receive2), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (scendere::epoch::epoch_2, node.store.block.version (node.store.tx_begin_read (), receive2->hash ()));
			ASSERT_EQ (scendere::epoch::epoch_2, receive2->sideband ().source_epoch);
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
...
TEST (wallet, epoch_2_receive_unopened)
{
	...
	while (++tries < max_tries)
	{
		...
		auto epoch2_unopened = scendere::state_block (key.pub, 0, 0, 0, node.network_params.ledger.epochs.link (scendere::epoch::epoch_2), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (key.pub, node.network_params.work.epoch_2));
		ASSERT_EQ (scendere::process_result::progress, node.process (epoch2_unopened).code);

		wallet.insert_adhoc (key.prv, false);

		auto receive1 = wallet.receive_action (send1->hash (), key.pub, amount, send1->link ().as_account (), 1);
		ASSERT_NE (nullptr, receive1);
		if (scendere::dev::network_params.work.difficulty (*receive1) < node.network_params.work.base)
		{
			ASSERT_GE (scendere::dev::network_params.work.difficulty (*receive1), node.network_params.work.epoch_2_receive);
			ASSERT_EQ (scendere::epoch::epoch_2, node.store.block.version (node.store.tx_begin_read (), receive1->hash ()));
			ASSERT_EQ (scendere::epoch::epoch_1, receive1->sideband ().source_epoch);
			break;
		}
	}
	ASSERT_LT (tries, max_tries);
...
TEST (wallet, foreach_representative_deadlock)
{
	...
	bool set = false;
	node.wallets.foreach_representative ([&node, &set, &system] (scendere::public_key const & pub, scendere::raw_key const & prv) {
		node.wallets.foreach_representative ([&node, &set, &system] (scendere::public_key const & pub, scendere::raw_key const & prv) {
...
TEST (wallet, receive_pruned)
{
	...
	// Send
	wallet1.insert_adhoc (scendere::dev::genesis_key.prv, false);
	auto amount = node2.config.receive_minimum.number ();
	auto send1 = wallet1.send_action (scendere::dev::genesis_key.pub, key.pub, amount, 1);
	auto send2 = wallet1.send_action (scendere::dev::genesis_key.pub, key.pub, 1, 1);
