...
TEST (election_scheduler, activate_one_timely)
{
	...
	system.nodes[0]->scheduler.activate (scendere::dev::genesis_key.pub, system.nodes[0]->store.tx_begin_read ());
...
TEST (election_scheduler, no_vacancy)
{
	...
	auto block1 = builder.make_block ()
				  ...
				  .build_shared ();
	ASSERT_EQ (scendere::process_result::progress, node.process (*block1).code);
	...
	auto block2 = builder.make_block ()
				  ...
				  .build_shared ();
	ASSERT_EQ (scendere::process_result::progress, node.process (*block2).code);
