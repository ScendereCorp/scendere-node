...
TEST (rate, basic)
{
	scendere::rate::token_bucket bucket (10, 10);
...
TEST (rate, network)
{
	...
	scendere::rate::token_bucket bucket (10, 5);
...
TEST (rate, reset)
{
	scendere::rate::token_bucket bucket (0, 0);
...
TEST (optional_ptr, basic)
{
	...
	scendere::optional_ptr<valtype> opt;
	ASSERT_FALSE (opt);
...
TEST (thread, thread_pool)
{
	...
	scendere::thread_pool workers (1u, scendere::thread_role::name::unknown);
...
TEST (thread_pool_alarm, one)
{
	...
	scendere::unique_lock<scendere::mutex> unique (mutex);
	condition.wait (unique, [&] () { return !!done; });
}

TEST (thread_pool_alarm, many)
{
	scendere::thread_pool workers (50u, scendere::thread_role::name::unknown);
...
TEST (filesystem, remove_all_files)
{
	...
	scendere::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	scendere::move_all_files_to_dir (dummy_directory, path);
...
{
	scendere::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);
