...
TEST (telemetry, consolidate_data)
{
	...
	data.uptime = 6;
	data.genesis_block = scendere::block_hash (3);
	...
	data1.uptime = 10;
	data1.genesis_block = scendere::block_hash (4);
	...
	data2.uptime = 3;
	data2.genesis_block = scendere::block_hash (4);
	...
	std::vector<scendere::telemetry_data> all_data{ data, data1, data2 };

	auto consolidated_telemetry_data = scendere::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data.genesis_block, scendere::block_hash (4));
	...
	all_data[2].genesis_block = scendere::block_hash (3);
	...
	auto consolidated_telemetry_data1 = scendere::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data1.genesis_block, scendere::block_hash (3));
...
TEST (telemetry, consolidate_data_remove_outliers)
{
	...
	std::vector<scendere::telemetry_data> all_data (20, data);

	// Insert some outliers
	scendere::telemetry_data lower_bound_outlier_data;
	...
	lower_bound_outlier_data.genesis_block = scendere::block_hash (2);
	...
	scendere::telemetry_data upper_bound_outlier_data;
	...
	upper_bound_outlier_data.genesis_block = scendere::block_hash (99);
...
TEST (telemetry, consolidate_data_remove_outliers_with_zero_bandwidth)
{
	...
	data1.uptime = 6;
	data1.genesis_block = scendere::block_hash (3);
	...
	std::vector<scendere::telemetry_data> all_data (100, data1);
	...
	data2.uptime = 6;
	data2.genesis_block = scendere::block_hash (3);
	data2.major_version = 20;
	data2.minor_version = 1;
	...
	auto consolidated_telemetry_data2 = scendere::consolidate_telemetry_data (all_data);
...
TEST (telemetry, basic)
{
	...
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	{
		...
		node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &server_endpoint, &telemetry_data] (scendere::telemetry_data_response const & response_a) {
	...
	scendere::compare_default_telemetry_response_data (telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (scendere::work_version::work_1), node_server->node_id);
	...
	std::this_thread::sleep_for (scendere::telemetry_cache_cutoffs::dev);
...
TEST (telemetry, receive_from_non_listening_channel)
{
	...
	scendere::telemetry_ack message{ scendere::dev::network_params.network, scendere::telemetry_data{} };
...
TEST (telemetry, over_udp)
{
	...
	std::atomic<bool> done{ false };
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &node_server] (scendere::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		scendere::compare_default_telemetry_response_data (response_a.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (scendere::work_version::work_1), node_server->node_id);
		done = true;
	});
...
TEST (telemetry, invalid_channel)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (nullptr, [&done] (scendere::telemetry_data_response const & response_a) {
...
TEST (telemetry, blocking_request)
{
	...
	scendere::compare_default_telemetry_response_data (telemetry_data_response.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (scendere::work_version::work_1), node_server->node_id);
...
TEST (telemetry, disconnects)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done] (scendere::telemetry_data_response const & response_a) {
		ASSERT_TRUE (response_a.error);
		done = true;
	});
...
TEST (telemetry, dos_tcp)
{
	...
	scendere::telemetry_req message{ scendere::dev::network_params.network };
	auto channel = node_client->network.tcp_channels.find_channel (scendere::transport::map_endpoint_to_tcp (node_server->network.endpoint ()));
	channel->send (message, [] (boost::system::error_code const & ec, size_t size_a) {
		ASSERT_FALSE (ec);
	});

	ASSERT_TIMELY (10s, 1 == node_server->stats.count (scendere::stat::type::message, scendere::stat::detail::telemetry_req, scendere::stat::dir::in));
	...
	ASSERT_TIMELY (10s, (scendere::telemetry_cache_cutoffs::dev + orig) <= std::chrono::steady_clock::now ());
	...
	while (node_server->stats.count (scendere::stat::type::message, scendere::stat::detail::telemetry_req, scendere::stat::dir::in) == 1)
	{
...
TEST (telemetry, dos_udp)
{
	...
	ASSERT_TIMELY (20s, 1 == node_server->stats.count (scendere::stat::type::message, scendere::stat::detail::telemetry_req, scendere::stat::dir::in));
	...
	ASSERT_TIMELY (20s, (scendere::telemetry_cache_cutoffs::dev + orig) <= std::chrono::steady_clock::now ());
...
TEST (telemetry, disable_metrics)
{
	...
	node_server->telemetry->get_metrics_single_peer_async (channel1, [&done, node_client] (scendere::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		scendere::compare_default_telemetry_response_data (response_a.telemetry_data, node_client->network_params, node_client->config.bandwidth_limit, node_client->default_difficulty (scendere::work_version::work_1), node_client->node_id);
		done = true;
	});
...
TEST (telemetry, max_possible_size)
{
	...
	scendere::telemetry_data data;
	data.unknown_data.resize (scendere::message_header::telemetry_size_mask.to_ulong () - scendere::telemetry_data::latest_size);

	scendere::telemetry_ack message{ scendere::dev::network_params.network, data };
	wait_peer_connections (system);
	...
	ASSERT_TIMELY (10s, 1 == node_server->stats.count (scendere::stat::type::message, scendere::stat::detail::telemetry_ack, scendere::stat::dir::in));
}
...
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3524
TEST (telemetry, DISABLED_remove_peer_different_genesis)
{
	...
	scendere::network_params network_params{ scendere::networks::scendere_dev_network };
	network_params.ledger.genesis = network_params.ledger.scendere_live_genesis;
	scendere::node_config config{ network_params };
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), config, system.work));
	node1->start ();
	system.nodes.push_back (node1);
	node0->network.merge_peer (node1->network.endpoint ());
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (10s, node0->stats.count (scendere::stat::type::telemetry, scendere::stat::detail::different_genesis_hash) != 0 && node1->stats.count (scendere::stat::type::telemetry, scendere::stat::detail::different_genesis_hash) != 0);

	ASSERT_TIMELY (1s, 0 == node0->network.size ());
	ASSERT_TIMELY (1s, 0 == node1->network.size ());
	ASSERT_GE (node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::out), 1);
	ASSERT_GE (node1->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::out), 1);

	scendere::lock_guard<scendere::mutex> guard (node0->network.excluded_peers.mutex);
	ASSERT_EQ (1, node0->network.excluded_peers.peers.get<scendere::peer_exclusion::tag_endpoint> ().count (node1->network.endpoint ().address ()));
	ASSERT_EQ (1, node1->network.excluded_peers.peers.get<scendere::peer_exclusion::tag_endpoint> ().count (node0->network.endpoint ().address ()));
}
...
TEST (telemetry, remove_peer_different_genesis_udp)
{
	...
	node_flags.disable_tcp_realtime = true;
	node_flags.disable_ongoing_telemetry_requests = true;
	scendere::system system (1, scendere::transport::transport_type::udp, node_flags);
	auto node0 (system.nodes[0]);
	ASSERT_EQ (0, node0->network.size ());
	scendere::network_params network_params{ scendere::networks::scendere_dev_network };
	network_params.ledger.genesis = network_params.ledger.scendere_live_genesis;
	scendere::node_config config{ network_params };
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), config, system.work, node_flags));
	node1->start ();
	system.nodes.push_back (node1);
	auto channel0 (std::make_shared<scendere::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node0->network_params.network.protocol_version));
	...
	node0->telemetry->get_metrics_single_peer_async (channel1, [&done0] (scendere::telemetry_data_response const & response_a) {
		done0 = true;
	});

	node1->telemetry->get_metrics_single_peer_async (channel0, [&done1] (scendere::telemetry_data_response const & response_a) {
		done1 = true;
	});
...
TEST (telemetry, remove_peer_invalid_signature)
{
	...
	node->telemetry->recent_or_initial_request_telemetry_data.emplace (channel->get_endpoint (), scendere::telemetry_data (), std::chrono::steady_clock::now (), true);
	...
	auto telemetry_ack = scendere::telemetry_ack{ scendere::dev::network_params.network, telemetry_data };
	node->network.inbound (telemetry_ack, channel);

	ASSERT_TIMELY (10s, node->stats.count (scendere::stat::type::telemetry, scendere::stat::detail::invalid_signature) > 0);
	ASSERT_NO_ERROR (system.poll_until_true (3s, [&node, address = channel->get_endpoint ().address ()] () -> bool {
		scendere::lock_guard<scendere::mutex> guard (node->network.excluded_peers.mutex);
		return node->network.excluded_peers.peers.get<scendere::peer_exclusion::tag_endpoint> ().count (address);
	}));
}
}
...
TEST (telemetry, maker_pruning)
{
	...
	ASSERT_EQ (scendere::telemetry_maker::nf_pruned_node, static_cast<scendere::telemetry_maker> (telemetry_data.maker));
}
