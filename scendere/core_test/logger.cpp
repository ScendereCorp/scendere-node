...
TEST (logger, changing_time_interval)
{
	...
	scendere::logger_mt my_logger (logging.min_time_between_log_output);
...
TEST (logger, try_log)
{
	auto path1 (scendere::unique_path ());
	std::stringstream ss;
	scendere::boost_log_cerr_redirect redirect_cerr (ss.rdbuf ());
	scendere::logger_mt my_logger (100ms);
...
TEST (logger, always_log)
{
	...
	std::stringstream ss;
	scendere::boost_log_cerr_redirect redirect_cerr (ss.rdbuf ());
	scendere::logger_mt my_logger (20s); // Make time interval effectively unreachable
...
TEST (logger, stable_filename)
{
	...
	scendere::logger_mt logger (logging.min_time_between_log_output);
