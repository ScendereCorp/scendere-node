...
TEST (election, quorum_minimum_flip_success)
{
	...
	auto const vote1 = std::make_shared<scendere::vote> (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::vote::timestamp_max, scendere::vote::duration_max, send2);
...
TEST (election, quorum_minimum_confirm_success)
{
	...
	auto vote1 (std::make_shared<scendere::vote> (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::vote::timestamp_max, scendere::vote::duration_max, send1));
	ASSERT_EQ (scendere::vote_code::vote, node1.active.vote (vote1));
	node1.block_processor.flush ();
...
TEST (election, quorum_minimum_update_weight_before_quorum_checks)
{
	...
	auto const amount = ((scendere::uint256_t (node_config.online_weight_minimum.number ()) * scendere::online_reps::online_weight_quorum) / 100).convert_to<scendere::uint128_t> () - 1;

	auto const latest = node1.latest (scendere::dev::genesis_key.pub);
	...
	auto const open1 = scendere::open_block_builder{}.make_block ().account (key1.pub).source (send1->hash ()).representative (key1.pub).sign (key1.prv, key1.pub).work (*system.work.generate (key1.pub)).build_shared ();
	...
	auto const vote1 = std::make_shared<scendere::vote> (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, scendere::vote::timestamp_max, scendere::vote::duration_max, send1);
	ASSERT_EQ (scendere::vote_code::vote, node1.active.vote (vote1));

	auto channel = node1.network.find_channel (node2.network.endpoint ());
	ASSERT_NE (channel, nullptr);

	auto const vote2 = std::make_shared<scendere::vote> (key1.pub, key1.prv, scendere::vote::timestamp_max, scendere::vote::duration_max, send1);
	ASSERT_TIMELY (10s, !node1.rep_crawler.response (channel, vote2));

	ASSERT_FALSE (election->confirmed ());
	{
		scendere::lock_guard<scendere::mutex> guard (node1.online_reps.mutex);
