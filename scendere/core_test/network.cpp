...
TEST (network, construction_with_specified_port)
{
	...
	auto const port = scendere::get_available_port ();
	auto const node = system.add_node (scendere::node_config{ port, system.logging });
...
TEST (network, self_discard)
{
	...
	scendere::system system (1, scendere::transport::transport_type::tcp, node_flags);
	scendere::message_buffer data;
	data.endpoint = system.nodes[0]->network.endpoint ();
	ASSERT_EQ (0, system.nodes[0]->stats.count (scendere::stat::type::error, scendere::stat::detail::bad_sender));
	system.nodes[0]->network.udp_channels.receive_action (&data);
	ASSERT_EQ (1, system.nodes[0]->stats.count (scendere::stat::type::error, scendere::stat::detail::bad_sender));
}

// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3611
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3612
TEST (network, DISABLED_send_node_id_handshake)
{
	...
	auto initial (node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in));
	auto initial_node1 (node1->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in));
	auto channel (std::make_shared<scendere::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.network.protocol_version));
	node0->network.send_keepalive (channel);
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node1->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in) != initial_node1);
	ASSERT_TIMELY (10s, node0->network.size () == 0 || node1->network.size () == 1);
	ASSERT_TIMELY (10s, node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in) == initial + 2);
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	auto initial (node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in));
	auto initial_node1 (node1->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in));
	auto initial_keepalive (node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive, scendere::stat::dir::in));
	std::weak_ptr<scendere::node> node_w (node0);
	node0->network.tcp_channels.start_tcp (node1->network.endpoint ());
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in) >= initial + 2);
	ASSERT_TIMELY (5s, node1->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in) >= initial_node1 + 2);
	ASSERT_TIMELY (5s, node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive, scendere::stat::dir::in) >= initial_keepalive + 2);
	ASSERT_TIMELY (5s, node1->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive, scendere::stat::dir::in) >= initial_keepalive + 2);
...
TEST (network, last_contacted)
{
	...
	scendere::node_config node1_config (scendere::get_available_port (), system.logging);
	node1_config.tcp_incoming_connections_max = 0; // Prevent ephemeral node1->node0 channel repacement with incoming connection
	auto node1 (std::make_shared<scendere::node> (system.io_ctx, scendere::unique_path (), node1_config, system.work));
	node1->start ();
	system.nodes.push_back (node1);

	auto channel1 = scendere::establish_tcp (system, *node1, node0->network.endpoint ());
...

	{
		...
		std::shared_ptr<scendere::socket> sock0 = channel0->socket.lock ();
		std::shared_ptr<scendere::socket> sock1 = channel1->socket.lock ();
	...
	auto keepalive_count = node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive, scendere::stat::dir::in);
	...
	ASSERT_TIMELY (3s, node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive, scendere::stat::dir::in) >= keepalive_count + 3);
...
TEST (network, multi_keepalive)
{
	...
	ASSERT_TIMELY (10s, node0->network.size () == 1 && node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive) >= 1);
	...
	ASSERT_TIMELY (10s, node1->network.size () == 2 && node0->network.size () == 2 && node2->network.size () == 2 && node0->stats.count (scendere::stat::type::message, scendere::stat::detail::keepalive) >= 2);
}
...
TEST (network, send_discarded_publish)
{
	...
	auto block (std::make_shared<scendere::send_block> (1, 1, 2, scendere::keypair ().prv, 4, *system.work.generate (scendere::root (1))));
	{
		...
		ASSERT_EQ (scendere::dev::genesis->hash (), node1.ledger.latest (transaction, scendere::dev::genesis_key.pub));
		ASSERT_EQ (scendere::dev::genesis->hash (), node2.latest (scendere::dev::genesis_key.pub));
	}
	ASSERT_TIMELY (10s, node2.stats.count (scendere::stat::type::message, scendere::stat::detail::publish, scendere::stat::dir::in) != 0);
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_EQ (scendere::dev::genesis->hash (), node1.ledger.latest (transaction, scendere::dev::genesis_key.pub));
	ASSERT_EQ (scendere::dev::genesis->hash (), node2.latest (scendere::dev::genesis_key.pub));
}
...
TEST (network, send_invalid_publish)
{
	...
	auto & node1 (*system.nodes[0]);
	auto & node2 (*system.nodes[1]);
	auto block (std::make_shared<scendere::send_block> (1, 1, 20, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::root (1))));
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.network.flood_block (block);
		ASSERT_EQ (scendere::dev::genesis->hash (), node1.ledger.latest (transaction, scendere::dev::genesis_key.pub));
...
TEST (network, send_valid_confirm_ack)
{
	...
	for (auto & type : types)
	{
		...
		scendere::send_block block2 (latest1, key2.pub, 50, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest1));
		scendere::block_hash latest2 (node2.latest (scendere::dev::genesis_key.pub));
		node1.process_active (std::make_shared<scendere::send_block> (block2));
		// Keep polling until latest block changes
		ASSERT_TIMELY (10s, node2.latest (scendere::dev::genesis_key.pub) != latest2);
		// Make sure the balance has decreased after processing the block.
		ASSERT_EQ (50, node2.balance (scendere::dev::genesis_key.pub));
	}
}
...
TEST (network, send_valid_publish)
{
	...
	for (auto & type : types)
	{
		...
		scendere::block_hash latest2 (node2.latest (scendere::dev::genesis_key.pub));
		node2.process_active (std::make_shared<scendere::send_block> (block2));
		ASSERT_TIMELY (10s, node1.stats.count (scendere::stat::type::message, scendere::stat::detail::publish, scendere::stat::dir::in) != 0);
...
TEST (network, send_insufficient_work_udp)
{
	...
	auto block (std::make_shared<scendere::send_block> (0, 1, 20, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, 0));
	scendere::publish publish{ scendere::dev::network_params.network, block };
	scendere::transport::channel_udp channel (node1.network.udp_channels, node2.network.endpoint (), node1.network_params.network.protocol_version);
	channel.send (publish, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work));
}
...
TEST (network, send_insufficient_work)
{
	...
	auto & node2 = *system.nodes[1];
	// Block zero work
	auto block1 (std::make_shared<scendere::send_block> (0, 1, 20, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, 0));
	scendere::publish publish1{ scendere::dev::network_params.network, block1 };
	auto tcp_channel (node1.network.tcp_channels.find_channel (scendere::transport::map_endpoint_to_tcp (node2.network.endpoint ())));
	tcp_channel->send (publish1, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work));
	// Legacy block work between epoch_2_recieve & epoch_1
	auto block2 (std::make_shared<scendere::send_block> (block1->hash (), 1, 20, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.work.epoch_2_receive, node1.network_params.work.epoch_1 - 1)));
	scendere::publish publish2{ scendere::dev::network_params.network, block2 };
	tcp_channel->send (publish2, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_TIMELY (10s, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work) != 1);
	ASSERT_EQ (2, node2.stats.count (scendere::stat::type::error, scendere::stat::detail::insufficient_work));
	// Legacy block work epoch_1
	auto block3 (std::make_shared<scendere::send_block> (block2->hash (), 1, 20, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (block2->hash (), node1.network_params.work.epoch_2)));
	scendere::publish publish3{ scendere::dev::network_params.network, block3 };
	tcp_channel->send (publish3, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node2.stats.count (scendere::stat::type::message, scendere::stat::detail::publish, scendere::stat::dir::in));
	ASSERT_TIMELY (10s, node2.stats.count (scendere::stat::type::message, scendere::stat::detail::publish, scendere::stat::dir::in) != 0);
	...
	auto block4 (std::make_shared<scendere::state_block> (scendere::dev::genesis_key.pub, block1->hash (), scendere::dev::genesis_key.pub, 20, 1, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.work.epoch_2_receive, node1.network_params.work.epoch_1 - 1)));
	scendere::publish publish4{ scendere::dev::network_params.network, block4 };
...
TEST (receivable_processor, confirm_insufficient_pos)
{
	scendere::system system (1);
	auto & node1 (*system.nodes[0]);
	auto block1 (std::make_shared<scendere::send_block> (scendere::dev::genesis->hash (), 0, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, 0));
	node1.work_generate_blocking (*block1);
	ASSERT_EQ (scendere::process_result::progress, node1.process (*block1).code);
	node1.scheduler.activate (scendere::dev::genesis_key.pub, node1.store.tx_begin_read ());
	scendere::keypair key1;
	auto vote (std::make_shared<scendere::vote> (key1.pub, key1.prv, 0, 0, block1));
...
TEST (receivable_processor, confirm_sufficient_pos)
{
	...
	ASSERT_EQ (scendere::process_result::progress, node1.process (*block1).code);
	node1.scheduler.activate (scendere::dev::genesis_key.pub, node1.store.tx_begin_read ());
	auto vote (std::make_shared<scendere::vote> (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, 0, 0, block1));
	scendere::confirm_ack con1{ scendere::dev::network_params.network, vote };
	node1.network.inbound (con1, node1.network.udp_channels.create (node1.network.endpoint ()));
}
...
TEST (receivable_processor, send_with_receive)
{
	...
	for (auto & type : types)
	{
		...
		scendere::block_hash latest1 (node1.latest (scendere::dev::genesis_key.pub));
		system.wallet (1)->insert_adhoc (key2.prv);
		auto block1 (std::make_shared<scendere::send_block> (latest1, key2.pub, amount - node1.config.receive_minimum.number (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest1)));
		ASSERT_EQ (amount, node1.balance (scendere::dev::genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount, node2.balance (scendere::dev::genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
		node1.process_active (block1);
		node1.block_processor.flush ();
		node2.process_active (block1);
		node2.block_processor.flush ();
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node1.balance (scendere::dev::genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node2.balance (scendere::dev::genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
...
TEST (network, receive_weight_change)
{
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<scendere::node> const & node_a) { return node_a->weight (key2.pub) == system.nodes[0]->config.receive_minimum.number (); }));
}
...
TEST (parse_endpoint, valid)
{
	...
	ASSERT_FALSE (scendere::parse_endpoint (string, endpoint));
...
TEST (parse_endpoint, invalid_port)
{
	...
	scendere::endpoint endpoint;
	ASSERT_TRUE (scendere::parse_endpoint (string, endpoint));
}
...
TEST (network, ipv6)
{
	...
	scendere::endpoint endpoint1 (address, 16384);
	std::vector<uint8_t> bytes1;
	{
		scendere::vectorstream stream (bytes1);
		scendere::write (stream, address.to_bytes ());
	}
	...
	scendere::bufferstream stream (bytes1.data (), bytes1.size ());
	auto error (scendere::try_read (stream, bytes2));
	ASSERT_FALSE (error);
	scendere::endpoint endpoint2 (boost::asio::ip::address_v6 (bytes2), 16384);
	ASSERT_EQ (endpoint1, endpoint2);
}

TEST (network, ipv6_from_ipv4)
{
	scendere::endpoint endpoint1 (boost::asio::ip::address_v4::loopback (), 16000);
	ASSERT_TRUE (endpoint1.address ().is_v4 ());
	scendere::endpoint endpoint2 (boost::asio::ip::address_v6::v4_mapped (endpoint1.address ().to_v4 ()), 16000);
...
{
	boost::asio::io_context io_ctx;
	auto port1 = scendere::get_available_port ();
	auto port2 = scendere::get_available_port ();
	scendere::endpoint endpoint1 (boost::asio::ip::address_v6::any (), port1);
	scendere::endpoint endpoint2 (boost::asio::ip::address_v4::any (), port2);
	std::array<uint8_t, 16> bytes1{};
	auto finish1 (false);
	scendere::endpoint endpoint3;
	...
	scendere::endpoint endpoint5 (boost::asio::ip::address_v4::loopback (), socket1.local_endpoint ().port ());
	scendere::endpoint endpoint6 (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4::loopback ()), socket2.local_endpoint ().port ());
...
TEST (network, reserved_address)
{
	scendere::system system (1);
	// 0 port test
	ASSERT_TRUE (scendere::transport::reserved_address (scendere::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 0)));
	// Valid address test
	ASSERT_FALSE (scendere::transport::reserved_address (scendere::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 1)));
	scendere::endpoint loopback (boost::asio::ip::make_address_v6 ("::1"), 1);
	ASSERT_FALSE (scendere::transport::reserved_address (loopback));
	scendere::endpoint private_network_peer (boost::asio::ip::make_address_v6 ("::ffff:10.0.0.0"), 1);
	ASSERT_TRUE (scendere::transport::reserved_address (private_network_peer, false));
	ASSERT_FALSE (scendere::transport::reserved_address (private_network_peer, true));
}
...
TEST (network, ipv6_bind_subnetwork)
{
	...
	auto address1_subnet (scendere::transport::ipv4_address_or_ipv6_subnet (address1));
	...
	auto address2_subnet (scendere::transport::ipv4_address_or_ipv6_subnet (address2));
...
TEST (network, network_range_ipv6)
{
	...
	auto address3_subnet (scendere::transport::map_address_to_subnetwork (address3));
...
TEST (network, network_range_ipv4)
{
	...
	auto address2_subnet (scendere::transport::map_address_to_subnetwork (address2));
...
{
	scendere::stat stats;
	scendere::message_buffer_manager buffer (stats, 512, 1);
...
{
	scendere::stat stats;
	scendere::message_buffer_manager buffer (stats, 512, 2);
	auto buffer1 (buffer.allocate ());
	ASSERT_NE (nullptr, buffer1);
...
{
	scendere::stat stats;
	scendere::message_buffer_manager buffer (stats, 512, 16);
...
TEST (message_buffer_manager, stats)
{
	...
	buffer.enqueue (buffer1);
	buffer.allocate ();
	ASSERT_EQ (1, stats.count (scendere::stat::type::udp, scendere::stat::detail::overflow));
}

TEST (tcp_listener, tcp_node_id_handshake)
{
	scendere::system system (1);
	auto socket (std::make_shared<scendere::client_socket> (*system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->bootstrap.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (scendere::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	scendere::node_id_handshake node_id_handshake{ scendere::dev::network_params.network, cookie, boost::none };
	...
	boost::optional<std::pair<scendere::account, scendere::signature>> response_zero (std::make_pair (scendere::account{}, scendere::signature (0)));
	scendere::node_id_handshake node_id_handshake_response{ scendere::dev::network_params.network, boost::none, response_zero };
...
// Test disabled because it's failing intermittently.
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3611
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3615
TEST (tcp_listener, DISABLED_tcp_listener_timeout_empty)
{
	scendere::system system (1);
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<scendere::client_socket> (*node0));
	...
	{
		{
			scendere::lock_guard<scendere::mutex> guard (node0->bootstrap.mutex);
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<scendere::client_socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (scendere::transport::map_tcp_to_endpoint (node0->bootstrap.endpoint ())));
	scendere::node_id_handshake node_id_handshake{ scendere::dev::network_params.network, cookie, boost::none };
	auto channel = std::make_shared<scendere::transport::channel_tcp> (*node0, socket);
	...
	ASSERT_TIMELY (5s, node0->stats.count (scendere::stat::type::message, scendere::stat::detail::node_id_handshake) != 0);
	{
		scendere::lock_guard<scendere::mutex> guard (node0->bootstrap.mutex);
...
TEST (network, replace_port)
{
	...
	scendere::node_config node0_config (scendere::get_available_port (), system.logging);
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto wrong_endpoint = scendere::endpoint (node1->network.endpoint ().address (), scendere::test_node_port ());
	auto channel0 (node0->network.udp_channels.insert (wrong_endpoint, node1->network_params.network.protocol_version));
	ASSERT_NE (nullptr, channel0);
	node0->network.udp_channels.modify (channel0, [&node1] (std::shared_ptr<scendere::transport::channel> const & channel_a) {
	...
	auto channel1 (std::make_shared<scendere::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.network.protocol_version));
...
// PR in which it got disabled: https://github.com/scenderecurrency/scendere-node/pull/3622
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3621
#ifndef _WIN32
TEST (network, peer_max_tcp_attempts)
{
	...
	{
		auto node2 (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work, node_flags));
		node2->start ();
		system.nodes.push_back (node2);
		// Start TCP attempt
	...
	ASSERT_TRUE (node->network.tcp_channels.reachout (scendere::endpoint (node->network.endpoint ().address (), scendere::get_available_port ())));
	ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_ip, scendere::stat::dir::out));
}
#endif
...
namespace transport
{
	TEST (network, peer_max_tcp_attempts_subnetwork)
	{
		...
		for (auto i (0); i < node->network_params.network.max_peers_per_subnetwork; ++i)
		{
			...
			scendere::endpoint endpoint (address, scendere::get_available_port ());
			ASSERT_FALSE (node->network.tcp_channels.reachout (endpoint));
		}
		ASSERT_EQ (0, node->network.size ());
		ASSERT_EQ (0, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_subnetwork, scendere::stat::dir::out));
		ASSERT_TRUE (node->network.tcp_channels.reachout (scendere::endpoint (boost::asio::ip::make_address_v6 ("::ffff:127.0.0.1"), scendere::get_available_port ())));
		ASSERT_EQ (1, node->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_subnetwork, scendere::stat::dir::out));
	}
}
}
...
TEST (network, duplicate_detection)
{
	...
	auto & node0 (*system.add_node (node_flags));
	auto & node1 (*system.add_node (node_flags));
	auto udp_channel (std::make_shared<scendere::transport::channel_udp> (node0.network.udp_channels, node1.network.endpoint (), node1.network_params.network.protocol_version));
	scendere::publish publish{ scendere::dev::network_params.network, scendere::dev::genesis };

	// Publish duplicate detection through UDP
	ASSERT_EQ (0, node1.stats.count (scendere::stat::type::filter, scendere::stat::detail::duplicate_publish));
	udp_channel->send (publish);
	udp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (scendere::stat::type::filter, scendere::stat::detail::duplicate_publish) == 1);

	// Publish duplicate detection through TCP
	auto tcp_channel (node0.network.tcp_channels.find_channel (scendere::transport::map_endpoint_to_tcp (node1.network.endpoint ())));
	ASSERT_EQ (1, node1.stats.count (scendere::stat::type::filter, scendere::stat::detail::duplicate_publish));
	tcp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (scendere::stat::type::filter, scendere::stat::detail::duplicate_publish) == 2);
}
...
TEST (network, duplicate_revert_publish)
{
	...
	auto other_node (std::make_shared<scendere::node> (system.io_ctx, scendere::get_available_port (), scendere::unique_path (), system.logging, system.work));
	other_node->start ();
	system.nodes.push_back (other_node);
	auto channel = scendere::establish_tcp (system, *other_node, node.network.endpoint ());
...
{
	scendere::system system;
	scendere::publish message{ scendere::dev::network_params.network, scendere::dev::genesis };
	...
	ASSERT_TIMELY (1s, 0 == node.stats.count (scendere::stat::type::drop, scendere::stat::detail::publish, scendere::stat::dir::out));

	// Send droppable message; drop stats should increase by one now
	channel1->send (message);
	ASSERT_TIMELY (1s, 1 == node.stats.count (scendere::stat::type::drop, scendere::stat::detail::publish, scendere::stat::dir::out));

	// Send non-droppable message, i.e. drop stats should not increase
	channel2->send (message, nullptr, scendere::buffer_drop_policy::no_limiter_drop);
	ASSERT_TIMELY (1s, 1 == node.stats.count (scendere::stat::type::drop, scendere::stat::detail::publish, scendere::stat::dir::out));
	...
	channel1->send (message);
	channel2->send (message);
	ASSERT_TIMELY (1s, 3 == node.stats.count (scendere::stat::type::drop, scendere::stat::detail::publish, scendere::stat::dir::out));
...
TEST (peer_exclusion, validate)
{
	...
	{
		scendere::tcp_endpoint endpoint (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (i)), 0);
	...
	auto & peers_by_endpoint (excluded_peers.peers.get<scendere::peer_exclusion::tag_endpoint> ());
	scendere::tcp_endpoint oldest (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x0)), 0);
	...
	scendere::tcp_endpoint first (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x1)), 0);
	ASSERT_NE (peers_by_endpoint.end (), peers_by_endpoint.find (first.address ()));
	scendere::tcp_endpoint second (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x2)), 0);
	...
	auto component (scendere::collect_container_info (excluded_peers, ""));
	auto composite (dynamic_cast<scendere::container_info_composite *> (component.get ()));
	ASSERT_NE (nullptr, component);
	auto & children (composite->get_children ());
	ASSERT_EQ (1, children.size ());
	auto child_leaf (dynamic_cast<scendere::container_info_leaf *> (children.front ().get ()));
...
TEST (network, tcp_no_connect_excluded_peers)
{
	...
	auto endpoint1_tcp (scendere::transport::map_endpoint_to_tcp (endpoint1));
	while (!node0->network.excluded_peers.check (endpoint1_tcp))
	{
		node0->network.excluded_peers.add (endpoint1_tcp, 1);
	}
	ASSERT_EQ (0, node0->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_excluded));
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (5s, node0->stats.count (scendere::stat::type::tcp, scendere::stat::detail::tcp_excluded) >= 1);
...
{
	scendere::tcp_message_manager manager (1);
	scendere::tcp_message_item item;
	item.node_id = scendere::account (100);
	...
	scendere::tcp_message_manager manager2 (2);
...
TEST (network, cleanup_purge)
{
	...
	std::weak_ptr<scendere::node> node_w = node1.shared ();
...
TEST (network, loopback_channel)
{
	...
	auto & node1 = *system.nodes[0];
	auto & node2 = *system.nodes[1];
	scendere::transport::channel_loopback channel1 (node1);
	ASSERT_EQ (channel1.get_type (), scendere::transport::transport_type::loopback);
	ASSERT_EQ (channel1.get_endpoint (), node1.network.endpoint ());
	ASSERT_EQ (channel1.get_tcp_endpoint (), scendere::transport::map_endpoint_to_tcp (node1.network.endpoint ()));
	...
	scendere::transport::channel_loopback channel2 (node2);
...
TEST (network, filter)
{
	scendere::system system{ 1 };
	auto & node1 = *system.nodes[0];
	scendere::keepalive keepalive{ scendere::dev::network_params.network };
	const_cast<scendere::networks &> (keepalive.header.network) = scendere::networks::scendere_dev_network;
	node1.network.inbound (keepalive, std::make_shared<scendere::transport::channel_loopback> (node1));
	ASSERT_EQ (0, node1.stats.count (scendere::stat::type::message, scendere::stat::detail::invalid_network));
	const_cast<scendere::networks &> (keepalive.header.network) = scendere::networks::invalid;
	node1.network.inbound (keepalive, std::make_shared<scendere::transport::channel_loopback> (node1));
	ASSERT_EQ (1, node1.stats.count (scendere::stat::type::message, scendere::stat::detail::invalid_network));
}

TEST (network, fill_keepalive_self)
{
	scendere::system system{ 2 };
	std::array<scendere::endpoint, 8> target;
