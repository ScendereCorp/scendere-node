...
{
	scendere::raw_key prv (0);
	auto pub (scendere::pub_key (prv));
	scendere::uint256_union message (0);
...
TEST (transaction_block, empty)
{
	...
	scendere::send_block block (0, 1, 13, key1.prv, key1.pub, 2);
	auto hash (block.hash ());
	ASSERT_FALSE (scendere::validate_message (key1.pub, hash, block.signature));
	block.signature.bytes[32] ^= 0x1;
	ASSERT_TRUE (scendere::validate_message (key1.pub, hash, block.signature));
}

TEST (block, send_serialize)
{
	scendere::send_block block1 (0, 1, 2, scendere::keypair ().prv, 4, 5);
	...
	ASSERT_NE (0, size);
	scendere::bufferstream stream2 (data, size);
	bool error (false);
	scendere::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (block, send_serialize_json)
{
	...
	bool error (false);
	scendere::send_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	scendere::receive_block block1 (0, 1, scendere::keypair ().prv, 3, 4);
	scendere::keypair key1;
	std::vector<uint8_t> bytes;
	{
		scendere::vectorstream stream1 (bytes);
	...
	bool error (false);
	scendere::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, receive_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	scendere::receive_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, open_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	scendere::open_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	scendere::change_block block1 (0, 1, scendere::keypair ().prv, 3, 4);
	std::string string1;
	block1.serialize_json (string1);
	ASSERT_NE (0, string1.size ());
	boost::property_tree::ptree tree1;
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	scendere::change_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	scendere::uint512_union input (scendere::uint512_t (0));
	std::string text;
	input.encode_hex (text);
	scendere::uint512_union output;
...
{
	scendere::uint512_union input (scendere::uint512_t (1));
	std::string text;
	input.encode_hex (text);
	scendere::uint512_union output;
	auto error (output.decode_hex (text));
...
{
	scendere::uint512_union input (scendere::uint512_t (1000));
	std::string text;
	input.encode_hex (text);
	text[5] = '!';
	scendere::uint512_union output;
...
{
	scendere::uint512_union input (std::numeric_limits<scendere::uint512_t>::max ());
	std::string text;
	input.encode_hex (text);
	scendere::uint512_union output;
	auto error (output.decode_hex (text));
	ASSERT_FALSE (error);
	ASSERT_EQ (input, output);
	ASSERT_EQ (scendere::uint512_t ("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), output.number ());
}
...
TEST (send_block, deserialize)
{
	...
	ASSERT_EQ (scendere::send_block::size, bytes.size ());
	scendere::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	scendere::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (receive_block, deserialize)
{
	...
	ASSERT_EQ (scendere::receive_block::size, bytes.size ());
	scendere::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	scendere::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (open_block, deserialize)
{
	...
	ASSERT_EQ (scendere::open_block::size, bytes.size ());
	scendere::bufferstream stream (bytes.data (), bytes.size ());
	bool error (false);
	scendere::open_block block2 (error, stream);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	scendere::change_block block1 (1, 2, scendere::keypair ().prv, 4, 5);
	ASSERT_EQ (block1.hash (), block1.hash ());
	std::vector<uint8_t> bytes;
	{
		scendere::vectorstream stream1 (bytes);
	...
	ASSERT_EQ (scendere::change_block::size, bytes.size ());
	auto data (bytes.data ());
	auto size (bytes.size ());
	ASSERT_NE (nullptr, data);
	ASSERT_NE (0, size);
	scendere::bufferstream stream2 (data, size);
	bool error (false);
	scendere::change_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	scendere::frontier_req request1{ scendere::dev::network_params.network };
	request1.start = 1;
	request1.age = 2;
	request1.count = 3;
	std::vector<uint8_t> bytes;
	...
	ASSERT_FALSE (error);
	scendere::frontier_req request2 (error, stream, header);
	ASSERT_FALSE (error);
...
TEST (block, publish_req_serialization)
{
	...
	scendere::publish req{ scendere::dev::network_params.network, block };
	std::vector<uint8_t> bytes;
	{
		scendere::vectorstream stream (bytes);
	...
	ASSERT_FALSE (error);
	scendere::publish req2 (error, stream2, header);
	ASSERT_FALSE (error);
	ASSERT_EQ (req, req2);
...
{
	scendere::send_block block (0, 1, 2, scendere::keypair ().prv, 4, 5);
	ASSERT_EQ (scendere::dev::network_params.work.difficulty (block), scendere::dev::network_params.work.difficulty (block.work_version (), block.root (), block.block_work ()));
}
...
TEST (state_block, serialization)
{
	...
	ASSERT_EQ (scendere::block_hash (1), block1->previous ());
	ASSERT_EQ (key2.pub, block1->hashables.representative);
	ASSERT_EQ (scendere::amount (2), block1->hashables.balance);
	ASSERT_EQ (scendere::uint256_union (4), block1->hashables.link);
	std::vector<uint8_t> bytes;
	{
		scendere::vectorstream stream (bytes);
		block1->serialize (stream);
	...
	ASSERT_EQ (scendere::state_block::size, bytes.size ());
	bool error1 (false);
	scendere::bufferstream stream (bytes.data (), bytes.size ());
	scendere::state_block block2 (error1, stream);
	ASSERT_FALSE (error1);
	ASSERT_EQ (*block1, block2);
	...
	bool error2 (false);
	scendere::state_block block3 (error2, tree);
	ASSERT_FALSE (error2);
...
{
	ASSERT_EQ (scendere::work_version::work_1, scendere::send_block ().work_version ());
	ASSERT_EQ (scendere::work_version::work_1, scendere::receive_block ().work_version ());
	ASSERT_EQ (scendere::work_version::work_1, scendere::change_block ().work_version ());
	ASSERT_EQ (scendere::work_version::work_1, scendere::open_block ().work_version ());
	ASSERT_EQ (scendere::work_version::work_1, scendere::state_block ().work_version ());
}

TEST (block_uniquer, null)
{
	scendere::block_uniquer uniquer;
...
TEST (block_uniquer, single)
{
	...
	ASSERT_EQ (*block1, *block2);
	std::weak_ptr<scendere::state_block> block3 (block2);
...
TEST (block_builder, from)
{
	...
	auto block = builder
				 .state ()
				 .account_address ("scen_15nhh1kzw3x8ohez6s75wy3jr6dqgq65oaede1fzk5hqxk4j8ehz7iqtb3to")
				 ...
				 .representative_address ("scen_1stofnrxuz3cai7ze75o174bpm7scwj9jn3nxsn8ntzg784jf1gzn1jjdkou")
...
TEST (block_builder, zeroed_state_block)
{
	scendere::block_builder builder;
	scendere::keypair key;
	scendere::state_block_builder state_builder;
	...
	ASSERT_FALSE (scendere::validate_message (key.pub, zero_block_build->hash (), zero_block_build->signature));
}
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
				 ...
				 .work (0)
				 .build (ec);
	ASSERT_EQ (ec, scendere::error_common::missing_representative);
}
...
TEST (block_builder, state_equality)
{
	...
	scendere::state_block block1 (key1.pub, 1, key2.pub, 2, 4, key1.prv, key1.pub, 5);
...
TEST (block_builder, state_errors)
{
	...
	ASSERT_EQ (ec, scendere::error_common::bad_account_number);

	builder.state ().zero ().account_address ("scen_1111111111111111111111111111111111111111111111111111hifc8npp").build (ec);
	ASSERT_NO_ERROR (ec);
}
...
TEST (block_builder, open)
{
	...
	auto block = builder
				 .open ()
				 .account_address ("scen_3t6k35gi95xu6tergt6p69ck76ogmitsa8mnijtpxm9fkcm736xtoncuohr3")
				 .representative_address ("scen_3t6k35gi95xu6tergt6p69ck76ogmitsa8mnijtpxm9fkcm736xtoncuohr3")
...
TEST (block_builder, open_equality)
{
	...
	// With constructor
	scendere::keypair key1, key2;
	scendere::open_block block1 (1, key1.pub, key2.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, change)
{
	...
	auto block = builder
				 .change ()
				 .representative_address ("scen_3rropjiqfxpmrrkooej4qtmm1pueu36f9ghinpho4esfdor8785a455d16nf")
...
TEST (block_builder, change_equality)
{
	...
	// With constructor
	scendere::keypair key1, key2;
	scendere::change_block block1 (1, key1.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, send)
{
	...
	auto block = builder
				 .send ()
				 .destination_address ("scen_1gys8r4crpxhp94n4uho5cshaho81na6454qni5gu9n53gksoyy1wcd4udyb")
	...
	ASSERT_TRUE (block->source ().is_zero ());
	ASSERT_EQ (block->destination ().to_account (), "scen_1gys8r4crpxhp94n4uho5cshaho81na6454qni5gu9n53gksoyy1wcd4udyb");
	ASSERT_TRUE (block->link ().is_zero ());
}
...
TEST (block_builder, send_equality)
{
	...
	// With constructor
	scendere::keypair key1, key2;
	scendere::send_block block1 (1, key1.pub, 2, key1.prv, key1.pub, 5);
...
TEST (block_builder, receive_equality)
{
	...
	// With constructor
	scendere::keypair key1;
	scendere::receive_block block1 (1, 2, key1.prv, key1.pub, 5);
