...
TEST (toml, diff)
{
	scendere::tomlconfig defaults, other;
...
{
	scendere::tomlconfig t;
	boost::filesystem::path data_path (".");
	scendere::daemon_config c{ data_path, scendere::dev::network_params };
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	scendere::tomlconfig t;
	t.read (ss);
	scendere::rpc_config conf{ scendere::dev::network_params.network };
	scendere::rpc_config defaults{ scendere::dev::network_params.network };
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	scendere::tomlconfig t;
	t.read (ss);
	scendere::daemon_config conf;
	scendere::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == scendere::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == scendere::error_config::invalid_value);
}

TEST (toml, put)
{
	scendere::tomlconfig config;
	scendere::tomlconfig config_node;
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["scen_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
	...
	ASSERT_EQ (scendere::rocksdb_config::using_rocksdb_in_tests (), defaults.node.rocksdb_config.enable);
...
TEST (toml, daemon_config_deserialize_errors)
{
...

	{
		...
		ASSERT_EQ (conf.node.frontiers_confirmation, scendere::frontiers_confirmation_mode::invalid);
	}
...
TEST (toml, daemon_read_config)
{
	...
	ASSERT_FALSE (boost::filesystem::exists (scendere::get_node_toml_config_path (path)));
	ASSERT_FALSE (scendere::read_node_config_toml (path, config));
	{
		auto error = scendere::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = scendere::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	scendere::tomlconfig toml;
	toml.write (scendere::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (scendere::get_node_toml_config_path (path)));
	ASSERT_FALSE (scendere::read_node_config_toml (path, config));
	{
		auto error = scendere::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
...
TEST (toml, tls_config_deserialize_no_defaults)
{
	...
	scendere::tomlconfig toml;
	toml.read (ss);
	scendere::tls_config conf;
	scendere::tls_config defaults;
	conf.deserialize_toml (toml);
