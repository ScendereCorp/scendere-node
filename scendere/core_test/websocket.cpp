#include <scendere/core_test/fakes/websocket_client.hpp>
...
TEST (websocket, subscription_edge)
{
	...
	config.websocket_config.port = scendere::get_available_port ();
	auto node1 (system.add_node (config));

	ASSERT_EQ (0, node1->websocket_server->subscriber_count (scendere::websocket::topic::confirmation));
	...
	auto task = ([config, &node1] () {
		...
		client.await_ack ();
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::confirmation));
		client.send_message (R"json({"action": "subscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::confirmation));
		client.send_message (R"json({"action": "unsubscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (0, node1->websocket_server->subscriber_count (scendere::websocket::topic::confirmation));
		client.send_message (R"json({"action": "unsubscribe", "topic": "confirmation", "ack": true})json");
		client.await_ack ();
		EXPECT_EQ (0, node1->websocket_server->subscriber_count (scendere::websocket::topic::confirmation));
	});
...
TEST (websocket, confirmation)
{
	...
	// Quick-confirm a block, legacy blocks should work without filtering
	{
		...
		balance -= send_amount;
		auto send (std::make_shared<scendere::send_block> (previous, key.pub, balance, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (previous)));
...
TEST (websocket, stopped_election)
{
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::stopped_election));
	...
	auto send1 (std::make_shared<scendere::send_block> (scendere::dev::genesis->hash (), key1.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (scendere::dev::genesis->hash ())));
...
TEST (websocket, confirmation_options_votes)
{
	...
	try
	{
		...
		for (auto & vote : votes_l.get ())
		{
			...
			ASSERT_EQ (scendere::dev::genesis_key.pub.to_account (), representative);
			...
			ASSERT_EQ (node1->latest (scendere::dev::genesis_key.pub).to_string (), hash);
			std::string weight (vote.second.get<std::string> ("weight"));
			ASSERT_EQ (node1->balance (scendere::dev::genesis_key.pub).convert_to<std::string> (), weight);
		}
	}
	catch (std::runtime_error const & ex)
	{
...
TEST (websocket, confirmation_options_update)
{
	...
	auto task = ([&added, &deleted, config, &node1] () {
		...
		std::string add_message = boost::str (boost::format (R"json({"action": "update", "topic": "confirmation", "ack": "true", "options": {"accounts_add": ["%1%"]}})json") % scendere::dev::genesis_key.pub.to_account ());
		...
		std::string delete_message = boost::str (boost::format (R"json({"action": "update", "topic": "confirmation", "ack": "true", "options": {"accounts_del": ["%1%"]}})json") % scendere::dev::genesis_key.pub.to_account ());
	...
	scendere::keypair key;
	scendere::state_block_builder builder;
	auto previous (node1->latest (scendere::dev::genesis_key.pub));
	auto send = builder
				.account (scendere::dev::genesis_key.pub)
				.previous (previous)
				.representative (scendere::dev::genesis_key.pub)
...
TEST (websocket, vote)
{
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::vote));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	auto send = builder
				...
				.previous (previous)
				.representative (scendere::dev::genesis_key.pub)
				.balance (scendere::dev::constants.genesis_amount - (node1->online_reps.delta () + 1))
				.link (key.pub)
				.sign (scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub)
				.work (*system.work.generate (previous))
				.build_shared ();
...
TEST (websocket, vote_options_type)
{
	...
	auto vote (std::make_shared<scendere::vote> (scendere::dev::genesis_key.pub, scendere::dev::genesis_key.prv, 0, 0, scendere::dev::genesis));
	scendere::websocket::message_builder builder;
	auto msg (builder.vote_received (vote, scendere::vote_code::replay));
...
TEST (websocket, vote_options_representatives)
{
	...
	auto task1 = ([&ack_ready, config, &node1] () {
		fake_websocket_client client (node1->websocket_server->listening_port ());
		std::string message = boost::str (boost::format (R"json({"action": "subscribe", "topic": "vote", "ack": "true", "options": {"representatives": ["%1%"]}})json") % scendere::dev::genesis_key.pub.to_account ());
...
TEST (websocket, work)
{
	...
	ASSERT_EQ (0, node1->websocket_server->subscriber_count (scendere::websocket::topic::work));
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::work));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	ASSERT_TIMELY (5s, ack_ready);
	ASSERT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::work));
	...
	ASSERT_EQ (request.get<std::string> ("version"), scendere::to_string (scendere::work_version::work_1));
	ASSERT_EQ (request.get<std::string> ("hash"), hash.to_string ());
	ASSERT_EQ (request.get<std::string> ("difficulty"), scendere::to_string_hex (node1->default_difficulty (scendere::work_version::work_1)));
	ASSERT_EQ (request.get<double> ("multiplier"), 1.0);

	ASSERT_EQ (1, contents.count ("result"));
	auto & result = contents.get_child ("result");
	uint64_t result_difficulty;
	scendere::from_string_hex (result.get<std::string> ("difficulty"), result_difficulty);
	ASSERT_GE (result_difficulty, node1->default_difficulty (scendere::work_version::work_1));
	ASSERT_NEAR (result.get<double> ("multiplier"), scendere::difficulty::to_multiplier (result_difficulty, node1->default_difficulty (scendere::work_version::work_1)), 1e-6);
	ASSERT_EQ (result.get<std::string> ("work"), scendere::to_string_hex (work.get ()));
...
TEST (websocket, bootstrap)
{
	...
	ASSERT_EQ (0, node1->websocket_server->subscriber_count (scendere::websocket::topic::bootstrap));
	...
	auto task = ([&ack_ready, config, &node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::bootstrap));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
...
TEST (websocket, bootstrap_exited)
{
	...
	scendere::util::counted_completion subscribed_completion (1);
...
TEST (websocket, telemetry)
{
	...
	auto task = ([config = node1->config, &node1, &done] () {
		...
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::telemetry));
		return client.get_response ();
	});
	...
	auto & contents = event.get_child ("message");
	scendere::jsonconfig telemetry_contents (contents);
	scendere::telemetry_data telemetry_data;
	telemetry_data.deserialize_json (telemetry_contents, false);
	compare_default_telemetry_response_data (telemetry_data, node2->network_params, node2->config.bandwidth_limit, node2->default_difficulty (scendere::work_version::work_1), node2->node_id);
	...
	EXPECT_EQ (0, node2->websocket_server->subscriber_count (scendere::websocket::topic::telemetry));
}
...
TEST (websocket, new_unconfirmed_block)
{
	...
	auto task = ([&ack_ready, config, node1] () {
		...
		client.await_ack ();
		ack_ready = true;
		EXPECT_EQ (1, node1->websocket_server->subscriber_count (scendere::websocket::topic::new_unconfirmed_block));
		return client.get_response ();
	});
	auto future = std::async (std::launch::async, task);
	...
	ASSERT_EQ (scendere::process_result::progress, node1->process_local (send1).code);
