...
#include <algorithm>
#include <scendere/qt_test/QTest>
#include <thread>
...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (scendere::random_wallet_id ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
// https://github.com/scenderecurrency/scendere-node/issues/3235
TEST (wallet, DISABLED_status)
{
	...
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet] (scendere_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (scendere::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (scendere_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (scendere_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (scendere_qt::status_types::disconnected));
}
...
TEST (wallet, status_with_peer)
{
	...
	ASSERT_TRUE (wallet_has (scendere_qt::status_types::nominal));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 scendere", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	scendere::system system (1);
	auto wallet_l (system.nodes[0]->wallets.create (scendere::random_wallet_id ()));
	scendere::public_key key1 (wallet_l->deterministic_insert ());
	scendere::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (scendere::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (wallet, send)
{
	...
	scendere::public_key key1 (system.wallet (1)->insert_adhoc (scendere::keypair ().prv));
	auto account (scendere::dev::genesis_key.pub);
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, scendere::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	scendere::uint128_t amount (wallet->node.balance (key1));
...
// Issue for investigating it: https://github.com/scenderecurrency/scendere-node/issues/3642
TEST (wallet, DISABLED_process_block)
{
	scendere_qt::eventloop_processor processor;
	scendere::system system (1);
	scendere::account account;
	scendere::block_hash latest (system.nodes[0]->latest (scendere::dev::genesis->account ()));
	system.wallet (0)->insert_adhoc (scendere::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	...
	scendere::send_block send (latest, key1.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
	std::string previous;
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, scendere::dev::genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	scendere::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (scendere::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	scendere::system system (1);
	system.wallet (0)->insert_adhoc (scendere::dev::genesis_key.prv);
	system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, 100);
	scendere::block_hash latest1 (system.nodes[0]->latest (scendere::dev::genesis_key.pub));
	system.wallet (0)->send_action (scendere::dev::genesis_key.pub, key.pub, 100);
	scendere::block_hash latest2 (system.nodes[0]->latest (scendere::dev::genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, scendere::dev::genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	scendere::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (scendere::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	scendere::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (scendere::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	scendere::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (scendere::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	auto store = scendere::make_store (system.nodes[0]->logger, scendere::unique_path (), scendere::dev::constants);
	ASSERT_TRUE (!store->init_error ());
	scendere::ledger ledger (*store, system.nodes[0]->stats, scendere::dev::constants);
	{
		auto transaction (store->tx_begin_write ());
		store->initialize (transaction, ledger.cache);
		scendere::keypair key;
		auto latest (ledger.latest (transaction, scendere::dev::genesis_key.pub));
		scendere::send_block send (latest, scendere::dev::genesis_key.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, send).code);
		scendere::receive_block receive (send.hash (), send.hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send.hash ()));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, receive).code);
		scendere::change_block change (receive.hash (), key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (receive.hash ()));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, change).code);
	}
	scendere_qt::history history (ledger, scendere::dev::genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (history, pruned_source)
{
	...
	scendere::ledger ledger (*store, system.nodes[0]->stats, scendere::dev::constants);
	ledger.pruning = true;
	scendere::block_hash next_pruning;
	// Basic pruning for legacy blocks. Previous block is pruned, source is pruned
	{
		auto transaction (store->tx_begin_write ());
		store->initialize (transaction, ledger.cache);
		auto latest (ledger.latest (transaction, scendere::dev::genesis_key.pub));
		scendere::send_block send1 (latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 100, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, send1).code);
		scendere::send_block send2 (send1.hash (), key.pub, scendere::dev::constants.genesis_amount - 200, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send1.hash ()));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, send2).code);
		scendere::receive_block receive (send2.hash (), send1.hash (), scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (send2.hash ()));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, receive).code);
		scendere::open_block open (send2.hash (), key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	scendere_qt::history history1 (ledger, scendere::dev::genesis_key.pub, *wallet);
	history1.refresh ();
	ASSERT_EQ (2, history1.model->rowCount ());
	scendere_qt::history history2 (ledger, key.pub, *wallet);
	history2.refresh ();
	...
	// Pruning for state blocks. Previous block is pruned, source is pruned
	{
		auto transaction (store->tx_begin_write ());
		auto latest (ledger.latest (transaction, scendere::dev::genesis_key.pub));
		scendere::state_block send (scendere::dev::genesis_key.pub, latest, scendere::dev::genesis_key.pub, scendere::dev::constants.genesis_amount - 200, key.pub, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, send).code);
		auto latest_key (ledger.latest (transaction, key.pub));
		scendere::state_block receive (key.pub, latest_key, key.pub, 200, send.hash (), key.prv, key.pub, *system.work.generate (latest_key));
		ASSERT_EQ (scendere::process_result::progress, ledger.process (transaction, receive).code);
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, scendere::dev::genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, scendere::dev::genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	scendere::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.latest (transaction, scendere::dev::genesis_key.pub));
		scendere::send_block block (latest, key.pub, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system.work.generate (latest));
		hash = block.hash ();
		ASSERT_EQ (scendere::process_result::progress, system.nodes[0]->ledger.process (transaction, block).code);
	}
	auto account (scendere::dev::genesis_key.pub);
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (scendere::dev::genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (scendere::dev::genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	scendere::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, scendere::dev::genesis_key.prv.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	scendere::system system (1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_GE (scendere::dev::network_params.work.difficulty (scendere::work_version::work_1, system.nodes[0]->ledger.latest_root (transaction, pub), work), system.nodes[0]->default_difficulty (scendere::work_version::work_1));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		auto latest (system1.nodes[0]->ledger.latest (transaction, scendere::dev::genesis->account ()));
		scendere::send_block send (latest, key1, 0, scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub, *system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (scendere_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (scendere_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (scendere_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
...
TEST (wallet, epoch_2_validation)
{
	...
	auto account (scendere::dev::genesis_key.pub);
	auto wallet (std::make_shared<scendere_qt::wallet> (*test_application, processor, *node, system.wallet (0), account));
	wallet->start ();
	wallet->client_window->show ();
	...
	auto create_and_process = [&] () -> scendere::block_hash {
		wallet->block_creation.create->click ();
		std::string json (wallet->block_creation.block->toPlainText ().toStdString ());
		...
		boost::property_tree::read_json (istream, tree1);
		bool error (false);
		scendere::state_block block (error, tree1);
		EXPECT_FALSE (error);
		EXPECT_EQ (scendere::process_result::progress, node->process (block).code);
		return block.hash ();
	};

	auto do_send = [&] (scendere::public_key const & destination) -> scendere::block_hash {
		wallet->block_creation.send->click ();
		wallet->block_creation.account->setText (scendere::dev::genesis_key.pub.to_account ().c_str ());
	...
	auto do_open = [&] (scendere::block_hash const & source, scendere::public_key const & account) -> scendere::block_hash {
		wallet->block_creation.open->click ();
	...
	auto do_receive = [&] (scendere::block_hash const & source) -> scendere::block_hash {
		wallet->block_creation.receive->click ();
		wallet->block_creation.source->setText (source.to_string ().c_str ());
		return create_and_process ();
	};

	auto do_change = [&] (scendere::public_key const & account, scendere::public_key const & representative) -> scendere::block_hash {
		wallet->block_creation.change->click ();
	...
	while (++tries < max_tries)
	{
		...
		do_receive (send2);
		do_change (key.pub, scendere::dev::genesis_key.pub);
	}
}
