...
REGISTER_ERROR_CODES (scendere, error_common);
REGISTER_ERROR_CODES (scendere, error_blocks);
REGISTER_ERROR_CODES (scendere, error_rpc);
REGISTER_ERROR_CODES (scendere, error_process);
REGISTER_ERROR_CODES (scendere, error_config);
...
public:
	error () = default;
	error (scendere::error const & error_a) = default;
	error (scendere::error && error_a) = default;
	...
	error & operator= (scendere::error const & err_a);
	error & operator= (scendere::error && err_a);
	...
	error & then (std::function<scendere::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = scendere::error_common::generic);
...
/**
 * A type that manages a scendere::error.
 * The default return type is scendere::error&, though shared_ptr<scendere::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = scendere::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, scendere::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<scendere::error>>::value, "Must be scendere::error& or shared_ptr<scendere::error>");
