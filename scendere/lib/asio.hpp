...
#include <scendere/boost/asio/write.hpp>
...
async_write (AsyncWriteStream & s, scendere::shared_const_buffer const & buffer, WriteHandler && handler)
{
	return boost::asio::async_write (s, buffer, std::forward<WriteHandler> (handler));
}

/**
 * Alternative to scendere::async_write where scatter/gather is desired for best performance, and where
