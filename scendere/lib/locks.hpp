...
#define USING_SCENDERE_TIMED_LOCKS (SCENDERE_TIMED_LOCKS > 0)
...
{
class mutex;
extern scendere::mutex * mutex_to_filter;
extern scendere::mutex mutex_to_filter_mutex;
...
template <typename Mutex>
void output_if_held_long_enough (scendere::timer<std::chrono::milliseconds> & timer, Mutex & mutex);

#ifndef SCENDERE_TIMED_LOCKS_IGNORE_BLOCKED
template <typename Mutex>
void output_if_blocked_long_enough (scendere::timer<std::chrono::milliseconds> & timer, Mutex & mutex);
#endif
...
template <>
class lock_guard<scendere::mutex> final
{
public:
	explicit lock_guard (scendere::mutex & mutex_a);
	~lock_guard () noexcept;
...
template <typename Mutex, typename = std::enable_if_t<std::is_same<Mutex, scendere::mutex>::value>>
...
/** Assumes std implementations of std::condition_variable never actually call scendere::unique_lock::lock/unlock,
...
public:
	...
	void wait (scendere::unique_lock<scendere::mutex> & lt);

	template <typename Pred>
	void wait (scendere::unique_lock<scendere::mutex> & lk, Pred pred)
	{
		while (!pred ())
		{
			wait (lk);
	...
	std::cv_status wait_until (scendere::unique_lock<scendere::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time)
	{
		if (!lk.mut || !lk.owns)
		{
	...
	bool wait_until (scendere::unique_lock<scendere::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time, Pred pred)
	{
		while (!pred ())
		{
	...
	void wait_for (scendere::unique_lock<scendere::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time)
	{
		wait_until (lk, std::chrono::steady_clock::now () + rel_time);
	}

	template <typename Rep, typename Period, typename Pred>
	bool wait_for (scendere::unique_lock<scendere::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time, Pred pred)
	{
