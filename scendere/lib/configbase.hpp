...
class configbase : public scendere::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<scendere::error> const & error_a) :
		error (error_a)
	{
	}

	/** Returns the current error */
	scendere::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a scendere::error_aware type. Child nodes share the error state. */
	std::shared_ptr<scendere::error> error;
