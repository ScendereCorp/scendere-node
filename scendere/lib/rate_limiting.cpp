...
scendere::rate::token_bucket::token_bucket (size_t max_token_count_a, size_t refill_rate_a)
{
	reset (max_token_count_a, refill_rate_a);
}

bool scendere::rate::token_bucket::try_consume (unsigned tokens_required_a)
{
	debug_assert (tokens_required_a <= 1e9);
	scendere::lock_guard<scendere::mutex> lk (bucket_mutex);
	refill ();
...
void scendere::rate::token_bucket::refill ()
{
...
size_t scendere::rate::token_bucket::largest_burst () const
{
	scendere::lock_guard<scendere::mutex> lk (bucket_mutex);
	return max_token_count - smallest_size;
}

void scendere::rate::token_bucket::reset (size_t max_token_count_a, size_t refill_rate_a)
{
	scendere::lock_guard<scendere::mutex> lk (bucket_mutex);
