...
scendere::error scendere::rpc_secure_config::serialize_toml (scendere::tomlconfig & toml) const
{
...
scendere::error scendere::rpc_secure_config::deserialize_toml (scendere::tomlconfig & toml)
{
...
scendere::rpc_config::rpc_config (scendere::network_constants & network_constants) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () }
{
}

scendere::rpc_config::rpc_config (scendere::network_constants & network_constants, uint16_t port_a, bool enable_control_a) :
	rpc_process{ network_constants },
	address{ boost::asio::ip::address_v6::loopback ().to_string () },
	port{ port_a },
...
scendere::error scendere::rpc_config::serialize_toml (scendere::tomlconfig & toml) const
{
	...
	scendere::tomlconfig rpc_process_l;
	...
	scendere::tomlconfig rpc_logging_l;
	rpc_logging_l.put ("log_rpc", rpc_logging.log_rpc, "Whether to log RPC calls.\ntype:bool");
	toml.put_child ("logging", rpc_logging_l);
	return toml.get_error ();
}

scendere::error scendere::rpc_config::deserialize_toml (scendere::tomlconfig & toml)
{
	if (!toml.empty ())
	{
		...
		if (rpc_secure_l)
		{
			return scendere::error ("The RPC secure configuration has moved to config-tls.toml. Please update the configuration.");
		}
...
scendere::rpc_process_config::rpc_process_config (scendere::network_constants & network_constants) :
...
namespace scendere
{
scendere::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, scendere::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	scendere::error error;
	auto toml_config_path = scendere::get_rpc_toml_config_path (data_path_a);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the scendere_rpc executable file path based on where the currently running executable is found.
