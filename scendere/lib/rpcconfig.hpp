...
{
public:
	rpc_process_config (scendere::network_constants & network_constants);
...
{
public:
	explicit rpc_config (scendere::network_constants & network_constants);
	explicit rpc_config (scendere::network_constants & network_constants, uint16_t, bool);
	scendere::error serialize_toml (scendere::tomlconfig &) const;
	scendere::error deserialize_toml (scendere::tomlconfig &);

	scendere::rpc_process_config rpc_process;
	std::string address;
	...
	scendere::rpc_logging_config rpc_logging;
	/** Optional TLS config */
	std::shared_ptr<scendere::tls_config> tls_config;
};

scendere::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, scendere::rpc_config & config_a, std::vector<std::string> const & config_overrides = std::vector<std::string> ());
