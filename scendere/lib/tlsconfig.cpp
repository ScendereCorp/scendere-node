...
namespace scendere
{
scendere::error scendere::tls_config::serialize_toml (scendere::tomlconfig & toml) const
{
...
scendere::error scendere::tls_config::deserialize_toml (scendere::tomlconfig & toml)
{
...
namespace
{
	bool on_verify_certificate (bool preverified, boost::asio::ssl::verify_context & ctx, scendere::tls_config & config_a, scendere::logger_mt & logger_a)
	{
	...
	void load_certs (scendere::tls_config & config_a, scendere::logger_mt & logger_a)
	{
		try
		{
...
scendere::error read_tls_config_toml (boost::filesystem::path const & data_path_a, scendere::tls_config & config_a, scendere::logger_mt & logger_a, std::vector<std::string> const & config_overrides)
{
	scendere::error error;
	auto toml_config_path = scendere::get_tls_toml_config_path (data_path_a);
...
#else
		auto msg ("https or wss is enabled in the TLS configuration, but the node is not built with SCENDERE_SECURE_RPC");
