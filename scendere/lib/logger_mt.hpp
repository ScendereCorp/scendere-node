...
inline boost::log::formatting_ostream & operator<< (boost::log::formatting_ostream & strm, boost::log::to_log_manip<scendere::severity_level, severity_tag> const & manip)
{
	...
	scendere::severity_level level = manip.get ();
...
private:
	...
	void output (scendere::severity_level severity_level, LogItems &&... log_items)
	{
...
public:
	 ...
	 */
	template <typename... LogItems>
	void always_log (scendere::severity_level severity_level, LogItems &&... log_items)
	{
	...
	{
		always_log (scendere::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
	 ...
	 */
	template <typename... LogItems>
	bool try_log (scendere::severity_level severity_level, LogItems &&... log_items)
	{
		auto error (true);
		auto time_now = std::chrono::steady_clock::now ();
		scendere::unique_lock<scendere::mutex> lk (last_log_time_mutex);
	...
	{
		return try_log (scendere::severity_level::normal, std::forward<LogItems> (log_items)...);
	}

	std::chrono::milliseconds min_log_delta{ 0 };

private:
	scendere::mutex last_log_time_mutex;
