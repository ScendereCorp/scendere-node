...
#include <scendere/boost/asio/deadline_timer.hpp>
#include <scendere/boost/asio/executor_work_guard.hpp>
#include <scendere/boost/asio/io_context.hpp>
#include <scendere/boost/asio/steady_timer.hpp>
#include <scendere/boost/asio/thread_pool.hpp>
...
namespace thread_role
{
	 ...
	 */
	scendere::thread_role::name get ();
	void set (scendere::thread_role::name);

	/*
	 * Get the thread name as a string from enum
	 */
	std::string get_string (scendere::thread_role::name);
...
{
public:
	explicit thread_pool (unsigned, scendere::thread_role::name);
	~thread_pool ();
...
private:
	...
	void set_thread_names (unsigned num_threads, scendere::thread_role::name thread_name);
};

std::unique_ptr<scendere::container_info_component> collect_container_info (thread_pool & thread_pool, std::string const & name);
}
