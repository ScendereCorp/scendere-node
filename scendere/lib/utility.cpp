...
std::size_t scendere::get_file_descriptor_limit ()
{
...
void scendere::set_file_descriptor_limit (std::size_t limit)
{
#ifndef _WIN32
	rlimit fd_limit{};
...
scendere::container_info_composite::container_info_composite (std::string const & name) :
	name (name)
{
}

bool scendere::container_info_composite::is_composite () const
{
	return true;
}

void scendere::container_info_composite::add_component (std::unique_ptr<container_info_component> child)
{
	children.push_back (std::move (child));
}

std::vector<std::unique_ptr<scendere::container_info_component>> const & scendere::container_info_composite::get_children () const
{
	return children;
}

std::string const & scendere::container_info_composite::get_name () const
{
	return name;
}

scendere::container_info_leaf::container_info_leaf (const container_info & info) :
	info (info)
{
}

bool scendere::container_info_leaf::is_composite () const
{
	return false;
}

scendere::container_info const & scendere::container_info_leaf::get_info () const
{
	return info;
}

void scendere::dump_crash_stacktrace ()
{
	boost::stacktrace::safe_dump_to ("scendere_node_backtrace.dump");
}

std::string scendere::generate_stacktrace ()
{
...
void scendere::remove_all_files_in_dir (boost::filesystem::path const & dir)
{
...
void scendere::move_all_files_to_dir (boost::filesystem::path const & from, boost::filesystem::path const & to)
{
...
void assert_internal (char const * check_expr, char const * func, char const * file, unsigned int line, bool is_release_assert, std::string_view error_msg)
{
	...
	auto backtrace_str = scendere::generate_stacktrace ();
...
#ifdef _WIN32
	{
		...
		std::string filename = is_release_assert ? "scendere_node_backtrace_release_assert.txt" : "scendere_node_backtrace_assert.txt";
		...
		scendere::set_secure_perm_file (filepath);
		file << backtrace_str;
	}
#endif
...
// Issue #3748
void scendere::sort_options_description (const boost::program_options::options_description & source, boost::program_options::options_description & target)
{
