...
scendere::tomlconfig::tomlconfig () :
	tree (cpptoml::make_table ())
{
	error = std::make_shared<scendere::error> ();
}

scendere::tomlconfig::tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<scendere::error> const & error_a) :
	scendere::configbase (error_a), tree (tree_a)
{
	if (!error)
	{
		error = std::make_shared<scendere::error> ();
...
void scendere::tomlconfig::doc (std::string const & key, std::string const & doc)
{
	tree->document (key, doc);
}

scendere::error & scendere::tomlconfig::read (boost::filesystem::path const & path_a)
{
...
scendere::error & scendere::tomlconfig::read (std::istream & stream_overrides, boost::filesystem::path const & path_a)
{
	std::fstream stream;
...
scendere::error & scendere::tomlconfig::read (std::istream & stream_a)
{
	std::stringstream stream_override_empty;
	stream_override_empty << std::endl;
...
scendere::error & scendere::tomlconfig::read (std::istream & stream_first_a, std::istream & stream_second_a)
{
	try
	{
...
void scendere::tomlconfig::write (boost::filesystem::path const & path_a)
{
	std::fstream stream;
	open_or_create (stream, path_a.string ());
	write (stream);
...
void scendere::tomlconfig::write (std::ostream & stream_a) const
{
...
void scendere::tomlconfig::open_or_create (std::fstream & stream_a, std::string const & path_a)
{
	if (!boost::filesystem::exists (path_a))
	{
		...
		scendere::set_secure_perm_file (path_a);
	}
...
std::shared_ptr<cpptoml::table> scendere::tomlconfig::get_tree ()
{
	return tree;
}

/** Returns true if the toml table is empty */
bool scendere::tomlconfig::empty () const
{
	return tree->empty ();
}

boost::optional<scendere::tomlconfig> scendere::tomlconfig::get_optional_child (std::string const & key_a)
{
...
scendere::tomlconfig scendere::tomlconfig::get_required_child (std::string const & key_a)
{
	if (!tree->contains (key_a))
	{
		*error = scendere::error_config::missing_value;
...
scendere::tomlconfig & scendere::tomlconfig::put_child (std::string const & key_a, scendere::tomlconfig & conf_a)
{
	tree->insert (key_a, conf_a.get_tree ());
	return *this;
}

scendere::tomlconfig & scendere::tomlconfig::replace_child (std::string const & key_a, scendere::tomlconfig & conf_a)
{
	tree->erase (key_a);
...
bool scendere::tomlconfig::has_key (std::string const & key_a)
{
	return tree->contains (key_a);
}

/** Erase the property of given key */
scendere::tomlconfig & scendere::tomlconfig::erase (std::string const & key_a)
{
	tree->erase (key_a);
	return *this;
}

std::shared_ptr<cpptoml::array> scendere::tomlconfig::create_array (std::string const & key, boost::optional<char const *> documentation_a)
{
	if (!has_key (key))
	{
...
/**
	 ...
	 */
void scendere::tomlconfig::erase_default_values (tomlconfig & defaults_a)
{
...
std::string scendere::tomlconfig::to_string ()
{
	std::stringstream ss;
...
std::string scendere::tomlconfig::to_string_commented_entries ()
{
...
scendere::tomlconfig & scendere::tomlconfig::get_config (bool optional, std::string const & key, uint8_t & target, uint8_t default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			{
				conditionally_set_error<uint8_t> (scendere::error_config::invalid_value, optional, key);
			}
			else
			{
		...
		{
			conditionally_set_error<uint8_t> (scendere::error_config::missing_value, optional, key);
		}
		else
		{
...
scendere::tomlconfig & scendere::tomlconfig::get_config (bool optional, std::string const & key, bool & target, bool default_value)
{
	auto bool_conv = [this, &target, &key, optional] (std::string val) {
		...
		{
			conditionally_set_error<bool> (scendere::error_config::invalid_value, optional, key);
		}
	};
	try
	{
		...
		{
			conditionally_set_error<bool> (scendere::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
...
void scendere::tomlconfig::erase_defaults (std::shared_ptr<cpptoml::table> const & base, std::shared_ptr<cpptoml::table> const & other, std::shared_ptr<cpptoml::table> const & update_target)
{
...
scendere::tomlconfig & scendere::tomlconfig::get_config (bool optional, std::string key, boost::asio::ip::address_v6 & target, boost::asio::ip::address_v6 const & default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			if (bec)
			{
				conditionally_set_error<boost::asio::ip::address_v6> (scendere::error_config::invalid_value, optional, key);
			}
		}
		else if (!optional)
		{
			conditionally_set_error<boost::asio::ip::address_v6> (scendere::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
