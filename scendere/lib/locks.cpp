...
{
	static scendere::mutex cout_mutex;
	auto stacktrace = scendere::generate_stacktrace ();
...
void output_if_held_long_enough (scendere::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_held >= std::chrono::milliseconds (SCENDERE_TIMED_LOCKS))
	{
		std::unique_lock lk (scendere::mutex_to_filter_mutex);
		if (!scendere::any_filters_registered () || (scendere::mutex_to_filter == &mutex))
		{
			lk.unlock ();
			output ("held", time_held, mutex);
		}
	}
	if (timer.current_state () != scendere::timer_state::stopped)
	{
		timer.stop ();
	}
}
...
void output_if_blocked_long_enough (scendere::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	...
	if (time_blocked >= std::chrono::milliseconds (SCENDERE_TIMED_LOCKS))
	{
		std::unique_lock lk (scendere::mutex_to_filter_mutex);
		if (!scendere::any_filters_registered () || (scendere::mutex_to_filter == &mutex))
		{
...
lock_guard<scendere::mutex>::lock_guard (scendere::mutex & mutex) :
	mut (mutex)
{
	timer.start ();
...
lock_guard<scendere::mutex>::~lock_guard () noexcept
{
	mut.unlock ();
...
template class unique_lock<scendere::mutex>;
...
void condition_variable::wait (scendere::unique_lock<scendere::mutex> & lk)
{
	if (!lk.mut || !lk.owns)
	{
		throw (std::system_error (std::make_error_code (std::errc::operation_not_permitted)));
...
scendere::mutex * mutex_to_filter{ nullptr };
scendere::mutex mutex_to_filter_mutex;

bool should_be_filtered (char const * name)
{
	return std::strcmp (name, xstr (SCENDERE_TIMED_LOCKS_FILTER)) == 0;
}

bool any_filters_registered ()
{
	return std::strcmp ("", xstr (SCENDERE_TIMED_LOCKS_FILTER)) != 0;
}
}
#endif

char const * scendere::mutex_identifier (mutexes mutex)
{
	switch (mutex)
	{
		case mutexes::active:
