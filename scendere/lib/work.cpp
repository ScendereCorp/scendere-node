...
std::string scendere::to_string (scendere::work_version const version_a)
{
	...
	switch (version_a)
	{
		case scendere::work_version::work_1:
			result = "work_1";
			break;
		case scendere::work_version::unspecified:
			result = "unspecified";
			break;
	}
	return result;
...
scendere::work_pool::work_pool (scendere::network_constants & network_constants, unsigned max_threads_a, std::chrono::nanoseconds pow_rate_limiter_a, std::function<boost::optional<uint64_t> (scendere::work_version const, scendere::root const &, uint64_t, std::atomic<int> &)> opencl_a) :
	...
	opencl (opencl_a)
{
	...
	for (auto i (0u); i < count; ++i)
	{
		threads.emplace_back (attrs, [this, i] () {
			scendere::thread_role::set (scendere::thread_role::name::work);
			scendere::work_thread_reprioritize ();
			loop (i);
		});
	}
}

scendere::work_pool::~work_pool ()
{
	stop ();
	for (auto & i : threads)
	{
...
void scendere::work_pool::loop (uint64_t thread)
{
	// Quick RNG for work attempts.
	xorshift1024star rng;
	scendere::random_pool::generate_block (reinterpret_cast<uint8_t *> (rng.s.data ()), rng.s.size () * sizeof (decltype (rng.s)::value_type));
	uint64_t work;
	uint64_t output;
	blake2b_state hash;
...
void scendere::work_pool::cancel (scendere::root const & root_a)
{
...
void scendere::work_pool::generate (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t> const &)> callback_a)
{
...
boost::optional<uint64_t> scendere::work_pool::generate (scendere::root const & root_a)
{
	debug_assert (network_constants.is_dev_network ());
	return generate (scendere::work_version::work_1, root_a, network_constants.work.base);
}

boost::optional<uint64_t> scendere::work_pool::generate (scendere::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_constants.is_dev_network ());
	return generate (scendere::work_version::work_1, root_a, difficulty_a);
}

boost::optional<uint64_t> scendere::work_pool::generate (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a)
{
...
size_t scendere::work_pool::size ()
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	return pending.size ();
}

std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (work_pool & work_pool, std::string const & name)
{
	size_t count;
	{
		scendere::lock_guard<scendere::mutex> guard (work_pool.mutex);
