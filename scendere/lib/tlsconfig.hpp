...
scendere::error read_tls_config_toml (boost::filesystem::path const & data_path_a, scendere::tls_config & config_a, scendere::logger_mt & logger_a, std::vector<std::string> const & config_overrides = std::vector<std::string> ());
}
