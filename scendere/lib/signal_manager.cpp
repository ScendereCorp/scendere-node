...
scendere::signal_manager::signal_manager () :
...
scendere::signal_manager::signal_descriptor::signal_descriptor (std::shared_ptr<boost::asio::signal_set> sigset_a, signal_manager & sigman_a, std::function<void (int)> handler_func_a, bool repeat_a) :
	sigset (sigset_a), sigman (sigman_a), handler_func (handler_func_a), repeat (repeat_a)
{
}

void scendere::signal_manager::register_signal_handler (int signum, std::function<void (int)> handler, bool repeat)
{
	...
	sigset->async_wait ([descriptor] (boost::system::error_code const & error, int signum) {
		scendere::signal_manager::base_handler (descriptor, error, signum);
	});

	log (boost::str (boost::format ("Registered signal handler for signal %d") % signum));
}

void scendere::signal_manager::base_handler (scendere::signal_manager::signal_descriptor descriptor, boost::system::error_code const & error, int signum)
{
	if (!error)
	{
