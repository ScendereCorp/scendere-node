...
*/
char const * const SCENDERE_VERSION_STRING = xstr (TAG_VERSION_STRING);
char const * const SCENDERE_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
char const * const SCENDERE_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
char const * const SCENDERE_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
char const * const SCENDERE_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
enum class networks : uint16_t
{
	...
	scendere_dev_network = 0x5241, // 'R', 'A'
	// Normal work parameters, secret beta genesis key, beta IP ports
	scendere_beta_network = 0x5242, // 'R', 'B'
	// Normal work parameters, secret live key, live IP ports
	scendere_live_network = 0x5243, // 'R', 'C'
	// Normal work parameters, secret test genesis key, test IP ports
	scendere_test_network = 0x5258, // 'R', 'X'
};
...
public:
	...
	work_thresholds operator= (scendere::work_thresholds const & other_a)
	{
		return other_a;
	}

	uint64_t threshold_entry (scendere::work_version const, scendere::block_type const) const;
	uint64_t threshold (scendere::block_details const &) const;
	// Ledger threshold
	uint64_t threshold (scendere::work_version const, scendere::block_details const) const;
	uint64_t threshold_base (scendere::work_version const) const;
	uint64_t value (scendere::root const & root_a, uint64_t work_a) const;
	...
	uint64_t difficulty (scendere::work_version const, scendere::root const &, uint64_t const) const;
	uint64_t difficulty (scendere::block const & block_a) const;
	bool validate_entry (scendere::work_version const, scendere::root const &, uint64_t const) const;
	bool validate_entry (scendere::block const &) const;

	/** Network work thresholds. Define these inline as constexpr when moving to cpp17. */
	static scendere::work_thresholds const publish_full;
	static scendere::work_thresholds const publish_beta;
	static scendere::work_thresholds const publish_dev;
	static scendere::work_thresholds const publish_test;
};

class network_constants
{
public:
	network_constants (scendere::work_thresholds & work, scendere::networks network_a) :
	...
	scendere::networks current_network{ scendere::network_constants::active_network };
	...
	scendere::networks network () const
	{
	 ...
	 */
	static void set_active_network (scendere::networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = scendere::networks::scendere_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = scendere::networks::scendere_beta_network;
		}
		else if (network_a == "dev")
		{
			active_network = scendere::networks::scendere_dev_network;
		}
		else if (network_a == "test")
		{
			active_network = scendere::networks::scendere_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == scendere::networks::scendere_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == scendere::networks::scendere_beta_network;
	}
	bool is_dev_network () const
	{
		return current_network == scendere::networks::scendere_dev_network;
	}
	bool is_test_network () const
	{
		return current_network == scendere::networks::scendere_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static scendere::networks active_network;
