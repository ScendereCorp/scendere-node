...
std::string scendere::error_system_messages::message (int ev) const
{
	switch (static_cast<scendere::error_system> (ev))
	{
		case scendere::error_system::generic:
			return "Unknown error";
		case scendere::error_system::deadline_expired:
...
std::shared_ptr<scendere::node> scendere::system::add_node (scendere::node_flags node_flags_a, scendere::transport::transport_type type_a)
{
	return add_node (scendere::node_config (scendere::get_available_port (), logging), node_flags_a, type_a);
}

/** Returns the node added. */
std::shared_ptr<scendere::node> scendere::system::add_node (scendere::node_config const & node_config_a, scendere::node_flags node_flags_a, scendere::transport::transport_type type_a)
{
	auto node (std::make_shared<scendere::node> (io_ctx, scendere::unique_path (), node_config_a, work, node_flags_a, node_sequence++));
	for (auto i : initialization_blocks)
	{
		auto result = node->ledger.process (node->store.tx_begin_write (), *i);
		debug_assert (result.code == scendere::process_result::progress);
	}
	...
	if (nodes.size () > 1)
	{
		...
		for (auto i (begin), j (begin + 1), n (nodes.end ()); j != n; ++i, ++j)
		{
			...
			if (type_a == scendere::transport::transport_type::tcp)
			{
			...
			{
				// UDP connection
				auto channel (std::make_shared<scendere::transport::channel_udp> ((*j)->network.udp_channels, (*i)->network.endpoint (), node1->network_params.network.protocol_version));
			...
			if (type_a == scendere::transport::transport_type::tcp && node_config_a.tcp_incoming_connections_max != 0 && !node_flags_a.disable_tcp_realtime)
			{
		...
		auto iterations1 (0);
		while (std::any_of (begin, nodes.end (), [] (std::shared_ptr<scendere::node> const & node_a) { return node_a->bootstrap_initiator.in_progress (); }))
		{
			poll ();
			++iterations1;
...
scendere::system::system (uint16_t count_a, scendere::transport::transport_type type_a, scendere::node_flags flags_a) :
	system ()
{
	nodes.reserve (count_a);
	for (uint16_t i (0); i < count_a; ++i)
	{
		scendere::node_config config (scendere::get_available_port (), logging);
...
void scendere::system::ledger_initialization_set (std::vector<scendere::keypair> const & reps, scendere::amount const & reserve)
{
	scendere::block_hash previous = scendere::dev::genesis->hash ();
	auto amount = (scendere::dev::constants.genesis_amount - reserve.number ()) / reps.size ();
	auto balance = scendere::dev::constants.genesis_amount;
	for (auto const & i : reps)
	{
		balance -= amount;
		scendere::state_block_builder builder;
		builder.account (scendere::dev::genesis_key.pub)
		.previous (previous)
...
std::shared_ptr<scendere::wallet> scendere::system::wallet (size_t index_a)
{
...
scendere::account scendere::system::account (scendere::transaction const & transaction_a, size_t index_a)
{
	...
	debug_assert (++keys == wallet_l->store.end ());
	return scendere::account (result);
}

uint64_t scendere::system::work_generate_limited (scendere::block_hash const & root_a, uint64_t min_a, uint64_t max_a)
{
	...
	} while (work.network_constants.work.difficulty (scendere::work_version::work_1, root_a, result) >= max_a);
	return result;
}
 ...
 */
std::unique_ptr<scendere::state_block> scendere::upgrade_epoch (scendere::work_pool & pool_a, scendere::ledger & ledger_a, scendere::epoch epoch_a)
{
	auto transaction (ledger_a.store.tx_begin_write ());
	auto dev_genesis_key = scendere::dev::genesis_key;
	...
	auto epoch = builder
				 ...
				 .work (*pool_a.generate (latest, pool_a.network_constants.work.threshold (scendere::work_version::work_1, scendere::block_details (epoch_a, false, false, true))))
				 .build (ec);

	bool error{ true };
	if (!ec && epoch)
	{
		error = ledger_a.process (transaction, *epoch).code != scendere::process_result::progress;
	}

	return !error ? std::move (epoch) : nullptr;
}

void scendere::blocks_confirm (scendere::node & node_a, std::vector<std::shared_ptr<scendere::block>> const & blocks_a, bool const forced_a)
{
...
std::unique_ptr<scendere::state_block> scendere::system::upgrade_genesis_epoch (scendere::node & node_a, scendere::epoch const epoch_a)
{
	return upgrade_epoch (work, node_a.ledger, epoch_a);
}

void scendere::system::deadline_set (std::chrono::duration<double, std::nano> const & delta_a)
{
	deadline = std::chrono::steady_clock::now () + delta_a * deadline_scaling_factor;
}

std::error_code scendere::system::poll (std::chrono::nanoseconds const & wait_time)
{
	...
	else if (count == 1 && timer.since_start ().count () >= SCENDERE_ASIO_HANDLER_TRACKING)
	{
	...
	{
		ec = scendere::error_system::deadline_expired;
		stop ();
	}
	return ec;
}

std::error_code scendere::system::poll_until_true (std::chrono::nanoseconds deadline_a, std::function<bool ()> predicate_a)
{
	std::error_code ec;
 ...
 */
void scendere::system::delay_ms (std::chrono::milliseconds const & delay)
{
...
{
public:
	traffic_generator (uint32_t count_a, uint32_t wait_a, std::shared_ptr<scendere::node> const & node_a, scendere::system & system_a) :
		count (count_a),
		wait (wait_a),
		node (node_a),
		system (system_a)
...
void scendere::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a)
{
...
void scendere::system::generate_usage_traffic (uint32_t count_a, uint32_t wait_a, size_t index_a)
{
	debug_assert (nodes.size () > index_a);
...
void scendere::system::generate_rollback (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
...
void scendere::system::generate_receive (scendere::node & node_a)
{
	std::shared_ptr<scendere::block> send_block;
	{
		auto transaction (node_a.store.tx_begin_read ());
		scendere::account random_account;
		random_pool::generate_block (random_account.bytes.data (), sizeof (random_account.bytes));
		auto i (node_a.store.pending.begin (transaction, scendere::pending_key (random_account, 0)));
		if (i != node_a.store.pending.end ())
		{
			scendere::pending_key const & send_hash (i->first);
			send_block = node_a.store.block.get (transaction, send_hash.hash);
		}
	}
	if (send_block != nullptr)
	{
		auto receive_error (wallet (0)->receive_sync (send_block, scendere::dev::genesis->account (), std::numeric_limits<scendere::uint128_t>::max ()));
		(void)receive_error;
	}
}

void scendere::system::generate_activity (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
...
scendere::account scendere::system::get_random_account (std::vector<scendere::account> & accounts_a)
{
	debug_assert (std::numeric_limits<CryptoPP::word32>::max () > accounts_a.size ());
	auto index (random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (accounts_a.size () - 1)));
...
scendere::uint128_t scendere::system::get_random_amount (scendere::transaction const & transaction_a, scendere::node & node_a, scendere::account const & account_a)
{
	scendere::uint128_t balance (node_a.ledger.account_balance (transaction_a, account_a));
	scendere::uint128_union random_amount;
	scendere::random_pool::generate_block (random_amount.bytes.data (), sizeof (random_amount.bytes));
	return (((scendere::uint256_t{ random_amount.number () } * balance) / scendere::uint256_t{ std::numeric_limits<scendere::uint128_t>::max () }).convert_to<scendere::uint128_t> ());
}

void scendere::system::generate_send_existing (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
	...
	scendere::account source;
	{
		...
		auto transaction (node_a.store.tx_begin_read ());
		scendere::store_iterator<scendere::account, scendere::account_info> entry (node_a.store.account.begin (transaction, account));
		if (entry == node_a.store.account.end ())
		{
			entry = node_a.store.account.begin (transaction);
		}
		debug_assert (entry != node_a.store.account.end ());
		destination = scendere::account (entry->first);
...
void scendere::system::generate_change_known (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
	scendere::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		scendere::account destination (get_random_account (accounts_a));
...
void scendere::system::generate_change_unknown (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
	scendere::account source (get_random_account (accounts_a));
	if (!node_a.latest (source).is_zero ())
	{
		scendere::keypair key;
		scendere::account destination (key.pub);
		auto change_error (wallet (0)->change_sync (source, destination));
		(void)change_error;
		debug_assert (!change_error);
	}
...
void scendere::system::generate_send_new (scendere::node & node_a, std::vector<scendere::account> & accounts_a)
{
...
void scendere::system::generate_mass_activity (uint32_t count_a, scendere::node & node_a)
{
	std::vector<scendere::account> accounts;
	auto dev_genesis_key = scendere::dev::genesis_key;
...
uint16_t scendere::get_available_port ()
{
...
void scendere::cleanup_dev_directories_on_exit ()
{
	// Makes sure everything is cleaned up
	scendere::logging::release_file_sink ();
	// Clean up tmp directories created by the tests. Since it's sometimes useful to
	// see log files after test failures, an environment variable is supported to
	// retain the files.
	if (std::getenv ("TEST_KEEP_TMPDIRS") == nullptr)
