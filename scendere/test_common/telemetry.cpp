...
void scendere::compare_default_telemetry_response_data_excluding_signature (scendere::telemetry_data const & telemetry_data_a, scendere::network_params const & network_params_a, uint64_t bandwidth_limit_a, uint64_t active_difficulty_a)
{
	...
	ASSERT_EQ (telemetry_data_a.major_version, scendere::get_major_node_version ());
	ASSERT_EQ (telemetry_data_a.minor_version, scendere::get_minor_node_version ());
	ASSERT_EQ (telemetry_data_a.patch_version, scendere::get_patch_node_version ());
	ASSERT_EQ (telemetry_data_a.pre_release_version, scendere::get_pre_release_node_version ());
	ASSERT_EQ (telemetry_data_a.maker, static_cast<std::underlying_type_t<scendere::telemetry_maker>> (scendere::telemetry_maker::nf_node));
...
void scendere::compare_default_telemetry_response_data (scendere::telemetry_data const & telemetry_data_a, scendere::network_params const & network_params_a, uint64_t bandwidth_limit_a, uint64_t active_difficulty_a, scendere::keypair const & node_id_a)
{
	ASSERT_FALSE (telemetry_data_a.validate_signature ());
	scendere::telemetry_data telemetry_data_l = telemetry_data_a;
