...
#include <scendere/scendere_node/daemon.hpp>
...
#ifndef _WIN32
#ifdef SCENDERE_STACKTRACE_BACKTRACE
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<scendere::config_key_value_pair>>()->multitoken(), "Pass node configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		("rpcconfig", boost::program_options::value<std::vector<scendere::config_key_value_pair>>()->multitoken(), "Pass rpc configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a scendere_node crash")
		("debug_generate_crash_report", "Consolidates the scendere_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for scendere_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for scendere_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for scendere_dev_network)")
	...
	if (ec == scendere::error_cli::unknown_command)
	{
		...
		{
			if (scendere::network_constants::active_network != scendere::networks::scendere_dev_network)
			{
				auto node_flags = scendere::inactive_node_flag_defaults ();
				scendere::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				scendere::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [] (decltype (bootstrap_weights.second) const & reps) -> scendere::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), scendere::uint128_t{ 0 }, [] (auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<scendere::account, scendere::uint128_t>> sorted;
				...
				struct mismatched_t
				{
					scendere::account rep;
					scendere::uint128_union hardcoded;
					scendere::uint128_union ledger;
					scendere::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (scendere::Mxrb_ratio, 0, true) % ledger.format_balance (scendere::Mxrb_ratio, 0, true) % diff.format_balance (scendere::Mxrb_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger] (auto const & rep) {
					...
					scendere::uint128_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				scendere::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), scendere::uint128_t{ 0 }, [] (auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				scendere::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				scendere::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), scendere::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()] (scendere::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					scendere::uint512_t const mean_diff = x > M ? x - M : M - x;
					scendere::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				scendere::uint128_union const mismatch_stddev = scendere::narrow_cast<scendere::uint128_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (scendere::Gxrb_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (scendere::Gxrb_ratio, mismatch_mean.number ());
				std::vector<std::pair<scendere::account, scendere::uint128_t>> newcomers;
				...
				auto newcomer_entry = [] (auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % scendere::uint128_union (rep.second).format_balance (scendere::Mxrb_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mscendere at %2% blocks\nledger weight %3% Mscendere at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mscendere\n\tmean %7% Mscendere\n\tsigma %8% Mscendere\n")
				% total_hardcoded.format_balance (scendere::Mxrb_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (scendere::Mxrb_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (scendere::Mxrb_ratio, 0, true)
				% mismatch_mean.format_balance (scendere::Mxrb_ratio, 0, true)
				% mismatch_stddev.format_balance (scendere::Mxrb_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = scendere::Gxrb_ratio;
		...
		{
			auto inactive_node = scendere::default_inactive_node (data_path, vm);
			...
			for (; i != end; ++i)
			{
				scendere::block_hash hash = i->first;
				scendere::block_w_sideband sideband = i->second;
				std::shared_ptr<scendere::block> b = sideband.block;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				scendere::uint256_union key;
				if (!key.decode_hex (key_it->second.as<std::string> ()))
				{
					scendere::keypair genesis (key.to_string ());
					scendere::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max () };
					std::cout << "Genesis: " << genesis.prv.to_string () << "\n"
							  << "Public: " << genesis.pub.to_string () << "\n"
							  << "Account: " << genesis.pub.to_account () << "\n";
					scendere::keypair landing;
					...
					scendere::uint128_t balance (std::numeric_limits<scendere::uint128_t>::max ());
					scendere::open_block genesis_block (reinterpret_cast<scendere::block_hash const &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (scendere::work_version::work_1, genesis.pub, network_params.work.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					scendere::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						scendere::uint128_t yearly_distribution (scendere::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							scendere::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (scendere::work_version::work_1, previous, network_params.work.epoch_1));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			auto node = inactive_node.node;
			auto transaction (node->store.tx_begin_read ());
			scendere::uint128_t total;
			auto rep_amounts = node->ledger.cache.rep_weights.get_rep_amounts ();
			std::map<scendere::account, scendere::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<scendere::block_hash> frontier_hashes;
		...
		{
			auto inactive_node = scendere::default_inactive_node (data_path, vm);
			scendere::uint256_union result;
			scendere::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
		...
		{
			uint64_t difficulty{ scendere::work_thresholds::publish_full.base };
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = scendere::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (scendere::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			scendere::work_pool work{ network_params.network, std::numeric_limits<unsigned>::max (), pow_rate_limiter };
			scendere::change_block block (0, 0, scendere::keypair ().prv, 0, 0);
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % scendere::to_string (scendere::difficulty::to_multiplier (difficulty, scendere::work_thresholds::publish_full.base), 4) % scendere::work_thresholds::publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (scendere::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							scendere::opencl_config config (platform, device, threads);
							auto opencl (scendere::opencl_work::create (true, config, logger, network_params.work));
							scendere::work_pool work_pool{ network_params.network, 0, std::chrono::nanoseconds (0), opencl ? [&opencl] (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
							...
							scendere::change_block block (0, 0, scendere::keypair ().prv, 0, 0);
							std::cerr << boost::str (boost::format ("Starting OpenCL generation profiling. Platform: %1%. Device: %2%. Threads: %3%. Difficulty: %4$#x (%5%x from base difficulty %6$#x)\n") % platform % device % threads % difficulty % scendere::to_string (scendere::difficulty::to_multiplier (difficulty, scendere::work_thresholds::publish_full.base), 4) % scendere::work_thresholds::publish_full.base);
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (scendere::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (boost::filesystem::exists ("scendere_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("scendere_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (boost::filesystem::exists ("scendere_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "scendere_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the scendere node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("scendere_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						scendere::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the scendere_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that scendere_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
				}
				else
			...
			{
				std::cerr << "Error: scendere_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		else if (vm.count ("debug_verify_profile"))
		{
			...
			scendere::uint256_union message;
			auto signature = scendere::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				scendere::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			scendere::uint256_union message;
			scendere::uint512_union signature (scendere::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			scendere::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				scendere::keypair key;
				scendere::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					scendere::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			scendere::update_flags (node_flags, vm);
			scendere::inactive_node inactive_node (scendere::unique_path (), data_path, node_flags);
			auto node = inactive_node.node;

			scendere::block_hash genesis_latest (node->latest (scendere::dev::genesis_key.pub));
			scendere::uint128_t genesis_balance (std::numeric_limits<scendere::uint128_t>::max ());
			// Generating keys
			std::vector<scendere::keypair> keys (num_accounts);
			std::vector<scendere::root> frontiers (num_accounts);
			std::vector<scendere::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<scendere::block>> blocks;
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
							...
							.work (*node->work.generate (scendere::work_version::work_1, genesis_latest, node->network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*node->work.generate (scendere::work_version::work_1, keys[i].pub, node->network_params.work.epoch_1))
							.build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
								...
								.work (*node->work.generate (scendere::work_version::work_1, frontiers[j], node->network_params.work.epoch_1))
								.build ();
					...
					auto receive = builder.state ()
								   ...
								   .work (*node->work.generate (scendere::work_version::work_1, frontiers[other], node->network_params.work.epoch_1))
								   .build ();
			...
			scendere::timer<std::chrono::seconds> timer_l (scendere::timer_state::started);
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			scendere::node_flags node_flags;
			scendere::update_flags (node_flags, vm);
			scendere::node_wrapper node_wrapper (scendere::unique_path (), data_path, node_flags);
			auto node = node_wrapper.node;

			scendere::block_hash genesis_latest (node->latest (scendere::dev::genesis_key.pub));
			scendere::uint128_t genesis_balance (std::numeric_limits<scendere::uint128_t>::max ());
			// Generating keys
			std::vector<scendere::keypair> keys (num_representatives);
			scendere::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<scendere::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<scendere::vote> (keys[j].pub, keys[j].prv, sequence, 0, std::vector<scendere::block_hash> (1, i->hash ())));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			auto path2 (scendere::unique_path ());
			logging.init (path1);
			...
			scendere::daemon_config daemon_config{ data_path, network_params };
			auto error = scendere::read_node_config_toml (data_path, daemon_config, config_overrides);

			scendere::node_config config1 = daemon_config.node;
			config1.peering_port = 24000;

			scendere::node_flags flags;
			scendere::update_flags (flags, vm);
			flags.disable_lazy_bootstrap = true;
			flags.disable_legacy_bootstrap = true;
			flags.disable_wallet_bootstrap = true;
			flags.disable_bootstrap_listener = true;
			auto node1 (std::make_shared<scendere::node> (io_ctx1, path1, config1, work, flags, 0));
			scendere::block_hash genesis_latest (node1->latest (scendere::dev::genesis_key.pub));
			scendere::uint128_t genesis_balance (std::numeric_limits<scendere::uint128_t>::max ());
			// Generating blocks
			std::deque<std::shared_ptr<scendere::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
							...
							.link (key.pub)
							.sign (scendere::dev::genesis_key.prv, scendere::dev::genesis_key.pub)
							.work (*work.generate (scendere::work_version::work_1, genesis_latest, scendere::dev::network_params.work.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*work.generate (scendere::work_version::work_1, key.pub, scendere::dev::network_params.work.epoch_1))
							.build ();
			...
			node1->start ();
			scendere::thread_runner runner1 (io_ctx1, node1->config.io_threads);
			...
			// Start new node
			scendere::node_config config2 = daemon_config.node;
			...
			auto node2 (std::make_shared<scendere::node> (io_ctx2, path2, config2, work, flags, 1));
			node2->start ();
			scendere::thread_runner runner2 (io_ctx2, node2->config.io_threads);
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./scendere_node --debug_random_feed | dieharder -a -g 200
			 */
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			scendere::inactive_node inactive_node_l (data_path, node_flags);

			scendere::node_rpc_config config;
			scendere::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<scendere::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
			handler_l->process_request ();
		}
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			scendere::timer<std::chrono::seconds> timer;
			timer.start ();
			auto node_flags = scendere::inactive_node_flag_defaults ();
			scendere::update_flags (node_flags, vm);
			node_flags.generate_cache.block_count = true;
			...
			std::deque<std::pair<scendere::account, scendere::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors] (std::string const & error_message_a) {
				if (!silent)
				{
					static scendere::mutex cerr_mutex;
					scendere::lock_guard<scendere::mutex> lock (cerr_mutex);
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count] (std::shared_ptr<scendere::node> const & node, scendere::read_transaction const & transaction, scendere::account const & account, scendere::account_info const & info) {
				++count;
				...
				scendere::block_hash calculated_hash (0);
				...
				scendere::account calculated_representative{};
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == scendere::block_type::open || block->type () == scendere::block_type::state)
					{
					...
					if (height == 0 && block->type () != scendere::block_type::open && block->type () != scendere::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<scendere::state_block &> (*block.get ()));
					...
					if (block->type () == scendere::block_type::open || block->type () == scendere::block_type::change || block->type () == scendere::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<scendere::pending_key, scendere::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count] (std::shared_ptr<scendere::node> const & node, scendere::read_transaction const & transaction, scendere::pending_key const & key, scendere::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<scendere::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<scendere::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			scendere::update_flags (node_flags, vm);
			scendere::inactive_node node (scendere::unique_path (), node_flags);
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			std::deque<scendere::unchecked_info> epoch_open_blocks;
			{
				auto node_flags = scendere::inactive_node_flag_defaults ();
				scendere::update_flags (node_flags, vm);
				node_flags.generate_cache.block_count = true;
				...
				for (auto i (source_node->store.account.begin (transaction)), n (source_node->store.account.end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							scendere::unchecked_info unchecked_info (block, account, scendere::signature_verification::unknown);
							node.node->block_processor.add (unchecked_info);
							if (block->type () == scendere::block_type::state && block->previous ().is_zero () && source_node->ledger.is_epoch_link (block->link ()))
							{
			...
			scendere::remove_temporary_directories ();
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % scendere::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("debug_cemented_block_count"))
		{
			...
			scendere::update_flags (node_flags, vm);
			scendere::inactive_node node (data_path, node_flags);
		...
		{
#ifdef BOOST_WINDOWS
			if (!scendere::event_log_reg_entry_exists () && !scendere::is_windows_elevated ())
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Scendere\\Scendere registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = scendere::default_inactive_node (data_path, vm);
			inactive_node->node->logger.always_log (scendere::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			auto inactive_node = scendere::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			auto const epoch_count = scendere::normalized_epoch (scendere::epoch::max) + static_cast<std::underlying_type<scendere::epoch>::type> (1);
			// Cache the accounts in a collection to make searching quicker against unchecked keys. Group by epoch
			scendere::locked<std::vector<boost::unordered_set<scendere::account>>> opened_account_versions_shared (epoch_count);
			using opened_account_versions_t = decltype (opened_account_versions_shared)::value_type;
			node->store.account.for_each_par (
			[&opened_account_versions_shared, epoch_count] (scendere::read_transaction const & /*unused*/, scendere::store_iterator<scendere::account, scendere::account_info> i, scendere::store_iterator<scendere::account, scendere::account_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					auto epoch_idx = scendere::normalized_epoch (account_info.epoch ());
			...
			boost::unordered_set<scendere::account> opened_accounts;
			{
				auto opened_account_versions = opened_account_versions_shared.lock ();
			...
			scendere::locked<boost::unordered_map<scendere::account, std::underlying_type_t<scendere::epoch>>> unopened_highest_pending_shared;
			using unopened_highest_pending_t = decltype (unopened_highest_pending_shared)::value_type;
			node->store.pending.for_each_par (
			[&unopened_highest_pending_shared, &opened_accounts] (scendere::read_transaction const & /*unused*/, scendere::store_iterator<scendere::pending_key, scendere::pending_info> i, scendere::store_iterator<scendere::pending_key, scendere::pending_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					if (!exists)
					{
						...
						auto epoch = scendere::normalized_epoch (info.epoch);
