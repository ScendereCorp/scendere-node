...
namespace
{
void scendere_abort_signal_handler (int signum)
{
	...
	scendere::dump_crash_stacktrace ();
	scendere::create_load_memory_address_files ();
...
#ifdef _WIN32
	std::signal (SIGSEGV, scendere_abort_signal_handler);
	std::signal (SIGABRT, scendere_abort_signal_handler);
#else
	struct sigaction sa = {};
	sa.sa_handler = scendere_abort_signal_handler;
...
static void load_and_set_bandwidth_params (std::shared_ptr<scendere::node> const & node, boost::filesystem::path const & data_path, scendere::node_flags const & flags)
{
	scendere::daemon_config config{ data_path, node->network_params };
...
void scendere_daemon::daemon::run (boost::filesystem::path const & data_path, scendere::node_flags const & flags)
{
	...
	if (!error)
	{
		...
		scendere::work_pool opencl_work (config.node.network_params.network, config.node.work_threads, config.node.pow_sleep_interval, opencl ? [&opencl] (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a, std::atomic<int> & ticket_a) {
		...
		try
		{
			...
			auto initialization_text = "Starting up Scendere node...";
			std::cout << initialization_text << std::endl;
			logger.always_log (initialization_text);

			scendere::set_file_descriptor_limit (OPEN_FILE_DESCRIPTORS_LIMIT);
			auto const file_descriptor_limit = scendere::get_file_descriptor_limit ();
			...
			auto node (std::make_shared<scendere::node> (io_ctx, data_path, config.node, opencl_work, flags));
			if (!node->init_error ())
			{
				auto network_label = node->network_params.network.get_current_network_as_string ();
				std::time_t dateTime = std::time (nullptr);

				std::cout << "Network: " << network_label << ", version: " << SCENDERE_VERSION_STRING << "\n"
				...
				node->start ();
				scendere::ipc::ipc_server ipc_server (*node, config.rpc);
				...
				/*if (config.pow_server.enable)
				{
					...
					{
						std::cerr << std::string ("scendere_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path << std::endl;
						std::exit (1);
					}
				...
				if (config.rpc_enable)
				{
					if (!config.rpc.child_process.enable)
					{
						...
						auto error = scendere::read_rpc_config_toml (data_path, rpc_config, flags.rpc_config_overrides);
						if (error)
						{
						...
						rpc_handler = std::make_unique<scendere::inprocess_rpc_handler> (*node, ipc_server, config.rpc, [&ipc_server, &workers = node->workers, &io_ctx] () {
							ipc_server.stop ();
				...
				runner = std::make_unique<scendere::thread_runner> (io_ctx, node->config.io_threads);
				runner->join ();
