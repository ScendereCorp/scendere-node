...
bool scendere_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<scendere_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<scendere_qt::eventloop_event *> (event_a)->action ();
	return true;
}

scendere_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
	QEvent (QEvent::Type::User),
	action (action_a)
{
}

scendere_qt::self_pane::self_pane (scendere_qt::wallet & wallet_a, scendere::account const & account_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	self_layout (new QHBoxLayout),
	self_window (new QWidget),
	your_account_label (new QLabel ("Your Scendere account:")),
	...
	wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % SCENDERE_VERSION_STRING % network).c_str ());
...
void scendere_qt::self_pane::set_balance_text (std::pair<scendere::uint128_t, scendere::uint128_t> balance_a)
{
...
scendere_qt::accounts::accounts (scendere_qt::wallet & wallet_a) :
...
void scendere_qt::accounts::refresh ()
{
	...
	{
		scendere::public_key key (i->first);
...
scendere_qt::import::import (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
scendere_qt::history::history (scendere::ledger & ledger_a, scendere::account const & account_a, scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	model (new QStandardItemModel),
	view (new QTableView),
...
namespace
{
class short_text_visitor : public scendere::block_visitor
{
public:
	short_text_visitor (scendere::transaction const & transaction_a, scendere::ledger & ledger_a) :
	...
	void open_block (scendere::open_block const & block_a)
	{
		...
		else
		{
			...
			amount = scendere::dev::constants.genesis_amount;
		}
	}
	...
	std::string type;
	scendere::uint128_t amount;
	scendere::account account;
};
}

void scendere_qt::history::refresh ()
{
...
scendere_qt::block_viewer::block_viewer (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void scendere_qt::block_viewer::rebroadcast_action (scendere::block_hash const & hash_a)
{
	auto done (true);
...
scendere_qt::account_viewer::account_viewer (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (SCENDERE): ") + wallet.format_balance (balance.first));
...
scendere_qt::stats_viewer::stats_viewer (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	refresh (new QPushButton ("Refresh")),
...
void scendere_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
scendere_qt::status::status (scendere_qt::wallet & wallet_a) :
	wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (scendere_qt::status_types::nominal);
	set_text ();
}

void scendere_qt::status::erase (scendere_qt::status_types status_a)
{
	debug_assert (status_a != scendere_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void scendere_qt::status::insert (scendere_qt::status_types status_a)
{
	debug_assert (status_a != scendere_qt::status_types::nominal);
...
std::string scendere_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case scendere_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case scendere_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case scendere_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case scendere_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case scendere_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case scendere_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case scendere_qt::status_types::nominal:
...
std::string scendere_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
scendere_qt::wallet::wallet (QApplication & application_a, scendere_qt::eventloop_processor & processor_a, scendere::node & node_a, std::shared_ptr<scendere::wallet> const & wallet_a, scendere::account & account_a) :
	rendering_ratio (scendere::Mxrb_ratio),
	node (node_a),
	wallet_m (wallet_a),
	account (account_a),
...
void scendere_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<scendere_qt::wallet> wallet_w (shared_from_this ());
...
void scendere_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<scendere_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<scendere::block> const & block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (scendere::election_status const & status_a, std::vector<scendere::vote_with_weight_info> const & votes_a, scendere::account const & account_a, scendere::uint128_t const & amount_a, bool, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (scendere::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (scendere_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (scendere_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w] (std::shared_ptr<scendere::transport::channel> const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (scendere_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (scendere_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (scendere_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (scendere_qt::status_types::working);
					}
				}
			}));
		}
...
void scendere_qt::wallet::refresh ()
{
	{
...
void scendere_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (scendere_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (scendere_qt::status_types::disconnected);
	}
}

void scendere_qt::wallet::empty_password ()
{
...
void scendere_qt::wallet::change_rendering_ratio (scendere::uint128_t const & rendering_ratio_a)
{
...
//TO-CHANGE
std::string scendere_qt::wallet::format_balance (scendere::uint128_t const & balance) const
{
	auto balance_str = scendere::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("scen");
	if (rendering_ratio == scendere::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void scendere_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void scendere_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

scendere_qt::settings::settings (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	password (new QLineEdit),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (change_rep, &QPushButton::released, [this] () {
		scendere::account representative_l;
...
void scendere_qt::settings::refresh_representative ()
{
...
void scendere_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void scendere_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (scendere_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (scendere_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (scendere_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (scendere_qt::status_types::vulnerable);
	}
}

scendere_qt::advanced_actions::advanced_actions (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	scendere_unit (new QRadioButton ("scen")), //TO-CHANGE
	...
	wallet (wallet_a)
{
	ratio_group->addButton (scendere_unit);
	ratio_group->setId (scendere_unit, ratio_group->buttons ().size () - 1);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (raw_unit, ratio_group->buttons ().size () - 1);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (scendere_unit);
	...
	QObject::connect (scendere_unit, &QRadioButton::toggled, [this] () {
		if (scendere_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (scendere::Mxrb_ratio);
			QSettings ().setValue (saved_ratio_key, ratio_group->id (scendere_unit));
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			this->wallet.change_rendering_ratio (scendere::raw_ratio);
	...
	{
		selected_ratio_button = scendere_unit;
	}
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] () {
		...
		auto error (scendere::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void scendere_qt::advanced_actions::refresh_peers ()
{
...
void scendere_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.account.begin (transaction)), j (wallet.node.ledger.store.account.end ()); i != j; ++i)
	{
		...
		std::string balance;
		scendere::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void scendere_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

scendere_qt::block_entry::block_entry (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	block (new QPlainTextEdit),
	status (new QLabel),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (scendere::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
scendere_qt::block_creation::block_creation (scendere_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void scendere_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void scendere_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void scendere_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void scendere_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void scendere_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void scendere_qt::block_creation::create_send ()
{
	scendere::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		scendere::amount amount_l;
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						scendere::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void scendere_qt::block_creation::create_receive ()
{
	scendere::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				scendere::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							scendere::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
							scendere::block_details details;
...
void scendere_qt::block_creation::create_change ()
{
	scendere::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					scendere::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
					scendere::block_details details;
					details.epoch = info.epoch ();
...
void scendere_qt::block_creation::create_open ()
{
	scendere::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending.get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								scendere::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
								scendere::block_details details;
								details.is_receive = true;
