...
std::string scendere::mapping_protocol::to_string ()
{
	std::stringstream ss;
...
std::string scendere::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

std::string scendere::port_mapping::to_string ()
{
	std::stringstream ss;
...
void scendere::port_mapping::refresh_devices ()
{
...
scendere::endpoint scendere::port_mapping::external_address ()
{
	scendere::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	scendere::lock_guard<scendere::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = scendere::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void scendere::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
		{
			auto upnp_description = std::string ("Scendere Node (") + node.network_params.network.get_current_network_as_string () + ")";
...
bool scendere::port_mapping::check_lost_or_old_mapping ()
{
...
void scendere::port_mapping::check_mapping_loop ()
{
...
void scendere::port_mapping::stop ()
{
	on = false;
	scendere::lock_guard<scendere::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
...
std::string scendere::upnp_state::to_string ()
{
	std::stringstream ss;
...
scendere::upnp_state::~upnp_state ()
{
	if (devices)
	{
...
scendere::upnp_state & scendere::upnp_state::operator= (scendere::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
