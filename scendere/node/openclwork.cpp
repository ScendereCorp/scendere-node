...
bool scendere::opencl_loaded{ true };
#else
bool scendere::opencl_loaded{ false };
#endif
...
enum IV_Derived {
    scendere_xor_iv0 = 0x6a09e667f2bdc900UL,  // iv1 ^ 0x1010000 ^ outlen
    scendere_xor_iv4 = 0x510e527fade682f9UL,  // iv4 ^ inbytes
    scendere_xor_iv6 = 0xe07c265404be4294UL,  // iv6 ^ ~0
};
...
{
    ulong2 vv[8] = {
        {scendere_xor_iv0, iv1}, {iv2, iv3},          {iv4, iv5},
        {iv6, iv7},          {iv0, iv1},          {iv2, iv3},
        {scendere_xor_iv4, iv5}, {scendere_xor_iv6, iv7},
    };
    ...
    return scendere_xor_iv0 ^ vv[0].s0 ^ vv[4].s0;
}
#undef G32
#undef G2v
#undef G2v_split
...
__kernel void scendere_work(__constant ulong *attempt,
...
scendere::opencl_environment::opencl_environment (bool & error_a)
{
	if (scendere::opencl_loaded)
	{
		...
		{
			scendere::opencl_platform platform;
			platform.platform = *i;
...
void scendere::opencl_environment::dump (std::ostream & stream)
{
	if (scendere::opencl_loaded)
	{
		auto index (0);
...
scendere::opencl_work::opencl_work (bool & error_a, scendere::opencl_config const & config_a, scendere::opencl_environment & environment_a, scendere::logger_mt & logger_a, scendere::work_thresholds & work) :
	config (config_a),
	context (0),
	attempt_buffer (0),
	result_buffer (0),
	...
	work{ work }
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			scendere::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							std::size_t item_size (sizeof (scendere::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "scendere_work", &kernel_error);
...
scendere::opencl_work::~opencl_work ()
{
	if (kernel != 0)
	{
...
boost::optional<uint64_t> scendere::opencl_work::generate_work (scendere::work_version const version_a, scendere::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> scendere::opencl_work::generate_work (scendere::work_version const version_a, scendere::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (work.difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (scendere::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<scendere::opencl_work> scendere::opencl_work::create (bool create_a, scendere::opencl_config const & config_a, scendere::logger_mt & logger_a, scendere::work_thresholds & work)
{
	std::unique_ptr<scendere::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new scendere::opencl_work (error, config_a, environment, logger_a, work));
			if (error)
			{
				result.reset ();
			}
