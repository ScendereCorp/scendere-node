...
class prioritization final
{
	...
	std::vector<scendere::uint128_t> minimums;
...
public:
	...
	void push (uint64_t time, std::shared_ptr<scendere::block> block);
	std::shared_ptr<scendere::block> top () const;
	void pop ();
	...
	std::unique_ptr<scendere::container_info_component> collect_container_info (std::string const &);
};
}
