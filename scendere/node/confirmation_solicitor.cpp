...
scendere::confirmation_solicitor::confirmation_solicitor (scendere::network & network_a, scendere::node_config const & config_a) :
...
void scendere::confirmation_solicitor::prepare (std::vector<scendere::representative> const & representatives_a)
{
...
bool scendere::confirmation_solicitor::broadcast (scendere::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	if (rebroadcasted++ < max_block_broadcasts)
	{
		...
		scendere::publish winner{ config.network_params.network, election_a.status.winner };
		unsigned count = 0;
		...
		network.flood_message (winner, scendere::buffer_drop_policy::limiter, 0.5f);
		error = false;
	}
	return error;
}

bool scendere::confirmation_solicitor::add (scendere::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	unsigned count = 0;
	auto const max_channel_requests (config.confirm_req_batches_max * scendere::network::confirm_req_hashes_max);
	auto const & hash (election_a.status.winner->hash ());
...
void scendere::confirmation_solicitor::flush ()
{
	debug_assert (prepared);
	for (auto const & request_queue : requests)
	{
		...
		for (auto const & root_hash : request_queue.second)
		{
			...
			if (roots_hashes_l.size () == scendere::network::confirm_req_hashes_max)
			{
				scendere::confirm_req req{ config.network_params.network, roots_hashes_l };
				channel->send (req);
