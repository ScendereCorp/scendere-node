...
scendere::ipc::broker::broker (scendere::node & node_a) :
	node (node_a)
{
}

std::shared_ptr<flatbuffers::Parser> scendere::ipc::subscriber::get_parser (scendere::ipc::ipc_config const & ipc_config_a)
{
	if (!parser)
	{
		parser = scendere::ipc::flatbuffers_handler::make_flatbuffers_parser (ipc_config_a);
	}
	return parser;
}

void scendere::ipc::broker::start ()
{
	node.observers.blocks.add ([this_l = shared_from_this ()] (scendere::election_status const & status_a, std::vector<scendere::vote_with_weight_info> const & votes_a, scendere::account const & account_a, scendere::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
		...
		try
		{
			...
			{
				auto confirmation (std::make_shared<scendereapi::EventConfirmationT> ());
				...
				switch (status_a.type)
				{
					case scendere::election_status_type::active_confirmed_quorum:
						confirmation->confirmation_type = scendereapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
						break;
					case scendere::election_status_type::active_confirmation_height:
						confirmation->confirmation_type = scendereapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height;
						break;
					case scendere::election_status_type::inactive_confirmation_height:
						confirmation->confirmation_type = scendereapi::TopicConfirmationType::TopicConfirmationType_inactive;
						break;
					default:
						debug_assert (false);
						break;
				...
				confirmation->confirmation_type = scendereapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
				confirmation->block = scendere::ipc::flatbuffers_builder::block_to_union (*status_a.winner, amount_a, is_state_send_a, is_state_epoch_a);
				confirmation->election_info = std::make_unique<scendereapi::ElectionInfoT> ();
		...
		catch (scendere::error const & err)
		{
...
void subscribe_or_unsubscribe (scendere::logger_mt & logger, COLL & subscriber_collection, std::weak_ptr<scendere::ipc::subscriber> const & subscriber_a, TOPIC_TYPE topic_a)
{
...
void scendere::ipc::broker::subscribe (std::weak_ptr<scendere::ipc::subscriber> const & subscriber_a, std::shared_ptr<scendereapi::TopicConfirmationT> const & confirmation_a)
{
...
void scendere::ipc::broker::broadcast (std::shared_ptr<scendereapi::EventConfirmationT> const & confirmation_a)
{
	using Filter = scendereapi::TopicConfirmationTypeFilter;
	decltype (confirmation_a->election_info) election_info;
	scendereapi::BlockUnion block;
	...
	while (itr != confirmation_subscribers->end ())
	{
		if (auto subscriber_l = itr->subscriber.lock ())
		{
			auto should_filter = [this, &itr, confirmation_a] () {
				...
				if ((confirmation_a->confirmation_type == scendereapi::TopicConfirmationType::TopicConfirmationType_active_quorum || confirmation_a->confirmation_type == scendereapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height) && (all_filter || active_filter))
				{
					should_filter_conf_type_l = false;
				}
				else if (confirmation_a->confirmation_type == scendereapi::TopicConfirmationType::TopicConfirmationType_inactive && (all_filter || inactive_filter))
				{
					should_filter_conf_type_l = false;
				}
			...
			{
				auto fb (scendere::ipc::flatbuffer_producer::make_buffer (*confirmation_a));

				if (subscriber_l->get_active_encoding () == scendere::ipc::payload_encoding::flatbuffers_json)
				{
					...
					{
						throw scendere::error ("Couldn't serialize response to JSON");
					}

					subscriber_l->async_send_message (reinterpret_cast<uint8_t const *> (json->data ()), json->size (), [json] (scendere::error const & err) {});
				}
				else
				{
					subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (scendere::error const & err) {});
				}
			}
			...
			if (block.type != scendereapi::Block::Block_NONE)
			{
...
std::size_t scendere::ipc::broker::confirmation_subscriber_count () const
{
	return confirmation_subscribers->size ();
}

void scendere::ipc::broker::service_register (std::string const & service_name_a, std::weak_ptr<scendere::ipc::subscriber> const & subscriber_a)
{
...
void scendere::ipc::broker::service_stop (std::string const & service_name_a)
{
	...
	for (auto & subcription : subscribers.get ())
	{
		if (auto subscriber_l = subcription.subscriber.lock ())
		{
			...
			{
				scendereapi::EventServiceStopT event_stop;
				auto fb (scendere::ipc::flatbuffer_producer::make_buffer (event_stop));
				subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (scendere::error const & err) {});
...
void scendere::ipc::broker::subscribe (std::weak_ptr<scendere::ipc::subscriber> const & subscriber_a, std::shared_ptr<scendereapi::TopicServiceStopT> const & service_stop_a)
{
	auto subscribers = service_stop_subscribers.lock ();
