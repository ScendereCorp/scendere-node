...
namespace ipc
{
	...
	{
	public:
		std::unordered_set<scendere::ipc::access_permission, enum_hash> permissions;
	...
	public:
		...
		std::string id;
		std::vector<scendere::ipc::access_role> roles;
		void clear () override;
	};
	...
	{
	public:
		bool has_access (std::string const & credentials_a, scendere::ipc::access_permission permission_a) const;
		bool has_access_to_all (std::string const & credentials_a, std::initializer_list<scendere::ipc::access_permission> permissions_a) const;
		bool has_access_to_oneof (std::string const & credentials_a, std::initializer_list<scendere::ipc::access_permission> permissions_a) const;
		scendere::error deserialize_toml (scendere::tomlconfig &);

	private:
		/** Process allow and deny entries for the given subject */
		void set_effective_permissions (scendere::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a);

		/** Clear current users, roles and default permissions */
		void clear ();

		std::unordered_map<std::string, scendere::ipc::access_user> users;
		std::unordered_map<std::string, scendere::ipc::access_role> roles;
