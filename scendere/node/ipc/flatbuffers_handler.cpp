...
namespace
{
auto handler_map = scendere::ipc::action_handler::handler_map ();
...
scendere::ipc::flatbuffers_handler::flatbuffers_handler (scendere::node & node_a, scendere::ipc::ipc_server & ipc_server_a, std::shared_ptr<scendere::ipc::subscriber> const & subscriber_a, scendere::ipc::ipc_config const & ipc_config_a) :
	node (node_a),
...
std::shared_ptr<flatbuffers::Parser> scendere::ipc::flatbuffers_handler::make_flatbuffers_parser (scendere::ipc::ipc_config const & ipc_config_a)
{
	...
	if (!api_path)
	{
		throw scendere::error ("Internal IPC error: unable to find api path");
	}

	char const * include_directories[] = { api_path->string ().c_str (), nullptr };
	std::string schemafile;
	if (!flatbuffers::LoadFile ((*api_path / "scendereapi.fbs").string ().c_str (), false, &schemafile))
	{
		throw scendere::error ("Internal IPC error: unable to load schema file");
	}
	...
	if (!parse_success)
	{
		...
		throw scendere::error (parser_error);
	}
	return parser;
}

void scendere::ipc::flatbuffers_handler::process_json (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
...
void scendere::ipc::flatbuffers_handler::process (uint8_t const * message_buffer_a, std::size_t buffer_size_a,
std::function<void (std::shared_ptr<flatbuffers::FlatBufferBuilder> const &)> const & response_handler)
{
	...
	try
	{
		...
		if (ipc_config.flatbuffers.verify_buffers)
		{
			...
			{
				throw scendere::error ("Envelope buffer did not pass verifier");
			}
		}

		auto incoming = scendereapi::GetEnvelope (message_buffer_a);
		if (incoming == nullptr)
		{
			scendere::error err ("Invalid message");
		...
		{
			scendere::error err ("Unknown message type");
			actionhandler->make_error (err.error_code_as_int (), err.get_message ());
		}
	}
	catch (scendere::error const & err)
