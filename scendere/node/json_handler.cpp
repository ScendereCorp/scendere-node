...
namespace
{
void construct_json (scendere::container_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void (scendere::json_handler *)>>;
...
bool block_confirmed (scendere::node & node, scendere::transaction & transaction, scendere::block_hash const & hash, bool include_active, bool include_only_confirmed);
char const * epoch_as_string (scendere::epoch);
}

scendere::json_handler::json_handler (scendere::node & node_a, scendere::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void (std::string const &)> const & response_a, std::function<void ()> stop_callback_a) :
	body (body_a),
	node (node_a),
	response (response_a),
...
std::function<void ()> scendere::json_handler::create_worker_task (std::function<void (std::shared_ptr<scendere::json_handler> const &)> const & action_a)
{
...
void scendere::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kscendere_from_raw" || action == "krai_from_raw")
			{
				mscendere_from_raw (scendere::kxrb_ratio);
			}
			else if (action == "kscendere_to_raw" || action == "krai_to_raw")
			{
				mscendere_to_raw (scendere::kxrb_ratio);
			}
			else if (action == "rai_from_raw")
			{
				mscendere_from_raw (scendere::xrb_ratio);
			}
			else if (action == "rai_to_raw")
			{
				mscendere_to_raw (scendere::xrb_ratio);
			}
			else if (action == "mscendere_from_raw" || action == "mrai_from_raw")
			{
				mscendere_from_raw ();
			}
			else if (action == "mscendere_to_raw" || action == "mrai_to_raw")
			{
				mscendere_to_raw ();
			}
			else if (action == "scendere_to_raw")
			{
				scendere_to_raw ();
			}
			else if (action == "raw_to_scendere")
			{
				raw_to_scendere ();
			}
...
void scendere::json_handler::response_errors ()
{
	if (!ec && response_l.empty ())
	{
		// Return an error code if no response data was given
		ec = scendere::error_rpc::empty_response;
	}
	if (ec)
	{
...
std::shared_ptr<scendere::wallet> scendere::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = scendere::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = scendere::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool scendere::json_handler::wallet_locked_impl (scendere::transaction const & transaction_a, std::shared_ptr<scendere::wallet> const & wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = scendere::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool scendere::json_handler::wallet_account_impl (scendere::transaction const & transaction_a, std::shared_ptr<scendere::wallet> const & wallet_a, scendere::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = scendere::error_common::account_not_found_wallet;
		}
	}
	return result;
}

scendere::account scendere::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
	...
	if (!ec)
	{
		...
		{
			// scendere- and xrb- prefixes are deprecated
...
scendere::account_info scendere::json_handler::account_info_impl (scendere::transaction const & transaction_a, scendere::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = scendere::error_common::account_not_found;
...
scendere::amount scendere::json_handler::amount_impl ()
{
	scendere::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = scendere::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<scendere::block> scendere::json_handler::block_impl (bool signature_work_required)
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	std::shared_ptr<scendere::block> result{ nullptr };
	if (!ec)
	{
		...
		else
		{
			...
			{
				ec = scendere::error_blocks::invalid_block;
			}
		}
		if (!ec)
		{
			...
			result = scendere::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = scendere::error_blocks::invalid_block;
			}
...
scendere::block_hash scendere::json_handler::hash_impl (std::string search_text)
{
	scendere::block_hash result (0);
	if (!ec)
	{
		std::string hash_text (request.get<std::string> (search_text));
		if (result.decode_hex (hash_text))
		{
			ec = scendere::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

scendere::amount scendere::json_handler::threshold_optional_impl ()
{
	scendere::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t scendere::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (scendere::from_string_hex (work_text.get (), result))
		{
			ec = scendere::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t scendere::json_handler::difficulty_optional_impl (scendere::work_version const version_a)
{
	...
	{
		if (scendere::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = scendere::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

uint64_t scendere::json_handler::difficulty_ledger (scendere::block const & block_a)
{
	scendere::block_details details (scendere::epoch::epoch_0, false, false, false);
	bool details_found (false);
	auto transaction (node.store.tx_begin_read ());
	// Previous block find
	std::shared_ptr<scendere::block> block_previous (nullptr);
	...
	if (!link.is_zero () && !details.is_send)
	{
		...
		if (block_link != nullptr && node.store.pending.exists (transaction, scendere::pending_key (block_a.account (), link.as_block_hash ())))
		{
...
double scendere::json_handler::multiplier_optional_impl (scendere::work_version const version_a, uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = scendere::difficulty::from_multiplier (multiplier, node.default_difficulty (version_a));
		}
		else
		{
			ec = scendere::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}

scendere::work_version scendere::json_handler::work_version_optional_impl (scendere::work_version const default_a)
{
	scendere::work_version result = default_a;
	boost::optional<std::string> version_text (request.get_optional<std::string> ("version"));
	if (!ec && version_text.is_initialized ())
	{
		if (*version_text == scendere::to_string (scendere::work_version::work_1))
		{
			result = scendere::work_version::work_1;
		}
		else
		{
			ec = scendere::error_rpc::bad_work_version;
		}
	}
	return result;
}
...
uint64_t scendere::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = scendere::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t scendere::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t scendere::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::invalid_offset;
		}
	}
	return result;
}

void scendere::json_handler::account_balance ()
{
...
void scendere::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void scendere::json_handler::account_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			bool const generate_work = rpc_l->request.get<bool> ("work", true);
			scendere::account new_key;
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = scendere::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = scendere::error_common::wallet_locked;
				}
			}
		}
...
void scendere::json_handler::account_get ()
{
	std::string key_text (request.get<std::string> ("key"));
	scendere::public_key pub;
	...
	{
		ec = scendere::error_common::bad_public_key;
	}
	response_errors ();
}

void scendere::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			scendere::amount balance_l (info.balance);
			std::string balance;
			balance_l.encode_dec (balance);

			response_l.put ("balance", balance);

			scendere::amount confirmed_balance_l;
			...
			std::shared_ptr<scendere::block> confirmed_frontier_block;
			...
			if (representative)
			{
				...
				if (include_confirmed)
				{
					scendere::account confirmed_representative{};
...
void scendere::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void scendere::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", scendere::account (i->first).to_account ());
...
void scendere::json_handler::account_move ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<scendere::public_key> accounts;
				...
				{
					rpc_l->ec = scendere::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = scendere::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	}));
...
void scendere::json_handler::account_remove ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
...
void scendere::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void scendere::json_handler::account_representative_set ()
{
	node.workers.push_task (create_worker_task ([work_generation_enabled = node.work_generation_enabled ()] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		auto representative (rpc_l->account_impl (representative_text, scendere::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						scendere::block_details details (info.epoch (), false, false, false);
						if (rpc_l->node.network_params.work.difficulty (scendere::work_version::work_1, info.head, work) < rpc_l->node.network_params.work.threshold (scendere::work_version::work_1, details))
						{
							rpc_l->ec = scendere::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = scendere::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				wallet->change_async (
				account, representative, [response_a, response_data] (std::shared_ptr<scendere::block> const & block) {
					if (block != nullptr)
					{
...
void scendere::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void scendere::json_handler::accounts_balances ()
{
...
void scendere::json_handler::accounts_representatives ()
{
...
void scendere::json_handler::accounts_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				scendere::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void scendere::json_handler::accounts_frontiers ()
{
...
void scendere::json_handler::accounts_pending ()
{
	response_l.put ("deprecated", "1");
	accounts_receivable ();
}

void scendere::json_handler::accounts_receivable ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending.begin (transaction, scendere::pending_key (account, 0))), n (node.store.pending.end ()); i != n && scendere::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
				scendere::pending_key const & key (i->first);
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([] (auto const & child1, auto const & child2) -> bool {
						return child1.second.template get<scendere::uint128_t> ("amount") > child2.second.template get<scendere::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([] (auto const & child1, auto const & child2) -> bool {
						return child1.second.template get<scendere::uint128_t> ("") > child2.second.template get<scendere::uint128_t> ("");
					});
				}
			}
			if (!peers_l.empty ())
...
void scendere::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	auto const multiplier_active = 1.0;
	auto const default_difficulty (node.default_difficulty (scendere::work_version::work_1));
	auto const default_receive_difficulty (node.default_receive_difficulty (scendere::work_version::work_1));
	...
	response_l.put ("deprecated", "1");
	response_l.put ("network_minimum", scendere::to_string_hex (default_difficulty));
	response_l.put ("network_receive_minimum", scendere::to_string_hex (default_receive_difficulty));
	response_l.put ("network_current", scendere::to_string_hex (scendere::difficulty::from_multiplier (multiplier_active, default_difficulty)));
	response_l.put ("network_receive_current", scendere::to_string_hex (scendere::difficulty::from_multiplier (receive_current_denormalized, default_receive_difficulty)));
...
void scendere::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis->account ())); // Cold storage genesis
	auto landing_balance (node.balance (scendere::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (scendere::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (scendere::account{}, false)).second); // Burning 0 account
	auto available (scendere::dev::constants.genesis_amount - genesis_balance - landing_balance - faucet_balance - burned_balance);
	response_l.put ("available", available.convert_to<std::string> ());
	response_errors ();
}

void scendere::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			scendere::account account (block->account ().is_zero () ? block->sideband ().account : block->account ());
			...
			if (block->type () == scendere::block_type::state)
			{
				auto subtype (scendere::state_subtype (block->sideband ().details));
				response_l.put ("subtype", subtype);
			}
		}
		else
		{
			ec = scendere::error_blocks::not_found;
		}
	}
	response_errors ();
}

void scendere::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				scendere::election_status status{ block_l, 0, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, scendere::election_status_type::active_confirmation_height };
				...
				if (!error_or_pruned)
				{
					if (auto state = dynamic_cast<scendere::state_block *> (block_l.get ()))
					{
...
void scendere::json_handler::blocks ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = scendere::error_blocks::bad_hash_number;
			}
		}
	}
...
void scendere::json_handler::blocks_info ()
{
	bool const pending = request.get<bool> ("pending", false);
	bool const receivable = request.get<bool> ("receivable", pending);
				...
				if (block != nullptr)
				{
					...
					if (receivable || receive_hash)
					{
						...
						if (destination.is_zero ())
						{
							...
							if (receive_hash)
							{
								entry.put ("receive_hash", scendere::block_hash (0).to_string ());
							}
						}
						else if (node.store.pending.exists (transaction, scendere::pending_key (destination, hash)))
						{
							if (receivable)
							{
								entry.put ("pending", "1");
						...
						else
						{
							...
							if (receive_hash)
							{
								std::shared_ptr<scendere::block> receive_block = node.ledger.find_receive_block_by_send_hash (transaction, destination, hash);
								std::string receive_hash = receive_block ? receive_block->hash ().to_string () : scendere::block_hash (0).to_string ();
					...
					if (source)
					{
						scendere::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void scendere::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void scendere::json_handler::block_count ()
{
...
void scendere::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	scendere::wallet_id wallet (0);
	// Default to work_1 if not specified
	auto work_version (work_version_optional_impl (scendere::work_version::work_1));
	...
	{
		representative = account_impl (representative_text.get (), scendere::error_rpc::bad_representative_number);
	}
	...
	{
		destination = account_impl (destination_text.get (), scendere::error_rpc::bad_destination);
	}
	scendere::block_hash source (0);
	...
	if (!ec && source_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::bad_source;
		}
	}
	...
	prv.clear ();
	scendere::block_hash previous (0);
	scendere::amount balance (0);
	if (work == 0 && !node.work_generation_enabled ())
	{
		ec = scendere::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::invalid_balance;
		}
	}
	...
	if (!ec && link_text.is_initialized ())
	{
		if (link.decode_account (link_text.get ()))
		{
			...
			{
				ec = scendere::error_rpc::bad_link;
			}
		}
	}
	else
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this] (scendere::block const & block_a) {
			boost::property_tree::ptree response_l;
			response_l.put ("hash", block_a.hash ().to_string ());
			response_l.put ("difficulty", scendere::to_string_hex (rpc_l->node.network_params.work.difficulty (block_a)));
			bool json_block_l = request.get<bool> ("json_block", false);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
		...
		auto get_callback_l = [rpc_l, block_response_put_l] (std::shared_ptr<scendere::block> const & block_a) {
			...
			return [block_a, rpc_l, block_response_put_l] (boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = scendere::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = scendere::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv != 0)
		{
			scendere::account pub (scendere::pub_key (prv));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = scendere::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = scendere::error_rpc::block_create_public_key_mismatch;
				}
			}
			scendere::block_builder builder_l;
			std::shared_ptr<scendere::block> block_l{ nullptr };
			scendere::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = scendere::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = scendere::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = scendere::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = scendere::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = scendere::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = scendere::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = scendere::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == scendere::error_common::missing_work))
			{
				if (work == 0)
				{
					...
					node.work_generate (work_version, root_l, difficulty_l, get_callback_l (block_l), scendere::account (pub));
				}
				else
				{
		...
		{
			ec = scendere::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void scendere::json_handler::block_hash ()
{
...
void scendere::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!scendere::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				std::string bootstrap_id (request.get<std::string> ("id", ""));
				node.bootstrap_initiator.bootstrap (scendere::endpoint (address, port), true, bootstrap_id);
				response_l.put ("success", "");
			}
			else
			{
				ec = scendere::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = scendere::error_common::invalid_port;
		}
	}
	else
	{
		ec = scendere::error_common::invalid_ip_address;
	}
	response_errors ();
}

void scendere::json_handler::bootstrap_any ()
{
...
void scendere::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	bool const force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = scendere::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void scendere::json_handler::bootstrap_status ()
{
	...
	{
		scendere::lock_guard<scendere::mutex> connections_lock (node.bootstrap_initiator.connections->mutex);
	...
	{
		scendere::lock_guard<scendere::mutex> attempts_lock (node.bootstrap_initiator.attempts.bootstrap_attempts_mutex);
...
void scendere::json_handler::chain (bool successors)
{
...
void scendere::json_handler::confirmation_active ()
{
...
void scendere::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = scendere::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void scendere::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void scendere::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (election != nullptr && !election->confirmed ())
		{
			...
			for (auto const & [tally, block] : info.tally)
			{
				...
				if (representatives)
				{
					std::multimap<scendere::uint128_t, scendere::account, std::greater<scendere::uint128_t>> representatives;
		...
		{
			ec = scendere::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = scendere::error_rpc::invalid_root;
	}
	response_errors ();
}

void scendere::json_handler::confirmation_quorum ()
{
...
void scendere::json_handler::database_txn_tracker ()
{
	...
	{
		ec = scendere::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void scendere::json_handler::delegators ()
{
	...
	if (!ec)
	{
		...
		for (auto i (node.store.account.begin (transaction, start_account.number () + 1)), n (node.store.account.end ()); i != n && delegators.size () < count; ++i)
		{
			...
			if (info.representative == representative)
			{
				...
				{
					std::string balance;
					scendere::uint128_union (info.balance).encode_dec (balance);
					scendere::account const & delegator (i->first);
...
void scendere::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void scendere::json_handler::deterministic_key ()
{
	...
	if (!seed.decode_hex (seed_text))
	{
		try
		{
			...
			scendere::raw_key prv = scendere::deterministic_key (seed, index);
		...
		{
			ec = scendere::error_common::invalid_index;
		}
	}
	else
	{
		ec = scendere::error_common::bad_seed;
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void scendere::json_handler::epoch_upgrade ()
{
	scendere::epoch epoch (scendere::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = scendere::epoch::epoch_1;
			break;
		case 2:
			epoch = scendere::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != scendere::epoch::invalid)
	{
		...
		if (!ec && threads_text.is_initialized ())
		{
			...
			{
				ec = scendere::error_rpc::invalid_threads_count;
			}
		}
		std::string key_text (request.get<std::string> ("key"));
		scendere::raw_key prv;
		...
		{
			if (scendere::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = scendere::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = scendere::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void scendere::json_handler::frontiers ()
{
...
void scendere::json_handler::account_count ()
{
...
namespace
{
class history_visitor : public scendere::block_visitor
{
public:
	history_visitor (scendere::json_handler & handler_a, bool raw_a, scendere::transaction & transaction_a, boost::property_tree::ptree & tree_a, scendere::block_hash const & hash_a, std::vector<scendere::public_key> const & accounts_filter_a) :
		handler (handler_a),
		raw (raw_a),
	...
	void send_block (scendere::send_block const & block_a)
	{
	...
	void receive_block (scendere::receive_block const & block_a)
	{
	...
	void open_block (scendere::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
		...
		else
		{
			...
			tree.put ("amount", scendere::dev::constants.genesis_amount.convert_to<std::string> ());
		}
	}
	void change_block (scendere::change_block const & block_a)
	{
	...
	void state_block (scendere::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (scendere::public_key const & account)
	{
		bool ignore (false);
	...
	scendere::json_handler & handler;
	bool raw;
	scendere::transaction & transaction;
	boost::property_tree::ptree & tree;
	scendere::block_hash const & hash;
	std::vector<scendere::public_key> const & accounts_filter;
};
}

void scendere::json_handler::account_history ()
{
	std::vector<scendere::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", scendere::to_string_hex (block->block_work ()));
...
void scendere::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void scendere::json_handler::key_create ()
{
	scendere::keypair pair;
...
void scendere::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	scendere::raw_key prv;
	if (!prv.decode_hex (key_text))
...
void scendere::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = scendere::error_rpc::invalid_timestamp;
			}
		}
		bool const sorting = request.get<bool> ("sorting", false);
		bool const representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<scendere::uint128_union, scendere::account>> ledger_l;
			for (auto i (node.store.account.begin (transaction, start)), n (node.store.account.end ()); i != n; ++i)
			{
				scendere::account_info const & info (i->second);
				scendere::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					scendere::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (receivable)
					{
						auto account_receivable = node.ledger.account_receivable (transaction, account);
...
void scendere::json_handler::mscendere_from_raw (scendere::uint128_t ratio)
{
...
void scendere::json_handler::mscendere_to_raw (scendere::uint128_t ratio)
{
	auto amount (amount_impl ());
	response_l.put ("deprecated", "1");
	if (!ec)
	{
		...
		{
			ec = scendere::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}

void scendere::json_handler::scendere_to_raw ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
		auto result (amount.number () * scendere::Mxrb_ratio);
		if (result > amount.number ())
		{
			response_l.put ("amount", result.convert_to<std::string> ());
		}
...
void scendere::json_handler::raw_to_scendere ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void scendere::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void scendere::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void scendere::json_handler::password_change ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void scendere::json_handler::password_enter ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void scendere::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::peers ()
{
	boost::property_tree::ptree peers_l;
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == scendere::transport::transport_type::tcp ? "tcp" : "udp");
...
void scendere::json_handler::pending ()
{
	response_l.put ("deprecated", "1");
	receivable ();
}

void scendere::json_handler::receivable ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto offset (offset_optional_impl (0));
	...
	if (!ec)
	{
		...
		std::vector<std::pair<std::string, scendere::uint128_t>> hash_amount_pairs;
		for (auto i (node.store.pending.begin (transaction, scendere::pending_key (account, 0))), n (node.store.pending.end ()); i != n && scendere::pending_key (i->first).account == account && (should_sort || peers_l.size () < count); ++i)
		{
			scendere::pending_key const & key (i->first);
			if (block_confirmed (node, transaction, key.hash, include_active, include_only_confirmed))
			{
		...
		if (should_sort)
		{
			if (source || min_version)
			{
				std::stable_sort (hash_ptree_pairs.begin (), hash_ptree_pairs.end (), [] (auto const & lhs, auto const & rhs) {
					return lhs.second.template get<scendere::uint128_t> ("amount") > rhs.second.template get<scendere::uint128_t> ("amount");
				});
...
void scendere::json_handler::pending_exists ()
{
	response_l.put ("deprecated", "1");
	receivable_exists ();
}

void scendere::json_handler::receivable_exists ()
{
	auto hash (hash_impl ());
	bool const include_active = request.get<bool> ("include_active", false);
	bool const include_only_confirmed = request.get<bool> ("include_only_confirmed", true);
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			if (!destination.is_zero ())
			{
				exists = node.store.pending.exists (transaction, scendere::pending_key (destination, hash));
			}
...
void scendere::json_handler::process ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == scendere::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<scendere::state_block> block_state (std::static_pointer_cast<scendere::state_block> (block));
				...
				{
					rpc_l->ec = scendere::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = scendere::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = scendere::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = scendere::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = scendere::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!rpc_l->node.network_params.work.validate_entry (*block))
			{
				if (!is_async)
				{
					...
					switch (result.code)
					{
						...
						case scendere::process_result::fork:
						{
							...
							{
								rpc_l->ec = scendere::error_process::fork;
							}
							break;
						}
						case scendere::process_result::insufficient_work:
						{
							rpc_l->ec = scendere::error_process::insufficient_work;
							break;
						}
						default:
						{
							rpc_l->ec = scendere::error_process::other;
							break;
						}
					}
				}
				else
				{
					...
					{
						rpc_l->ec = scendere::error_common::is_not_state_block;
					}
				}
			}
			else
			{
				rpc_l->ec = scendere::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	}));
...
void scendere::json_handler::pruned_exists ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		{
			ec = scendere::error_rpc::pruning_disabled;
		}
	}
	response_errors ();
}

void scendere::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (node.ledger.block_or_pruned_exists (block_transaction, hash))
			{
				...
				if (!node.store.pending.get (block_transaction, scendere::pending_key (account, hash), pending_info))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						scendere::account_info info;
						scendere::root head;
						scendere::epoch epoch = pending_info.epoch;
						...
						scendere::block_details details (epoch, false, true, false);
						if (node.network_params.work.difficulty (scendere::work_version::work_1, head, work) < node.network_params.work.threshold (scendere::work_version::work_1, details))
						{
							ec = scendere::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						scendere::account representative (wallet->store.representative (wallet_transaction));
						bool generate_work (work == 0); // Disable work generation if "work" option is provided
						auto response_a (response);
						wallet->receive_async (
						hash, representative, scendere::dev::constants.genesis_amount, account, [response_a] (std::shared_ptr<scendere::block> const & block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = scendere::error_process::unreceivable;
				}
			}
			else
			{
...
void scendere::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void scendere::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void scendere::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<scendere::account, scendere::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<scendere::uint128_t, std::string>> representation;
...
void scendere::json_handler::representatives_online ()
{
...
void scendere::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = scendere::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<scendere::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block.get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					scendere::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending.exists (transaction, scendere::pending_key (destination, hash)))
						{
							scendere::block_hash previous (node.ledger.latest (transaction, destination));
...
void scendere::json_handler::search_pending ()
{
	response_l.put ("deprecated", "1");
	search_receivable ();
}

void scendere::json_handler::search_receivable ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::search_pending_all ()
{
	response_l.put ("deprecated", "1");
	search_receivable_all ();
}

void scendere::json_handler::search_receivable_all ()
{
	if (!ec)
	{
...
void scendere::json_handler::send ()
{
	auto wallet (wallet_impl ());
	auto amount (amount_impl ());
	...
	auto source (account_impl (source_text, scendere::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, scendere::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		...
		if (!ec)
		{
			...
			if (!ec && work)
			{
				scendere::block_details details (info.epoch (), true, false, false);
				if (node.network_params.work.difficulty (scendere::work_version::work_1, info.head, work) < node.network_params.work.threshold (scendere::work_version::work_1, details))
				{
					ec = scendere::error_common::invalid_work;
				}
			}
		...
		if (!ec)
		{
			...
			wallet->send_async (
			source, destination, amount.number (), [balance, amount, response_a, response_data] (std::shared_ptr<scendere::block> const & block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec (scendere::error_common::insufficient_balance);
...
void scendere::json_handler::sign ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	// Retrieving hash
	scendere::block_hash hash (0);
	...
	{
		ec = scendere::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		scendere::raw_key prv;
		...
		{
			scendere::public_key pub (scendere::pub_key (prv));
			scendere::signature signature (scendere::sign_message (prv, pub, hash));
...
void scendere::json_handler::stats ()
{
	...
	{
		ec = scendere::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void scendere::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void scendere::json_handler::stop ()
{
	response_l.put ("success", "");
	response_errors ();
	if (!ec)
...
void scendere::json_handler::telemetry ()
{
	auto rpc_l (shared_from_this ());
	...
	if (address_text.is_initialized () || port_text.is_initialized ())
	{
		...
		if (address_text.is_initialized () && port_text.is_initialized ())
		{
			...
			if (!scendere::parse_port (*port_text, port))
			{
				...
				if (!scendere::parse_address (*address_text, address))
				{
					scendere::endpoint endpoint (address, port);
					if (address.is_loopback () && port == rpc_l->node.network.endpoint ().port ())
					{
						// Requesting telemetry metrics locally
						auto telemetry_data = scendere::local_telemetry_data (rpc_l->node.ledger, rpc_l->node.network, rpc_l->node.unchecked, rpc_l->node.config.bandwidth_limit, rpc_l->node.network_params, rpc_l->node.startup_time, rpc_l->node.default_difficulty (scendere::work_version::work_1), rpc_l->node.node_id);

						scendere::jsonconfig config_l;
					...
					{
						channel = node.network.find_channel (scendere::transport::map_endpoint_to_v6 (endpoint));
						if (!channel)
						{
							ec = scendere::error_rpc::peer_not_found;
						}
					}
				}
				else
		...
		{
			ec = scendere::error_rpc::requires_port_and_address;
		}
			...
			if (node.telemetry)
			{
				node.telemetry->get_metrics_single_peer_async (channel, [rpc_l] (auto const & telemetry_response_a) {
					if (!telemetry_response_a.error)
					{
						...
						{
							rpc_l->ec = scendere::error_rpc::generic;
						}
					}
					else
					{
	...
	else
	{
		...
		if (node.telemetry)
		{
			...
			{
				scendere::jsonconfig config_l;
				std::vector<scendere::telemetry_data> telemetry_datas;
				...
				auto average_telemetry_metrics = scendere::consolidate_telemetry_data (telemetry_datas);
...
void scendere::json_handler::unchecked ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		{
			scendere::unchecked_info const & info (i->second);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
...
void scendere::json_handler::unchecked_clear ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
...
void scendere::json_handler::unchecked_get ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		{
			scendere::unchecked_key const & key (i->first);
			if (key.hash == hash)
			{
				scendere::unchecked_info const & info (i->second);
				response_l.put ("modified_timestamp", std::to_string (info.modified ()));
...
void scendere::json_handler::unchecked_keys ()
{
	bool const json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	scendere::block_hash key (0);
...
void scendere::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	scendere::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending.begin (transaction, scendere::pending_key (start, 0)));
		auto end (node.store.pending.end ());
		scendere::account current_account (start);
		scendere::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			scendere::pending_key key (iterator->first);
			scendere::account account (key.account);
			scendere::pending_info info (iterator->second);
			if (node.store.account.exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<scendere::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending.begin (transaction, scendere::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void scendere::json_handler::validate_account_number ()
{
	auto account (account_impl ());
	(void)account;
...
void scendere::json_handler::wallet_add ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = scendere::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	}));
...
void scendere::json_handler::wallet_add_watch ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = scendere::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void scendere::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		scendere::uint128_t balance (0);
		scendere::uint128_t receivable (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			scendere::confirmation_height_info confirmation_info{};
			...
			scendere::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == scendere::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == scendere::key_type::adhoc)
			{
				adhoc_count++;
			}
...
void scendere::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			scendere::account const & account (i->first);
			scendere::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				scendere::uint128_t receivable = node.ledger.account_receivable (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void scendere::json_handler::wallet_change_seed ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					scendere::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = scendere::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	}));
...
void scendere::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void scendere::json_handler::wallet_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		scendere::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = scendere::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				scendere::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				debug_assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void scendere::json_handler::wallet_destroy ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				rpc_l->ec = scendere::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = scendere::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	}));
}

void scendere::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account.get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<scendere::public_key> no_filter;
...
void scendere::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::wallet_ledger ()
{
	bool const representative = request.get<bool> ("representative", false);
	bool const weight = request.get<bool> ("weight", false);
	bool const pending = request.get<bool> ("pending", false);
...
void scendere::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		scendere::raw_key empty;
		empty.clear ();
...
void scendere::json_handler::wallet_pending ()
{
	response_l.put ("deprecated", "1");
	wallet_receivable ();
}

void scendere::json_handler::wallet_receivable ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			scendere::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending.begin (block_transaction, scendere::pending_key (account, 0))), nn (node.store.pending.end ()); ii != nn && scendere::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				scendere::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						scendere::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void scendere::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::wallet_representative_set ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					wallet->change_async (
					account, representative, [] (std::shared_ptr<scendere::block> const &) {}, 0, false);
				}
			}
		}
		rpc_l->response_errors ();
...
void scendere::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void scendere::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void scendere::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), scendere::to_string_hex (work));
		}
...
void scendere::json_handler::work_generate ()
{
	boost::optional<scendere::account> account;
	...
	if (!ec)
	{
		...
		if (!ec && (difficulty > node.max_work_generate_difficulty (work_version) || difficulty < node.network_params.work.threshold_entry (work_version, scendere::block_type::state)))
		{
			ec = scendere::error_rpc::difficulty_limit;
		}
		...
		if (!ec && request.count ("block"))
		{
			...
			if (block != nullptr)
			{
				if (hash != block->root ())
				{
					ec = scendere::error_rpc::block_root_mismatch;
				}
				...
				{
					ec = scendere::error_rpc::block_work_version_mismatch;
				}
				// Difficulty calculation
				...
				{
					ec = scendere::error_rpc::block_work_enough;
				}
			}
		}
		if (!ec && response_l.empty ())
		{
			...
			auto callback = [rpc_l, hash, work_version, this] (boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", scendere::to_string_hex (work));
					std::stringstream ostream;
					auto result_difficulty (rpc_l->node.network_params.work.difficulty (work_version, hash, work));
					response_l.put ("difficulty", scendere::to_string_hex (result_difficulty));
					auto result_multiplier = scendere::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
					response_l.put ("multiplier", scendere::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				if (node.local_work_generation_enabled ())
				{
					...
					if (error)
					{
						ec = scendere::error_common::failure_work_generation;
					}
				}
				else
				{
					ec = scendere::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void scendere::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void scendere::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void scendere::json_handler::work_set ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<scendere::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
...
void scendere::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	// Default to work_1 if not specified
	...
	if (!ec)
	{
		...
		response_l.put ("valid_receive", (result_difficulty >= node.network_params.work.threshold (work_version, scendere::block_details (scendere::epoch::epoch_2, false, true, false))) ? "1" : "0");
		response_l.put ("difficulty", scendere::to_string_hex (result_difficulty));
		auto result_multiplier = scendere::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
		response_l.put ("multiplier", scendere::to_string (result_multiplier));
	}
...
void scendere::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void scendere::json_handler::work_peers ()
{
...
void scendere::json_handler::work_peers_clear ()
{
...
void scendere::inprocess_rpc_handler::process_request (std::string const &, std::string const & body_a, std::function<void (std::string const &)> response_a)
{
	// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
	auto handler (std::make_shared<scendere::json_handler> (node, node_rpc_config, body_a, response_a, [this] () {
		this->stop_callback ();
		this->stop ();
	}));
...
void scendere::inprocess_rpc_handler::process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void (std::shared_ptr<std::string> const &)> response_a)
{
	std::string body_l = params_a.json_envelope (body_a);
	auto handler (std::make_shared<scendere::ipc::flatbuffers_handler> (node, ipc_server, nullptr, node.config.ipc_config));
...
void construct_json (scendere::container_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<scendere::container_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<scendere::container_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &scendere::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &scendere::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &scendere::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &scendere::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &scendere::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &scendere::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &scendere::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &scendere::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &scendere::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &scendere::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &scendere::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &scendere::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &scendere::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &scendere::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &scendere::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_representatives", &scendere::json_handler::accounts_representatives);
	no_arg_funcs.emplace ("accounts_create", &scendere::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &scendere::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &scendere::json_handler::accounts_pending);
	no_arg_funcs.emplace ("accounts_receivable", &scendere::json_handler::accounts_receivable);
	no_arg_funcs.emplace ("active_difficulty", &scendere::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &scendere::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &scendere::json_handler::block_info);
	no_arg_funcs.emplace ("block", &scendere::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &scendere::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &scendere::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &scendere::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &scendere::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &scendere::json_handler::block_count);
	no_arg_funcs.emplace ("block_create", &scendere::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &scendere::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &scendere::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &scendere::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &scendere::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &scendere::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &scendere::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &scendere::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &scendere::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &scendere::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &scendere::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &scendere::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &scendere::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &scendere::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &scendere::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &scendere::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &scendere::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &scendere::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &scendere::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &scendere::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &scendere::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &scendere::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &scendere::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &scendere::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &scendere::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &scendere::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &scendere::json_handler::password_enter);
	no_arg_funcs.emplace ("peers", &scendere::json_handler::peers);
	no_arg_funcs.emplace ("pending", &scendere::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &scendere::json_handler::pending_exists);
	no_arg_funcs.emplace ("receivable", &scendere::json_handler::receivable);
	no_arg_funcs.emplace ("receivable_exists", &scendere::json_handler::receivable_exists);
	no_arg_funcs.emplace ("process", &scendere::json_handler::process);
	no_arg_funcs.emplace ("pruned_exists", &scendere::json_handler::pruned_exists);
	no_arg_funcs.emplace ("receive", &scendere::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &scendere::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &scendere::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &scendere::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &scendere::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &scendere::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &scendere::json_handler::search_pending);
	no_arg_funcs.emplace ("search_receivable", &scendere::json_handler::search_receivable);
	no_arg_funcs.emplace ("search_pending_all", &scendere::json_handler::search_pending_all);
	no_arg_funcs.emplace ("search_receivable_all", &scendere::json_handler::search_receivable_all);
	no_arg_funcs.emplace ("send", &scendere::json_handler::send);
	no_arg_funcs.emplace ("sign", &scendere::json_handler::sign);
	no_arg_funcs.emplace ("stats", &scendere::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &scendere::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &scendere::json_handler::stop);
	no_arg_funcs.emplace ("telemetry", &scendere::json_handler::telemetry);
	no_arg_funcs.emplace ("unchecked", &scendere::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &scendere::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &scendere::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &scendere::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &scendere::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &scendere::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &scendere::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &scendere::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &scendere::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &scendere::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &scendere::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &scendere::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &scendere::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &scendere::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &scendere::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &scendere::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &scendere::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &scendere::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &scendere::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &scendere::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &scendere::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &scendere::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &scendere::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &scendere::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_receivable", &scendere::json_handler::wallet_receivable);
	no_arg_funcs.emplace ("wallet_representative", &scendere::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &scendere::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &scendere::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &scendere::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &scendere::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &scendere::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &scendere::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &scendere::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &scendere::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &scendere::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &scendere::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &scendere::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
char const * epoch_as_string (scendere::epoch epoch)
{
	switch (epoch)
	{
		case scendere::epoch::epoch_2:
			return "2";
		case scendere::epoch::epoch_1:
			return "1";
		default:
			return "0";
	}
