...
scendere::socket::socket (scendere::node & node_a, endpoint_type_t endpoint_type_a) :
	strand{ node_a.io_ctx.get_executor () },
	tcp_socket{ node_a.io_ctx },
	node{ node_a },
	endpoint_type_m{ endpoint_type_a },
	timeout{ std::numeric_limits<uint64_t>::max () },
	last_completion_time_or_init{ scendere::seconds_since_epoch () },
	last_receive_time_or_init{ scendere::seconds_since_epoch () },
...
scendere::socket::~socket ()
{
	close_internal ();
}

void scendere::socket::async_connect (scendere::tcp_endpoint const & endpoint_a, std::function<void (boost::system::error_code const &)> callback_a)
{
	...
	[this_l, callback = std::move (callback_a), endpoint_a] (boost::system::error_code const & ec) {
		if (ec)
		{
			this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_connect_error, scendere::stat::dir::in);
		}
		else
		{
...
void scendere::socket::async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::size_t size_a, std::function<void (boost::system::error_code const &, std::size_t)> callback_a)
{
		...
		if (!closed)
		{
			...
			boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback = std::move (callback_a), size_a, this_l] () mutable {
				...
				[this_l, buffer_a, cbk = std::move (callback)] (boost::system::error_code const & ec, std::size_t size_a) {
					if (ec)
					{
						this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_read_error, scendere::stat::dir::in);
					}
					else
					{
						this_l->node.stats.add (scendere::stat::type::traffic_tcp, scendere::stat::dir::in, size_a);
						this_l->set_last_completion ();
	...
	{
		debug_assert (false && "scendere::socket::async_read called with incorrect buffer size");
...
void scendere::socket::async_write (scendere::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> callback_a)
{
	if (closed)
	{
		if (callback_a)
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback = std::move (callback_a), this_l = shared_from_this ()] () mutable {
		...
		scendere::async_write (this_l->tcp_socket, buffer_a,
		boost::asio::bind_executor (this_l->strand,
		[buffer_a, cbk = std::move (callback), this_l] (boost::system::error_code ec, std::size_t size_a) {
			...
			if (ec)
			{
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_error, scendere::stat::dir::in);
			}
			else
			{
				this_l->node.stats.add (scendere::stat::type::traffic_tcp, scendere::stat::dir::out, size_a);
				this_l->set_last_completion ();
			}
...
void scendere::socket::set_default_timeout ()
{
 ...
 */
void scendere::socket::set_timeout (std::chrono::seconds timeout_a)
{
	timeout = timeout_a.count ();
}

void scendere::socket::set_last_completion ()
{
	last_completion_time_or_init = scendere::seconds_since_epoch ();
}

void scendere::socket::set_last_receive_time ()
{
	last_receive_time_or_init = scendere::seconds_since_epoch ();
}

void scendere::socket::checkup ()
{
	std::weak_ptr<scendere::socket> this_w (shared_from_this ());
	node.workers.add_timed_task (std::chrono::steady_clock::now () + std::chrono::seconds (2), [this_w] () {
		if (auto this_l = this_w.lock ())
		{
			uint64_t now (scendere::seconds_since_epoch ());
			...
			{
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_silent_connection_drop, scendere::stat::dir::in);
			...
			{
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_io_timeout_drop,
				this_l->endpoint_type () == endpoint_type_t::server ? scendere::stat::dir::in : scendere::stat::dir::out);
				condition_to_disconnect = true;
			}
...
bool scendere::socket::has_timed_out () const
{
	return timed_out;
}

void scendere::socket::set_default_timeout_value (std::chrono::seconds timeout_a)
{
	default_timeout = timeout_a;
}

void scendere::socket::set_silent_connection_tolerance_time (std::chrono::seconds tolerance_time_a)
{
	auto this_l (shared_from_this ());
...
void scendere::socket::close_internal ()
{
	if (!closed.exchange (true))
	{
		...
		if (ec)
		{
			...
			node.stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::error_socket_close);
		}
	}
}

scendere::tcp_endpoint scendere::socket::remote_endpoint () const
{
	return remote;
}

scendere::tcp_endpoint scendere::socket::local_endpoint () const
{
	return tcp_socket.local_endpoint ();
}

scendere::server_socket::server_socket (scendere::node & node_a, boost::asio::ip::tcp::endpoint local_a, std::size_t max_connections_a) :
...
void scendere::server_socket::start (boost::system::error_code & ec_a)
{
...
void scendere::server_socket::close ()
{
	auto this_l (std::static_pointer_cast<scendere::server_socket> (shared_from_this ()));
...
boost::asio::ip::network_v6 scendere::socket_functions::get_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
	return boost::asio::ip::make_network_v6 (ip_address, network_prefix);
}

boost::asio::ip::address scendere::socket_functions::first_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
...
boost::asio::ip::address scendere::socket_functions::last_ipv6_subnet_address (boost::asio::ip::address_v6 const & ip_address, size_t network_prefix)
{
	auto range = get_ipv6_subnet_address (ip_address, network_prefix).hosts ();
	debug_assert (!range.empty ());
	return *(--range.end ());
...
size_t scendere::socket_functions::count_subnetwork_connections (
scendere::address_socket_mmap const & per_address_connections,
...
bool scendere::server_socket::limit_reached_for_incoming_subnetwork_connections (std::shared_ptr<scendere::socket> const & new_connection)
{
	debug_assert (strand.running_in_this_thread ());
	if (node.flags.disable_max_peers_per_subnetwork || scendere::transport::is_ipv4_or_v4_mapped_address (new_connection->remote.address ()))
	{
...
bool scendere::server_socket::limit_reached_for_incoming_ip_connections (std::shared_ptr<scendere::socket> const & new_connection)
{
	debug_assert (strand.running_in_this_thread ());
...
void scendere::server_socket::on_connection (std::function<bool (std::shared_ptr<scendere::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<scendere::server_socket> (shared_from_this ()));
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, callback = std::move (callback_a)] () mutable {
		...
		auto new_connection = std::make_shared<scendere::socket> (this_l->node, endpoint_type_t::server);
		...
		[this_l, new_connection, cbk = std::move (callback)] (boost::system::error_code const & ec_a) mutable {
			...
			if (this_l->connections_per_address.size () >= this_l->max_inbound_connections)
			{
				...
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_failure, scendere::stat::dir::in);
			...
			if (this_l->limit_reached_for_incoming_ip_connections (new_connection))
			{
				...
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_ip, scendere::stat::dir::in);
				this_l->on_connection_requeue_delayed (std::move (cbk));
				return;
			}
			...
			if (this_l->limit_reached_for_incoming_subnetwork_connections (new_connection))
			{
				...
				% remote_ip_address.to_string ());
				this_l->node.logger.try_log (log_message);
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_subnetwork, scendere::stat::dir::in);
				this_l->on_connection_requeue_delayed (std::move (cbk));
				return;
			}
			...
			if (!ec_a)
			{
				...
				this_l->node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_accept_success, scendere::stat::dir::in);
...
void scendere::server_socket::on_connection_requeue_delayed (std::function<bool (std::shared_ptr<scendere::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<scendere::server_socket> (shared_from_this ()));
...
void scendere::server_socket::evict_dead_connections ()
{
	debug_assert (strand.running_in_this_thread ());
