...
public:
	...
	representative (scendere::account account_a, scendere::amount weight_a, std::shared_ptr<scendere::transport::channel> const & channel_a) :
		account (account_a), weight (weight_a), channel (channel_a)
	{
		debug_assert (channel != nullptr);
	}
	std::reference_wrapper<scendere::transport::channel const> channel_ref () const
	{
		return *channel;
	};
	bool operator== (scendere::representative const & other_a) const
	{
		return account == other_a.account;
	}
	scendere::account account{};
	scendere::amount weight{ 0 };
...
class rep_crawler final
{
	...
	mi::indexed_by<
		mi::hashed_unique<mi::member<representative, scendere::account, &representative::account>>,
		mi::random_access<>,
		...
		mi::ordered_non_unique<mi::tag<tag_weight>,
			mi::member<representative, scendere::amount, &representative::weight>, std::greater<scendere::amount>>,
		mi::hashed_non_unique<mi::tag<tag_channel_ref>,
			mi::const_mem_fun<representative, std::reference_wrapper<scendere::transport::channel const>, &representative::channel_ref>>>>;
	// clang-format on

public:
	rep_crawler (scendere::node & node_a);

	/** Start crawling */
	void start ();

	/** Remove block hash from list of active rep queries */
	void remove (scendere::block_hash const &);

	/** Remove block hash from with delay depending on vote processor size */
	void throttled_remove (scendere::block_hash const &, uint64_t const);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::vector<std::shared_ptr<scendere::transport::channel>> const & channels_a);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::shared_ptr<scendere::transport::channel> const & channel_a);

	/** Query if a peer manages a principle representative */
	bool is_pr (scendere::transport::channel const &) const;
	 ...
	 */
	bool response (std::shared_ptr<scendere::transport::channel> const &, std::shared_ptr<scendere::vote> const &);

	/** Get total available weight from representatives */
	scendere::uint128_t total_weight () const;
	...
	std::vector<representative> representatives (std::size_t count_a = std::numeric_limits<std::size_t>::max (), scendere::uint128_t const weight_a = 0, boost::optional<decltype (scendere::network_constants::protocol_version)> const & opt_version_min_a = boost::none);
	...
	std::vector<representative> principal_representatives (std::size_t count_a = std::numeric_limits<std::size_t>::max (), boost::optional<decltype (scendere::network_constants::protocol_version)> const & opt_version_min_a = boost::none);

	/** Request a list of the top \p count_a known representative endpoints. */
	std::vector<std::shared_ptr<scendere::transport::channel>> representative_endpoints (std::size_t count_a);
...
private:
	...
	scendere::mutex active_mutex;

	/** We have solicted votes for these random blocks */
	std::unordered_set<scendere::block_hash> active;
	...
	std::vector<std::shared_ptr<scendere::transport::channel>> get_crawl_targets (scendere::uint128_t total_weight_a);

	/** When a rep request is made, this is called to update the last-request timestamp. */
	void on_rep_request (std::shared_ptr<scendere::transport::channel> const & channel_a);
	...
	mutable scendere::mutex probable_reps_mutex;
	...
	std::deque<std::pair<std::shared_ptr<scendere::transport::channel>, std::shared_ptr<scendere::vote>>> responses;
};
}
