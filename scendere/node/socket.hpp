...
class socket : public std::enable_shared_from_this<scendere::socket>
{
...
public:
	 ...
	 */
	explicit socket (scendere::node & node, endpoint_type_t endpoint_type_a);
	virtual ~socket ();
	...
	void async_write (scendere::shared_const_buffer const &, std::function<void (boost::system::error_code const &, std::size_t)> = {});
	...
	{
		return type () == scendere::socket::type_t::realtime || type () == scendere::socket::type_t::realtime_response_server;
	}
	bool is_closed ()
	{
		return closed;
...
namespace socket_functions
{
	...
	size_t count_subnetwork_connections (scendere::address_socket_mmap const &, boost::asio::ip::address_v6 const &, size_t);
}
...
public:
	 ...
	 */
	explicit server_socket (scendere::node & node_a, boost::asio::ip::tcp::endpoint local_a, std::size_t max_connections_a);
	...
	void on_connection (std::function<bool (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const &)>);
...
private:
	...
	void on_connection_requeue_delayed (std::function<bool (std::shared_ptr<scendere::socket> const & new_connection, boost::system::error_code const &)>);
	/** Checks whether the maximum number of connections per IP was reached. If so, it returns true. */
	bool limit_reached_for_incoming_ip_connections (std::shared_ptr<scendere::socket> const & new_connection);
	bool limit_reached_for_incoming_subnetwork_connections (std::shared_ptr<scendere::socket> const & new_connection);
};
...
public:
	 ...
	 * @param node_a Owning node
	 */
	explicit client_socket (scendere::node & node_a) :
