...
bool scendere::prioritization::value_type::operator< (value_type const & other_a) const
{
...
void scendere::prioritization::next ()
{
	++current;
...
void scendere::prioritization::seek ()
{
	next ();
...
void scendere::prioritization::populate_schedule ()
{
 ...
 */
scendere::prioritization::prioritization (uint64_t maximum) :
	maximum{ maximum }
{
	static std::size_t constexpr bucket_count = 129;
	buckets.resize (bucket_count);
	scendere::uint128_t minimum{ 1 };
 ...
 */
void scendere::prioritization::push (uint64_t time, std::shared_ptr<scendere::block> block)
{
	auto was_empty = empty ();
	auto block_has_balance = block->type () == scendere::block_type::state || block->type () == scendere::block_type::send;
...
std::shared_ptr<scendere::block> scendere::prioritization::top () const
{
	debug_assert (!empty ());
...
void scendere::prioritization::pop ()
{
	debug_assert (!empty ());
	debug_assert (!buckets[*current].empty ());
...
std::size_t scendere::prioritization::size () const
{
	std::size_t result{ 0 };
...
std::size_t scendere::prioritization::bucket_count () const
{
	return buckets.size ();
}

/** Returns number of items in bucket with index 'index' */
std::size_t scendere::prioritization::bucket_size (std::size_t index) const
{
	return buckets[index].size ();
}

/** Returns true if all buckets are empty */
bool scendere::prioritization::empty () const
{
...
void scendere::prioritization::dump () const
{
...
std::unique_ptr<scendere::container_info_component> scendere::prioritization::collect_container_info (std::string const & name)
{
