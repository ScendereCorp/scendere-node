...
#include <scendere/node/ipc/ipc_config.hpp>
...
{
public:
	node_config (scendere::network_params & network_params = scendere::dev::network_params);
	node_config (const std::optional<uint16_t> &, scendere::logging const &, scendere::network_params & network_params = scendere::dev::network_params);
	scendere::error serialize_toml (scendere::tomlconfig &) const;
	scendere::error deserialize_toml (scendere::tomlconfig &);
	bool upgrade_json (unsigned, scendere::jsonconfig &);
	scendere::account random_representative () const;
	scendere::network_params & network_params;
	std::optional<uint16_t> peering_port{};
	scendere::logging logging;
	std::vector<std::pair<std::string, uint16_t>> work_peers;
	std::vector<std::pair<std::string, uint16_t>> secondary_work_peers{ { "127.0.0.1", 8076 } }; /* Default of scendere-pow-server */
	std::vector<std::string> preconfigured_peers;
	std::vector<scendere::account> preconfigured_representatives;
	unsigned bootstrap_fraction_numerator{ 1 };
	scendere::amount receive_minimum{ scendere::xrb_ratio };
	scendere::amount vote_minimum{ scendere::Gxrb_ratio };
	scendere::amount rep_crawler_weight_minimum{ "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF" };
	std::chrono::milliseconds vote_generator_delay{ std::chrono::milliseconds (100) };
	unsigned vote_generator_threshold{ 3 };
	scendere::amount online_weight_minimum{ 10000 * scendere::Gxrb_ratio }; //TO-CHANGE Default 60000
	...
	scendere::websocket::config websocket_config;
	scendere::diagnostics_config diagnostics_config;
	...
	scendere::stat_config stat_config;
	scendere::ipc::ipc_config ipc_config;
	...
	scendere::lmdb_config lmdb_config;
	scendere::frontiers_confirmation_mode frontiers_confirmation{ scendere::frontiers_confirmation_mode::automatic };
	std::string serialize_frontiers_confirmation (scendere::frontiers_confirmation_mode) const;
	scendere::frontiers_confirmation_mode deserialize_frontiers_confirmation (std::string const &);
...
public:
	...
	scendere::confirmation_height_mode confirmation_height_processor_mode{ scendere::confirmation_height_mode::automatic };
	scendere::generate_cache generate_cache;
