...
scendere::pull_info::pull_info (scendere::hash_or_account const & account_or_head_a, scendere::block_hash const & head_a, scendere::block_hash const & end_a, uint64_t bootstrap_id_a, count_t count_a, unsigned retry_limit_a) :
...
scendere::bulk_pull_client::bulk_pull_client (std::shared_ptr<scendere::bootstrap_client> const & connection_a, std::shared_ptr<scendere::bootstrap_attempt> const & attempt_a, scendere::pull_info const & pull_a) :
...
scendere::bulk_pull_client::~bulk_pull_client ()
{
	...
	{
		pull.head = expected;
		if (attempt->mode != scendere::bootstrap_mode::legacy)
		{
...
void scendere::bulk_pull_client::request ()
{
	...
	req, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_request_failure, scendere::stat::dir::in);
		}
	},
	scendere::buffer_drop_policy::no_limiter_drop);
}

void scendere::bulk_pull_client::throttled_receive_block ()
{
...
void scendere::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	connection->socket->async_read (connection->receive_buffer, 1, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_receive_block_failure, scendere::stat::dir::in);
			this_l->network_error = true;
		}
	});
}

void scendere::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	scendere::block_type type (static_cast<scendere::block_type> (connection->receive_buffer->data ()[0]));
	...
	switch (type)
	{
		...
		{
			socket_l->async_read (connection->receive_buffer, scendere::send_block::size, [this_l, type] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			socket_l->async_read (connection->receive_buffer, scendere::receive_block::size, [this_l, type] (boost::system::error_code const & ec, std::size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, scendere::open_block::size, [this_l, type] (boost::system::error_code const & ec, std::size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, scendere::change_block::size, [this_l, type] (boost::system::error_code const & ec, std::size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, scendere::state_block::size, [this_l, type] (boost::system::error_code const & ec, std::size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void scendere::bulk_pull_client::received_block (boost::system::error_code const & ec, std::size_t size_a, scendere::block_type type_a)
{
	if (!ec)
	{
		scendere::bufferstream stream (connection->receive_buffer->data (), size_a);
		...
		if (block != nullptr && !connection->node->network_params.work.validate_entry (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see scendere::bootstrap_attempt::lazy_destinations_increment (...)
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (attempt->mode != scendere::bootstrap_mode::legacy || unexpected_count < 16384)
				{
		...
		else if (block == nullptr)
		{
			...
			connection->node->stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_deserialize_receive_block, scendere::stat::dir::in);
		}
		else // Work invalid
		{
			if (connection->node->config.logging.bulk_pull_logging ())
	...
	else
	{
		...
		connection->node->stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_receive_block_failure, scendere::stat::dir::in);
		network_error = true;
	}
}

scendere::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<scendere::bootstrap_client> const & connection_a, std::shared_ptr<scendere::bootstrap_attempt> const & attempt_a, scendere::account const & account_a) :
	connection (connection_a),
	attempt (attempt_a),
	account (account_a),
	pull_blocks (0)
...
scendere::bulk_pull_account_client::~bulk_pull_account_client ()
{
	attempt->pull_finished ();
}

void scendere::bulk_pull_account_client::request ()
{
	scendere::bulk_pull_account req{ connection->node->network_params.network };
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = scendere::bulk_pull_account_flags::pending_hash_and_amount;
	if (connection->node->config.logging.bulk_pull_logging ())
	{
	...
	req, [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_error_starting_request, scendere::stat::dir::in);
		}
	},
	scendere::buffer_drop_policy::no_limiter_drop);
}

void scendere::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	std::size_t size_l (sizeof (scendere::uint256_union) + sizeof (scendere::uint128_union));
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		if (size_a == size_l)
		{
			if (!ec)
			{
				scendere::block_hash pending;
				scendere::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (scendere::uint256_union));
				auto error1 (scendere::try_read (frontier_stream, pending));
				(void)error1;
				debug_assert (!error1);
				scendere::amount balance;
				scendere::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (scendere::uint256_union), sizeof (scendere::uint128_union));
				auto error2 (scendere::try_read (balance_stream, balance));
				(void)error2;
				debug_assert (!error2);
 ...
 */
void scendere::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void scendere::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		...
		{
			scendere::vectorstream stream (send_buffer);
			scendere::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
		...
		connection->socket->async_write (scendere::shared_const_buffer (std::move (send_buffer)), [this_l] (boost::system::error_code const & ec, std::size_t size_a) {
...
std::shared_ptr<scendere::block> scendere::bulk_pull_server::get_next ()
{
...
void scendere::bulk_pull_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void scendere::bulk_pull_server::send_finished ()
{
	scendere::shared_const_buffer send_buffer (static_cast<uint8_t> (scendere::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void scendere::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, std::size_t size_a)
{
	if (!ec)
	{
...
scendere::bulk_pull_server::bulk_pull_server (std::shared_ptr<scendere::bootstrap_server> const & connection_a, std::unique_ptr<scendere::bulk_pull> request_a) :
	connection (connection_a),
 ...
 */
void scendere::bulk_pull_account_server::set_params ()
{
	debug_assert (request != nullptr);
	...
	if (request->flags == scendere::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == scendere::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == scendere::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void scendere::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	if (!invalid_request)
	{
		...
		scendere::uint128_union account_frontier_balance (account_frontier_balance_int);

		// Write the frontier block hash and balance into a buffer
		std::vector<uint8_t> send_buffer;
		{
			scendere::vectorstream output_stream (send_buffer);
...
void scendere::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<scendere::pending_key>, std::unique_ptr<scendere::pending_info>> scendere::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<scendere::pending_key>, std::unique_ptr<scendere::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == scendere::store_iterator<scendere::pending_key, scendere::pending_info> (nullptr))
		{
			break;
		}

		scendere::pending_key key (stream->first);
		scendere::pending_info info (stream->second);
		...
		result.first = std::make_unique<scendere::pending_key> (key);
		result.second = std::make_unique<scendere::pending_info> (info);
...
void scendere::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, std::size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void scendere::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		scendere::vectorstream output_stream (send_buffer);
		scendere::uint256_union account_zero (0);
		scendere::uint128_union balance_zero (0);
...
void scendere::bulk_pull_account_server::complete (boost::system::error_code const & ec, std::size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
scendere::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<scendere::bootstrap_server> const & connection_a, std::unique_ptr<scendere::bulk_pull_account> request_a) :
	connection (connection_a),
	request (std::move (request_a)),
	current_key (0, 0)
{
