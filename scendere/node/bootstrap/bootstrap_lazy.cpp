...
constexpr std::chrono::seconds scendere::bootstrap_limits::lazy_flush_delay_sec;
constexpr uint64_t scendere::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit;
constexpr double scendere::bootstrap_limits::lazy_batch_pull_count_resize_ratio;
constexpr std::size_t scendere::bootstrap_limits::lazy_blocks_restart_limit;

scendere::bootstrap_attempt_lazy::bootstrap_attempt_lazy (std::shared_ptr<scendere::node> const & node_a, uint64_t incremental_id_a, std::string const & id_a) :
	scendere::bootstrap_attempt (node_a, scendere::bootstrap_mode::lazy, incremental_id_a, id_a)
{
	node->bootstrap_initiator.notify_listeners (true);
}

scendere::bootstrap_attempt_lazy::~bootstrap_attempt_lazy ()
{
	debug_assert (lazy_blocks.size () == lazy_blocks_count);
	node->bootstrap_initiator.notify_listeners (false);
}

bool scendere::bootstrap_attempt_lazy::lazy_start (scendere::hash_or_account const & hash_or_account_a, bool confirmed)
{
...
void scendere::bootstrap_attempt_lazy::lazy_add (scendere::hash_or_account const & hash_or_account_a, unsigned retry_limit)
{
...
void scendere::bootstrap_attempt_lazy::lazy_add (scendere::pull_info const & pull_a)
{
...
void scendere::bootstrap_attempt_lazy::lazy_requeue (scendere::block_hash const & hash_a, scendere::block_hash const & previous_a)
{
	scendere::unique_lock<scendere::mutex> lock (mutex);
	...
	if (lazy_blocks_processed (hash_a))
	{
		...
		lock.unlock ();
		node->bootstrap_initiator.connections->requeue_pull (scendere::pull_info (hash_a, hash_a, previous_a, incremental_id, static_cast<scendere::pull_info::count_t> (1), node->network_params.bootstrap.lazy_destinations_retry_limit));
	}
}

uint32_t scendere::bootstrap_attempt_lazy::lazy_batch_size ()
{
	auto result (node->network_params.bootstrap.lazy_max_pull_blocks);
	if (total_blocks > scendere::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit && lazy_blocks_count != 0)
	{
		auto lazy_blocks_ratio (static_cast<double> (total_blocks / lazy_blocks_count));
		if (lazy_blocks_ratio > scendere::bootstrap_limits::lazy_batch_pull_count_resize_ratio)
		{
			// Increasing blocks ratio weight as more important (^3). Small batch count should lower blocks ratio below target
			double lazy_blocks_factor (std::pow (lazy_blocks_ratio / scendere::bootstrap_limits::lazy_batch_pull_count_resize_ratio, 3.0));
			// Decreasing total block count weight as less important (sqrt)
			double total_blocks_factor (std::sqrt (total_blocks / scendere::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit));
...
void scendere::bootstrap_attempt_lazy::lazy_pull_flush (scendere::unique_lock<scendere::mutex> & lock_a)
{
	static std::size_t const max_pulls (static_cast<std::size_t> (scendere::bootstrap_limits::bootstrap_connection_scale_target_blocks) * 3);
	if (pulling < max_pulls)
	{
		debug_assert (node->network_params.bootstrap.lazy_max_pull_blocks <= std::numeric_limits<scendere::pull_info::count_t>::max ());
		scendere::pull_info::count_t batch_count (lazy_batch_size ());
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			{
				lock_a.unlock ();
				node->bootstrap_initiator.connections->add_pull (scendere::pull_info (pull_start.first, pull_start.first.as_block_hash (), scendere::block_hash (0), incremental_id, batch_count, pull_start.second));
				++pulling;
				++count;
				lock_a.lock ();
			}
...
bool scendere::bootstrap_attempt_lazy::lazy_finished ()
{
	debug_assert (!mutex.try_lock ());
	if (stopped)
	{
...
bool scendere::bootstrap_attempt_lazy::lazy_has_expired () const
{
	bool result (false);
	...
	else if (!node->flags.disable_legacy_bootstrap && lazy_blocks_count > scendere::bootstrap_limits::lazy_blocks_restart_limit)
	{
		result = true;
	}
	return result;
...
void scendere::bootstrap_attempt_lazy::run ()
{
	...
	while ((still_pulling () || !lazy_finished ()) && !lazy_has_expired ())
	{
		...
		{
			condition.wait (lock, [&stopped = stopped, &pulling = pulling, &lazy_pulls = lazy_pulls, this_l] { return stopped || pulling == 0 || (pulling < scendere::bootstrap_limits::bootstrap_connection_scale_target_blocks && !lazy_pulls.empty ()) || this_l->lazy_has_expired (); });
			++iterations;
			// Flushing lazy pulls
...
bool scendere::bootstrap_attempt_lazy::process_block (std::shared_ptr<scendere::block> const & block_a, scendere::account const & known_account_a, uint64_t pull_blocks_processed, scendere::bulk_pull::count_t max_blocks, bool block_expected, unsigned retry_limit)
{
...
bool scendere::bootstrap_attempt_lazy::process_block_lazy (std::shared_ptr<scendere::block> const & block_a, scendere::account const & known_account_a, uint64_t pull_blocks_processed, scendere::bulk_pull::count_t max_blocks, unsigned retry_limit)
{
	bool stop_pull (false);
	...
	if (!lazy_blocks_processed (hash))
	{
		...
		if (pull_blocks_processed == 1 && (block_a->type () == scendere::block_type::state || block_a->type () == scendere::block_type::send))
		{
...
void scendere::bootstrap_attempt_lazy::lazy_block_state (std::shared_ptr<scendere::block> const & block_a, unsigned retry_limit)
{
	std::shared_ptr<scendere::state_block> block_l (std::static_pointer_cast<scendere::state_block> (block_a));
	if (block_l != nullptr)
	{
		auto transaction (node->store.tx_begin_read ());
		scendere::uint128_t balance (block_l->hashables.balance.number ());
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && !lazy_blocks_processed (link.as_block_hash ()) && !node->ledger.block_or_pruned_exists (transaction, link.as_block_hash ()))
		{
			...
			{
				lazy_state_backlog.emplace (previous, scendere::lazy_state_backlog_item{ link, balance, retry_limit });
			}
		}
	}
}

void scendere::bootstrap_attempt_lazy::lazy_block_state_backlog_check (std::shared_ptr<scendere::block> const & block_a, scendere::block_hash const & hash_a)
{
	...
	if (find_state != lazy_state_backlog.end ())
	{
		...
		if (block_a->type () == scendere::block_type::state || block_a->type () == scendere::block_type::send)
		{
...
void scendere::bootstrap_attempt_lazy::lazy_backlog_cleanup ()
{
	uint64_t read_count (0);
	auto transaction (node->store.tx_begin_read ());
...
void scendere::bootstrap_attempt_lazy::lazy_blocks_insert (scendere::block_hash const & hash_a)
{
	debug_assert (!mutex.try_lock ());
	auto inserted (lazy_blocks.insert (std::hash<::scendere::block_hash> () (hash_a)));
	if (inserted.second)
	{
		++lazy_blocks_count;
...
void scendere::bootstrap_attempt_lazy::lazy_blocks_erase (scendere::block_hash const & hash_a)
{
	debug_assert (!mutex.try_lock ());
	auto erased (lazy_blocks.erase (std::hash<::scendere::block_hash> () (hash_a)));
	if (erased)
	{
		--lazy_blocks_count;
...
bool scendere::bootstrap_attempt_lazy::lazy_blocks_processed (scendere::block_hash const & hash_a)
{
	return lazy_blocks.find (std::hash<::scendere::block_hash> () (hash_a)) != lazy_blocks.end ();
}

bool scendere::bootstrap_attempt_lazy::lazy_processed_or_exists (scendere::block_hash const & hash_a)
{
	bool result (false);
	scendere::unique_lock<scendere::mutex> lock (mutex);
	if (lazy_blocks_processed (hash_a))
...
unsigned scendere::bootstrap_attempt_lazy::lazy_retry_limit_confirmed ()
{
	debug_assert (!mutex.try_lock ());
	...
	return multiplier * std::max (node->network_params.bootstrap.lazy_retry_limit, 2 * scendere::narrow_cast<unsigned> (peer_count));
}

void scendere::bootstrap_attempt_lazy::get_information (boost::property_tree::ptree & tree_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
...
scendere::bootstrap_attempt_wallet::bootstrap_attempt_wallet (std::shared_ptr<scendere::node> const & node_a, uint64_t incremental_id_a, std::string id_a) :
	scendere::bootstrap_attempt (node_a, scendere::bootstrap_mode::wallet_lazy, incremental_id_a, id_a)
{
	node->bootstrap_initiator.notify_listeners (true);
}

scendere::bootstrap_attempt_wallet::~bootstrap_attempt_wallet ()
{
	node->bootstrap_initiator.notify_listeners (false);
}

void scendere::bootstrap_attempt_wallet::request_pending (scendere::unique_lock<scendere::mutex> & lock_a)
{
	lock_a.unlock ();
	...
	if (connection_l && !stopped)
	{
		...
		node->background ([connection_l, this_l, account] () {
			auto client (std::make_shared<scendere::bulk_pull_account_client> (connection_l, this_l, account));
			client->request ();
		});
	}
}

void scendere::bootstrap_attempt_wallet::requeue_pending (scendere::account const & account_a)
{
...
void scendere::bootstrap_attempt_wallet::wallet_start (std::deque<scendere::account> & accounts_a)
{
	{
		scendere::lock_guard<scendere::mutex> lock (mutex);
...
bool scendere::bootstrap_attempt_wallet::wallet_finished ()
{
	debug_assert (!mutex.try_lock ());
...
void scendere::bootstrap_attempt_wallet::run ()
{
	debug_assert (started);
...
std::size_t scendere::bootstrap_attempt_wallet::wallet_size ()
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	return wallet_accounts.size ();
}

void scendere::bootstrap_attempt_wallet::get_information (boost::property_tree::ptree & tree_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
