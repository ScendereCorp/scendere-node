...
constexpr double scendere::bootstrap_limits::bootstrap_connection_scale_target_blocks;
constexpr double scendere::bootstrap_limits::bootstrap_minimum_blocks_per_sec;
constexpr double scendere::bootstrap_limits::bootstrap_minimum_termination_time_sec;
constexpr unsigned scendere::bootstrap_limits::bootstrap_max_new_connections;
constexpr unsigned scendere::bootstrap_limits::requeued_pulls_processed_blocks_factor;

scendere::bootstrap_client::bootstrap_client (std::shared_ptr<scendere::node> const & node_a, scendere::bootstrap_connections & connections_a, std::shared_ptr<scendere::transport::channel_tcp> const & channel_a, std::shared_ptr<scendere::socket> const & socket_a) :
	node (node_a),
...
scendere::bootstrap_client::~bootstrap_client ()
{
	--connections.connections_count;
}

double scendere::bootstrap_client::sample_block_rate ()
{
	auto elapsed = std::max (elapsed_seconds (), scendere::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
	block_rate = static_cast<double> (block_count.load ()) / elapsed;
	return block_rate;
}

void scendere::bootstrap_client::set_start_time (std::chrono::steady_clock::time_point start_time_a)
{
	scendere::lock_guard<scendere::mutex> guard (start_time_mutex);
	start_time_m = start_time_a;
}

double scendere::bootstrap_client::elapsed_seconds () const
{
	scendere::lock_guard<scendere::mutex> guard (start_time_mutex);
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time_m).count ();
}

void scendere::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
scendere::bootstrap_connections::bootstrap_connections (scendere::node & node_a) :
	node (node_a)
{
}

std::shared_ptr<scendere::bootstrap_client> scendere::bootstrap_connections::connection (std::shared_ptr<scendere::bootstrap_attempt> const & attempt_a, bool use_front_connection)
{
	...
	std::shared_ptr<scendere::bootstrap_client> result;
...
void scendere::bootstrap_connections::pool_connection (std::shared_ptr<scendere::bootstrap_client> const & client_a, bool new_client, bool push_front)
{
	scendere::unique_lock<scendere::mutex> lock (mutex);
...
void scendere::bootstrap_connections::add_connection (scendere::endpoint const & endpoint_a)
{
	connect_client (scendere::tcp_endpoint (endpoint_a.address (), endpoint_a.port ()), true);
}

std::shared_ptr<scendere::bootstrap_client> scendere::bootstrap_connections::find_connection (scendere::tcp_endpoint const & endpoint_a)
{
...
void scendere::bootstrap_connections::connect_client (scendere::tcp_endpoint const & endpoint_a, bool push_front)
{
	++connections_count;
	...
	[this_l, socket, endpoint_a, push_front] (boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<scendere::bootstrap_client> (this_l->node.shared (), *this_l, std::make_shared<scendere::transport::channel_tcp> (*this_l->node.shared (), socket), socket));
...
unsigned scendere::bootstrap_connections::target_connections (std::size_t pulls_remaining, std::size_t attempts_count) const
{
	auto const attempts_factor = scendere::narrow_cast<unsigned> (node.config.bootstrap_connections * attempts_count);
	...
	double step_scale = std::min (1.0, std::max (0.0, (double)pulls_remaining / scendere::bootstrap_limits::bootstrap_connection_scale_target_blocks));
...
{
	bool operator() (std::shared_ptr<scendere::bootstrap_client> const & lhs, std::shared_ptr<scendere::bootstrap_client> const & rhs) const
	{
		return lhs->block_rate > rhs->block_rate;
	}
};

void scendere::bootstrap_connections::populate_connections (bool repeat)
{
	double rate_sum = 0.0;
	std::size_t num_pulls = 0;
	std::size_t attempts_count = node.bootstrap_initiator.attempts.size ();
	std::priority_queue<std::shared_ptr<scendere::bootstrap_client>, std::vector<std::shared_ptr<scendere::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<scendere::tcp_endpoint> endpoints;
	{
		scendere::unique_lock<scendere::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<scendere::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				...
				if (client->elapsed_seconds () > scendere::bootstrap_limits::bootstrap_connection_warmup_time_sec && client->block_count > 0)
				{
				...
				if (elapsed_sec > scendere::bootstrap_limits::bootstrap_minimum_termination_time_sec && blocks_per_sec < scendere::bootstrap_limits::bootstrap_minimum_blocks_per_sec)
				{
					if (node.config.logging.bulk_pull_logging ())
					{
						node.logger.try_log (boost::str (boost::format ("Stopping slow peer %1% (elapsed sec %2%s > %3%s and %4% blocks per second < %5%)") % client->channel->to_string () % elapsed_sec % scendere::bootstrap_limits::bootstrap_minimum_termination_time_sec % blocks_per_sec % scendere::bootstrap_limits::bootstrap_minimum_blocks_per_sec));
					}
	...
	{
		auto delta = std::min ((target - connections_count) * 2, scendere::bootstrap_limits::bootstrap_max_new_connections);
		// TODO - tune this better
		// Not many peers respond, need to try to make more connections than we need.
		for (auto i = 0u; i < delta; i++)
		{
			auto endpoint (node.network.bootstrap_peer (true));
			if (endpoint != scendere::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && (node.flags.allow_bootstrap_peers_duplicates || endpoints.find (endpoint) == endpoints.end ()) && !node.network.excluded_peers.check (endpoint))
			{
	...
	if (!stopped && repeat)
	{
		std::weak_ptr<scendere::bootstrap_connections> this_w (shared_from_this ());
...
void scendere::bootstrap_connections::start_populate_connections ()
{
...
void scendere::bootstrap_connections::add_pull (scendere::pull_info const & pull_a)
{
	scendere::pull_info pull (pull_a);
...
void scendere::bootstrap_connections::request_pull (scendere::unique_lock<scendere::mutex> & lock_a)
{
	lock_a.unlock ();
	...
	{
		std::shared_ptr<scendere::bootstrap_attempt> attempt_l;
		scendere::pull_info pull;
		...
		while (attempt_l == nullptr && !pulls.empty ())
		{
			...
			if (attempt_l != nullptr && attempt_l->mode == scendere::bootstrap_mode::lazy && !pull.head.is_zero () && attempt_l->lazy_processed_or_exists (pull.head))
			{
		...
		if (attempt_l != nullptr)
		{
			...
			node.background ([connection_l, attempt_l, pull] () {
				auto client (std::make_shared<scendere::bulk_pull_client> (connection_l, attempt_l, pull));
				client->request ();
			});
		}
	}
...
void scendere::bootstrap_connections::requeue_pull (scendere::pull_info const & pull_a, bool network_error)
{
	auto pull (pull_a);
	if (!network_error)
	{
	...
	if (attempt_l != nullptr)
	{
		...
		if (attempt_l->mode == scendere::bootstrap_mode::lazy)
		{
			pull.count = attempt_l->lazy_batch_size ();
		}
		if (attempt_l->mode == scendere::bootstrap_mode::legacy && (pull.attempts < pull.retry_limit + (pull.processed / scendere::bootstrap_limits::requeued_pulls_processed_blocks_factor)))
		{
			{
				scendere::lock_guard<scendere::mutex> lock (mutex);
		...
		else if (attempt_l->mode == scendere::bootstrap_mode::lazy && (pull.attempts <= pull.retry_limit + (pull.processed / node.network_params.bootstrap.lazy_max_pull_blocks)))
		{
		...
		else
		{
			...
			node.stats.inc (scendere::stat::type::bootstrap, scendere::stat::detail::bulk_pull_failed_account, scendere::stat::dir::in);

			if (attempt_l->mode == scendere::bootstrap_mode::lazy && pull.processed > 0)
			{
				attempt_l->lazy_add (pull);
			}
			else if (attempt_l->mode == scendere::bootstrap_mode::legacy)
			{
...
void scendere::bootstrap_connections::clear_pulls (uint64_t bootstrap_id_a)
{
	{
		scendere::lock_guard<scendere::mutex> lock (mutex);
		auto i (pulls.begin ());
...
void scendere::bootstrap_connections::run ()
{
...
void scendere::bootstrap_connections::stop ()
{
	scendere::unique_lock<scendere::mutex> lock (mutex);
	stopped = true;
	lock.unlock ();
