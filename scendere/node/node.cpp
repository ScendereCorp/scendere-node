...
double constexpr scendere::node::price_max;
double constexpr scendere::node::free_cutoff;
std::size_t constexpr scendere::block_arrival::arrival_size_min;
std::chrono::seconds constexpr scendere::block_arrival::arrival_time_min;

namespace scendere
{
extern unsigned char scendere_bootstrap_weights_live[];
extern std::size_t scendere_bootstrap_weights_live_size;
extern unsigned char scendere_bootstrap_weights_beta[];
extern std::size_t scendere_bootstrap_weights_beta_size;
}

void scendere::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a] (boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (scendere::transport::map_endpoint_to_v6 (i->endpoint ()));
				std::weak_ptr<scendere::node> node_w (node_l);
...
std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (rep_crawler & rep_crawler, std::string const & name)
{
	std::size_t count;
	{
		scendere::lock_guard<scendere::mutex> guard (rep_crawler.active_mutex);
...
scendere::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, scendere::logging const & logging_a, scendere::work_pool & work_a, scendere::node_flags flags_a, unsigned seq) :
	node (io_ctx_a, application_path_a, scendere::node_config (peering_port_a, logging_a), work_a, flags_a, seq)
{
}

scendere::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, scendere::node_config const & config_a, scendere::work_pool & work_a, scendere::node_flags flags_a, unsigned seq) :
	...
	workers (std::max (3u, config.io_threads / 4), scendere::thread_role::name::worker),
	flags (flags_a),
	work (work_a),
	distributed_work (*this),
	logger (config_a.logging.min_time_between_log_output),
	store_impl (scendere::make_store (logger, application_path_a, network_params.ledger, flags.read_only, true, config_a.rocksdb_config, config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_config, config_a.backup_before_upgrade)),
	store (*store_impl),
	unchecked{ store, flags.disable_block_processor_unchecked_deletion },
	wallets_store_impl (std::make_unique<scendere::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config)),
	...
	telemetry (std::make_shared<scendere::telemetry> (network, workers, observers.telemetry, stats, network_params, flags.disable_ongoing_telemetry_requests)),
	...
	node_seq (seq)
{
	unchecked.satisfied = [this] (scendere::unchecked_info const & info) {
	...
	if (!init_error ())
	{
		...
		{
			auto endpoint_l (scendere::tcp_endpoint (boost::asio::ip::make_address_v6 (config.websocket_config.address), config.websocket_config.port));
			websocket_server = std::make_shared<scendere::websocket::listener> (config.websocket_config.tls_config, logger, wallets, io_ctx, endpoint_l);
		...
		network.channel_observer = [this] (std::shared_ptr<scendere::transport::channel> const & channel_a) {
		...
		{
			observers.blocks.add ([this] (scendere::election_status const & status_a, std::vector<scendere::vote_with_weight_info> const & votes_a, scendere::account const & account_a, scendere::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				auto block_a (status_a.winner);
				if ((status_a.type == scendere::election_status_type::active_confirmed_quorum || status_a.type == scendere::election_status_type::active_confirmation_height) && this->block_arrival.recent (block_a->hash ()))
				{
					auto node_l (shared_from_this ());
					background ([node_l, block_a, account_a, amount_a, is_state_send_a, is_state_epoch_a] () {
						...
						// Subtype field
						else if (block_a->type () == scendere::block_type::state)
						{
						...
						resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
							...
							else
							{
								...
								node_l->stats.inc (scendere::stat::type::error, scendere::stat::detail::http_callback, scendere::stat::dir::out);
							}
						});
					});
				}
		...
		{
			observers.blocks.add ([this] (scendere::election_status const & status_a, std::vector<scendere::vote_with_weight_info> const & votes_a, scendere::account const & account_a, scendere::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
				debug_assert (status_a.type != scendere::election_status_type::ongoing);

				if (this->websocket_server->any_subscriber (scendere::websocket::topic::confirmation))
				{
					auto block_a (status_a.winner);
					std::string subtype;
					if (is_state_send_a)
			...
			observers.active_stopped.add ([this] (scendere::block_hash const & hash_a) {
				if (this->websocket_server->any_subscriber (scendere::websocket::topic::stopped_election))
				{
			...
			observers.telemetry.add ([this] (scendere::telemetry_data const & telemetry_data, scendere::endpoint const & endpoint) {
				if (this->websocket_server->any_subscriber (scendere::websocket::topic::telemetry))
				{
					scendere::websocket::message_builder builder;
		...
		observers.blocks.add ([this] (scendere::election_status const & status_a, std::vector<scendere::vote_with_weight_info> const & votes_a, scendere::account const & account_a, scendere::amount const & amount_a, bool is_state_send_a, bool is_state_epoch_a) {
			...
			switch (status_a.type)
			{
				case scendere::election_status_type::active_confirmed_quorum:
					this->stats.inc (scendere::stat::type::confirmation_observer, scendere::stat::detail::active_quorum, scendere::stat::dir::out);
					break;
				case scendere::election_status_type::active_confirmation_height:
					this->stats.inc (scendere::stat::type::confirmation_observer, scendere::stat::detail::active_conf_height, scendere::stat::dir::out);
					break;
				case scendere::election_status_type::inactive_confirmation_height:
					this->stats.inc (scendere::stat::type::confirmation_observer, scendere::stat::detail::inactive_conf_height, scendere::stat::dir::out);
					break;
				default:
					break;
			}
		});
		observers.endpoint.add ([this] (std::shared_ptr<scendere::transport::channel> const & channel_a) {
			if (channel_a->get_type () == scendere::transport::transport_type::udp)
			{
		...
		observers.vote.add ([this] (std::shared_ptr<scendere::vote> vote_a, std::shared_ptr<scendere::transport::channel> const & channel_a, scendere::vote_code code_a) {
			debug_assert (code_a != scendere::vote_code::invalid);
			// The vote_code::vote is handled inside the election
			if (code_a == scendere::vote_code::indeterminate)
			{
		...
		{
			observers.vote.add ([this] (std::shared_ptr<scendere::vote> vote_a, std::shared_ptr<scendere::transport::channel> const & channel_a, scendere::vote_code code_a) {
				if (this->websocket_server->any_subscriber (scendere::websocket::topic::vote))
				{
					scendere::websocket::message_builder builder;
		...
		observers.work_cancel.add ([this] (scendere::root const & root_a) {
			this->work.cancel (root_a);
			this->distributed_work.cancel (root_a);
		});

		logger.always_log ("Node starting, version: ", SCENDERE_VERSION_STRING);
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", scendere::uint128_union (rep.second).format_balance (Mxrb_ratio, 0, true), " XRB");
				}
			}
...
void scendere::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> const & target, std::shared_ptr<std::string> const & body, std::shared_ptr<boost::asio::ip::tcp::resolver> const & resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver] (boost::system::error_code const & ec, std::size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (scendere::stat::type::http_callback, scendere::stat::detail::initiate, scendere::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool scendere::node::copy_with_compaction (boost::filesystem::path const & destination)
{
	return store.copy_db (destination);
}

std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (node & node, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
...
void scendere::node::process_active (std::shared_ptr<scendere::block> const & incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming);
}

scendere::process_return scendere::node::process (scendere::block & block_a)
{
...
scendere::process_return scendere::node::process_local (std::shared_ptr<scendere::block> const & block_a)
{
	...
	auto const transaction (store.tx_begin_write ({ tables::accounts, tables::blocks, tables::frontiers, tables::pending }));
	return block_processor.process_one (transaction, post_events, info, false, scendere::block_origin::local);
}

void scendere::node::process_local_async (std::shared_ptr<scendere::block> const & block_a)
{
	// Add block hash as recently arrived to trigger automatic rebroadcast and election
	block_arrival.add (block_a->hash ());
	// Set current time to trigger automatic rebroadcast and election
...
void scendere::node::start ()
{
	...
	wallets.start ();
	if (config.frontiers_confirmation != scendere::frontiers_confirmation_mode::disabled)
	{
...
void scendere::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
...
scendere::block_hash scendere::node::latest (scendere::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

scendere::uint128_t scendere::node::balance (scendere::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<scendere::block> scendere::node::block (scendere::block_hash const & hash_a)
{
	auto const transaction (store.tx_begin_read ());
	return store.block.get (transaction, hash_a);
}

std::pair<scendere::uint128_t, scendere::uint128_t> scendere::node::balance_pending (scendere::account const & account_a, bool only_confirmed_a)
{
	std::pair<scendere::uint128_t, scendere::uint128_t> result;
	auto const transaction (store.tx_begin_read ());
...
scendere::uint128_t scendere::node::weight (scendere::account const & account_a)
{
	return ledger.weight (account_a);
}

scendere::block_hash scendere::node::rep_block (scendere::account const & account_a)
{
	auto const transaction (store.tx_begin_read ());
...
scendere::uint128_t scendere::node::minimum_principal_weight ()
{
	return minimum_principal_weight (online_reps.trended ());
}

scendere::uint128_t scendere::node::minimum_principal_weight (scendere::uint128_t const & online_stake)
{
	return online_stake / network_params.network.principal_weight_factor;
}

void scendere::node::long_inactivity_cleanup ()
{
...
void scendere::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
	vote_processor.calculate_weights ();
	std::weak_ptr<scendere::node> node_w (shared_from_this ());
...
void scendere::node::ongoing_bootstrap ()
{
	...
	auto previous_bootstrap_count (stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::initiate, scendere::stat::dir::out) + stats.count (scendere::stat::type::bootstrap, scendere::stat::detail::initiate_legacy_age, scendere::stat::dir::out));
	/* 
...
void scendere::node::ongoing_peer_store ()
{
...
void scendere::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		scendere::set_secure_perm_directory (backup_path, error_chmod);
...
void scendere::node::search_receivable_all ()
{
...
void scendere::node::bootstrap_wallet ()
{
	std::deque<scendere::account> accounts;
	{
		scendere::lock_guard<scendere::mutex> lock (wallets.mutex);
		...
		for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n && accounts.size () < 128; ++i)
		{
			...
			{
				scendere::account account (j->first);
...
void scendere::node::unchecked_cleanup ()
{
	std::vector<scendere::uint128_t> digests;
	std::deque<scendere::unchecked_key> cleaning_list;
	...
	{
		auto const now (scendere::seconds_since_epoch ());
		auto const transaction (store.tx_begin_read ());
		// Max 1M records to clean, max 2 minutes reading to prevent slow i/o systems issues
		for (auto [i, n] = unchecked.full_range (transaction); i != n && cleaning_list.size () < 1024 * 1024 && scendere::seconds_since_epoch () - now < 120; ++i)
		{
...
void scendere::node::ongoing_unchecked_cleanup ()
{
	unchecked_cleanup ();
...
void scendere::node::ongoing_backlog_population ()
{
	populate_backlog ();
...
bool scendere::node::collect_ledger_pruning_targets (std::deque<scendere::block_hash> & pruning_targets_a, scendere::account & last_account_a, uint64_t const batch_read_size_a, uint64_t const max_depth_a, uint64_t const cutoff_time_a)
{
	...
	for (auto i (store.confirmation_height.begin (transaction, last_account_a)), n (store.confirmation_height.end ()); i != n && !finish_transaction;)
	{
		...
		scendere::block_hash hash (i->second.frontier);
		uint64_t depth (0);
...
void scendere::node::ledger_pruning (uint64_t const batch_size_a, bool bootstrap_weight_reached_a, bool log_to_cout_a)
{
	uint64_t const max_depth (config.max_pruning_depth != 0 ? config.max_pruning_depth : std::numeric_limits<uint64_t>::max ());
	uint64_t const cutoff_time (bootstrap_weight_reached_a ? scendere::seconds_since_epoch () - config.max_pruning_age.count () : std::numeric_limits<uint64_t>::max ());
	uint64_t pruned_count (0);
	uint64_t transaction_write_count (0);
	scendere::account last_account (1); // 0 Burn account is never opened. So it can be used to break loop
	std::deque<scendere::block_hash> pruning_targets;
	...
	while ((transaction_write_count != 0 || !target_finished) && !stopped)
	{
		...
		{
			auto scoped_write_guard = write_database_queue.wait (scendere::writer::pruning);
...
void scendere::node::ongoing_ledger_pruning ()
{
	auto bootstrap_weight_reached (ledger.cache.block_count >= ledger.bootstrap_weight_max_blocks);
...
int scendere::node::price (scendere::uint128_t const & balance_a, int amount_a)
{
	debug_assert (balance_a >= amount_a * scendere::Gxrb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= scendere::Gxrb_ratio;
		auto balance_scaled ((balance_l / scendere::Mxrb_ratio).convert_to<double> ());
...
uint64_t scendere::node::default_difficulty (scendere::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case scendere::work_version::work_1:
...
uint64_t scendere::node::default_receive_difficulty (scendere::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
...
uint64_t scendere::node::max_work_generate_difficulty (scendere::work_version const version_a) const
{
	return scendere::difficulty::from_multiplier (config.max_work_generate_multiplier, default_difficulty (version_a));
}

bool scendere::node::local_work_generation_enabled () const
{
	return config.work_threads > 0 || work.opencl;
}

bool scendere::node::work_generation_enabled () const
{
	return work_generation_enabled (config.work_peers);
}

bool scendere::node::work_generation_enabled (std::vector<std::pair<std::string, uint16_t>> const & peers_a) const
{
	return !peers_a.empty () || local_work_generation_enabled ();
}

boost::optional<uint64_t> scendere::node::work_generate_blocking (scendere::block & block_a, uint64_t difficulty_a)
{
...
void scendere::node::work_generate (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t>)> callback_a, boost::optional<scendere::account> const & account_a, bool secondary_work_peers_a)
{
...
boost::optional<uint64_t> scendere::node::work_generate_blocking (scendere::work_version const version_a, scendere::root const & root_a, uint64_t difficulty_a, boost::optional<scendere::account> const & account_a)
{
...
boost::optional<uint64_t> scendere::node::work_generate_blocking (scendere::block & block_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (block_a, default_difficulty (scendere::work_version::work_1));
}

boost::optional<uint64_t> scendere::node::work_generate_blocking (scendere::root const & root_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (root_a, default_difficulty (scendere::work_version::work_1));
}

boost::optional<uint64_t> scendere::node::work_generate_blocking (scendere::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (scendere::work_version::work_1, root_a, difficulty_a);
}

void scendere::node::add_initial_peers ()
{
	...
	{
		scendere::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
...
void scendere::node::block_confirm (std::shared_ptr<scendere::block> const & block_a)
{
...
bool scendere::node::block_confirmed (scendere::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.block_confirmed (transaction, hash_a);
}

bool scendere::node::block_confirmed_or_being_confirmed (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a)
{
	return confirmation_height_processor.is_processing_block (hash_a) || ledger.block_confirmed (transaction_a, hash_a);
}

void scendere::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<scendere::node> node_w (shared_from_this ());
...
bool scendere::node::online () const
{
	return rep_crawler.total_weight () > online_reps.delta ();
}

void scendere::node::ongoing_online_weight_calculation ()
{
	online_reps.sample ();
	ongoing_online_weight_calculation_queue ();
}

void scendere::node::receive_confirmed (scendere::transaction const & block_transaction_a, scendere::block_hash const & hash_a, scendere::account const & destination_a)
{
	scendere::unique_lock<scendere::mutex> lk (wallets.mutex);
	...
	for ([[maybe_unused]] auto const & [id, wallet] : wallets_l)
	{
		if (wallet->store.exists (wallet_transaction, destination_a))
		{
			...
			auto error (store.pending.get (block_transaction_a, scendere::pending_key (destination_a, hash_a), pending));
			if (!error)
			{
				auto amount (pending.amount.number ());
				wallet->receive_async (hash_a, representative, amount, destination_a, [] (std::shared_ptr<scendere::block> const &) {});
			}
			else
			{
...
void scendere::node::process_confirmed_data (scendere::transaction const & transaction_a, std::shared_ptr<scendere::block> const & block_a, scendere::block_hash const & hash_a, scendere::account & account_a, scendere::uint128_t & amount_a, bool & is_state_send_a, bool & is_state_epoch_a, scendere::account & pending_account_a)
{
	...
	{
		amount_a = scendere::dev::constants.genesis_amount;
	}
	if (auto state = dynamic_cast<scendere::state_block *> (block_a.get ()))
	{
	...
	if (auto send = dynamic_cast<scendere::send_block *> (block_a.get ()))
	{
		pending_account_a = send->hashables.destination;
	}
}

void scendere::node::process_confirmed (scendere::election_status const & status_a, uint64_t iteration_a)
{
	...
	{
		iteration_a++;
		std::weak_ptr<scendere::node> node_w (shared ());
...
bool scendere::block_arrival::add (scendere::block_hash const & hash_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.get<tag_sequence> ().emplace_back (scendere::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool scendere::block_arrival::recent (scendere::block_hash const & hash_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (block_arrival & block_arrival, std::string const & name)
{
	std::size_t count = 0;
	{
		scendere::lock_guard<scendere::mutex> guard (block_arrival.mutex);
...
std::shared_ptr<scendere::node> scendere::node::shared ()
{
	return shared_from_this ();
}

int scendere::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version.get (transaction);
}

bool scendere::node::init_error () const
{
	return store.init_error () || wallets_store.init_error ();
}

bool scendere::node::epoch_upgrader (scendere::raw_key const & prv_a, scendere::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	if (!error)
	{
		...
		if (!error)
		{
			*epoch_upgrade = std::async (std::launch::async, &scendere::node::epoch_upgrader_impl, this, prv_a, epoch_a, count_limit, threads);
		}
	}
	return error;
}

void scendere::node::set_bandwidth_params (std::size_t limit, double ratio)
{
...
void scendere::node::epoch_upgrader_impl (scendere::raw_key const & prv_a, scendere::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	scendere::thread_role::set (scendere::thread_role::name::epoch_upgrader);
	auto upgrader_process = [] (scendere::node & node_a, std::atomic<uint64_t> & counter, std::shared_ptr<scendere::block> const & epoch, uint64_t difficulty, scendere::public_key const & signer_a, scendere::root const & root_a, scendere::account const & account_a) {
		epoch->block_work_set (node_a.work_generate_blocking (scendere::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!scendere::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (node_a.network_params.work.difficulty (*epoch) >= difficulty);
		scendere::process_result result (scendere::process_result::old);
		if (valid_signature && valid_work)
		{
			result = node_a.process_local (epoch).code;
		}
		if (result == scendere::process_result::progress)
		{
			++counter;
		}
		else
		{
			bool fork (result == scendere::process_result::fork);
	...
	scendere::raw_key raw_key;
	raw_key = prv_a;
	auto signer (scendere::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	scendere::mutex upgrader_mutex;
	scendere::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, scendere::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.account.begin (transaction)), n (store.account.end ()); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (scendere::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped; ++i)
			{
				auto transaction (store.tx_begin_read ());
				scendere::account_info info;
				scendere::account const & account (i->account);
				if (!store.account.get (transaction, account, info) && info.epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (network_params.work.threshold (scendere::work_version::work_1, scendere::block_details (epoch_a, false, false, true)));
					scendere::root const & root (info.head);
					std::shared_ptr<scendere::block> epoch = builder.state ()
														 .account (account)
														 .previous (info.head)
					...
					{
						{
							scendere::unique_lock<scendere::mutex> lock (upgrader_mutex);
							++workers;
						...
						this->workers.push_task ([node_l = shared_from_this (), &upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account] () {
							...
							{
								scendere::lock_guard<scendere::mutex> lock (upgrader_mutex);
								--workers;
							}
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction (store.tx_begin_read ());
			for (auto i (store.pending.begin (transaction, scendere::pending_key (1, 0))), n (store.pending.end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account.exists (transaction, key.account))
				{
					...
					if (info.epoch < epoch_a)
					{
						...
						release_assert (scendere::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (network_params.work.threshold (scendere::work_version::work_1, scendere::block_details (epoch_a, false, false, true)));
						scendere::root const & root (key.account);
						scendere::account const & account (key.account);
						std::shared_ptr<scendere::block> epoch = builder.state ()
															 .account (key.account)
															 .previous (0)
															 .representative (0)
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<scendere::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = store.pending.begin (transaction, scendere::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
...
std::pair<uint64_t, decltype (scendere::ledger::bootstrap_weights)> scendere::node::get_bootstrap_weights () const
{
	std::unordered_map<scendere::account, scendere::uint128_t> weights;
	uint8_t const * weight_buffer = network_params.network.is_live_network () ? scendere_bootstrap_weights_live : scendere_bootstrap_weights_beta;
	std::size_t weight_size = network_params.network.is_live_network () ? scendere_bootstrap_weights_live_size : scendere_bootstrap_weights_beta_size;
	scendere::bufferstream weight_stream ((uint8_t const *)weight_buffer, weight_size);
	scendere::uint128_union block_height;
	uint64_t max_blocks = 0;
	if (!scendere::try_read (weight_stream, block_height))
	{
		max_blocks = scendere::narrow_cast<uint64_t> (block_height.number ());
		while (true)
		{
			scendere::account account;
			if (scendere::try_read (weight_stream, account.bytes))
			{
				break;
			}
			scendere::amount weight;
			if (scendere::try_read (weight_stream, weight.bytes))
			{
				break;
			}
...
void scendere::node::populate_backlog ()
{
	auto done = false;
	uint64_t const chunk_size = 65536;
	scendere::account next = 0;
	uint64_t total = 0;
	while (!stopped && !done)
	{
		auto transaction = store.tx_begin_read ();
...
uint64_t scendere::node::get_confirmation_height (scendere::transaction const & transaction_a, scendere::account & account_a)
{
...
scendere::node_wrapper::node_wrapper (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, scendere::node_flags const & node_flags_a) :
	network_params{ scendere::network_constants::active_network },
	...
	work{ network_params.network, 1 }
{
	...
	scendere::set_secure_perm_directory (path_a, error_chmod);
	scendere::daemon_config daemon_config{ path_a, network_params };
	auto error = scendere::read_node_config_toml (config_path_a, daemon_config, node_flags_a.config_overrides);
	if (error)
	{
	...
	node = std::make_shared<scendere::node> (*io_context, path_a, node_config, work, node_flags_a);
}
...
scendere::inactive_node::inactive_node (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, scendere::node_flags const & node_flags_a) :
...
scendere::inactive_node::inactive_node (boost::filesystem::path const & path_a, scendere::node_flags const & node_flags_a) :
	inactive_node (path_a, path_a, node_flags_a)
{
}

scendere::node_flags const & scendere::inactive_node_flag_defaults ()
{
	static scendere::node_flags node_flags;
...
std::unique_ptr<scendere::store> scendere::make_store (scendere::logger_mt & logger, boost::filesystem::path const & path, scendere::ledger_constants & constants, bool read_only, bool add_db_postfix, scendere::rocksdb_config const & rocksdb_config, scendere::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, scendere::lmdb_config const & lmdb_config_a, bool backup_before_upgrade)
{
	if (rocksdb_config.enable)
	{
		return std::make_unique<scendere::rocksdb_store> (logger, add_db_postfix ? path / "rocksdb" : path, constants, rocksdb_config, read_only);
	}

	return std::make_unique<scendere::mdb_store> (logger, add_db_postfix ? path / "data.ldb" : path, constants, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, backup_before_upgrade);
}
