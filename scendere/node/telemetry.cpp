...
scendere::telemetry::telemetry (scendere::network & network_a, scendere::thread_pool & workers_a, scendere::observer_set<scendere::telemetry_data const &, scendere::endpoint const &> & observers_a, scendere::stat & stats_a, scendere::network_params & network_params_a, bool disable_ongoing_requests_a) :
	network (network_a),
	workers (workers_a),
...
void scendere::telemetry::stop ()
{
	stopped = true;
}

void scendere::telemetry::set (scendere::telemetry_ack const & message_a, scendere::transport::channel const & channel_a)
{
	if (!stopped)
	{
		scendere::unique_lock<scendere::mutex> lk (mutex);
		scendere::endpoint endpoint = channel_a.get_endpoint ();
		...
		if (it == recent_or_initial_request_telemetry_data.cend () || !it->undergoing_request)
		{
			...
			stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::unsolicited_telemetry_ack);
			return;
		}

		recent_or_initial_request_telemetry_data.modify (it, [&message_a] (scendere::telemetry_info & telemetry_info_a) {
...
bool scendere::telemetry::verify_message (scendere::telemetry_ack const & message_a, scendere::transport::channel const & channel_a)
{
	...
	if (!node_id_mismatch)
	{
		...
		if (!remove_channel)
		{
			...
			if (remove_channel)
			{
				stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::different_genesis_hash);
			}
		}
		else
		{
			stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::invalid_signature);
		}
	}
	else
	{
		stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::node_id_mismatch);
	}
...
std::chrono::milliseconds scendere::telemetry::cache_plus_buffer_cutoff_time () const
{
...
bool scendere::telemetry::within_cache_plus_buffer_cutoff (telemetry_info const & telemetry_info) const
{
...
bool scendere::telemetry::within_cache_cutoff (telemetry_info const & telemetry_info) const
{
	auto is_within = (telemetry_info.last_response + cache_cutoff) >= std::chrono::steady_clock::now ();
	return !telemetry_info.awaiting_first_response () && is_within;
}

void scendere::telemetry::ongoing_req_all_peers (std::chrono::milliseconds next_request_interval)
{
		...
		if (auto this_l = this_w.lock ())
		{
			...
			if (!this_l->stopped)
			{
				...
				struct channel_wrapper
				{
					...
					channel_wrapper (std::shared_ptr<scendere::transport::channel> const & channel_a) :
						channel (channel_a)
					{
					}
					scendere::endpoint endpoint () const
					{
				...
				mi::indexed_by<
					mi::hashed_unique<mi::tag<tag_endpoint>,
						mi::const_mem_fun<channel_wrapper, scendere::endpoint, &channel_wrapper::endpoint>>,
					mi::hashed_unique<mi::tag<tag_channel>,
						mi::member<channel_wrapper, std::shared_ptr<scendere::transport::channel>, &channel_wrapper::channel>>>> peers;
				// clang-format on
...

				{
					...
					scendere::lock_guard<scendere::mutex> guard (this_l->mutex);
...
std::unordered_map<scendere::endpoint, scendere::telemetry_data> scendere::telemetry::get_metrics ()
{
	std::unordered_map<scendere::endpoint, scendere::telemetry_data> telemetry_data;
	...
	// clang-format off
	scendere::transform_if (range.begin (), range.end (), std::inserter (telemetry_data, telemetry_data.end ()),
		[this](auto const & telemetry_info) { return this->within_cache_plus_buffer_cutoff (telemetry_info); },
		[](auto const & telemetry_info) { return std::pair<scendere::endpoint const, scendere::telemetry_data>{ telemetry_info.endpoint, telemetry_info.data }; });
	// clang-format on

	return telemetry_data;
}

void scendere::telemetry::get_metrics_single_peer_async (std::shared_ptr<scendere::transport::channel> const & channel_a, std::function<void (telemetry_data_response const &)> const & callback_a)
{
	auto invoke_callback_with_error = [&callback_a, &workers = this->workers, channel_a] () {
		...
		workers.push_task ([callback_a, endpoint] () {
			...
			callback_a ({ scendere::telemetry_data{}, endpoint, error });
		});
	};
	...
	if (!stopped)
	{
		if (channel_a)
		{
			auto add_callback_async = [&workers = this->workers, &callback_a] (telemetry_data const & telemetry_data_a, scendere::endpoint const & endpoint_a) {
			...
			else
			{
				...
				else
				{
					if (it == recent_or_initial_request_telemetry_data.cend ())
					{
						...
						recent_or_initial_request_telemetry_data.emplace (channel_a->get_endpoint (), scendere::telemetry_data (), std::chrono::steady_clock::now (), true);
						it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
					}
					else
					{
						recent_or_initial_request_telemetry_data.modify (it, [] (scendere::telemetry_info & telemetry_info_a) {
...
scendere::telemetry_data_response scendere::telemetry::get_metrics_single_peer (std::shared_ptr<scendere::transport::channel> const & channel_a)
{
...
void scendere::telemetry::fire_request_message (std::shared_ptr<scendere::transport::channel> const & channel_a)
{
	uint64_t round_l;
	{
		auto it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
	...
	std::weak_ptr<scendere::telemetry> this_w (shared_from_this ());
	scendere::telemetry_req message{ network_params.network };
	// clang-format off
	channel_a->send (message, [this_w, endpoint = channel_a->get_endpoint (), round_l](boost::system::error_code const & ec, std::size_t size_a) {
		if (auto this_l = this_w.lock ())
		{
			if (ec)
			{
				...
				this_l->stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::failed_send_telemetry_req);
				scendere::lock_guard<scendere::mutex> guard (this_l->mutex);
			...
			else
			{
				...
				this_l->workers.add_timed_task (std::chrono::steady_clock::now () + this_l->response_time_cutoff, [round_l, this_w, endpoint]() {
					if (auto this_l = this_w.lock ())
					{
						...
						{
							this_l->stats.inc (scendere::stat::type::telemetry, scendere::stat::detail::no_response_received);
							this_l->channel_processed (endpoint, true);
						}
					}
				});			
	...
	scendere::buffer_drop_policy::no_socket_drop);
	// clang-format on
}

void scendere::telemetry::channel_processed (scendere::endpoint const & endpoint_a, bool error_a)
{
...
void scendere::telemetry::flush_callbacks_async (scendere::endpoint const & endpoint_a, bool error_a)
{
	// Post to thread_pool so that it's truly async and not on the calling thread (same problem as std::async otherwise)
	workers.push_task ([endpoint_a, error_a, this_w = std::weak_ptr<scendere::telemetry> (shared_from_this ())] () {
		if (auto this_l = this_w.lock ())
		{
			scendere::unique_lock<scendere::mutex> lk (this_l->mutex);
...
void scendere::telemetry::invoke_callbacks (scendere::endpoint const & endpoint_a, bool error_a)
{
	std::vector<std::function<void (telemetry_data_response const &)>> callbacks_l;
	telemetry_data_response response_data{ scendere::telemetry_data (), endpoint_a, error_a };
	{
...
std::size_t scendere::telemetry::telemetry_data_size ()
{
	scendere::lock_guard<scendere::mutex> guard (mutex);
	return recent_or_initial_request_telemetry_data.size ();
}

scendere::telemetry_info::telemetry_info (scendere::endpoint const & endpoint_a, scendere::telemetry_data const & data_a, std::chrono::steady_clock::time_point last_response_a, bool undergoing_request_a) :
	endpoint (endpoint_a),
	data (data_a),
...
bool scendere::telemetry_info::awaiting_first_response () const
{
	return data == scendere::telemetry_data ();
}

std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (telemetry & telemetry, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
	std::size_t callbacks_count;
	{
		scendere::lock_guard<scendere::mutex> guard (telemetry.mutex);
...
scendere::telemetry_data scendere::consolidate_telemetry_data (std::vector<scendere::telemetry_data> const & telemetry_datas)
{
	...
	std::unordered_map<scendere::block_hash, int> genesis_blocks;
	...
	auto strip_outliers_and_sum = [num_either_side_to_remove] (auto & counts) {
		...
		return std::accumulate (counts.begin (), counts.end (), scendere::uint128_t (0), [] (scendere::uint128_t total, auto count) {
			return total += count;
		});
	};
	...
	scendere::telemetry_data consolidated_data;
...
scendere::telemetry_data scendere::local_telemetry_data (scendere::ledger const & ledger_a, scendere::network & network_a, scendere::unchecked_map const & unchecked, uint64_t bandwidth_limit_a, scendere::network_params const & network_params_a, std::chrono::steady_clock::time_point statup_time_a, uint64_t active_difficulty_a, scendere::keypair const & node_id_a)
{
	...
	telemetry_data.peer_count = scendere::narrow_cast<decltype (telemetry_data.peer_count)> (network_a.size ());
	telemetry_data.account_count = ledger_a.cache.account_count;
	telemetry_data.major_version = scendere::get_major_node_version ();
	telemetry_data.minor_version = scendere::get_minor_node_version ();
	telemetry_data.patch_version = scendere::get_patch_node_version ();
	telemetry_data.pre_release_version = scendere::get_pre_release_node_version ();
