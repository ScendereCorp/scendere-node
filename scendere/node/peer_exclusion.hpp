...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<scendere::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (scendere::tcp_endpoint const &, std::size_t const);
	bool check (scendere::tcp_endpoint const &);
	void remove (scendere::tcp_endpoint const &);
