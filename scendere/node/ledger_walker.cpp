...
scendere::ledger_walker::ledger_walker (scendere::ledger const & ledger_a) :
	ledger{ ledger_a },
...
void scendere::ledger_walker::walk_backward (scendere::block_hash const & start_block_hash_a, should_visit_callback const & should_visit_callback_a, visitor_callback const & visitor_callback_a)
{
...
void scendere::ledger_walker::walk (scendere::block_hash const & end_block_hash_a, should_visit_callback const & should_visit_callback_a, visitor_callback const & visitor_callback_a)
{
	std::uint64_t last_walked_block_order_index = 0;
	dht::DiskHash<scendere::block_hash> walked_blocks_order{ scendere::unique_path ().c_str (), static_cast<int> (std::to_string (std::numeric_limits<std::uint64_t>::max ()).size ()) + 1, dht::DHOpenRW };
...
void scendere::ledger_walker::walk_backward (scendere::block_hash const & start_block_hash_a, visitor_callback const & visitor_callback_a)
{
	walk_backward (
	start_block_hash_a,
	[&] (auto const & /* block */) {
...
void scendere::ledger_walker::walk (scendere::block_hash const & end_block_hash_a, visitor_callback const & visitor_callback_a)
{
	walk (
	end_block_hash_a,
	[&] (auto const & /* block */) {
...
void scendere::ledger_walker::enqueue_block (scendere::block_hash block_hash_a)
{
...
void scendere::ledger_walker::enqueue_block (std::shared_ptr<scendere::block> const & block_a)
{
	debug_assert (block_a);
	enqueue_block (block_a->hash ());
}

bool scendere::ledger_walker::add_to_walked_blocks (scendere::block_hash const & block_hash_a)
{
	if (use_in_memory_walked_blocks)
	{
		...
		walked_blocks_disk.emplace (scendere::unique_path ().c_str (), sizeof (scendere::block_hash::bytes) + 1, dht::DHOpenRW);
...
bool scendere::ledger_walker::add_to_walked_blocks_disk (scendere::block_hash const & block_hash_a)
{
	debug_assert (!use_in_memory_walked_blocks);
	debug_assert (walked_blocks_disk.has_value ());

	std::array<decltype (scendere::block_hash::chars)::value_type, sizeof (scendere::block_hash::bytes) + 1> block_hash_key{};
...
void scendere::ledger_walker::clear_queue ()
{
	use_in_memory_walked_blocks = true;
...
std::shared_ptr<scendere::block> scendere::ledger_walker::dequeue_block (scendere::transaction const & transaction_a)
{
