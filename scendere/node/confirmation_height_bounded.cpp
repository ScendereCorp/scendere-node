...
#include <scendere/node/confirmation_height_bounded.hpp>
...
scendere::confirmation_height_bounded::confirmation_height_bounded (scendere::ledger & ledger_a, scendere::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, scendere::logging const & logging_a, scendere::logger_mt & logger_a, std::atomic<bool> & stopped_a, uint64_t & batch_write_size_a, std::function<void (std::vector<std::shared_ptr<scendere::block>> const &)> const & notify_observers_callback_a, std::function<void (scendere::block_hash const &)> const & notify_block_already_cemented_observers_callback_a, std::function<uint64_t ()> const & awaiting_processing_size_callback_a) :
	ledger (ledger_a),
...
scendere::confirmation_height_bounded::top_and_next_hash scendere::confirmation_height_bounded::get_next_block (boost::optional<top_and_next_hash> const & next_in_receive_chain_a, boost::circular_buffer_space_optimized<scendere::block_hash> const & checkpoints_a, boost::circular_buffer_space_optimized<receive_source_pair> const & receive_source_pairs, boost::optional<receive_chain_details> & receive_details_a, scendere::block const & original_block)
{
...
void scendere::confirmation_height_bounded::process (std::shared_ptr<scendere::block> original_block)
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	boost::circular_buffer_space_optimized<scendere::block_hash> checkpoints{ max_items };
	...
	do
	{
		...
		scendere::account account (block->account ());
		if (account.is_zero ())
		{
		...
		if (!hit_receive || (receive_source_pairs.size () == 1 && top_most_non_receive_block_hash != current))
		{
			...
			if ((max_batch_write_size_reached || should_output || force_write) && !pending_writes.empty ())
			{
				...
				if (write_database_queue.process (scendere::writer::confirmation_height))
				{
...
scendere::block_hash scendere::confirmation_height_bounded::get_least_unconfirmed_hash_from_top_level (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::account const & account_a, scendere::confirmation_height_info const & confirmation_height_info_a, uint64_t & block_height_a)
{
	scendere::block_hash least_unconfirmed_hash = hash_a;
...
bool scendere::confirmation_height_bounded::iterate (scendere::read_transaction const & transaction_a, uint64_t bottom_height_a, scendere::block_hash const & bottom_hash_a, boost::circular_buffer_space_optimized<scendere::block_hash> & checkpoints_a, scendere::block_hash & top_most_non_receive_block_hash_a, scendere::block_hash const & top_level_hash_a, boost::circular_buffer_space_optimized<receive_source_pair> & receive_source_pairs_a, scendere::account const & account_a)
{
	...
	while (!hash.is_zero () && !reached_target && !stopped)
	{
		...
		if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block.exists (transaction_a, source))
		{
			...
			auto next = !sideband.successor.is_zero () && sideband.successor != top_level_hash_a ? boost::optional<scendere::block_hash> (sideband.successor) : boost::none;
...
void scendere::confirmation_height_bounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
...
void scendere::confirmation_height_bounded::cement_blocks (scendere::write_guard & scoped_write_guard_a)
{
	...
	std::vector<std::shared_ptr<scendere::block>> cemented_blocks;
	...
	scendere::timer<> cemented_batch_timer;
	auto error = false;
	{
		// This only writes to the confirmation_height table and is the only place to do so in a single process
		auto transaction (ledger.store.tx_begin_write ({}, { scendere::tables::confirmation_height }));
		...
		while (!error && !pending_writes.empty ())
		{
			...
			auto write_confirmation_height = [&account, &ledger = ledger, &transaction] (uint64_t num_blocks_cemented, uint64_t confirmation_height, scendere::block_hash const & confirmed_frontier) {
#ifndef NDEBUG
				// Extra debug checks
				scendere::confirmation_height_info confirmation_height_info;
				ledger.store.confirmation_height.get (transaction, account, confirmation_height_info);
...
#endif
				ledger.store.confirmation_height.put (transaction, account, scendere::confirmation_height_info{ confirmation_height, confirmed_frontier });
				ledger.cache.cemented_count += num_blocks_cemented;
				ledger.stats.add (scendere::stat::type::confirmation_height, scendere::stat::detail::blocks_confirmed, scendere::stat::dir::in, num_blocks_cemented);
				ledger.stats.add (scendere::stat::type::confirmation_height, scendere::stat::detail::blocks_confirmed_bounded, scendere::stat::dir::in, num_blocks_cemented);
			};
			...
			if (pending.top_height > confirmation_height_info.height)
			{
				...
				scendere::block_hash new_cemented_frontier;
				...
				for (auto num_blocks_iterated = 0; num_blocks_confirmed - num_blocks_iterated != 0; ++num_blocks_iterated)
				{
					...
					if (cemented_blocks.size () > batch_write_size + (batch_write_size / 10))
					{
						...
						{
							scoped_write_guard_a = write_database_queue.wait (scendere::writer::confirmation_height);
							transaction.renew ();
						}
...
bool scendere::confirmation_height_bounded::pending_empty () const
{
	return pending_writes.empty ();
}

void scendere::confirmation_height_bounded::clear_process_vars ()
{
	accounts_confirmed_info.clear ();
	accounts_confirmed_info_size = 0;
}

scendere::confirmation_height_bounded::receive_chain_details::receive_chain_details (scendere::account const & account_a, uint64_t height_a, scendere::block_hash const & hash_a, scendere::block_hash const & top_level_a, boost::optional<scendere::block_hash> next_a, uint64_t bottom_height_a, scendere::block_hash const & bottom_most_a) :
	account (account_a),
	height (height_a),
	hash (hash_a),
...
scendere::confirmation_height_bounded::write_details::write_details (scendere::account const & account_a, uint64_t bottom_height_a, scendere::block_hash const & bottom_hash_a, uint64_t top_height_a, scendere::block_hash const & top_hash_a) :
	account (account_a),
	bottom_height (bottom_height_a),
...
scendere::confirmation_height_bounded::receive_source_pair::receive_source_pair (confirmation_height_bounded::receive_chain_details const & receive_details_a, const block_hash & source_a) :
	receive_details (receive_details_a),
	source_hash (source_a)
{
}

scendere::confirmation_height_bounded::confirmed_info::confirmed_info (uint64_t confirmed_height_a, scendere::block_hash const & iterated_frontier_a) :
	confirmed_height (confirmed_height_a),
	iterated_frontier (iterated_frontier_a)
{
}

std::unique_ptr<scendere::container_info_component> scendere::collect_container_info (confirmation_height_bounded & confirmation_height_bounded, std::string const & name_a)
{
