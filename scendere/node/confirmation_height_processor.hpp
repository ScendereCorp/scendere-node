...
{
public:
	confirmation_height_processor (scendere::ledger &, scendere::write_database_queue &, std::chrono::milliseconds, scendere::logging const &, scendere::logger_mt &, boost::latch & initialized_latch, confirmation_height_mode = confirmation_height_mode::automatic);
	...
	bool is_processing_added_block (scendere::block_hash const & hash_a) const;
	bool is_processing_block (scendere::block_hash const &) const;
	scendere::block_hash current () const;

	void add_cemented_observer (std::function<void (std::shared_ptr<scendere::block> const &)> const &);
	void add_block_already_cemented_observer (std::function<void (scendere::block_hash const &)> const &);

private:
	mutable scendere::mutex mutex{ mutex_identifier (mutexes::confirmation_height_processor) };
	// Hashes which have been added to the confirmation height processor, but not yet processed
	// clang-format off
	struct block_wrapper
	{
		block_wrapper (std::shared_ptr<scendere::block> const & block_a) :
		block (block_a)
		{
		}

		std::reference_wrapper<scendere::block_hash const> hash () const
		{
			return block->hash ();
		}
	...
	mi::indexed_by<
		...
		mi::hashed_unique<mi::tag<tag_hash>,
			mi::const_mem_fun<block_wrapper, std::reference_wrapper<scendere::block_hash const>, &block_wrapper::hash>>>> awaiting_processing;
	// clang-format on

	// Hashes which have been added and processed, but have not been cemented
	std::unordered_set<scendere::block_hash> original_hashes_pending;
	bool paused{ false };

	/** This is the last block popped off the confirmation height pending collection */
	std::shared_ptr<scendere::block> original_block;
	...
	std::vector<std::function<void (std::shared_ptr<scendere::block> const &)>> cemented_observers;
	std::vector<std::function<void (scendere::block_hash const &)>> block_already_cemented_observers;
	...
	void set_next_hash ();
	void notify_observers (std::vector<std::shared_ptr<scendere::block>> const &);
	void notify_observers (scendere::block_hash const &);
