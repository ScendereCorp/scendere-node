...
scendere::rocksdb_store::rocksdb_store (scendere::logger_mt & logger_a, boost::filesystem::path const & path_a, scendere::ledger_constants & constants, scendere::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
	// clang-format off
	store_partial{
		constants,
		block_store_partial,
	...
	max_block_write_batch_num_m{ scendere::narrow_cast<unsigned> (blocks_memtable_size_bytes () / (2 * (sizeof (scendere::block_type) + scendere::state_block::size + scendere::block_sideband::size (scendere::block_type::state)))) },
...
std::unordered_map<char const *, scendere::tables> scendere::rocksdb_store::create_cf_name_table_map () const
{
	std::unordered_map<char const *, scendere::tables> map{ { rocksdb::kDefaultColumnFamilyName.c_str (), tables::default_unused },
...
void scendere::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
void scendere::rocksdb_store::generate_tombstone_map ()
{
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (scendere::tables::unchecked), std::forward_as_tuple (0, 50000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (scendere::tables::blocks), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (scendere::tables::accounts), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (scendere::tables::pending), std::forward_as_tuple (0, 25000));
}

rocksdb::ColumnFamilyOptions scendere::rocksdb_store::get_common_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
rocksdb::ColumnFamilyOptions scendere::rocksdb_store::get_cf_options (std::string const & cf_name_a) const
{
	rocksdb::ColumnFamilyOptions cf_options;
...
std::vector<rocksdb::ColumnFamilyDescriptor> scendere::rocksdb_store::create_column_families ()
{
...
scendere::write_transaction scendere::rocksdb_store::tx_begin_write (std::vector<scendere::tables> const & tables_requiring_locks_a, std::vector<scendere::tables> const & tables_no_locks_a)
{
	std::unique_ptr<scendere::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<scendere::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<scendere::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return scendere::write_transaction{ std::move (txn) };
}

scendere::read_transaction scendere::rocksdb_store::tx_begin_read () const
{
	return scendere::read_transaction{ std::make_unique<scendere::read_rocksdb_txn> (db.get ()) };
}

std::string scendere::rocksdb_store::vendor_get () const
{
	return boost::str (boost::format ("RocksDB %1%.%2%.%3%") % ROCKSDB_MAJOR % ROCKSDB_MINOR % ROCKSDB_PATCH);
}

rocksdb::ColumnFamilyHandle * scendere::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool scendere::rocksdb_store::exists (scendere::transaction const & transaction_a, tables table_a, scendere::rocksdb_val const & key_a) const
{
...
int scendere::rocksdb_store::del (scendere::write_transaction const & transaction_a, tables table_a, scendere::rocksdb_val const & key_a)
{
...
void scendere::rocksdb_store::flush_tombstones_check (tables table_a)
{
...
void scendere::rocksdb_store::flush_table (scendere::tables table_a)
{
	db->Flush (rocksdb::FlushOptions{}, table_to_column_family (table_a));
}

void scendere::version_rocksdb_store::version_put (scendere::write_transaction const & transaction_a, int version_a)
{
	...
	auto status (rocksdb_store.put (transaction_a, tables::meta, version_key, scendere::rocksdb_val (version_value)));
	release_assert (rocksdb_store.success (status));
}

rocksdb::Transaction * scendere::rocksdb_store::tx (scendere::transaction const & transaction_a) const
{
	debug_assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int scendere::rocksdb_store::get (scendere::transaction const & transaction_a, tables table_a, scendere::rocksdb_val const & key_a, scendere::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
int scendere::rocksdb_store::put (scendere::write_transaction const & transaction_a, tables table_a, scendere::rocksdb_val const & key_a, scendere::rocksdb_val const & value_a)
{
	debug_assert (transaction_a.contains (table_a));
...
bool scendere::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool scendere::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int scendere::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t scendere::rocksdb_store::count (scendere::transaction const & transaction_a, tables table_a) const
{
	uint64_t sum = 0;
...
int scendere::rocksdb_store::drop (scendere::write_transaction const & transaction_a, tables table_a)
{
	debug_assert (transaction_a.contains (table_a));
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peer.begin (transaction_a), n = peer.end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, scendere::rocksdb_val (i->first));
...
int scendere::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
scendere::unchecked_rocksdb_store::unchecked_rocksdb_store (scendere::rocksdb_store & rocksdb_store_a) :
	scendere::unchecked_store_partial<rocksdb::Slice, scendere::rocksdb_store> (rocksdb_store_a),
	rocksdb_store{ rocksdb_store_a } {};

scendere::version_rocksdb_store::version_rocksdb_store (scendere::rocksdb_store & rocksdb_store_a) :
	scendere::version_store_partial<rocksdb::Slice, scendere::rocksdb_store> (rocksdb_store_a),
	rocksdb_store{ rocksdb_store_a } {};

void scendere::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options scendere::rocksdb_store::get_db_options ()
{
...
rocksdb::BlockBasedTableOptions scendere::rocksdb_store::get_active_table_options (std::size_t lru_size) const
{
...
rocksdb::BlockBasedTableOptions scendere::rocksdb_store::get_small_table_options () const
{
	rocksdb::BlockBasedTableOptions table_options;
	// Improve point lookup performance be using the data block hash index (uses about 5% more space).
	table_options.data_block_index_type = rocksdb::BlockBasedTableOptions::DataBlockIndexType::kDataBlockBinaryAndHash;
...
rocksdb::ColumnFamilyOptions scendere::rocksdb_store::get_small_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a) const
{
...
rocksdb::ColumnFamilyOptions scendere::rocksdb_store::get_active_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
void scendere::rocksdb_store::on_flush (rocksdb::FlushJobInfo const & flush_job_info_a)
{
...
std::vector<scendere::tables> scendere::rocksdb_store::all_tables () const
{
	return std::vector<scendere::tables>{ tables::accounts, tables::blocks, tables::confirmation_height, tables::final_votes, tables::frontiers, tables::meta, tables::online_weight, tables::peers, tables::pending, tables::pruned, tables::unchecked, tables::vote };
}

bool scendere::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		scendere::rocksdb_store rocksdb_store (logger, destination_path.string (), constants, rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

void scendere::rocksdb_store::rebuild_db (scendere::write_transaction const & transaction_a)
{
	// Not available for RocksDB
}

bool scendere::rocksdb_store::init_error () const
{
	return error;
}

void scendere::rocksdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	uint64_t val;
...
unsigned long long scendere::rocksdb_store::blocks_memtable_size_bytes () const
{
	return base_memtable_size_bytes ();
}

unsigned long long scendere::rocksdb_store::base_memtable_size_bytes () const
{
...
unsigned scendere::rocksdb_store::max_block_write_batch_num () const
{
	return max_block_write_batch_num_m;
}

std::string scendere::rocksdb_store::error_string (int status) const
{
	return std::to_string (status);
}

scendere::rocksdb_store::tombstone_info::tombstone_info (uint64_t num_since_last_flush_a, uint64_t const max_a) :
...
template class scendere::store_partial<rocksdb::Slice, scendere::rocksdb_store>;
