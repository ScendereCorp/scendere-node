...
scendere::transport::channel_tcp::channel_tcp (scendere::node & node_a, std::weak_ptr<scendere::socket> socket_a) :
	channel (node_a),
	socket (std::move (socket_a))
{
}

scendere::transport::channel_tcp::~channel_tcp ()
{
...
std::size_t scendere::transport::channel_tcp::hash_code () const
{
	std::hash<::scendere::tcp_endpoint> hash;
	return hash (get_tcp_endpoint ());
}

bool scendere::transport::channel_tcp::operator== (scendere::transport::channel const & other_a) const
{
	bool result (false);
	auto other_l (dynamic_cast<scendere::transport::channel_tcp const *> (&other_a));
	if (other_l != nullptr)
	{
		return *this == *other_l;
	}
...
void scendere::transport::channel_tcp::send_buffer (scendere::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, std::size_t)> const & callback_a, scendere::buffer_drop_policy policy_a)
{
	if (auto socket_l = socket.lock ())
	{
		if (!socket_l->max () || (policy_a == scendere::buffer_drop_policy::no_socket_drop && !socket_l->full ()))
		{
			socket_l->async_write (
			buffer_a, [endpoint_a = socket_l->remote_endpoint (), node = std::weak_ptr<scendere::node> (node.shared ()), callback_a] (boost::system::error_code const & ec, std::size_t size_a) {
		...
		{
			if (policy_a == scendere::buffer_drop_policy::no_socket_drop)
			{
				node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_no_socket_drop, scendere::stat::dir::out);
			}
			else
			{
				node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_write_drop, scendere::stat::dir::out);
			}
			if (callback_a)
			{
...
std::string scendere::transport::channel_tcp::to_string () const
{
	return boost::str (boost::format ("%1%") % get_tcp_endpoint ());
}

void scendere::transport::channel_tcp::set_endpoint ()
{
	scendere::lock_guard<scendere::mutex> lk (channel_mutex);
	debug_assert (endpoint == scendere::tcp_endpoint (boost::asio::ip::address_v6::any (), 0)); // Not initialized endpoint value
...
scendere::transport::tcp_channels::tcp_channels (scendere::node & node, std::function<void (scendere::message const &, std::shared_ptr<scendere::transport::channel> const &)> sink) :
	node{ node },
	sink{ std::move (sink) }
{
}

bool scendere::transport::tcp_channels::insert (std::shared_ptr<scendere::transport::channel_tcp> const & channel_a, std::shared_ptr<scendere::socket> const & socket_a, std::shared_ptr<scendere::bootstrap_server> const & bootstrap_server_a)
{
	auto endpoint (channel_a->get_tcp_endpoint ());
	debug_assert (endpoint.address ().is_v6 ());
	auto udp_endpoint (scendere::transport::map_tcp_to_endpoint (endpoint));
	bool error (true);
...
void scendere::transport::tcp_channels::erase (scendere::tcp_endpoint const & endpoint_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	channels.get<endpoint_tag> ().erase (endpoint_a);
}

std::size_t scendere::transport::tcp_channels::size () const
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	return channels.size ();
}

std::shared_ptr<scendere::transport::channel_tcp> scendere::transport::tcp_channels::find_channel (scendere::tcp_endpoint const & endpoint_a) const
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
...
std::unordered_set<std::shared_ptr<scendere::transport::channel>> scendere::transport::tcp_channels::random_set (std::size_t count_a, uint8_t min_version, bool include_temporary_channels_a) const
{
...
void scendere::transport::tcp_channels::random_fill (std::array<scendere::endpoint, 8> & target_a) const
{
...
bool scendere::transport::tcp_channels::store_all (bool clear_peers)
{
	...
	std::vector<scendere::endpoint> endpoints;
	{
		...
		std::back_inserter (endpoints), [] (auto const & channel) { return scendere::transport::map_tcp_to_endpoint (channel.endpoint ()); });
	}
	bool result (false);
	if (!endpoints.empty ())
	{
		...
		{
			node.store.peer.put (transaction, scendere::endpoint_key{ endpoint.address ().to_v6 ().to_bytes (), endpoint.port () });
		}
		result = true;
	}
	return result;
...
std::shared_ptr<scendere::transport::channel_tcp> scendere::transport::tcp_channels::find_node_id (scendere::account const & node_id_a)
{
	std::shared_ptr<scendere::transport::channel_tcp> result;
	scendere::lock_guard<scendere::mutex> lock (mutex);
...
scendere::tcp_endpoint scendere::transport::tcp_channels::bootstrap_peer (uint8_t connection_protocol_version_min)
{
...
void scendere::transport::tcp_channels::process_messages ()
{
	while (!stopped)
	{
...
void scendere::transport::tcp_channels::process_message (scendere::message const & message_a, scendere::tcp_endpoint const & endpoint_a, scendere::account const & node_id_a, std::shared_ptr<scendere::socket> const & socket_a)
{
	auto type_a = socket_a->type ();
	if (!stopped && message_a.header.version_using >= node.network_params.network.protocol_version_min)
	{
		auto channel (node.network.find_channel (scendere::transport::map_tcp_to_endpoint (endpoint_a)));
		if (channel)
		{
		...
		else
		{
			...
			else if (!node.network.excluded_peers.check (endpoint_a))
			{
				if (!node_id_a.is_zero ())
				{
					...
					auto temporary_channel (std::make_shared<scendere::transport::channel_tcp> (node, socket_a));
					...
					debug_assert (type_a == scendere::socket::type_t::realtime || type_a == scendere::socket::type_t::realtime_response_server);
					// Don't insert temporary channels for response_server
					if (type_a == scendere::socket::type_t::realtime)
					{
				...
				else
				{
					...
					debug_assert (message_a.header.type == scendere::message_type::node_id_handshake);
...
void scendere::transport::tcp_channels::start ()
{
	ongoing_keepalive ();
}

void scendere::transport::tcp_channels::stop ()
{
	stopped = true;
	scendere::unique_lock<scendere::mutex> lock (mutex);
...
bool scendere::transport::tcp_channels::max_ip_connections (scendere::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_ip, scendere::stat::dir::out);
	}
	return result;
}

bool scendere::transport::tcp_channels::max_subnetwork_connections (scendere::tcp_endpoint const & endpoint_a)
{
	...
	if (result)
	{
		node.stats.inc (scendere::stat::type::tcp, scendere::stat::detail::tcp_max_per_subnetwork, scendere::stat::dir::out);
	}
	return result;
}

bool scendere::transport::tcp_channels::max_ip_or_subnetwork_connections (scendere::tcp_endpoint const & endpoint_a)
{
	return max_ip_connections (endpoint_a) || max_subnetwork_connections (endpoint_a);
}

bool scendere::transport::tcp_channels::reachout (scendere::endpoint const & endpoint_a)
{
	auto tcp_endpoint (scendere::transport::map_endpoint_to_tcp (endpoint_a));
...
std::unique_ptr<scendere::container_info_component> scendere::transport::tcp_channels::collect_container_info (std::string const & name)
{
...
void scendere::transport::tcp_channels::purge (std::chrono::steady_clock::time_point const & cutoff_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
...
void scendere::transport::tcp_channels::ongoing_keepalive ()
{
	scendere::keepalive message{ node.network_params.network };
	node.network.random_fill (message.peers);
	scendere::unique_lock<scendere::mutex> lock (mutex);
	// Wake up channels
	std::vector<std::shared_ptr<scendere::transport::channel_tcp>> send_list;
	...
	scendere::tcp_endpoint invalid_endpoint (boost::asio::ip::address_v6::any (), 0);
	if (!node.network_params.network.is_dev_network () && !node.flags.disable_udp)
	{
		...
		for (auto i (0); i <= random_count; ++i)
		{
			...
			{
				start_tcp (scendere::transport::map_tcp_to_endpoint (tcp_endpoint));
			}
		}
	}
...
void scendere::transport::tcp_channels::list (std::deque<std::shared_ptr<scendere::transport::channel>> & deque_a, uint8_t minimum_version_a, bool include_temporary_channels_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	// clang-format off
...
void scendere::transport::tcp_channels::modify (std::shared_ptr<scendere::transport::channel_tcp> const & channel_a, std::function<void (std::shared_ptr<scendere::transport::channel_tcp> const &)> modify_callback_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
...
void scendere::transport::tcp_channels::update (scendere::tcp_endpoint const & endpoint_a)
{
	scendere::lock_guard<scendere::mutex> lock (mutex);
	auto existing (channels.get<endpoint_tag> ().find (endpoint_a));
	if (existing != channels.get<endpoint_tag> ().end ())
...
void scendere::transport::tcp_channels::start_tcp (scendere::endpoint const & endpoint_a)
{
	...
	auto socket = std::make_shared<scendere::client_socket> (node);
	std::weak_ptr<scendere::socket> socket_w (socket);
	auto channel (std::make_shared<scendere::transport::channel_tcp> (node, socket_w));
	std::weak_ptr<scendere::node> node_w (node.shared ());
	socket->async_connect (scendere::transport::map_endpoint_to_tcp (endpoint_a),
	[node_w, channel, socket, endpoint_a] (boost::system::error_code const & ec) {
		if (auto node_l = node_w.lock ())
		{
			if (!ec && channel)
			{
				...
				scendere::node_id_handshake message (node_l->network_params.network, cookie, boost::none);
...
void scendere::transport::tcp_channels::start_tcp_receive_node_id (std::shared_ptr<scendere::transport::channel_tcp> const & channel_a, scendere::endpoint const & endpoint_a, std::shared_ptr<std::vector<uint8_t>> const & receive_buffer_a)
{
	std::weak_ptr<scendere::node> node_w (node.shared ());
	if (auto socket_l = channel_a->socket.lock ())
	{
		auto cleanup_node_id_handshake_socket = [socket_w = channel_a->socket, node_w] (scendere::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
		...
		auto cleanup_and_udp_fallback = [socket_w = channel_a->socket, node_w, cleanup_node_id_handshake_socket] (scendere::endpoint const & endpoint_a) {
			if (auto node_l = node_w.lock ())
			{
				node_l->network.tcp_channels.udp_fallback (endpoint_a);
		...
		socket_l->async_read (receive_buffer_a, 8 + sizeof (scendere::account) + sizeof (scendere::account) + sizeof (scendere::signature), [node_w, channel_a, endpoint_a, receive_buffer_a, cleanup_and_udp_fallback, cleanup_node_id_handshake_socket] (boost::system::error_code const & ec, std::size_t size_a) {
			if (auto node_l = node_w.lock ())
			{
				if (!ec && channel_a)
				{
					node_l->stats.inc (scendere::stat::type::message, scendere::stat::detail::node_id_handshake, scendere::stat::dir::in);
					auto error (false);
					scendere::bufferstream stream (receive_buffer_a->data (), size_a);
					scendere::message_header header (error, stream);
					if (!error && header.type == scendere::message_type::node_id_handshake)
					{
						if (header.version_using >= node_l->network_params.network.protocol_version_min)
						{
							scendere::node_id_handshake message (error, stream, header);
							if (!error && message.response && message.query)
							{
								...
								if (process)
								{
									...
									boost::optional<std::pair<scendere::account, scendere::signature>> response (std::make_pair (node_l->node_id.pub, scendere::sign_message (node_l->node_id.prv, node_l->node_id.pub, *message.query)));
									scendere::node_id_handshake response_message (node_l->network_params.network, boost::none, response);
									if (node_l->config.logging.network_node_id_handshake_logging ())
									{
										...
										if (auto node_l = node_w.lock ())
										{
											if (!ec && channel_a)
											{
												...
												if (auto socket_l = channel_a->socket.lock ())
												{
													...
													auto response_server = std::make_shared<scendere::bootstrap_server> (socket_l, node_l);
													node_l->network.tcp_channels.insert (channel_a, socket_l, response_server);
													// Listen for possible responses
													response_server->socket->type_set (scendere::socket::type_t::realtime_response_server);
													...
													{
														node_l->telemetry->get_metrics_single_peer_async (channel_a, [] (scendere::telemetry_data_response const &) {
						...
						else
						{
							...
							cleanup_node_id_handshake_socket (endpoint_a);
							{
								scendere::lock_guard<scendere::mutex> lock (node_l->network.tcp_channels.mutex);
								node_l->network.tcp_channels.attempts.get<endpoint_tag> ().erase (scendere::transport::map_endpoint_to_tcp (endpoint_a));
							}
						}
					}
					else
...
void scendere::transport::tcp_channels::udp_fallback (scendere::endpoint const & endpoint_a)
{
	{
		scendere::lock_guard<scendere::mutex> lock (mutex);
		attempts.get<endpoint_tag> ().erase (scendere::transport::map_endpoint_to_tcp (endpoint_a));
	}
