...
scendere::mdb_store::mdb_store (scendere::logger_mt & logger_a, boost::filesystem::path const & path_a, scendere::ledger_constants & constants, scendere::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, scendere::lmdb_config const & lmdb_config_a, bool backup_before_upgrade_a) :
	// clang-format off
	store_partial{
		constants,
		block_store_partial,
	...
	logger (logger_a),
	env (error, path_a, scendere::mdb_env::options::make ().set_config (lmdb_config_a).set_use_no_mem_init (true)),
...
bool scendere::mdb_store::vacuum_after_upgrade (boost::filesystem::path const & path_a, scendere::lmdb_config const & lmdb_config_a)
{
	...
	if (vacuum_success)
	{
		...
		auto options = scendere::mdb_env::options::make ()
...
void scendere::mdb_store::serialize_mdb_tracker (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	mdb_txn_tracker.serialize_json (json, min_read_time, min_write_time);
}

void scendere::mdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	MDB_stat stats;
...
scendere::write_transaction scendere::mdb_store::tx_begin_write (std::vector<scendere::tables> const &, std::vector<scendere::tables> const &)
{
	return env.tx_begin_write (create_txn_callbacks ());
}

scendere::read_transaction scendere::mdb_store::tx_begin_read () const
{
	return env.tx_begin_read (create_txn_callbacks ());
}

std::string scendere::mdb_store::vendor_get () const
{
	return boost::str (boost::format ("LMDB %1%.%2%.%3%") % MDB_VERSION_MAJOR % MDB_VERSION_MINOR % MDB_VERSION_PATCH);
}

scendere::mdb_txn_callbacks scendere::mdb_store::create_txn_callbacks () const
{
	scendere::mdb_txn_callbacks mdb_txn_callbacks;
	if (txn_tracking_enabled)
	{
		mdb_txn_callbacks.txn_start = ([&mdb_txn_tracker = mdb_txn_tracker] (scendere::transaction_impl const * transaction_impl) {
			mdb_txn_tracker.add (transaction_impl);
		});
		mdb_txn_callbacks.txn_end = ([&mdb_txn_tracker = mdb_txn_tracker] (scendere::transaction_impl const * transaction_impl) {
...
void scendere::mdb_store::open_databases (bool & error_a, scendere::transaction const & transaction_a, unsigned flags)
{
...
bool scendere::mdb_store::do_upgrades (scendere::write_transaction & transaction_a, bool & needs_vacuuming)
{
	auto error (false);
	auto version_l = version.get (transaction_a);
	switch (version_l)
...
void scendere::mdb_store::upgrade_v14_to_v15 (scendere::write_transaction & transaction_a)
{
	logger.always_log ("Preparing v14 to v15 database upgrade...");

	std::vector<std::pair<scendere::account, scendere::account_info>> account_infos;
	...
	scendere::mdb_merge_iterator<scendere::account, scendere::account_info_v14> i_account (transaction_a, accounts_v0_handle, accounts_v1_handle);
	scendere::mdb_merge_iterator<scendere::account, scendere::account_info_v14> n_account{};
	for (; i_account != n_account; ++i_account)
	{
		scendere::account account (i_account->first);
		scendere::account_info_v14 account_info_v14 (i_account->second);
		...
		account_infos.emplace_back (account, scendere::account_info{ account_info_v14.head, rep_block->representative (), account_info_v14.open_block, account_info_v14.balance, account_info_v14.modified, account_info_v14.block_count, i_account.from_first_database ? scendere::epoch::epoch_0 : scendere::epoch::epoch_1 });
		// Move confirmation height from account_info database to its own table
		mdb_put (env.tx (transaction_a), confirmation_height_handle, scendere::mdb_val (account), scendere::mdb_val (account_info_v14.confirmation_height), MDB_APPEND);
	...
	for (auto const & account_account_info_pair : account_infos)
	{
		...
		mdb_put (env.tx (transaction_a), accounts_handle, scendere::mdb_val (account_account_info_pair.first), scendere::mdb_val (account_info), MDB_APPEND);
	}
	...
	scendere::mdb_merge_iterator<scendere::block_hash, scendere::state_block_w_sideband_v14> i_state (transaction_a, state_blocks_v0_handle, state_blocks_v1_handle);
	scendere::mdb_merge_iterator<scendere::block_hash, scendere::state_block_w_sideband_v14> n_state{};
	auto num = 0u;
	for (; i_state != n_state; ++i_state, ++num)
	{
		scendere::block_hash hash (i_state->first);
		scendere::state_block_w_sideband_v14 state_block_w_sideband_v14 (i_state->second);
		auto & sideband_v14 = state_block_w_sideband_v14.sideband;

		scendere::block_sideband_v18 sideband (sideband_v14.account, sideband_v14.successor, sideband_v14.balance, sideband_v14.height, sideband_v14.timestamp, i_state.from_first_database ? scendere::epoch::epoch_0 : scendere::epoch::epoch_1, false, false, false);
		...
		scendere::mdb_val value{ data.size (), (void *)data.data () };
		auto s = mdb_put (env.tx (transaction_a), state_blocks_new, scendere::mdb_val (hash), value, MDB_APPEND);
	...
	std::vector<std::pair<scendere::pending_key, scendere::pending_info>> pending_infos;
	pending_infos.reserve (pending_counters.before_v0 + pending_counters.before_v1);

	scendere::mdb_merge_iterator<scendere::pending_key, scendere::pending_info_v14> i_pending (transaction_a, pending_v0_handle, pending_v1_handle);
	scendere::mdb_merge_iterator<scendere::pending_key, scendere::pending_info_v14> n_pending{};
	for (; i_pending != n_pending; ++i_pending)
	{
		scendere::pending_info_v14 info (i_pending->second);
		pending_infos.emplace_back (scendere::pending_key (i_pending->first), scendere::pending_info{ info.source, info.amount, i_pending.from_first_database ? scendere::epoch::epoch_0 : scendere::epoch::epoch_1 });
	...
	{
		mdb_put (env.tx (transaction_a), pending_handle, scendere::mdb_val (pending_key_pending_info_pair.first), scendere::mdb_val (pending_key_pending_info_pair.second), MDB_APPEND);
	}
...
void scendere::mdb_store::upgrade_v15_to_v16 (scendere::write_transaction const & transaction_a)
{
...
void scendere::mdb_store::upgrade_v16_to_v17 (scendere::write_transaction const & transaction_a)
{
	...
	std::vector<std::pair<scendere::account, scendere::confirmation_height_info>> confirmation_height_infos;
	auto num = 0u;
	for (scendere::mdb_iterator<scendere::account, uint64_t> i (transaction_a, confirmation_height_handle), n (scendere::mdb_iterator<scendere::account, uint64_t>{}); i != n; ++i, ++account_info_i, ++num)
	{
		scendere::account account (i->first);
		...
		{
			confirmation_height_infos.emplace_back (account, confirmation_height_info{ 0, scendere::block_hash (0) });
		}
		else
		{
	...
	{
		mdb_put (env.tx (transaction_a), confirmation_height_handle, scendere::mdb_val (confirmation_height_info_pair.first), scendere::mdb_val (confirmation_height_info_pair.second), MDB_APPEND);
	}
...
void scendere::mdb_store::upgrade_v17_to_v18 (scendere::write_transaction const & transaction_a)
{
	...
	auto num = 0u;
	for (scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::state_block>> state_i (transaction_a, state_blocks_handle), state_n{}; state_i != state_n; ++state_i, ++num)
	{
		scendere::block_w_sideband_v18<scendere::state_block> block_w_sideband (state_i->second);
		...
		scendere::block_sideband_v18 new_sideband (sideband.account, sideband.successor, sideband.balance, sideband.height, sideband.timestamp, sideband.details.epoch, is_send, is_receive, is_epoch);
		// Write these out
		std::vector<uint8_t> data;
		{
			scendere::vectorstream stream (data);
...
void scendere::mdb_store::upgrade_v18_to_v19 (scendere::write_transaction const & transaction_a)
{
	...
	std::map<scendere::block_hash, scendere::block_w_sideband> legacy_open_receive_change_blocks;

	for (auto i (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::change_block>> (std::make_unique<scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::change_block>>> (transaction_a, change_blocks_handle))), n (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::change_block>> (nullptr)); i != n; ++i)
	{
		scendere::block_sideband_v18 const & old_sideband (i->second.sideband);
		scendere::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, scendere::epoch::epoch_0, false, false, false, scendere::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { scendere::block_w_sideband{ i->second.block, new_sideband } };
	}

	for (auto i (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::open_block>> (std::make_unique<scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::open_block>>> (transaction_a, open_blocks_handle))), n (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::open_block>> (nullptr)); i != n; ++i)
	{
		scendere::block_sideband_v18 const & old_sideband (i->second.sideband);
		scendere::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, scendere::epoch::epoch_0, false, false, false, scendere::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { scendere::block_w_sideband{ i->second.block, new_sideband } };
	...
	for (auto i (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::receive_block>> (std::make_unique<scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::receive_block>>> (transaction_a, receive_blocks_handle))), n (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::receive_block>> (nullptr)); i != n; ++i)
	{
		scendere::block_sideband_v18 const & old_sideband (i->second.sideband);
		scendere::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, scendere::epoch::epoch_0, false, false, false, scendere::epoch::epoch_0);
		legacy_open_receive_change_blocks[i->first] = { scendere::block_w_sideband{ i->second.block, new_sideband } };
	...
	MDB_dbi temp_legacy_open_receive_change_blocks;
	{
		...
		for (auto const & legacy_block : legacy_open_receive_change_blocks)
		{
			...
			{
				scendere::vectorstream stream (data);
				scendere::serialize_block (stream, *legacy_block.second.block);
				legacy_block.second.sideband.serialize (stream, legacy_block.second.block->type ());
			}

			scendere::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_open_receive_change_blocks, scendere::mdb_val (legacy_block.first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	MDB_dbi temp_legacy_send_blocks;
	{
		...
		for (auto i (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::send_block>> (std::make_unique<scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::send_block>>> (transaction_a, send_blocks_handle))), n (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::send_block>> (nullptr)); i != n; ++i)
		{
			...
			{
				scendere::vectorstream stream (data);
				scendere::serialize_block (stream, *block_w_sideband_v18.block);
				block_w_sideband_v18.sideband.serialize (stream, scendere::block_type::send); // Equal to new version for legacy blocks
			}

			scendere::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_send_blocks, scendere::mdb_val (i->first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	MDB_dbi temp_legacy_send_open_receive_change_blocks;
	{
		...
		scendere::mdb_merge_iterator<scendere::block_hash, scendere::block_w_sideband> i (transaction_a, temp_legacy_open_receive_change_blocks, temp_legacy_send_blocks);
		scendere::mdb_merge_iterator<scendere::block_hash, scendere::block_w_sideband> n{};
		for (; i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp_legacy_send_open_receive_change_blocks, scendere::mdb_val (i->first), scendere::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
	...
	{
		auto type_state (scendere::block_type::state);
		mdb_dbi_open (env.tx (transaction_a), "temp_state_blocks", MDB_CREATE, &temp_state_blocks);

		for (auto i (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::state_block>> (std::make_unique<scendere::mdb_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::state_block>>> (transaction_a, state_blocks_handle))), n (scendere::store_iterator<scendere::block_hash, scendere::block_w_sideband_v18<scendere::state_block>> (nullptr)); i != n; ++i)
		{
			auto const & block_w_sideband_v18 (i->second);
			scendere::block_sideband_v18 const & old_sideband (block_w_sideband_v18.sideband);
			...
			if (old_sideband.details.is_receive)
			{
				...
				{
					scendere::bufferstream stream (reinterpret_cast<uint8_t const *> (db_val.get ().data ()), db_val.get ().size ());
					auto source_block (scendere::deserialize_block (stream, type_state));
					release_assert (source_block != nullptr);
					scendere::block_sideband_v18 source_sideband;
					auto error (source_sideband.deserialize (stream, type_state));
					release_assert (!error);
					source_epoch = source_sideband.details.epoch;
				}
			}
			scendere::block_sideband new_sideband (old_sideband.account, old_sideband.successor, old_sideband.balance, old_sideband.height, old_sideband.timestamp, old_sideband.details.epoch, old_sideband.details.is_send, old_sideband.details.is_receive, old_sideband.details.is_epoch, source_epoch);

			std::vector<uint8_t> data;
			{
				scendere::vectorstream stream (data);
				scendere::serialize_block (stream, *block_w_sideband_v18.block);
				new_sideband.serialize (stream, scendere::block_type::state);
			}

			scendere::mdb_val value{ data.size (), (void *)data.data () };
			auto s = mdb_put (env.tx (transaction_a), temp_state_blocks, scendere::mdb_val (i->first), value, MDB_APPEND);
			release_assert_success (*this, s);
		}
	}
	...
	scendere::mdb_merge_iterator<scendere::block_hash, scendere::block_w_sideband> i (transaction_a, temp_legacy_send_open_receive_change_blocks, temp_state_blocks);
	scendere::mdb_merge_iterator<scendere::block_hash, scendere::block_w_sideband> n{};
	mdb_dbi_open (env.tx (transaction_a), "blocks", MDB_CREATE, &blocks_handle);
	for (; i != n; ++i)
	{
		auto s = mdb_put (env.tx (transaction_a), blocks_handle, scendere::mdb_val (i->first), scendere::mdb_val (i->second), MDB_APPEND);
		release_assert_success (*this, s);
	}
...
void scendere::mdb_store::upgrade_v19_to_v20 (scendere::write_transaction const & transaction_a)
{
...
void scendere::mdb_store::upgrade_v20_to_v21 (scendere::write_transaction const & transaction_a)
{
...
void scendere::mdb_store::create_backup_file (scendere::mdb_env & env_a, boost::filesystem::path const & filepath_a, scendere::logger_mt & logger_a)
{
...
scendere::unchecked_mdb_store::unchecked_mdb_store (scendere::mdb_store & mdb_store_a) :
	unchecked_store_partial<MDB_val, mdb_store> (mdb_store_a){};

bool scendere::mdb_store::exists (scendere::transaction const & transaction_a, tables table_a, scendere::mdb_val const & key_a) const
{
	scendere::mdb_val junk;
...
int scendere::mdb_store::get (scendere::transaction const & transaction_a, tables table_a, scendere::mdb_val const & key_a, scendere::mdb_val & value_a) const
{
	return mdb_get (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a);
}

int scendere::mdb_store::put (scendere::write_transaction const & transaction_a, tables table_a, scendere::mdb_val const & key_a, scendere::mdb_val const & value_a) const
{
	return (mdb_put (env.tx (transaction_a), table_to_dbi (table_a), key_a, value_a, 0));
}

int scendere::mdb_store::del (scendere::write_transaction const & transaction_a, tables table_a, scendere::mdb_val const & key_a) const
{
	return (mdb_del (env.tx (transaction_a), table_to_dbi (table_a), key_a, nullptr));
}

int scendere::mdb_store::drop (scendere::write_transaction const & transaction_a, tables table_a)
{
	return clear (transaction_a, table_to_dbi (table_a));
}

int scendere::mdb_store::clear (scendere::write_transaction const & transaction_a, MDB_dbi handle_a)
{
	return mdb_drop (env.tx (transaction_a), handle_a, 0);
}

uint64_t scendere::mdb_store::count (scendere::transaction const & transaction_a, tables table_a) const
{
	return count (transaction_a, table_to_dbi (table_a));
}

uint64_t scendere::mdb_store::count (scendere::transaction const & transaction_a, MDB_dbi db_a) const
{
	MDB_stat stats;
...
MDB_dbi scendere::mdb_store::table_to_dbi (tables table_a) const
{
	switch (table_a)
	{
...
bool scendere::mdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool scendere::mdb_store::success (int status) const
{
	return (MDB_SUCCESS == status);
}

int scendere::mdb_store::status_code_not_found () const
{
	return MDB_NOTFOUND;
}

std::string scendere::mdb_store::error_string (int status) const
{
	return mdb_strerror (status);
}

bool scendere::mdb_store::copy_db (boost::filesystem::path const & destination_file)
{
	return !mdb_env_copy2 (env.environment, destination_file.string ().c_str (), MDB_CP_COMPACT);
}

void scendere::mdb_store::rebuild_db (scendere::write_transaction const & transaction_a)
{
	...
	for (auto const & table : tables)
	{
		...
		for (auto i (scendere::store_iterator<scendere::uint256_union, scendere::mdb_val> (std::make_unique<scendere::mdb_iterator<scendere::uint256_union, scendere::mdb_val>> (transaction_a, table))), n (scendere::store_iterator<scendere::uint256_union, scendere::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, scendere::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert_success (*this, s);
		}
		...
		for (auto i (scendere::store_iterator<scendere::uint256_union, scendere::mdb_val> (std::make_unique<scendere::mdb_iterator<scendere::uint256_union, scendere::mdb_val>> (transaction_a, temp))), n (scendere::store_iterator<scendere::uint256_union, scendere::mdb_val> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), table, scendere::mdb_val (i->first), i->second, MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, table) == count (transaction_a, temp));
	...
	// Pending table
	{
		...
		mdb_dbi_open (env.tx (transaction_a), "temp_table", MDB_CREATE, &temp);
		// Copy all values to temporary table
		for (auto i (scendere::store_iterator<scendere::pending_key, scendere::pending_info> (std::make_unique<scendere::mdb_iterator<scendere::pending_key, scendere::pending_info>> (transaction_a, pending_handle))), n (scendere::store_iterator<scendere::pending_key, scendere::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), temp, scendere::mdb_val (i->first), scendere::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, pending_handle) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), pending_handle, 0);
		// Put values from copy
		for (auto i (scendere::store_iterator<scendere::pending_key, scendere::pending_info> (std::make_unique<scendere::mdb_iterator<scendere::pending_key, scendere::pending_info>> (transaction_a, temp))), n (scendere::store_iterator<scendere::pending_key, scendere::pending_info> (nullptr)); i != n; ++i)
		{
			auto s = mdb_put (env.tx (transaction_a), pending_handle, scendere::mdb_val (i->first), scendere::mdb_val (i->second), MDB_APPEND);
			release_assert_success (*this, s);
		}
		release_assert (count (transaction_a, pending_handle) == count (transaction_a, temp));
		mdb_drop (env.tx (transaction_a), temp, 1);
...
bool scendere::mdb_store::init_error () const
{
	return error;
}

std::shared_ptr<scendere::block> scendere::mdb_store::block_get_v18 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a) const
{
	...
	if (value.size () != 0)
	{
		...
		release_assert (!error);
		result->sideband_set (scendere::block_sideband (sideband.account, sideband.successor, sideband.balance, sideband.height, sideband.timestamp, sideband.details.epoch, sideband.details.is_send, sideband.details.is_receive, sideband.details.is_epoch, scendere::epoch::epoch_0));
	}
	return result;
}

scendere::mdb_val scendere::mdb_store::block_raw_get_v18 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::block_type & type_a) const
{
	scendere::mdb_val result;
	// Table lookups are ordered by match probability
	scendere::block_type block_types[]{ scendere::block_type::state, scendere::block_type::send, scendere::block_type::receive, scendere::block_type::open, scendere::block_type::change };
...
boost::optional<scendere::mdb_val> scendere::mdb_store::block_raw_get_by_type_v18 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::block_type & type_a) const
{
	scendere::mdb_val value;
	scendere::mdb_val hash (hash_a);
	...
	boost::optional<scendere::mdb_val> result;
	if (success (status))
	{
		result = value;
	}
...
scendere::uint128_t scendere::mdb_store::block_balance_v18 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a) const
{
	auto block (block_get_v18 (transaction_a, hash_a));
	release_assert (block);
	scendere::uint128_t result (this->block.balance_calculated (block));
	return result;
}

// All the v14 functions below are only needed during upgrades
std::size_t scendere::mdb_store::block_successor_offset_v14 (scendere::transaction const & transaction_a, std::size_t entry_size_a, scendere::block_type type_a) const
{
	return entry_size_a - scendere::block_sideband_v14::size (type_a);
}

scendere::block_hash scendere::mdb_store::block_successor_v14 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a) const
{
	scendere::block_type type;
	...
	if (value.size () != 0)
	{
		...
		scendere::bufferstream stream (reinterpret_cast<uint8_t const *> (value.data ()) + block_successor_offset_v14 (transaction_a, value.size (), type), result.bytes.size ());
...
scendere::mdb_val scendere::mdb_store::block_raw_get_v14 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::block_type & type_a, bool * is_state_v1) const
{
	scendere::mdb_val result;
	// Table lookups are ordered by match probability
	scendere::block_type block_types[]{ scendere::block_type::state, scendere::block_type::send, scendere::block_type::receive, scendere::block_type::open, scendere::block_type::change };
...
boost::optional<scendere::mdb_val> scendere::mdb_store::block_raw_get_by_type_v14 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::block_type & type_a, bool * is_state_v1) const
{
	scendere::mdb_val value;
	scendere::mdb_val hash (hash_a);
	int status = status_code_not_found ();
...
std::shared_ptr<scendere::block> scendere::mdb_store::block_get_v14 (scendere::transaction const & transaction_a, scendere::block_hash const & hash_a, scendere::block_sideband_v14 * sideband_a, bool * is_state_v1) const
{
	scendere::block_type type;
...
scendere::mdb_store::upgrade_counters::upgrade_counters (uint64_t count_before_v0, uint64_t count_before_v1) :
	before_v0 (count_before_v0),
	before_v1 (count_before_v1)
{
}

bool scendere::mdb_store::upgrade_counters::are_equal () const
{
	return (before_v0 == after_v0) && (before_v1 == after_v1);
}

unsigned scendere::mdb_store::max_block_write_batch_num () const
{
	return std::numeric_limits<unsigned>::max ();
}

// Explicitly instantiate
template class scendere::store_partial<MDB_val, scendere::mdb_store>;
