...
{
public:
	explicit matches_txn (scendere::transaction_impl const * transaction_impl_a) :
		transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (scendere::mdb_txn_stats const & mdb_txn_stats)
	{
...
scendere::read_mdb_txn::read_mdb_txn (scendere::mdb_env const & environment_a, scendere::mdb_txn_callbacks txn_callbacks_a) :
...
scendere::read_mdb_txn::~read_mdb_txn ()
{
...
void scendere::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void scendere::read_mdb_txn::renew ()
{
...
void * scendere::read_mdb_txn::get_handle () const
{
	return handle;
}

scendere::write_mdb_txn::write_mdb_txn (scendere::mdb_env const & environment_a, scendere::mdb_txn_callbacks txn_callbacks_a) :
	env (environment_a),
	txn_callbacks (txn_callbacks_a)
{
	renew ();
...
scendere::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void scendere::write_mdb_txn::commit ()
{
	if (active)
	{
		auto status (mdb_txn_commit (handle));
...
void scendere::write_mdb_txn::renew ()
{
...
void * scendere::write_mdb_txn::get_handle () const
{
	return handle;
}

bool scendere::write_mdb_txn::contains (scendere::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

scendere::mdb_txn_tracker::mdb_txn_tracker (scendere::logger_mt & logger_a, scendere::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
	logger (logger_a),
...
void scendere::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (std::size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			scendere::jsonconfig mdb_lock_config;
			...
			{
				scendere::jsonconfig frame_json;
			...
			scendere::jsonconfig stack (stacktrace_config);
...
void scendere::mdb_txn_tracker::log_if_held_long_enough (scendere::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == scendere::thread_role::get_string (scendere::thread_role::name::block_processing);
...
void scendere::mdb_txn_tracker::add (scendere::transaction_impl const * transaction_impl)
{
	scendere::lock_guard<scendere::mutex> guard (mutex);
...
void scendere::mdb_txn_tracker::erase (scendere::transaction_impl const * transaction_impl)
{
...
scendere::mdb_txn_stats::mdb_txn_stats (scendere::transaction_impl const * transaction_impl) :
	transaction_impl (transaction_impl),
	thread_name (scendere::thread_role::get_string ()),
	stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool scendere::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<scendere::write_transaction_impl const *> (transaction_impl) != nullptr);
}
