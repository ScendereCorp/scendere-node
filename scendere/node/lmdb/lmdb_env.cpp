...
scendere::mdb_env::mdb_env (bool & error_a, boost::filesystem::path const & path_a, scendere::mdb_env::options options_a)
{
	init (error_a, path_a, options_a);
}

void scendere::mdb_env::init (bool & error_a, boost::filesystem::path const & path_a, scendere::mdb_env::options options_a)
{
	boost::system::error_code error_mkdir, error_chmod;
	if (path_a.has_parent_path ())
	{
		boost::filesystem::create_directories (path_a.parent_path (), error_mkdir);
		scendere::set_secure_perm_directory (path_a.parent_path (), error_chmod);
		if (!error_mkdir)
		{
			...
			if (options_a.config.sync == scendere::lmdb_config::sync_strategy::nosync_safe)
			{
				environment_flags |= MDB_NOMETASYNC;
			}
			else if (options_a.config.sync == scendere::lmdb_config::sync_strategy::nosync_unsafe)
			{
				environment_flags |= MDB_NOSYNC;
			}
			else if (options_a.config.sync == scendere::lmdb_config::sync_strategy::nosync_unsafe_large_memory)
			{
...
scendere::mdb_env::operator MDB_env * () const
{
	return environment;
}

scendere::read_transaction scendere::mdb_env::tx_begin_read (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return scendere::read_transaction{ std::make_unique<scendere::read_mdb_txn> (*this, mdb_txn_callbacks) };
}

scendere::write_transaction scendere::mdb_env::tx_begin_write (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return scendere::write_transaction{ std::make_unique<scendere::write_mdb_txn> (*this, mdb_txn_callbacks) };
}

MDB_txn * scendere::mdb_env::tx (scendere::transaction const & transaction_a) const
{
