...
{
public:
	mdb_iterator (scendere::transaction const & transaction_a, MDB_dbi db_a, MDB_val const & val_a = MDB_val{}, bool const direction_asc = true)
	{
	...
	mdb_iterator (scendere::mdb_iterator<T, U> && other_a)
	{
	...
	mdb_iterator (scendere::mdb_iterator<T, U> const &) = delete;
	...
	scendere::store_iterator_impl<T, U> & operator++ () override
	{
	...
	std::pair<scendere::db_val<MDB_val>, scendere::db_val<MDB_val>> * operator-> ()
	{
		return &current;
	}

	bool operator== (scendere::store_iterator_impl<T, U> const & base_a) const override
	{
		auto const other_a (boost::polymorphic_downcast<scendere::mdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = scendere::db_val<MDB_val> ();
		current.second = scendere::db_val<MDB_val> ();
		debug_assert (is_end_sentinal ());
	}

	scendere::mdb_iterator<T, U> & operator= (scendere::mdb_iterator<T, U> && other_a)
	{
		if (cursor != nullptr)
		{
			mdb_cursor_close (cursor);
	...
	std::pair<scendere::db_val<MDB_val>, scendere::db_val<MDB_val>> current;
...
{
public:
	mdb_merge_iterator (scendere::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a) :
		impl1 (std::make_unique<scendere::mdb_iterator<T, U>> (transaction_a, db1_a)),
		impl2 (std::make_unique<scendere::mdb_iterator<T, U>> (transaction_a, db2_a))
	{
	}

	mdb_merge_iterator () :
		impl1 (std::make_unique<scendere::mdb_iterator<T, U>> ()),
		impl2 (std::make_unique<scendere::mdb_iterator<T, U>> ())
	{
	}

	mdb_merge_iterator (scendere::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a, MDB_val const & val_a) :
		impl1 (std::make_unique<scendere::mdb_iterator<T, U>> (transaction_a, db1_a, val_a)),
		impl2 (std::make_unique<scendere::mdb_iterator<T, U>> (transaction_a, db2_a, val_a))
	{
	}

	mdb_merge_iterator (scendere::mdb_merge_iterator<T, U> && other_a)
	{
		impl1 = std::move (other_a.impl1);
		impl2 = std::move (other_a.impl2);
	}

	mdb_merge_iterator (scendere::mdb_merge_iterator<T, U> const &) = delete;
	...
	bool operator== (scendere::store_iterator_impl<T, U> const & base_a) const override
	{
		debug_assert ((dynamic_cast<scendere::mdb_merge_iterator<T, U> const *> (&base_a) != nullptr) && "Incompatible iterator comparison");
		auto & other (static_cast<scendere::mdb_merge_iterator<T, U> const &> (base_a));
	...
	scendere::mdb_merge_iterator<T, U> & operator= (scendere::mdb_merge_iterator<T, U> &&) = default;
	scendere::mdb_merge_iterator<T, U> & operator= (scendere::mdb_merge_iterator<T, U> const &) = delete;

	mutable bool from_first_database{ false };

private:
	scendere::mdb_iterator<T, U> & least_iterator () const
	{
		scendere::mdb_iterator<T, U> * result;
	...
	std::unique_ptr<scendere::mdb_iterator<T, U>> impl1;
	std::unique_ptr<scendere::mdb_iterator<T, U>> impl2;
};
}
