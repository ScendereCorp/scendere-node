...
scendere::rep_crawler::rep_crawler (scendere::node & node_a) :
	node (node_a)
{
	if (!node.flags.disable_rep_crawler)
	{
		node.observers.endpoint.add ([this] (std::shared_ptr<scendere::transport::channel> const & channel_a) {
			this->query (channel_a);
		});
	}
}

void scendere::rep_crawler::remove (scendere::block_hash const & hash_a)
{
	scendere::lock_guard<scendere::mutex> lock (active_mutex);
	active.erase (hash_a);
}

void scendere::rep_crawler::start ()
{
	ongoing_crawl ();
}

void scendere::rep_crawler::validate ()
{
	...
	for (auto const & i : responses_l)
	{
		...
		if (channel->get_type () == scendere::transport::transport_type::loopback)
		{
		...
		scendere::uint128_t rep_weight = node.ledger.weight (vote->account);
		...
		auto inserted = false;
		auto updated = false;
		std::shared_ptr<scendere::transport::channel> prev_channel;

		scendere::unique_lock<scendere::mutex> lock (probable_reps_mutex);

		auto existing (probable_reps.find (vote->account));
		if (existing != probable_reps.end ())
		{
			probable_reps.modify (existing, [rep_weight, &updated, &vote, &channel, &prev_channel] (scendere::representative & info) {
		...
		{
			probable_reps.emplace (scendere::representative (vote->account, rep_weight, channel));
			inserted = true;
		}
...
void scendere::rep_crawler::ongoing_crawl ()
{
...
std::vector<std::shared_ptr<scendere::transport::channel>> scendere::rep_crawler::get_crawl_targets (scendere::uint128_t total_weight_a)
{
	...
	std::vector<std::shared_ptr<scendere::transport::channel>> result;
	result.insert (result.end (), random_peers.begin (), random_peers.end ());
	return result;
}

void scendere::rep_crawler::query (std::vector<std::shared_ptr<scendere::transport::channel>> const & channels_a)
{
...
void scendere::rep_crawler::query (std::shared_ptr<scendere::transport::channel> const & channel_a)
{
	std::vector<std::shared_ptr<scendere::transport::channel>> peers;
	peers.emplace_back (channel_a);
	query (peers);
}

void scendere::rep_crawler::throttled_remove (scendere::block_hash const & hash_a, uint64_t const target_finished_processed)
{
...
bool scendere::rep_crawler::is_pr (scendere::transport::channel const & channel_a) const
{
	scendere::lock_guard<scendere::mutex> lock (probable_reps_mutex);
...
bool scendere::rep_crawler::response (std::shared_ptr<scendere::transport::channel> const & channel_a, std::shared_ptr<scendere::vote> const & vote_a)
{
	bool error = true;
	scendere::lock_guard<scendere::mutex> lock (active_mutex);
...
scendere::uint128_t scendere::rep_crawler::total_weight () const
{
	scendere::lock_guard<scendere::mutex> lock (probable_reps_mutex);
...
void scendere::rep_crawler::on_rep_request (std::shared_ptr<scendere::transport::channel> const & channel_a)
{
	scendere::lock_guard<scendere::mutex> lock (probable_reps_mutex);
	if (channel_a->get_tcp_endpoint ().address () != boost::asio::ip::address_v6::any ())
	{
		...
		{
			channel_ref_index.modify (itr_pair.first, [] (scendere::representative & value_a) {
...
void scendere::rep_crawler::cleanup_reps ()
{
	std::vector<std::shared_ptr<scendere::transport::channel>> channels;
	{
	...
	{
		bool equal (false);
		if (i->get_type () == scendere::transport::transport_type::tcp)
		{
			auto find_channel (node.network.tcp_channels.find_channel (i->get_tcp_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<scendere::transport::channel_tcp *> (i.get ()))
			{
				equal = true;
			}
		}
		else if (i->get_type () == scendere::transport::transport_type::udp)
		{
			auto find_channel (node.network.udp_channels.channel (i->get_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<scendere::transport::channel_udp *> (i.get ()))
			{
				equal = true;
			}
		}
...
void scendere::rep_crawler::update_weights ()
{
	scendere::lock_guard<scendere::mutex> lock (probable_reps_mutex);
	for (auto i (probable_reps.get<tag_last_request> ().begin ()), n (probable_reps.get<tag_last_request> ().end ()); i != n;)
	{
		...
		if (weight > 0)
		{
			...
			{
				probable_reps.get<tag_last_request> ().modify (i, [weight] (scendere::representative & info) {
					info.weight = weight;
				});
			}
			++i;
...
std::vector<scendere::representative> scendere::rep_crawler::representatives (std::size_t count_a, scendere::uint128_t const weight_a, boost::optional<decltype (scendere::network_constants::protocol_version)> const & opt_version_min_a)
{
...
std::vector<scendere::representative> scendere::rep_crawler::principal_representatives (std::size_t count_a, boost::optional<decltype (scendere::network_constants::protocol_version)> const & opt_version_min_a)
{
	return representatives (count_a, node.minimum_principal_weight (), opt_version_min_a);
}

std::vector<std::shared_ptr<scendere::transport::channel>> scendere::rep_crawler::representative_endpoints (std::size_t count_a)
{
	std::vector<std::shared_ptr<scendere::transport::channel>> result;
...
std::size_t scendere::rep_crawler::representative_count ()
{
	scendere::lock_guard<scendere::mutex> lock (probable_reps_mutex);
