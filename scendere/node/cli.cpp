...
namespace
{
void reset_confirmation_heights (scendere::write_transaction const & transaction, scendere::ledger_constants & constants, scendere::store & store);
...
std::string scendere::error_cli_messages::message (int ev) const
{
	switch (static_cast<scendere::error_cli> (ev))
	{
		case scendere::error_cli::generic:
			return "Unknown error";
		case scendere::error_cli::parse_error:
			return "Coud not parse command line";
		case scendere::error_cli::invalid_arguments:
			return "Invalid arguments";
		case scendere::error_cli::unknown_command:
			return "Unknown command";
		case scendere::error_cli::database_write_error:
			return "Database write error";
		case scendere::error_cli::reading_config:
			return "Config file read error";
		case scendere::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case scendere::error_cli::ambiguous_udp_options:
			return "Flags --disable_udp and --enable_udp cannot be used together";
		case scendere::error_cli::ambiguous_pruning_voting_options:
...
void scendere::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void scendere::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code scendere::update_flags (scendere::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	if (config != vm.end ())
	{
		flags_a.config_overrides = scendere::config_overrides (config->second.as<std::vector<scendere::config_key_value_pair>> ());
	}
	auto rpcconfig (vm.find ("rpcconfig"));
	if (rpcconfig != vm.end ())
	{
		flags_a.rpc_config_overrides = scendere::config_overrides (rpcconfig->second.as<std::vector<scendere::config_key_value_pair>> ());
	}
	return ec;
}

std::error_code scendere::flags_config_conflicts (scendere::node_flags const & flags_a, scendere::node_config const & config_a)
{
	std::error_code ec;
	if (flags_a.enable_pruning && config_a.enable_voting)
	{
		ec = scendere::error_cli::ambiguous_pruning_voting_options;
	}
	return ec;
}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = scendere::error_cli::database_write_error;
}
...
std::error_code scendere::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : scendere::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = scendere::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						scendere::remove_all_files_in_dir (backup_path);
						scendere::move_all_files_to_dir (source_path, backup_path);
						scendere::move_all_files_to_dir (vacuum_path, source_path);
	...
	{
		auto data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : scendere::working_path ();
		auto node_flags = scendere::inactive_node_flag_defaults ();
	...
	else if (vm.count ("generate_config"))
	{
		...
		{
			valid_type = true;
			scendere::rpc_config config{ scendere::dev::network_params.network };
			config.serialize_toml (toml);
		}
		else if (type == "tls")
		{
			valid_type = true;
			scendere::tls_config config;
			config.serialize_toml (toml);
		}
		else
		{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Scendere. Visit https://docs.scendere.org/running-a-node/configuration/ for more information.\n#\n"
					  ...
					  << "# Additional information for notable configuration options is available in https://docs.scendere.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		key.clear ();
		scendere::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		scendere::raw_key junk1;
		junk1.clear ();
		scendere::uint256_union junk2 (0);
		scendere::kdf kdf{ inactive_node->node->config.network_params.kdf_work };
		kdf.phs (junk1, "", junk2);
		std::cout << "Testing time retrieval latency... " << std::flush;
		scendere::timer<std::chrono::nanoseconds> timer (scendere::timer_state::started);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = scendere::default_inactive_node (data_path, vm);
			auto wallet_key = scendere::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (scendere::pub_key (key) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									scendere::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << scendere::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = scendere::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<scendere::inactive_node> scendere::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = scendere::inactive_node_flag_defaults ();
	scendere::update_flags (node_flags, vm_a);
	return std::make_unique<scendere::inactive_node> (path_a, node_flags);
}
...
bool is_using_rocksdb (boost::filesystem::path const & data_path, boost::program_options::variables_map const & vm, std::error_code & ec)
{
	...
	{
		config_overrides = scendere::config_overrides (config_arg->second.as<std::vector<scendere::config_key_value_pair>> ());
	}

	// config override...
	auto error = scendere::read_node_config_toml (data_path, config, config_overrides);
	if (!error)
	{
	...
	{
		ec = scendere::error_cli::reading_config;
	}
