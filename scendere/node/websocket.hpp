...
#include <scendere/node/websocket_stream.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (scendere::websocket::topic topic_a) :
			topic (topic_a)
		{
		}
		message (scendere::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
			topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		scendere::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<scendere::block> const & block_a, scendere::account const & account_a, scendere::amount const & amount_a, std::string subtype, bool include_block, scendere::election_status const & election_status_a, std::vector<scendere::vote_with_weight_info> const & election_votes_a, scendere::websocket::confirmation_options const & options_a);
		message stopped_election (scendere::block_hash const & hash_a);
		message vote_received (std::shared_ptr<scendere::vote> const & vote_a, scendere::vote_code code_a);
		message work_generation (scendere::work_version const version_a, scendere::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (scendere::work_version const version_a, scendere::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (scendere::work_version const version_a, scendere::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (scendere::telemetry_data const &, scendere::endpoint const &);
		message new_block_arrived (scendere::block const & block_a);
	...
	{
	public:
		confirmation_options (scendere::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, scendere::wallets & wallets_a, scendere::logger_mt & logger_a);
	...
	private:
		...
		boost::optional<scendere::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, scendere::logger_mt & logger_a);
	...
	public:
		...
		explicit session (scendere::websocket::listener & listener_a, socket_type socket_a, boost::asio::ssl::context & ctx_a);
#endif
		/** Constructor that takes ownership over \p socket_a */
		explicit session (scendere::websocket::listener & listener_a, socket_type socket_a);
		...
		void write (scendere::websocket::message message_a);

	private:
		/** The owning listener */
		scendere::websocket::listener & ws_listener;
		/** Websocket stream, supporting both plain and tls connections */
		scendere::websocket::stream ws;
		...
		scendere::mutex subscriptions_mutex;
	...
	{
	public:
		listener (std::shared_ptr<scendere::tls_config> const & tls_config_a, scendere::logger_mt & logger_a, scendere::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<scendere::block> const & block_a, scendere::account const & account_a, scendere::amount const & amount_a, std::string const & subtype, scendere::election_status const & election_status_a, std::vector<scendere::vote_with_weight_info> const & election_votes_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (scendere::websocket::message message_a);

		scendere::logger_mt & get_logger () const
		{
			return logger;
		}
		...
		scendere::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (scendere::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		std::size_t subscriber_count (scendere::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend scendere::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (scendere::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (scendere::websocket::topic const & topic_a);
