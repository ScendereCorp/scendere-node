...
scendere::rpc_handler::rpc_handler (scendere::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void (std::string const &)> const & response_a, scendere::rpc_handler_interface & rpc_handler_interface_a, scendere::logger_mt & logger) :
	body (body_a),
...
void scendere::rpc_handler::process_request (scendere::rpc_handler_request_params const & request_params)
{
	try
	{
		...
		else
		{
			if (request_params.rpc_version == 1)
			{
				...
				std::error_code rpc_control_disabled_ec = scendere::error_rpc::rpc_control_disabled;
				...
				else
				{
					...
					{
						scendere::uint128_union random_id;
						scendere::random_pool::generate_block (random_id.bytes.data (), random_id.bytes.size ());
