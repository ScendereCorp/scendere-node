...
public:
	 ...
	 **/
	bool apply (uint8_t const * bytes_a, size_t count_a, scendere::uint128_t * digest_a = nullptr);

	/**
	 * Sets the corresponding element in the filter to zero, if it matches \p digest_a exactly.
	 **/
	void clear (scendere::uint128_t const & digest_a);

	/**
	 * Clear many digests from the filter
	 **/
	void clear (std::vector<scendere::uint128_t> const &);
	 ...
	 */
	template <typename OBJECT>
	scendere::uint128_t hash (OBJECT const & object_a) const;
...
private:
	 ...
	 **/
	scendere::uint128_t & get_element (scendere::uint128_t const & hash_a);

	/**
	 * Hashes \p count_a bytes starting from \p bytes_a .
	 * @return the siphash digest of the contents in \p bytes_a .
	 **/
	scendere::uint128_t hash (uint8_t const * bytes_a, size_t count_a) const;

	std::vector<scendere::uint128_t> items;
	CryptoPP::SecByteBlock key{ siphash_t::KEYLENGTH };
	scendere::mutex mutex{ mutex_identifier (mutexes::network_filter) };
};
}
