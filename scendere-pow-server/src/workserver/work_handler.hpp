...
{
public:
	/** We register a scendere_pow device for every device entry in the config file */
	class registered_device
	{
	public:
		registered_device (scendere_pow_server::config::device device_config_a,
		    std::shared_ptr<scendere_pow::driver> const & driver_a)
		...
		scendere_pow_server::config::device device_config;
		std::shared_ptr<scendere_pow::driver> driver;
		std::atomic<bool> busy{ false };
	};

	work_handler (scendere_pow_server::config const & config_a, std::shared_ptr<spdlog::logger> const & logger_a);
	~work_handler ();
	...
	void push_job (scendere_pow_server::job const & job)
	{
	...
	boost::optional<scendere_pow_server::job> pop_job ()
	{
		std::lock_guard<std::mutex> lk (jobs_mutex);
		boost::optional<scendere_pow_server::job> res;
		if (!jobs.empty ())
		{
			res = jobs.top ();
			jobs.pop ();
...
private:
	...
	scendere_pow_server::config const & config;
