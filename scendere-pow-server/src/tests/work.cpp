...
TEST (bigint, basic)
{
	scendere_pow_server::u128 val128;
	...
	scendere_pow_server::u256 val256;
	...
	scendere_pow_server::u512 val512;
...
TEST (queue, priority)
{
	scendere_pow_server::config config;
	scendere_pow_server::work_handler handler (config, nullptr);

	constexpr unsigned count = 10;
	constexpr unsigned count_priority_boosted = 4;
	std::vector<scendere_pow_server::job> jobs (count);
	for (unsigned i = 0; i < count; i++)
	{
		jobs[i].request.root_hash = scendere_pow_server::u256 (i);
...
TEST (queue, remove)
{
	...
	handler.remove_job (scendere_pow_server::u256 (5));
...
TEST (difficulty, low)
{
	...
	scendere_pow_server::u128 base (0x2000000000000000);
	scendere_pow_server::u128 diff (0x2000000000000001);
	double multiplier = scendere_pow_server::to_multiplier (diff, base);
...
TEST (difficulty, high)
{
	...
	scendere_pow_server::u128 base (0x2000000000000000);
	scendere_pow_server::u128 diff (0xffffcfcaef105e00);
	double multiplier = scendere_pow_server::to_multiplier (diff, base);
	ASSERT_NEAR (expected_multiplier, multiplier, 1e-10);
}
...
{
	scendere_pow_server::u128 base (0x2000000000000000);
	scendere_pow_server::u128 diff;
	diff.from_hex ("100000000000000000000");
	double multiplier = scendere_pow_server::to_multiplier (diff, base);
	auto diff_from_multiplier = scendere_pow_server::from_multiplier (multiplier, base).number ();
