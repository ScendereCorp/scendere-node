# Scendere PoW Server
[![Build Status](https://travis-ci.org/scenderecurrency/scendere-pow-server.svg)](https://travis-ci.org/scenderecurrency/scendere-pow-server)

This project is a standalone work server for [scendere-pow](https://github.com/scenderecurrency/scendere-pow), the proof-of-work algorithm used by Scendere.

## Download

* [Linux](s3://repo.scendere.org/pow-server/scendere_pow_server-latest-Linux.tar.gz)
* [Mac](s3://repo.scendere.org/pow-server/scendere_pow_server-latest-Darwin.tar.gz)
* [Windows](s3://repo.scendere.org/pow-server/scendere_pow_server-latest-win64.tar.gz)
...
```
git clone --recursive https://github.com/scenderecurrency/scendere-pow-server.git
cd scendere-pow-server; mkdir build; cd build
cmake -DSCENDERE_POW_STANDALONE=ON ..
make
```
...
```
./scendere_pow_server --config server.log_to_stderr=true
```
...
To override defaults using a config file, create a file called `scendere-pow-server.toml` and add the required keys and values under their respective TOML table. The file name can optionally be specified with the `--config_path` option (if not specified, the working directory is searched)

**A documented configuration file can be created with the --generate_config command**

**A documented configuration file is created in the build root when the scendere_pow_server target is created**
...
Authentication and access control is beyond the scope of the Scendere PoW Server. To expose functionality, consider using a reverse proxy; this can be used to control access to the REST, WebSocket and admin interfaces.
