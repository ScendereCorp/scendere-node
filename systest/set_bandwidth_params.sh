...
# if SCENDERE_NODE_EXE is unset ot empty then "../../build/scendere_node" is used
SCENDERE_NODE_EXE=${SCENDERE_NODE_EXE:-../../build/scendere_node}

mkdir -p "$DATADIR/log"
clean_data_dir

# start scendere_node and store its pid so we can later send it
# the SIGHUP signal and so we can terminate it
msg start scendere_node
$SCENDERE_NODE_EXE --daemon --data_path "$DATADIR" >/dev/null &
pid=$!
msg pid=$pid
...
# send scendere_node the SIGHUP signal
kill -HUP $pid
...
# terminate scendere node and wait for it to die
kill $pid
wait $pid
