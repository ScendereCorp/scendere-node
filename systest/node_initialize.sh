...
test_initialize_cmd() {
    ...
    $SCENDERE_NODE_EXE --initialize --data_path "$DATADIR" "$netcmd" "$netarg"
    ...
    $SCENDERE_NODE_EXE --debug_block_count --data_path "$DATADIR" "$netcmd" "$netarg" | grep -q 'Block count: 1'

    # check the genesis block is correct
    $SCENDERE_NODE_EXE --debug_block_dump --data_path "$DATADIR" "$netcmd" "$netarg" | head -n 1 | grep -qi "$genesishash"
}
